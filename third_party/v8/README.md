...
`RUSTY_V8_ARCHIVE`/`RUSTY_V8_SRC_BINDING_PATH` with Codewen release assets. Bazel
...
1. Bump the `v8` crate version and refresh `codewen-rs/Cargo.lock`.
...
by Codewen's Bazel Windows GNU toolchain.
...
downloaded `RUSTY_V8_SRC_BINDING_PATH` to point at those `openai/codewen` release
...
the exact resolved `v8` crate version in `codewen-rs/Cargo.lock`.
