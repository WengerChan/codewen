...
./scripts/stage_npm_packages.py \
  ...
  --package codewen \
  --package codewen-responses-api-proxy \
  --package codewen-sdk
```
...
When `--package codewen` is provided, the staging helper builds the lightweight
`@yourname/codewen` meta package plus all platform-native `@yourname/codewen` variants
