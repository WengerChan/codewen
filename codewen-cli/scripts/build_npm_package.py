#!/usr/bin/env python3
"""Stage and optionally package the @yourname/codewen npm module."""
...
CODEWEN_CLI_ROOT = SCRIPT_DIR.parent
REPO_ROOT = CODEWEN_CLI_ROOT.parent
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "codewen-rs" / "responses-api-proxy" / "npm"
CODEWEN_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"
CODEWEN_NPM_NAME = "@yourname/codewen"
CODEWEN_PACKAGE_COMPONENT = "codewen-package"

# `npm_name` is the local optional-dependency alias consumed by `bin/codewen.js`.
# The underlying package published to npm is always `@yourname/codewen`.
CODEWEN_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "codewen-linux-x64": {
        "npm_name": "@yourname/codewen-linux-x64",
        "npm_tag": "linux-x64",
    ...
    "codewen-linux-arm64": {
        "npm_name": "@yourname/codewen-linux-arm64",
        "npm_tag": "linux-arm64",
    ...
    "codewen-darwin-x64": {
        "npm_name": "@yourname/codewen-darwin-x64",
        "npm_tag": "darwin-x64",
        "target_triple": "x86_64-apple-darwin",
        "os": "darwin",
        "cpu": "x64",
    },
    "codewen-darwin-arm64": {
        "npm_name": "@yourname/codewen-darwin-arm64",
    ...
    "codewen-win32-x64": {
        "npm_name": "@yourname/codewen-win32-x64",
        "npm_tag": "win32-x64",
    ...
    "codewen-win32-arm64": {
        "npm_name": "@yourname/codewen-win32-arm64",
        "npm_tag": "win32-arm64",
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "codewen": ["codewen", *CODEWEN_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "codewen": [],
    "codewen-linux-x64": [CODEWEN_PACKAGE_COMPONENT],
    "codewen-linux-arm64": [CODEWEN_PACKAGE_COMPONENT],
    "codewen-darwin-x64": [CODEWEN_PACKAGE_COMPONENT],
    "codewen-darwin-arm64": [CODEWEN_PACKAGE_COMPONENT],
    "codewen-win32-x64": [CODEWEN_PACKAGE_COMPONENT],
    "codewen-win32-arm64": [CODEWEN_PACKAGE_COMPONENT],
    "codewen-responses-api-proxy": ["codewen-responses-api-proxy"],
    "codewen-sdk": [],
}

PACKAGE_TARGET_FILTERS: dict[str, str] = {
    package_name: package_config["target_triple"]
    for package_name, package_config in CODEWEN_PLATFORM_PACKAGES.items()
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="codewen",
        help="Which npm package to stage (default: codewen).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "codewen":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/codewen.js --version\n"
                    f"    node {staging_dir_str}/bin/codewen.js --help\n\n"
                )
            elif package == "codewen-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/codewen-responses-api-proxy.js --help\n\n"
                )
            elif package in CODEWEN_PLATFORM_PACKAGES:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="codewen-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "codewen":
        ...
        shutil.copy2(CODEWEN_CLI_ROOT / "bin" / "codewen.js", bin_dir / "codewen.js")

        readme_src = REPO_ROOT / "README.md"
        if readme_src.exists():
            shutil.copy2(readme_src, staging_dir / "README.md")

        package_json_path = CODEWEN_CLI_ROOT / "package.json"
    elif package in CODEWEN_PLATFORM_PACKAGES:
        platform_package = CODEWEN_PLATFORM_PACKAGES[package]
        ...
        with open(CODEWEN_CLI_ROOT / "package.json", "r", encoding="utf-8") as fh:
            codex_package_json = json.load(fh)

        package_json = {
            "name": CODEWEN_NPM_NAME,
        ...
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "codewen-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "codewen-responses-api-proxy.js")
    ...
    elif package == "codewen-sdk":
        package_json_path = CODEWEN_SDK_ROOT / "package.json"
    ...
    if package == "codewen":
        package_json["files"] = ["bin/codewen.js"]
        package_json["optionalDependencies"] = {
            CODEWEN_PLATFORM_PACKAGES[platform_package]["npm_name"]: (
                f"npm:{CODEWEN_NPM_NAME}@"
                f"{compute_platform_package_version(version, CODEWEN_PLATFORM_PACKAGES[platform_package]['npm_tag'])}"
            )
            for platform_package in PACKAGE_EXPANSIONS["codewen"]
            if platform_package != "codewen"
        }
        ...
        if not isinstance(dependencies, dict):
            dependencies = {}
        dependencies[CODEWEN_NPM_NAME] = version
...
def stage_codex_sdk_sources(staging_dir: Path) -> None:
    ...
    if not dist_src.exists():
        raise RuntimeError("codewen-sdk build did not produce a dist directory.")
...
def copy_native_binaries(
    ...
    for target_dir in vendor_src.iterdir():
        ...
        if CODEWEN_PACKAGE_COMPONENT in components_set:
        ...
        for component in sorted(components_set - {CODEWEN_PACKAGE_COMPONENT}):
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="codewen-npm-pack-") as pack_dir_str:
