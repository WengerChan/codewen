...
docker exec --user root "$CONTAINER_NAME" bash -c "mkdir -p /etc/codewen"
for domain in $OPENAI_ALLOWED_DOMAINS; do
  ...
  echo "$domain" | docker exec --user root -i "$CONTAINER_NAME" bash -c "cat >> /etc/codewen/allowed_domains.txt"
done

# Set proper permissions on the domains file
docker exec --user root "$CONTAINER_NAME" bash -c "chmod 444 /etc/codewen/allowed_domains.txt && chown root:root /etc/codewen/allowed_domains.txt"
...
docker exec -it "$CONTAINER_NAME" bash -c "cd \"/app$WORK_DIR\" && codewen --sandbox workspace-write --ask-for-approval on-request ${quoted_args}"
