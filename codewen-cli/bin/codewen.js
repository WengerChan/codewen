#!/usr/bin/env node
// Unified entry point for the Codewen CLI.
...
const codewenPackageRoot = realpathSync(path.join(__dirname, ".."));
...
function findCodexExecutable() {
  ...
  const codexExecutable = path.join(
    ...
    targetTriple,
    "bin",
    process.platform === "win32" ? "codewen.exe" : "codewen",
  );
  ...
  const updateCommand =
    packageManager === "bun"
      ? "bun install -g @yourname/codewen@latest"
      : packageManager === "pnpm"
        ? "pnpm add -g @yourname/codewen@latest"
        : "npm install -g @yourname/codewen@latest";
  throw new Error(
...
function isPnpmOwnedCodexInstall(nodeModulesDir) {
  ...
  try {
    return (
      realpathSync(path.join(nodeModulesDir, "@openai", "codewen")) ===
      codewenPackageRoot
    );
  } catch {
    return false;
...
function detectPackageManager() {
  ...
  for (const startDir of new Set([codewenPackageRoot, entrypointDir])) {
...
const env = {
  ...process.env,
  CODEX_MANAGED_PACKAGE_ROOT: codewenPackageRoot,
};
