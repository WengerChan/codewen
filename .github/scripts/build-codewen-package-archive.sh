...
usage() {
  cat <<'EOF'
Usage: build-codewen-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-manifest <path>] \
  [--codewen-command-runner-bin <path>] \
  [--codewen-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --codewen-command-runner-bin)
      resource_args+=(
        --codewen-command-runner-bin
        "${2:?--codewen-command-runner-bin requires a value}"
      )
    ...
    --codewen-windows-sandbox-setup-bin)
      resource_args+=(
        --codewen-windows-sandbox-setup-bin
        "${2:?--codewen-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="codewen"
    entrypoint="codewen"
    archive_stem="codewen-package"
    ;;
  app-server)
    variant="codewen-app-server"
    entrypoint="codewen-app-server"
    archive_stem="codewen-app-server-package"
    ;;
  *)
...
code_mode_host_bin="${entrypoint_dir%/}/codewen-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/codewen-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/codewen-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--codewen-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--codewen-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
