...
"""Verify that codewen-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the codewen-rs directory naming conventions.
...
CARGO_RS_ROOT = ROOT / "codewen-rs"
WORKSPACE_PACKAGE_FIELDS = ("version", "edition", "license")
TOP_LEVEL_NAME_EXCEPTIONS = {
    "windows-sandbox-rs": "codewen-windows-sandbox",
}
UTILITY_NAME_EXCEPTIONS = {
    "path-utils": "codewen-utils-path",
}
MANIFEST_FEATURE_EXCEPTIONS = {
    "codewen-rs/code-mode/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
    "codewen-rs/v8-poc/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under codewen-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `codewen-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `codewen-rs/<crate>/Cargo.toml` and "
        "`codewen-rs/utils/<crate>/Cargo.toml`."
    )
    print(
...
def expected_package_name(path: Path) -> str | None:
    ...
    if len(parts) == 2 and parts[1] == "Cargo.toml":
        ...
        return TOP_LEVEL_NAME_EXCEPTIONS.get(
            directory,
            directory if directory.startswith("codewen-") else f"codewen-{directory}",
        )
    if len(parts) == 3 and parts[0] == "utils" and parts[2] == "Cargo.toml":
        directory = parts[1]
        return UTILITY_NAME_EXCEPTIONS.get(directory, f"codewen-utils-{directory}")
    return None
