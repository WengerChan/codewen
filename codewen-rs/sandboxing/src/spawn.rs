...
pub async fn spawn_process(request: SpawnRequest<'_>) -> Result<SpawnedProcess> {
    if request.sandbox == SandboxType::WindowsRestrictedToken {
        #[cfg(target_os = "windows")]
        {
            ...
            let codewen_home = codewen_utils_home_dir::find_codewen_home()
                .context("windows sandbox: failed to resolve codewen_home")?;
            let empty_paths = &[];
            let overrides = windows.filesystem_overrides;

            return codewen_windows_sandbox::spawn_windows_sandbox_session_for_level(
                codewen_windows_sandbox::WindowsSandboxSessionRequest {
                    permission_profile: windows.permission_profile,
                    workspace_roots: windows.workspace_roots,
                    codewen_home: codewen_home.as_path(),
    ...
    if request.tty {
        codewen_utils_pty::pty::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        codewen_utils_pty::pipe::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        codewen_utils_pty::pipe::spawn_process_no_stdin(
            program,
            args,
            request.cwd,
            request.env,
