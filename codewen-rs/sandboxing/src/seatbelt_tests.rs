...
use codewen_network_proxy::ConfigReloader;
use codewen_network_proxy::ConfigReloaderFuture;
use codewen_network_proxy::ConfigState;
use codewen_network_proxy::ManagedNetworkSandboxContext;
use codewen_network_proxy::NetworkMode;
use codewen_network_proxy::NetworkProxy;
use codewen_network_proxy::NetworkProxyConfig;
use codewen_network_proxy::NetworkProxyConstraints;
use codewen_network_proxy::NetworkProxyState;
use codewen_network_proxy::build_config_state;
...
use codewen_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/codewen-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.codewen".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/codewen-readable");
    let unreadable = absolute_path("/tmp/codewen-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/codewen-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/codewen-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/codewen-proxy-use";
    let explicit_socket = "/tmp/codewen-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_codewen_subpaths() {
    ...
    // write to .codewen/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.codewen carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_codewen_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .codewen/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_codewen_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .codewen is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_codewen_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_codex = repo_root.join(".codewen");
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .codewen/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".codewen")).expect("create .codewen");
    fs::write(
        vulnerable_root.join(".codewen").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .codewen/config.toml");
    ...
    let dot_codewen_canonical = vulnerable_root_canonical.join(".codewen");
