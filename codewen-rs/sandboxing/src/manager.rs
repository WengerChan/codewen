...
pub struct SandboxTransformRequest<'a> {
    ...
    pub codewen_linux_sandbox_exe: Option<&'a Path>,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing codewen-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let pending = pending_sandboxed_request?;
                let exe = codewen_linux_sandbox_exe
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let codewen_home = codewen_utils_home_dir::find_codewen_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            self.transform_for_direct_spawn_with_codewen_home(request, codewen_home.as_path())
        }
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_codewen_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        codewen_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    codewen_home: &Path,
    proxy_settings_mode: codewen_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = codewen_windows_sandbox::resolve_exe_for_launch(source.as_path(), codewen_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        codewen_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
