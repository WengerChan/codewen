# codewen-linux-sandbox

This crate is responsible for producing:

- a `codewen-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Codewen CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `codewen-exec` CLI can check if its arg0 is `codewen-linux-sandbox` and, if so, execute as if it were `codewen-linux-sandbox`
  - this should also be true of the `codewen` multitool CLI

On Linux, Codewen prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `codewen-resources/bwrap` binary shipped
with Codewen.
Codewen also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Codewen surfaces the same startup warning
...
it cannot create the required user namespaces, so Codewen rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `codewen-resources/bwrap` path.
- If `bwrap` is missing, Codewen also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Codewen surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Codewen rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.codewen`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `codewen sandbox`; the host OS selects the sandbox backend.
