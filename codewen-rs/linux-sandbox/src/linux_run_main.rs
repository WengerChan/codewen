...
) -> CodewenResult<crate::bwrap::BwrapArgs> {
...
fn exit_with_bwrap_build_error(err: codewen_protocol::error::CodewenErr) -> ! {
...
fn preflight_proc_mount_support(network_mode: BwrapNetworkMode) -> CodewenResult<bool> {
...
fn synthetic_mount_registry_root() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "codewen-bwrap-synthetic-mount-targets-{effective_uid}"
    ))
}
