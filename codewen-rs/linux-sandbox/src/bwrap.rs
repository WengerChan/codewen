...
//! - sensitive subpaths such as `.git`, `.agents`, and `.codewen` remain
...
use codewen_protocol::permissions::is_protected_metadata_name;
...
use codewen_protocol::protocol::FileSystemSpecialPath;
use codewen_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.codewen`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".codewen")
                {
                    return None;
                }
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_codex = temp_dir.path().join("real-codewen");
        let logical_codex = logical_home.join(".codewen");
        ...
        std::os::unix::fs::symlink(&real_codex, &logical_codex)
            .expect("create symlinked codewen home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".codewen"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_codex = path_to_string(&temp_dir.path().join(".codewen"));
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.codewen"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.codewen"),
            ]
        );
        assert_eq!(
            args.args,
