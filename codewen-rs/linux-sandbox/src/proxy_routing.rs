use codewen_network_proxy::PROXY_ATTRIBUTION_TOKEN_ENV_KEY;
use codewen_network_proxy::write_attribution_frame;
...
const PROXY_SOCKET_DIR_PREFIX: &str = "codewen-linux-sandbox-proxy-";
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(codewen_home) = std::env::var_os("CODEWEN_HOME") {
        let candidate = PathBuf::from(codewen_home).join("tmp");
...
fn harden_bridge_process() -> io::Result<()> {
    ...
    codewen_process_hardening::disable_process_dumping()
}
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("codewen-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("codewen-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("codewen-linux-sandbox-proxy-1234-1000-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("codewen-linux-sandbox-proxy-x"),
            None
        );
