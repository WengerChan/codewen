...
use codewen_core::exec::process_exec_tool_call;
...
fn codewen_linux_sandbox_exe() -> PathBuf {
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> codewen_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<codewen_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let codewen_linux_sandbox_exe = Some(codewen_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &codewen_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(CodewenErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(CodewenErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<codewen_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> codewen_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(CodewenErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let codewen_linux_sandbox_exe: Option<PathBuf> = Some(codewen_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_codewen_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_codex = tmpdir.path().join(".codewen");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_codex).expect("create .codewen");

    let git_target = dot_git.join("config");
    let codewen_target = dot_codex.join("config.toml");
    ...
    let codewen_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", codewen_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".codewen write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(codewen_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_codewen_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-codewen");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_codex = tmpdir.path().join(".codewen");
    symlink(&decoy, &dot_codex).expect("create .codewen symlink");
    ...
    let codewen_output = expect_denied(
        ...
        .await,
        ".codewen symlink replacement should be denied",
    );
    assert_ne!(codewen_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_codewen_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    {
        Err(CodewenErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".codewen symlink build failure should deny: {result:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|codewen|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_codewen_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".codewen"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .codewen directory creation should be denied",
    );
    assert_ne!(mkdir_codewen_output.exit_code, 0);
    assert!(!subdir.join(".codewen").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = codewen_linux_sandbox_exe()
        .parent()
