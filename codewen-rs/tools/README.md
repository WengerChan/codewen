...
`codewen-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `codewen-core`.
...
compatibility-sensitive orchestration in `codewen-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `codewen-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `codewen-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `codewen-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `codewen-core`.
3. Leave compatibility-sensitive adapters in `codewen-core` while downstream
...
`codewen-core`, that is a sign to revisit the extraction boundary before adding
more here.
