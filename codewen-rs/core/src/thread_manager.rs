use crate::CodewenAppsToolsCache;
use crate::SkillsService;
use crate::agent::AgentControl;
use crate::attestation::AttestationProvider;
use crate::codewen_thread::CodewenThread;
...
use codewen_agent_graph_store::AgentGraphStore;
use codewen_agent_graph_store::LocalAgentGraphStore;
...
use codewen_code_mode::CodeModeSessionProvider;
use codewen_code_mode::InProcessCodeModeSessionProvider;
use codewen_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use codewen_extension_api::ExtensionRegistry;
...
use codewen_login::default_client::CODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
use codewen_model_provider_info::OPENAI_PROVIDER_ID;
...
use codewen_thread_store::InMemoryThreadStore;
use codewen_thread_store::LoadThreadHistoryParams;
use codewen_thread_store::LocalThreadStore;
use codewen_thread_store::LocalThreadStoreConfig;
use codewen_thread_store::ReadThreadByRolloutPathParams;
use codewen_thread_store::ReadThreadParams;
use codewen_thread_store::StoredModelContext;
use codewen_thread_store::StoredThread;
use codewen_thread_store::ThreadMetadataPatch;
use codewen_thread_store::ThreadStore;
use codewen_thread_store::ThreadStoreError;
use codewen_thread_store::UpdateThreadMetadataParams;
...
pub struct ThreadManager {
    ...
    _test_codewen_home_guard: Option<TempCodexHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<codewen_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<CodewenThread>>>>,
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        codewen_apps_tools_cache: CodewenAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
    ...
    /// Construct with a dummy AuthManager containing the provided CodewenAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let codewen_home = std::env::temp_dir().join(format!(
            "codewen-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&codewen_home)
            .unwrap_or_else(|err| panic!("temp codewen home dir create failed: {err}"));
        ...
        manager._test_codewen_home_guard = Some(TempCodexHomeGuard { path: codewen_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided CodewenAuth and codewen home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let skills_codewen_home = match AbsolutePathBuf::from_absolute_path_checked(&codewen_home) {
            Ok(codewen_home) => codewen_home,
            Err(err) => panic!("test codewen_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite_home: codewen_home.clone(),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(codewen_home, /*config_model_catalog*/ None),
                environment_manager,
                skills_service,
                plugins_manager,
                mcp_manager,
    ...
    pub fn validate_environment_selections(
        ...
        for environment in environments {
            if !environment_ids.insert(environment.environment_id.as_str()) {
                return Err(CodewenErr::InvalidRequest(format!(
            ...
            self.state
                ...
                .ok_or_else(|| {
                    CodewenErr::InvalidRequest(format!(
    ...
    pub async fn list_models(
        ...
        http_client_factory: codewen_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> CodewenResult<Arc<CodewenThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `CodewenThread`/`LiveThread`, so metadata changes stay ordered
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    CodewenErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    CodewenErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
    ...
    pub async fn start_thread(&self, config: Config) -> CodewenResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<codewen_protocol::dynamic_tools::DynamicToolSpec>,
    ) -> CodewenResult<NewThread> {
    ...
    /// as `Arc<CodewenThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<CodewenThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        ...
        match threads.get(&thread_id) {
            ...
            Some(_) | None => Err(CodewenErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => CodewenErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    ) -> CodewenResult<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
    ...
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> CodewenResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<CodewenThread>> {
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(CodewenErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(CodewenThread::new(
                    session,
                    io,
        ...
        Err(CodewenErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> codewen_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return codewen_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(codewen_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> CodewenErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => CodewenErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => CodewenErr::InvalidRequest(message),
        err => CodewenErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> CodewenErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => CodewenErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => CodewenErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => CodewenErr::UnsupportedOperation(format!(
