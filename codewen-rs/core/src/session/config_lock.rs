...
use codewen_config::config_toml::OrchestratorFeatureToml;
use codewen_config::config_toml::OrchestratorToml;
...
use codewen_features::RolloutBudgetConfigToml;
...
pub(crate) async fn validate_config_lock_if_configured(
    ...
    let options = ConfigLockReplayOptions {
        allow_codewen_version_mismatch: config.config_lock_allow_codewen_version_mismatch,
    };
...
mod tests {
    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        for spec in codewen_features::FEATURES {
            assert_eq!(
    ...
    async fn lock_contains_exact_managed_requirements() {
        ...
        let sqlite_home = codewen_home.path().join("managed-state");
        let log_dir = codewen_home.path().join("managed-logs");
        let catalog_path = codewen_home.path().join("managed-models.json");
...
"#,
    ...
    async fn lock_drops_unmanaged_model_catalog_input() {
        let codewen_home = tempfile::tempdir().expect("create temp dir");
        let catalog_path = codewen_home.path().join("user-models.json");
        write_model_catalog(&catalog_path);
    ...
    #[tokio::test]
    async fn lock_validation_rejects_codewen_version_mismatch_by_default() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.codewen_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");
        ...
        assert!(
            message.contains("debug.config_lockfile.allow_codewen_version_mismatch=true"),
            "{message}"
        );
    }

    #[tokio::test]
    async fn lock_validation_can_ignore_codewen_version_mismatch() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.codewen_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");
