...
use codewen_analytics::CompactionPhase;
use codewen_analytics::CompactionReason;
use codewen_analytics::InvocationType;
use codewen_analytics::TurnResolvedConfigFact;
use codewen_analytics::build_track_events_context;
use codewen_async_utils::OrCancelExt;
use codewen_core_plugins::RecommendedPluginCandidatesInput;
use codewen_core_skills::injection::InjectedHostSkillPrompts;
use codewen_extension_api::TurnInputContext;
use codewen_extension_api::TurnInputEnvironment;
use codewen_features::Feature;
use codewen_file_system::FindUpErrorPolicy;
use codewen_file_system::find_nearest_ancestor_with_markers;
...
use codewen_protocol::items::PlanItem;
...
use codewen_protocol::protocol::AgentMessageContentDeltaEvent;
use codewen_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use codewen_protocol::protocol::PlanDeltaEvent;
use codewen_protocol::protocol::RawResponseCompletedEvent;
use codewen_protocol::protocol::ReasoningContentDeltaEvent;
use codewen_protocol::protocol::ReasoningRawContentDeltaEvent;
use codewen_protocol::protocol::SafetyBufferingEvent;
...
use codewen_tools::ToolName;
use codewen_tools::filter_request_plugin_install_discoverable_tools_for_client;
use codewen_utils_path_uri::PathUri;
use codewen_utils_stream_parser::AssistantTextChunk;
use codewen_utils_stream_parser::AssistantTextStreamParser;
use codewen_utils_stream_parser::ProposedPlanSegment;
use codewen_utils_stream_parser::extract_proposed_plan_text;
use codewen_utils_stream_parser::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<codewen_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
    prewarmed_client_session: Option<ModelClientSession>,
    cancellation_token: CancellationToken,
) -> CodewenResult<Option<String>> {
    ...
    if let Err(err) = run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
        if matches!(err, CodewenErr::TurnAborted) {
            return Err(err);
        }
        let error = err.to_codewen_protocol_error();
    ...
    // Although from the perspective of codewen.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: CodewenResult<_> = async {
            ...
            let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
                ...
                window_id,
                CodewenResponsesRequestKind::Turn,
            );
        ...
        match sampling_request_result {
            ...
            Err(err @ CodewenErr::TurnAborted) => {
                return Err(err);
            }
            Err(codewen_error @ CodewenErr::InvalidImageRequest()) => {
                sess.track_turn_codewen_error(turn_context.as_ref(), &codewen_error);
                let error = CodewenErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    codewen_error_info: Some(error),
                });
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_codewen_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_codewen_error(turn_context.as_ref(), &e);
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = codewen_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
...
async fn capture_current_model_fallback_step_context(
    ...
    let uses_codewen_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(codewen_login::AuthManager::current_auth_uses_codewen_backend);
    if !uses_codewen_backend
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = codewen_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<codewen_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &CodewenResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> CodewenResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(CodewenErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(CodewenErr::ContextWindowExceeded);
            }
            Err(CodewenErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(CodewenErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
...
pub(super) fn agent_message_text(item: &codewen_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: codewen_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(codewen_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &codewen_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, CodewenResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> CodewenResult<()> {
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> CodewenResult<SamplingRequestResult> {
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy.value(),
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, CodewenResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: CodewenResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            codewen.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            codewen.usage.reasoning_output_tokens = field::Empty,
            codewen.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(codewen_async_utils::CancelErr::Cancelled) => break Err(CodewenErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![codewen_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(CodewenErr::TurnAborted);
    }
