...
use crate::codewen_thread::TryStartTurnIfIdleRejectionReason;
...
use codewen_config::ConfigLayerStackOrdering;
use codewen_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use codewen_config::LoaderOverrides;
use codewen_config::NetworkConstraints;
use codewen_config::NetworkDomainPermissionToml;
use codewen_config::NetworkDomainPermissionsToml;
use codewen_config::RequirementSource;
use codewen_config::Sourced;
use codewen_config::loader::project_trust_key;
use codewen_config::types::McpServerConfig;
use codewen_config::types::McpServerTransportConfig;
use codewen_config::types::ToolSuggestDisabledTool;
use codewen_core_skills::HostSkillsSnapshot;
...
use codewen_protocol::AgentPath;
use codewen_protocol::ResponseItemId;
use codewen_protocol::SessionId;
use codewen_protocol::ThreadId;
use codewen_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use codewen_protocol::config_types::ServiceTier;
use codewen_protocol::config_types::TrustLevel;
use codewen_protocol::exec_output::ExecToolCallOutput;
use codewen_protocol::models::AgentMessageInputContent;
use codewen_protocol::models::FileSystemPermissions;
use codewen_protocol::models::FunctionCallOutputBody;
use codewen_protocol::models::FunctionCallOutputContentItem;
use codewen_protocol::models::FunctionCallOutputPayload;
use codewen_protocol::models::ImageDetail;
use codewen_protocol::models::PermissionProfile;
use codewen_protocol::models::SandboxEnforcement;
use codewen_protocol::openai_models::ModelServiceTier;
use codewen_protocol::permissions::FileSystemAccessMode;
use codewen_protocol::permissions::FileSystemPath;
use codewen_protocol::permissions::FileSystemSandboxEntry;
use codewen_protocol::permissions::FileSystemSandboxPolicy;
use codewen_protocol::permissions::FileSystemSpecialPath;
use codewen_protocol::protocol::NonSteerableTurnKind;
use codewen_protocol::protocol::SandboxPolicy;
use codewen_protocol::protocol::TurnEnvironmentSelections;
use codewen_protocol::request_permissions::PermissionGrantScope;
use codewen_protocol::request_permissions::RequestPermissionProfile;
use codewen_utils_path_uri::PathUri;
use tracing::Span;
...
use codewen_config::config_toml::ConfigToml;
use codewen_config::config_toml::ProjectConfig;
use codewen_config::permissions_toml::FilesystemPermissionToml;
use codewen_config::permissions_toml::FilesystemPermissionsToml;
use codewen_config::permissions_toml::NetworkToml;
use codewen_config::permissions_toml::PermissionProfileToml;
use codewen_config::permissions_toml::PermissionsToml;
use codewen_execpolicy::Decision;
use codewen_execpolicy::NetworkRuleProtocol;
use codewen_execpolicy::Policy;
use codewen_network_proxy::NetworkProxyConfig;
use codewen_otel::MetricsClient;
use codewen_otel::MetricsConfig;
use codewen_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use codewen_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use codewen_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use codewen_otel::THREAD_SKILLS_TRUNCATED_METRIC;
use codewen_otel::TelemetryAuthMode;
...
use codewen_protocol::items::HookPromptFragment;
use codewen_protocol::items::build_hook_prompt_message;
use codewen_protocol::models::BaseInstructions;
use codewen_protocol::models::ContentItem;
use codewen_protocol::models::InternalChatMessageMetadataPassthrough;
use codewen_protocol::models::ResponseItem;
use codewen_protocol::protocol::AskForApproval;
use codewen_protocol::protocol::CodewenErrorInfo;
use codewen_protocol::protocol::CompactedItem;
use codewen_protocol::protocol::ConversationAudioParams;
use codewen_protocol::protocol::CreditsSnapshot;
use codewen_protocol::protocol::GranularApprovalConfig;
use codewen_protocol::protocol::InitialHistory;
use codewen_protocol::protocol::InterAgentCommunication;
use codewen_protocol::protocol::MultiAgentVersion;
use codewen_protocol::protocol::NetworkApprovalProtocol;
use codewen_protocol::protocol::RateLimitSnapshot;
use codewen_protocol::protocol::RateLimitWindow;
use codewen_protocol::protocol::RealtimeAudioFrame;
use codewen_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use codewen_protocol::protocol::RealtimeVoice;
...
use codewen_protocol::protocol::SessionMeta;
use codewen_protocol::protocol::SessionMetaLine;
use codewen_protocol::protocol::SkillScope;
use codewen_protocol::protocol::Submission;
use codewen_protocol::protocol::ThreadRolledBackEvent;
use codewen_protocol::protocol::ThreadSettingsOverrides;
use codewen_protocol::protocol::TokenCountEvent;
use codewen_protocol::protocol::TokenUsage;
use codewen_protocol::protocol::TokenUsageInfo;
use codewen_protocol::protocol::TurnAbortedEvent;
use codewen_protocol::protocol::TurnCompleteEvent;
use codewen_protocol::protocol::TurnStartedEvent;
use codewen_protocol::protocol::UserMessageEvent;
use codewen_protocol::protocol::W3cTraceContext;
use codewen_rmcp_client::ElicitationAction;
...
use core_test_support::test_codewen::local;
...
use codewen_protocol::mcp::CallToolResult as McpCallToolResult;
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("codewen-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        codewen_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        codewen_home.join(codewen_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<codewen_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&codewen_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: codewen_hooks::StartHookTarget::SessionStart {
                source: codewen_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn codewen_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { codewen_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<()> for ProbeToolRuntime {
        ...
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: codewen_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
                #[allow(deprecated)]
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> codewen_sandboxing::SandboxablePreference {
            codewen_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(codewen_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: codewen_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let codewen_home = session.codewen_home().await;
    std::fs::create_dir_all(&codewen_home).expect("create codewen home");
    let config_toml_path = codewen_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = codewen_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let codewen_home = session.codewen_home().await;
    std::fs::create_dir_all(&codewen_home).expect("create codewen home");
    let base_config_path = codewen_home.join(CONFIG_TOML_FILE);
    let profile_config_path = codewen_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .codewen_home(codewen_home.to_path_buf())
    ...
    assert_eq!(
        config
            ...
            .map(codewen_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    ...
    .await?;
    let codewen_home = session.codewen_home().await;
    std::fs::create_dir_all(&codewen_home)?;
    let config_toml_path = codewen_home.join(CONFIG_TOML_FILE);
    let user_config: codewen_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let config = session.get_config().await;
    let hook_list = codewen_hooks::list_hooks(codewen_hooks::HooksConfig {
        feature_enabled: true,
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: codewen_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: codewen_config::MatcherGroup {
                matcher: None,
                hooks: vec![codewen_config::HookHandlerConfig::Command {
        ...
        let identity = codewen_config::TomlValue::try_from(identity)?;
...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    ...
    std::fs::create_dir_all(&codewen_home).expect("create codewen home");
    std::fs::write(
        codewen_home.join(CONFIG_TOML_FILE),
        r#"[apps.calendar]
enabled = false
destructive_enabled = false
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl codewen_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a codewen_extension_api::ExtensionData,
            thread_store: &'a codewen_extension_api::ExtensionData,
            turn_store: &'a codewen_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> codewen_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl codewen_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: codewen_extension_api::TurnStartInput<'a>,
        ) -> codewen_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl codewen_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: codewen_extension_api::TurnErrorInput<'a>,
        ) -> codewen_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: CodewenErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, CodewenErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl codewen_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &codewen_extension_api::ExtensionData,
            thread_store: &codewen_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: codewen_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_codewen().with_config(move |config| {
        config
            .permissions
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codewen().with_config(|config| {
        config.permissions.approval_policy =
            codewen_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        codewen_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "codewen_core__codewen_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: codewen_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            codewen_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            codewen_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            codewen_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.codewen_error_info,
        Some(CodewenErrorInfo::ThreadRollbackFailed)
    );
    assert_eq!(
...
async fn set_rate_limits_retains_previous_credits() {
    let codewen_home = tempfile::tempdir().expect("create temp dir");
    ...
    let session_configuration = SessionConfiguration {
        ...
        codewen_home: config.codewen_home.clone(),
        thread_name: None,
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(codewen_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("codewen_other".to_string()),
        limit_name: Some("codewen_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(codewen_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("codewen".to_string()),
            limit_name: None,
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.codewen_error_info == Some(CodewenErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(codewen_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(CodewenAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("codewen-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "codewen_thread_initialized")
                    }) {
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = codewen_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        codewen_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let codewen_home = tempfile::tempdir().expect("create codewen home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        codewen_home.path().join(codewen_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let codewen_home = parent_config.codewen_home.clone();
    let skill_dir = codewen_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&codewen_exec_server::LOCAL_FS));
    ...
    let role_path = codewen_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.codewen_home.to_path_buf()));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(codewen_code_mode::InProcessCodeModeSessionProvider),
        Arc::new(codewen_extension_api::ExtensionRegistryBuilder::new().build()),
        codewen_extension_api::ExtensionDataInit::default(),
        /*supports_openai_form_elicitation*/ false,
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(codewen_thread_store::LocalThreadStore::new(
            codewen_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        codewen_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(codewen_mcp::McpRuntime::new(
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: codewen_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(codewen_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: codewen_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: codewen_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: codewen_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(codewen_thread_store::LocalThreadStore::new(
            codewen_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .codewen_home(config.codewen_home.to_path_buf())
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(codewen_thread_store::LocalThreadStore::new(
            codewen_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                codewen_state::StateRuntime::init(
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            codewen_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(codewen_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(codewen_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            codewen_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(codewen_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = codewen_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(codewen_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    codewen_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(codewen_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: codewen_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(codewen_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            thread_settings: codewen_protocol::protocol::ThreadSettingsOverrides {
                ...
                approvals_reviewer: Some(codewen_config::types::ApprovalsReviewer::AutoReview),
                sandbox_policy: Some(config.legacy_sandbox_policy()),
                summary: config.model_reasoning_summary,
                personality: config.personality,
                collaboration_mode: Some(codewen_protocol::config_types::CollaborationMode {
                    mode: codewen_protocol::config_types::ModeKind::Default,
                    settings: codewen_protocol::config_types::Settings {
    ...
    assert_eq!(
        ...
        codewen_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = codewen_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(codewen_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn codewen_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        codewen_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl codewen_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: codewen_extension_api::TurnAbortInput<'a>,
        ) -> codewen_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(codewen_home).await;
...
async fn refresh_mcp_servers_keeps_the_previous_runtime_alive() {
    ...
    assert!(Arc::ptr_eq(&step_context.mcp, &old_runtime));
    assert_eq!(
        codewen_mcp::configured_mcp_servers(new_runtime.config()),
...
impl codewen_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
...
async fn deferred_environment_roots_refresh_plugin_availability() {
    ...
    impl codewen_extension_api::McpServerContributor<Config> for ReadyPluginContributor {
        ...
        ) -> codewen_extension_api::ExtensionFuture<'a, Vec<codewen_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                available
                    .then(
                        || codewen_extension_api::McpServerContribution::SelectedPluginPackage {
    ...
    let (mut session, turn_context) = make_session_and_context().await;
    let mut registry = codewen_extension_api::ExtensionRegistryBuilder::new();
    ...
    session.services.mcp_manager = Arc::new(McpManager::new_with_extensions(
        ...
        registry,
        crate::CodewenAppsToolsCache::default(),
    ));
    let session = Arc::new(session);
    session
    ...
    let selected_root = codewen_protocol::capabilities::SelectedCapabilityRoot {
        id: "skill-only".to_string(),
        location: codewen_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    registration
        .complete(Ok(codewen_exec_server::EnvironmentReadyInfo {
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    let environment_manager = session.services.turn_environments.environment_manager();
    let selected_root =
        |environment_id: &str, path: &str| codewen_protocol::capabilities::SelectedCapabilityRoot {
    ...
    for selected_root in &selected_roots {
        let codewen_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
...
impl codewen_extension_api::ContextContributor for PromptExtensionTestContributor {
    ...
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    codewen_extension_api::PromptFragment::developer_policy(
...
-> Arc<codewen_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = codewen_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl codewen_extension_api::ContextContributor for TurnContextExtensionTestContributor {
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<codewen_thread_store::InMemoryThreadStore> {
    let store = Arc::new(codewen_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn codewen_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &codewen_thread_store::InMemoryThreadStore,
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = codewen_protocol::user_input::TextElement::new(
        codewen_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(codewen_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use codewen_protocol::protocol::AskForApproval;
    use codewen_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: codewen_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: codewen_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let codewen_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_codex = project_root.join(".codewen");
    let nested_dot_codex = nested.join(".codewen");
    ...
    write_project_trust_config(&codewen_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .codewen_home(codewen_home)
    ...
    let expected_source_path = codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_codex = project_root.join(".codewen");
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let codewen_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&codewen_home)?;
        write_project_trust_config(&codewen_home, &trust_entries).await?;
