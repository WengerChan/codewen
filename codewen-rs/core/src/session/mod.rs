...
use codewen_analytics::SubAgentThreadStartedInput;
use codewen_analytics::TurnCodexErrorFact;
use codewen_config::types::AuthKeyringBackendKind;
use codewen_config::types::OAuthCredentialsStoreMode;
use codewen_connectors::connector_runtime_context_key;
use codewen_core_skills::injection::HostSkillsCatalogInWorldState;
use codewen_exec_server::Environment;
use codewen_exec_server::EnvironmentManager;
use codewen_execpolicy::prefix_rule_migration;
use codewen_extension_api::ExtensionDataInit;
use codewen_extension_api::LoadedUserInstructions;
use codewen_extension_api::PromptFragment;
use codewen_extension_api::PromptSlot;
use codewen_extension_api::TurnContextContributionInput;
use codewen_features::FEATURES;
use codewen_features::Feature;
use codewen_features::unstable_features_warning_event;
use codewen_hooks::Hooks;
use codewen_hooks::HooksConfig;
use codewen_login::AuthManager;
use codewen_login::CodewenAuth;
use codewen_login::auth_env_telemetry::collect_auth_env_telemetry;
use codewen_mcp::McpConnectionManager;
use codewen_mcp::McpResourceClient;
use codewen_mcp::McpRuntime;
...
use codewen_network_proxy::NetworkProxy;
use codewen_network_proxy::NetworkProxyAuditMetadata;
use codewen_network_proxy::normalize_host;
use codewen_otel::current_span_trace_id;
use codewen_otel::current_span_w3c_trace_context;
use codewen_otel::set_parent_from_w3c_trace_context;
use codewen_protocol::SessionId;
use codewen_protocol::ThreadId;
use codewen_protocol::approvals::ElicitationRequest;
use codewen_protocol::approvals::ElicitationRequestEvent;
use codewen_protocol::approvals::ExecPolicyAmendment;
use codewen_protocol::approvals::NetworkPolicyAmendment;
use codewen_protocol::approvals::NetworkPolicyRuleAction;
use codewen_protocol::config_types::ApprovalsReviewer;
use codewen_protocol::config_types::AutoCompactTokenLimitScope;
...
use codewen_protocol::config_types::WebSearchMode;
use codewen_protocol::dynamic_tools::DynamicToolResponse;
use codewen_protocol::dynamic_tools::DynamicToolSpec;
use codewen_protocol::items::EnteredReviewModeItem;
use codewen_protocol::items::TurnItem;
use codewen_protocol::items::UserMessageItem;
use codewen_protocol::models::ActivePermissionProfile;
use codewen_protocol::models::AdditionalPermissionProfile;
...
use codewen_protocol::models::format_allow_prefixes;
...
use codewen_protocol::protocol::AdditionalContextEntry;
use codewen_protocol::protocol::FileChange;
use codewen_protocol::protocol::HasLegacyEvent;
...
use codewen_protocol::protocol::RawResponseItemEvent;
...
use codewen_protocol::protocol::ThreadHistoryMode;
use codewen_protocol::protocol::ThreadSource;
use codewen_protocol::protocol::TurnAbortReason;
use codewen_protocol::protocol::TurnContextItem;
use codewen_protocol::protocol::TurnContextNetworkItem;
...
use codewen_protocol::protocol::WorldStateItem;
...
use codewen_protocol::request_permissions::RequestPermissionsArgs;
use codewen_protocol::request_permissions::RequestPermissionsEvent;
use codewen_protocol::request_permissions::RequestPermissionsResponse;
use codewen_protocol::request_user_input::RequestUserInputArgs;
use codewen_protocol::request_user_input::RequestUserInputResponse;
use codewen_rmcp_client::ElicitationResponse;
use codewen_rollout::state_db;
use codewen_rollout_trace::AgentResultTracePayload;
use codewen_rollout_trace::ThreadStartedTraceMetadata;
use codewen_rollout_trace::ThreadTraceContext;
use codewen_sandboxing::policy_transforms::intersect_permission_profiles;
use codewen_shell_command::parse_command::parse_command;
use codewen_terminal_detection::user_agent;
use codewen_thread_store::CreateThreadParams;
use codewen_thread_store::LiveThread;
use codewen_thread_store::LiveThreadInitGuard;
use codewen_thread_store::LocalThreadStore;
use codewen_thread_store::ReadThreadParams;
use codewen_thread_store::ResumeThreadParams;
use codewen_thread_store::ThreadPersistenceMetadata;
...
use crate::codewen_thread::ThreadConfigSnapshot;
#[cfg(test)]
...
#[cfg(test)]
use codewen_protocol::exec_output::StreamOutput;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                codewen_error_info: Some(CodewenErrorInfo::BadRequest),
            },
...
use codewen_core_plugins::RecommendedPluginCandidatesInput;
use codewen_git_utils::get_git_repo_root;
use codewen_mcp::McpConfig;
use codewen_mcp::effective_mcp_servers;
use codewen_otel::SessionTelemetry;
use codewen_otel::THREAD_STARTED_METRIC;
...
use codewen_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use codewen_protocol::config_types::WindowsSandboxLevel;
use codewen_protocol::models::LocalImagePreparation;
use codewen_protocol::models::ResponseInputItem;
use codewen_protocol::models::ResponseItem;
use codewen_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use codewen_protocol::protocol::ApplyPatchApprovalRequestEvent;
...
use codewen_protocol::protocol::DeprecationNoticeEvent;
use codewen_protocol::protocol::ErrorEvent;
...
use codewen_protocol::protocol::McpServerRefreshConfig;
use codewen_protocol::protocol::ModelRerouteEvent;
use codewen_protocol::protocol::ModelRerouteReason;
use codewen_protocol::protocol::ModelVerification;
use codewen_protocol::protocol::ModelVerificationEvent;
use codewen_protocol::protocol::NetworkApprovalContext;
...
use codewen_protocol::protocol::RequestUserInputEvent;
...
use codewen_protocol::protocol::SessionNetworkProxyRuntime;
use codewen_protocol::protocol::StreamErrorEvent;
use codewen_protocol::protocol::Submission;
use codewen_protocol::protocol::ThreadMemoryMode;
...
use codewen_protocol::protocol::TurnModerationMetadataEvent;
use codewen_protocol::protocol::WarningEvent;
use codewen_protocol::user_input::UserInput;
use codewen_tools::UnifiedExecShellMode;
use codewen_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use codewen_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn codewen_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<codewen_extension_api::ExtensionRegistry<crate::config::Config>>,
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/codewen/concepts/cyber-safety";

impl Session {
    /// Spawn and initialize a new session.
    pub(crate) async fn spawn(args: SessionSpawnArgs) -> CodewenResult<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if codewen_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodewenResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            {
                let codewen_home = config.codewen_home.clone();
                let policy_path = default_policy_path(codewen_home.as_path());
                if let Err(err) = prefix_rule_migration(
                    codewen_home.as_path(),
                    policy_path.as_path(),
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| CodewenErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            codewen_models_manager::manager::RefreshStrategy::Offline
        } else {
            codewen_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.codewen_home)
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> CodewenResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> CodewenResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| CodewenErr::InternalAgentDied)?;
        Ok(())
    }

    pub(crate) async fn shutdown_and_wait(&self) -> CodewenResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(CodewenErr::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> CodewenResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> CodewenResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-codewen-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &codewen_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn codewen_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn codewen_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn codewen_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.codewen_home().clone()
    }
    ...
    /// Record a terminal CodewenErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_codewen_error(&self, turn_context: &TurnContext, error: &CodewenErr) {
        self.services
            .analytics_events_client
            .track_turn_codewen_error(TurnCodexErrorFact::from_codewen_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .codewen_error_info
                .as_ref()
                .is_some_and(CodewenErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_codewen_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &codewen_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| codewen_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&codewen_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                codewen_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-codewen, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(codewen_hooks::SessionStartSource::Compact);
        }
    }
    ...
    pub(crate) async fn build_initial_context_with_world_state(
        ...
        let step_store = codewen_extension_api::ExtensionData::new(turn_context.sub_id.clone());
    ...
    pub(crate) async fn build_initial_context_with_world_state_and_mcp(
        ...
        step_store: &codewen_extension_api::ExtensionData,
    ) -> Vec<ResponseItem> {
    ...
    pub(crate) async fn notify_stream_error(
        ...
        codewen_error: CodewenErr,
    ) {
        let additional_details = codewen_error.to_string();
        let codewen_error_info = CodewenErrorInfo::ResponseStreamDisconnected {
            http_status_code: codewen_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<codewen_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::CodewenHooks),
