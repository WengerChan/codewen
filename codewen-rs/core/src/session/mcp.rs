...
use codewen_exec_server::ExecutorCapabilityDiscoveryCache;
use codewen_exec_server::ExecutorCapabilityDiscoverySnapshot;
use codewen_exec_server::MAX_SELECTED_CAPABILITY_ROOTS;
use codewen_exec_server::ResolvedSelectedCapabilityRoot;
use codewen_mcp::ElicitationReviewRequest;
use codewen_mcp::ElicitationReviewer;
use codewen_mcp::ElicitationReviewerHandle;
use codewen_protocol::capabilities::CapabilityRootLocation;
use codewen_protocol::capabilities::SelectedCapabilityRoot;
use codewen_protocol::config_types::ApprovalsReviewer;
use codewen_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use codewen_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use codewen_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use codewen_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use codewen_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use codewen_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use codewen_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use codewen_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use codewen_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use codewen_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use codewen_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use codewen_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use codewen_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use codewen_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        codewen_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> codewen_mcp::ElicitationLifecycle {
        let elicitations = self.services.elicitations.clone();
        codewen_mcp::ElicitationLifecycle::new(move || elicitations.register())
    }
    ...
    pub async fn request_mcp_server_elicitation(
            ...
            .elicitations_auto_deny()
        {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: codewen_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                codewen_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                codewen_protocol::mcp::RequestId::Integer(value)
            }
        };
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let tool_plugin_provenance = codewen_mcp::tool_plugin_provenance(&mcp_config);
        ...
        let codewen_apps_auth_manager =
            codewen_mcp::host_owned_codewen_apps_enabled(&mcp_config, auth.as_ref())
        ...
        let refreshed_manager = McpConnectionManager::new(
            ...
            mcp_config.codewen_home.clone(),
            self.services.mcp_manager.codewen_apps_tools_cache(),
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let codewen_protocol::approvals::ElicitationRequest::Form {
