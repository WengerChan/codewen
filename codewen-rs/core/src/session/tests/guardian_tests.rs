...
use codewen_protocol::models::AdditionalPermissionProfile as PermissionProfile;
...
use core_test_support::codewen_linux_sandbox_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                codewen_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    turn_context_raw.permission_profile = codewen_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config
        .permissions
        .set_permission_profile(codewen_protocol::models::PermissionProfile::Disabled)
        .expect("test setup should allow disabling the permission profile");
    config.codewen_linux_sandbox_exe = codewen_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        codewen_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: codewen_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let codewen_home = tempdir().expect("create codewen home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_codewen_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(codewen_thread_store::LocalThreadStore::new(
        codewen_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(codewen_code_mode::InProcessCodeModeSessionProvider),
        extensions: codewen_extension_api::empty_extension_registry(),
        ...
        parent_rollout_thread_trace: codewen_rollout_trace::ThreadTraceContext::disabled(),
        user_shell_override: None,
        parent_trace: None,
        environment_selections: Vec::new(),
        thread_extension_init: codewen_extension_api::ExtensionDataInit::default(),
