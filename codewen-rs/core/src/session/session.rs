...
pub(crate) struct SessionConfiguration {
    ...
    pub(super) codewen_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    ...
    pub(crate) fn codewen_home(&self) -> &AbsolutePathBuf {
        &self.codewen_home
    }
    ...
    pub(super) fn sandbox_policy(&self) -> SandboxPolicy {
        ...
        codewen_sandboxing::compatibility_sandbox_policy_for_permission_profile(
            &permission_profile,
            self.cwd(),
        )
    }
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: codewen_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        config: Arc<Config>,
        user_instructions: Option<codewen_extension_api::UserInstructions>,
        ...
        code_mode_session_provider: Arc<dyn codewen_code_mode::CodeModeSessionProvider>,
        extensions: Arc<codewen_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        thread_extension_init.insert(codewen_extension_api::ThreadOriginator(
            session_configuration.originator.clone(),
        ));
        let mcp_thread_init = thread_extension_init.clone();
        let thread_extension_data = codewen_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let auth_and_mcp_fut = async move {
            ...
            let mcp_servers = codewen_mcp::effective_mcp_servers(mcp_config, auth.as_ref());
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let trace_agent_path = session_configuration
                .session_source
                .get_agent_path()
                .unwrap_or_else(codewen_protocol::AgentPath::root);
            let trace_task_name =
            ...
            let config_path = config.codewen_home.join(CONFIG_TOML_FILE);
            ...
            let auth_env_telemetry = collect_auth_env_telemetry(
                &session_configuration.provider,
                auth_manager.codewen_api_key_env_enabled(),
            );
            ...
            let session_extension_data =
                codewen_extension_api::ExtensionData::new(session_id.to_string());
            session_extension_data.insert(McpResourceClient::new(Arc::clone(&mcp_runtime)));
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(codewen_extension_api::ThreadStartInput {
            ...
            let codewen_apps_auth_manager =
                codewen_mcp::host_owned_codewen_apps_enabled(&mcp_projection.config, auth)
            ...
            let mcp_connection_manager = McpConnectionManager::new(
                ...
                sess.services.mcp_manager.codewen_apps_tools_cache(),
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => codewen_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    codewen_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => codewen_hooks::SessionStartSource::Clear,
            };
