...
async fn exec_full_buffer_capture_ignores_expiration() -> Result<()> {
    ...
    let output = exec(
        ExecParams {
            command,
            cwd: codewen_utils_absolute_path::AbsolutePathBuf::current_dir()?,
            expiration: 1.into(),
...
async fn process_exec_tool_call_preserves_full_buffer_capture_policy() -> Result<()> {
    ...
    let cwd = codewen_utils_absolute_path::AbsolutePathBuf::current_dir()?;
...
fn windows_restricted_token_rejects_managed_root_write_profiles() {
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        codewen_protocol::permissions::FileSystemSandboxEntry {
            ...
            access: codewen_protocol::permissions::FileSystemAccessMode::Write,
...
fn windows_elevated_allows_split_restricted_read_policies() {
    ...
    let docs = codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        codewen_protocol::permissions::FileSystemSandboxEntry {
            path: codewen_protocol::permissions::FileSystemPath::Path { path: docs },
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        codewen_protocol::permissions::FileSystemSandboxEntry {
            path: codewen_protocol::permissions::FileSystemPath::Path {
                path: codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                    .expect("absolute docs"),
            },
            access: codewen_protocol::permissions::FileSystemAccessMode::Read,
            missing_path_behavior: None,
...
fn windows_restricted_token_supports_full_read_split_write_read_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        codewen_protocol::permissions::FileSystemSandboxEntry {
            path: codewen_protocol::permissions::FileSystemPath::Path { path: docs.clone() },
            access: codewen_protocol::permissions::FileSystemAccessMode::Read,
            missing_path_behavior: None,
        },
    ]);
    ...
    // `.codewen`, so the restricted-token overlay only needs the extra read-only
    // docs carveout.
...
fn windows_restricted_token_rejects_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        codewen_protocol::permissions::FileSystemSandboxEntry {
            path: codewen_protocol::permissions::FileSystemPath::Path { path: blocked },
...
fn windows_elevated_supports_split_write_read_carveouts() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            additional_deny_read_paths: vec![],
            additional_deny_write_paths: vec![
                codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_docs)
                    .expect("absolute docs"),
            ],
        }))
    );
...
fn windows_workspace_defaults_do_not_hide_explicit_metadata_carveouts() {
    ...
    for name in codewen_protocol::permissions::PROTECTED_METADATA_PATH_NAMES {
...
fn windows_elevated_supports_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        codewen_protocol::permissions::FileSystemSandboxEntry {
            path: codewen_protocol::permissions::FileSystemPath::Path {
                path: codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&blocked)
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
                    expected_blocked.clone(),
                )
                .expect("absolute blocked"),
            ],
            additional_deny_write_paths: vec![
                codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_blocked)
                    .expect("absolute blocked"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_globs() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(secret)
...
fn windows_elevated_rejects_reopened_writable_descendants() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        codewen_protocol::permissions::FileSystemSandboxEntry {
            path: codewen_protocol::permissions::FileSystemPath::Path {
                path: codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&nested)
...
fn process_exec_tool_call_uses_platform_sandbox_for_network_only_restrictions() {
    let expected = codewen_sandboxing::get_platform_sandbox(/*windows_sandbox_enabled*/ false)
    ...
    assert_eq!(
        select_process_exec_tool_sandbox_type(
            &FileSystemSandboxPolicy::unrestricted(),
            NetworkSandboxPolicy::Restricted,
            codewen_protocol::config_types::WindowsSandboxLevel::Disabled,
...
async fn kill_child_process_group_kills_grandchildren_on_timeout() -> Result<()> {
    ...
    let params = ExecParams {
        ...
        network_environment_id: None,
        sandbox_permissions: SandboxPermissions::UseDefault,
        windows_sandbox_level: codewen_protocol::config_types::WindowsSandboxLevel::Disabled,
        windows_sandbox_private_desktop: false,
        justification: None,
        arg0: None,
    };
