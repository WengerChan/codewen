...
use codewen_protocol::openai_models::ToolMode;
use codewen_protocol::protocol::MultiAgentVersion;
use codewen_tools::ResponsesApiNamespace;
use codewen_tools::ResponsesApiNamespaceTool;
use codewen_tools::TOOL_SEARCH_TOOL_NAME;
use codewen_tools::ToolCall as ExtensionToolCall;
use codewen_tools::ToolEnvironmentMode;
use codewen_tools::ToolExecutor;
...
use codewen_tools::can_request_original_image_detail;
use codewen_tools::collect_code_mode_exec_prompt_tool_definitions;
use codewen_tools::collect_request_plugin_install_entries;
use codewen_tools::default_namespace_description;
use codewen_tools::request_user_input_available_modes;
use codewen_tools::shell_command_backend_for_features;
use codewen_tools::shell_type_for_model_and_features;
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && codewen_code_mode::is_code_mode_nested_tool(spec.name())
    {
        codewen_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    provider.uses_openai_actor_authorization()
        || (provider.requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_codewen_backend))
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure != ToolExposure::DirectModelOnly
        && codewen_code_mode::is_code_mode_nested_tool(
            &codewen_tools::code_mode_name_for_tool_name(tool_name),
        )
}
...
fn build_code_mode_executors(
    ...
    let default_exec_yield_time_ms =
        default_exec_yield_time_override_ms(&turn_context.config.features)
            .unwrap_or(codewen_code_mode::DEFAULT_EXEC_YIELD_TIME_MS);
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, codewen_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| codewen_code_mode::ToolNamespaceDescription {
...
fn append_extension_tool_executors(
    ...
    let tool_mode = effective_tool_mode(turn_context);
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly) {
        reserved_tool_names.insert(ToolName::plain(codewen_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(codewen_code_mode::WAIT_TOOL_NAME));
    }
    if search_tool_enabled(turn_context)
        && planned_tools
            .runtimes()
...
fn compare_code_mode_tools(
    left: &codewen_code_mode::ToolDefinition,
    right: &codewen_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, codewen_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &codewen_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, codewen_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
