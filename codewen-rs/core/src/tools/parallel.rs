...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, CodewenErr>> {
        ...
        async move {
            match future.await {
                ...
                Err(FunctionCallError::Fatal(message)) => Err(CodewenErr::Fatal(message)),
    ...
    fn failure_response(call: ToolCall, err: FunctionCallError) -> ResponseInputItem {
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                call_id: call.call_id,
                name: None,
                output: codewen_protocol::models::FunctionCallOutputPayload {
                    body: codewen_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
    ...
    fn drop(&mut self) {
        ...
        macro_rules! log_tool_call {
            ($dispatch_duration_ms:expr, $handler_duration_ms:expr, $total_duration_ms:expr) => {
                info!(
                    event.name = "codewen.tool_call",
                    trace_id = %codewen_otel::current_span_trace_id().unwrap_or_default(),
...
mod tests {
    ...
    fn tool_call_timing_guard_ignores_code_mode_source() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            let call = ToolCall {
                tool_name: codewen_tools::ToolName::plain("test_tool"),
    ...
    async fn cancellation_before_dispatch_admission_logs_dispatch_only_timing() -> anyhow::Result<()>
    {
        ...
        let tool_name = codewen_tools::ToolName::plain("test_tool");
        ...
        let timing_events = logs
            .lines()
            .filter(|line| line.contains("event.name=\"codewen.tool_call\""))
            .collect::<Vec<_>>();
        assert_eq!(
            timing_events.len(),
            1,
    ...
    impl codewen_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl codewen_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: codewen_extension_api::ToolFinishInput<'a>,
        ) -> codewen_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = codewen_tools::ToolName::plain("cleanup_tool");
