...
use codewen_analytics::PluginInstallRequestSource;
use codewen_analytics::PluginInstallRequested;
use codewen_analytics::PluginInstallRequestedPlugin;
...
use codewen_tools::DiscoverableToolAction;
use codewen_tools::DiscoverableToolType;
use codewen_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use codewen_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use codewen_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use codewen_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use codewen_tools::RequestPluginInstallArgs;
use codewen_tools::RequestPluginInstallResult;
use codewen_tools::ToolName;
use codewen_tools::ToolSpec;
use codewen_tools::all_requested_connectors_picked_up;
use codewen_tools::build_request_plugin_install_elicitation_request;
use codewen_tools::filter_request_plugin_install_discoverable_tools_for_client;
use codewen_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("codewen-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in codewen-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.codewen_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    codewen_home: &codewen_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    session: &crate::session::session::Session,
    turn: &crate::session::turn_context::TurnContext,
    manager: &codewen_mcp::McpConnectionManager,
    tool: &DiscoverableTool,
    auth: Option<&codewen_login::CodewenAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match manager.hard_refresh_codewen_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh codewen apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
