...
pub(crate) fn collab_spawn_error(err: CodewenErr) -> FunctionCallError {
    match err {
        CodewenErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        CodewenErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: CodewenErr) -> FunctionCallError {
    match err {
        CodewenErr::ThreadNotFound(id) => {
