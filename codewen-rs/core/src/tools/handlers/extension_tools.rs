...
use codewen_tools::ConversationHistory;
use codewen_tools::ExtensionTurnItem;
use codewen_tools::ToolCall as ExtensionToolCall;
use codewen_tools::ToolEnvironment;
use codewen_tools::ToolName;
use codewen_tools::ToolSearchInfo;
use codewen_tools::ToolSpec;
use codewen_tools::TurnItemEmissionFuture;
use codewen_tools::TurnItemEmitter;
use codewen_utils_string::to_ascii_json_string;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn codewen_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn codewen_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
impl ToolExecutor<ToolInvocation> for ExtensionToolAdapter {
    ...
    fn handle(&self, invocation: ToolInvocation) -> codewen_tools::ToolExecutorFuture<'_> {
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall {
    ...
    let codewen_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use codewen_extension_items::web_search::WebSearchItem;
    use codewen_protocol::items::TurnItem;
    ...
    impl codewen_extension_api::ToolExecutor<codewen_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> codewen_tools::ToolName {
            codewen_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> codewen_tools::ToolSpec {
            codewen_tools::ToolSpec::Function(codewen_tools::ResponsesApiTool {
                name: "extension_echo".to_string(),
                description: "Echoes arguments.".to_string(),
                strict: true,
                parameters: codewen_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(&self, _call: codewen_tools::ToolCall) -> codewen_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(codewen_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn codewen_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<codewen_tools::ToolCall>>>,
    }

    impl codewen_extension_api::ToolExecutor<codewen_tools::ToolCall> for CapturingExtensionExecutor {
        fn tool_name(&self) -> codewen_tools::ToolName {
            codewen_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> codewen_tools::ToolSpec {
            codewen_tools::ToolSpec::Function(codewen_tools::ResponsesApiTool {
                ...
                strict: false,
                parameters: codewen_tools::JsonSchema::default(),
                output_schema: None,
                defer_loading: None,
            })
        }

        fn handle(&self, call: codewen_tools::ToolCall) -> codewen_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: codewen_tools::ToolCall,
        ) -> Result<Box<dyn codewen_tools::ToolOutput>, codewen_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: codewen_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = codewen_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        codewen_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        codewen_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
