...
use codewen_apply_patch::ApplyPatchAction;
use codewen_apply_patch::ApplyPatchFileChange;
use codewen_apply_patch::Hunk;
use codewen_apply_patch::StreamingPatchParser;
...
use codewen_protocol::protocol::PatchApplyUpdatedEvent;
use codewen_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use codewen_sandboxing::policy_transforms::merge_permission_profiles;
use codewen_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn format_update_chunks_for_progress(chunks: &[codewen_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &codewen_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    codewen_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
fn patch_permissions_without_path_matching(
    ...
    (
        ...
        codewen_protocol::permissions::FileSystemSandboxPolicy::unrestricted(),
    )
}
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match codewen_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match codewen_apply_patch::verify_apply_patch_args(
            args,
        ...
        .await
        {
            codewen_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            codewen_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            codewen_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            codewen_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match codewen_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs, Some(&sandbox))
        .await
    {
        codewen_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
        ...
        codewen_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
