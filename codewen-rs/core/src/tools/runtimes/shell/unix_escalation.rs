...
use codewen_execpolicy::MatchOptions;
...
use codewen_protocol::protocol::NetworkPolicyRuleAction;
...
use codewen_shell_command::bash::parse_shell_lc_plain_commands;
use codewen_shell_command::bash::parse_shell_lc_single_command_prefix;
use codewen_shell_escalation::EscalateServer;
use codewen_shell_escalation::EscalationDecision;
use codewen_shell_escalation::EscalationExecution;
use codewen_shell_escalation::EscalationPermissions;
use codewen_shell_escalation::EscalationPolicy;
use codewen_shell_escalation::EscalationPolicyFuture;
use codewen_shell_escalation::EscalationSession;
use codewen_shell_escalation::ExecParams;
use codewen_shell_escalation::ExecResult;
use codewen_shell_escalation::PreparedExec;
use codewen_shell_escalation::ResolvedPermissionProfile;
use codewen_shell_escalation::ShellCommandExecutor;
use codewen_shell_escalation::ShellCommandExecutorFuture;
use codewen_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        codewen_linux_sandbox_exe: ctx.turn.config.codewen_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    codewen_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<codewen_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["CODEWEN_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            codewen_linux_sandbox_exe: self.codewen_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Codex(CodewenErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::Codex(CodewenErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
