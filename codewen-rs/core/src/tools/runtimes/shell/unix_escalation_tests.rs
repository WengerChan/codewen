...
use codewen_execpolicy::PolicyParser;
use codewen_execpolicy::RuleMatch;
use codewen_hooks::Hooks;
use codewen_hooks::HooksConfig;
use codewen_network_proxy::PROXY_ACTIVE_ENV_KEY;
use codewen_network_proxy::PROXY_ENV_KEYS;
...
use codewen_sandboxing::policy_transforms::effective_permission_profile;
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(codewen_execpolicy::Policy::empty())),
        ...
        stopwatch: codewen_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = codewen_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = codewen_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.codewen_home)
        .context("recreate codewen home for hook fixtures")?;
    ...
    let config_toml_path = turn_context
        .config
        .codewen_home
        .join(codewen_config::CONFIG_TOML_FILE);
    ...
    let action = tokio::time::timeout(
        ...
        codewen_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        codewen_shell_escalation::EscalationDecision::Escalate(
            codewen_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, codewen_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        codewen_shell_escalation::EscalationDecision::Deny {
