...
use codewen_code_mode::CellId;
use codewen_code_mode::CodeModeNestedToolCall;
use codewen_code_mode::CodeModeSession;
use codewen_code_mode::CodeModeSessionProvider;
use codewen_code_mode::CodeModeToolKind;
use codewen_code_mode::RuntimeResponse;
...
use codewen_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use codewen_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = codewen_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = codewen_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = codewen_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) async fn execute(
        &self,
        mut request: codewen_code_mode::ExecuteRequest,
    ) -> Result<codewen_code_mode::StartedCell, String> {
    ...
    pub(crate) async fn wait(
        &self,
        request: codewen_code_mode::WaitRequest,
    ) -> Result<codewen_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &codewen_code_mode::CellId) {
...
mod tests {
    ...
    use crate::tools::context::ToolPayload;
    use codewen_code_mode::CodeModeToolKind;
    use codewen_code_mode::ExecuteRequest;
    use codewen_code_mode::FunctionCallOutputContentItem as CodeModeOutputContentItem;
    ...
    async fn missing_process_host_falls_back_to_in_process_session() {
        let service = CodeModeService::new(
            Arc::new(ProcessOwnedCodeModeSessionProvider::with_host_program(
                "codewen-code-mode-host-does-not-exist".into(),
            )),
        ...
        assert_eq!(
            response,
            RuntimeResponse::Result {
                cell_id: codewen_code_mode::CellId::new("1".to_string()),
