use codewen_code_mode::ImageDetail as CodeModeImageDetail;
...
pub(super) fn into_function_call_output_content_items(
    items: Vec<codewen_code_mode::FunctionCallOutputContentItem>,
...
impl IntoProtocol<FunctionCallOutputContentItem>
    for codewen_code_mode::FunctionCallOutputContentItem
{
    fn into_protocol(self) -> FunctionCallOutputContentItem {
        let value = self;
        match value {
            codewen_code_mode::FunctionCallOutputContentItem::InputText { text } => {
                FunctionCallOutputContentItem::InputText { text }
            }
            codewen_code_mode::FunctionCallOutputContentItem::InputImage { image_url, detail } => {
            ...
            codewen_code_mode::FunctionCallOutputContentItem::InputAudio { audio_url } => {
