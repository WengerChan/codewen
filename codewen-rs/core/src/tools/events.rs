...
use codewen_apply_patch::AppliedPatchDelta;
...
use codewen_protocol::items::CommandExecutionStatus;
...
impl ToolEmitter {
    ...
    pub async fn finish(
        ...
        let (event, result) = match out {
            ...
            Err(ToolError::Codex(CodewenErr::Sandbox(SandboxErr::Timeout { output }))) => {
                let response = self.format_exec_output_for_model(&output, ctx);
                let event = ToolEventStage::Failure(ToolEventFailure::Output(*output));
                let result = Err(FunctionCallError::RespondToModel(response));
                (event, result)
            }
            Err(ToolError::Codex(CodewenErr::Sandbox(SandboxErr::Denied { output, .. }))) => {
                let response = self.format_exec_output_for_model(&output, ctx);
...
mod tests {
    ...
    async fn assert_failed_apply_patch_tracks_committed_delta(
        ...
        let delta = codewen_apply_patch::apply_patch(
    ...
    async fn denied_apply_patch_tracks_committed_delta() {
        ...
        assert_failed_apply_patch_tracks_committed_delta(
            Err(ToolError::Codex(CodewenErr::Sandbox(SandboxErr::Denied {
