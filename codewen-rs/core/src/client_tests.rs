...
use codewen_model_provider::BearerAuthProvider;
use codewen_model_provider::SharedModelProvider;
use codewen_model_provider::create_model_provider;
use codewen_model_provider_info::CHATGPT_CODEX_BASE_URL;
...
use codewen_rollout_trace::ExecutionStatus;
...
use codewen_rollout_trace::RolloutTrace;
...
async fn compact_uses_bearer_after_agent_identity_session_fallback() -> anyhow::Result<()> {
    ...
    let auth_manager = chatgpt_auth_manager(&codewen_home, server.uri()).await;
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: codewen_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn write_chatgpt_auth_json(codewen_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn chatgpt_auth_manager(
    ...
    write_chatgpt_auth_json(codewen_home.path());
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::CodewenTurnStarted {
...
fn output_message(id: &str, text: &str) -> ResponseItem {
    ResponseItem::Message {
        id: Some(codewen_protocol::ResponseItemId::with_suffix("msg", id)),
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_codewen_responses() {
...
#[tokio::test]
async fn non_chatgpt_codewen_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
