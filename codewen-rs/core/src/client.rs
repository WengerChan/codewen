...
//! per-turn state such as the `x-codewen-turn-state` token used for sticky routing.
//!
...
use codewen_api::AgentIdentityTelemetry;
use codewen_api::ApiError;
use codewen_api::AuthProvider;
use codewen_api::CompactClient as ApiCompactClient;
use codewen_api::CompactionInput as ApiCompactionInput;
use codewen_api::Compression;
use codewen_api::MemoriesClient as ApiMemoriesClient;
use codewen_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use codewen_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use codewen_api::Provider as ApiProvider;
use codewen_api::RawMemory as ApiRawMemory;
use codewen_api::RealtimeCallClient as ApiRealtimeCallClient;
use codewen_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use codewen_api::Reasoning;
use codewen_api::ReasoningContext;
use codewen_api::RequestTelemetry;
use codewen_api::ReqwestTransport;
use codewen_api::ResponseCreateWsRequest;
use codewen_api::ResponsesApiRequest;
use codewen_api::ResponsesClient as ApiResponsesClient;
use codewen_api::ResponsesOptions as ApiResponsesOptions;
use codewen_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use codewen_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use codewen_api::ResponsesWsRequest;
use codewen_api::SharedAuthProvider;
use codewen_api::SseTelemetry;
use codewen_api::StreamOptions;
use codewen_api::TransportError;
use codewen_api::WebsocketTelemetry;
use codewen_api::auth_header_telemetry;
use codewen_api::build_session_headers;
use codewen_api::create_text_param_for_request;
use codewen_api::response_create_client_metadata;
use codewen_http_client::ClientRouteClass;
use codewen_http_client::HttpClientFactory;
use codewen_login::AuthManager;
use codewen_login::CodewenAuth;
use codewen_login::RefreshTokenError;
use codewen_login::UnauthorizedRecovery;
use codewen_login::default_client::add_originator_header;
use codewen_login::default_client::create_client_for_route;
...
use codewen_protocol::config_types::Verbosity as VerbosityConfig;
...
use codewen_protocol::protocol::InternalSessionSource;
use codewen_protocol::protocol::SessionSource;
use codewen_protocol::protocol::W3cTraceContext;
use codewen_rollout_trace::CompactionTraceContext;
use codewen_rollout_trace::InferenceTraceAttempt;
use codewen_rollout_trace::InferenceTraceContext;
use codewen_tools::create_tools_json_for_responses_api;
...
use codewen_feedback::FeedbackRequestTags;
use codewen_feedback::emit_feedback_request_tags_with_auth_env;
use codewen_login::auth::AgentIdentityAuthPolicy;
use codewen_login::auth_env_telemetry::AuthEnvTelemetry;
use codewen_login::auth_env_telemetry::collect_auth_env_telemetry;
use codewen_model_provider::AgentIdentitySessionFallback;
use codewen_model_provider::ProviderAuthScope;
use codewen_model_provider::SharedModelProvider;
use codewen_model_provider::create_model_provider;
#[cfg(test)]
use codewen_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use codewen_protocol::error::Result;
use codewen_response_debug_context::extract_response_debug_context;
use codewen_response_debug_context::extract_response_debug_context_from_api_error;
use codewen_response_debug_context::telemetry_api_error_message;
use codewen_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_CODEX_INSTALLATION_ID_HEADER: &str = "x-codewen-installation-id";
pub const X_CODEX_TURN_STATE_HEADER: &str = "x-codewen-turn-state";
pub const X_CODEX_TURN_METADATA_HEADER: &str = "x-codewen-turn-metadata";
pub const X_CODEX_PARENT_THREAD_ID_HEADER: &str = "x-codewen-parent-thread-id";
pub const X_CODEX_WINDOW_ID_HEADER: &str = "x-codewen-window-id";
...
const X_CODEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-codewen-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_codewen_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_CODEX_RESPONSES_LITE_HEADER: &str =
    "x-openai-internal-codewen-responses-lite";
...
/// - The `x-codewen-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-codewen-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-codewen-turn-state` request header for
...
impl ModelClient {
    ...
    pub fn new(
        ...
        let codewen_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.codewen_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), codewen_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &CodewenResponsesMetadata) -> String {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "codewen.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &CodewenResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(CodewenAuth::auth_mode),
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: codewen_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: codewen_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &CodewenResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                codewen_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&CodewenAuth>) -> Compression {
...
///
/// - `x-codewen-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-codewen-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-codewen-beta-features", header_value);
    }
...
fn map_response_stream(
    api_stream: codewen_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let codewen_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = codewen_api::ResponseStream {
        rx_event,
...
/// the mapped `CodewenErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: codewen_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a CodewenResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(CodewenErr::RefreshTokenFailed(failed))
            }
