...
use codewen_exec_server::ExecProcess;
use codewen_exec_server::ExecProcessEventReceiver;
use codewen_exec_server::ExecProcessFuture;
use codewen_exec_server::ProcessId;
use codewen_exec_server::ProcessSignal;
use codewen_exec_server::ReadResponse;
use codewen_exec_server::StartedExecProcess;
use codewen_exec_server::WriteResponse;
use codewen_exec_server::WriteStatus;
...
use core_test_support::test_codewen::test_env as remote_test_env;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, codewen_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, codewen_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), codewen_exec_server::ExecServerError> {
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export CODEX_INTERACTIVE_SHELL_VAR=codewen\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = codewen_exec_server::Environment::default_for_tests();
