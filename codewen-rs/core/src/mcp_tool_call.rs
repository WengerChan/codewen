...
use codewen_analytics::AppInvocation;
use codewen_analytics::InvocationType;
use codewen_analytics::build_track_events_context;
use codewen_config::ConfigLayerSource;
use codewen_config::types::AppToolApproval;
use codewen_config::types::ApprovalsReviewer;
use codewen_connectors::AppToolPolicy;
use codewen_connectors::AppToolPolicyEvaluator;
use codewen_connectors::AppToolPolicyInput;
use codewen_features::Feature;
use codewen_hooks::PermissionRequestDecision;
use codewen_mcp::CODEX_APPS_MCP_SERVER_NAME;
use codewen_mcp::MCP_TOOL_CODEX_APPS_META_KEY;
use codewen_mcp::McpConnectionManager;
use codewen_mcp::McpPermissionPromptAutoApproveContext;
use codewen_mcp::SandboxState;
use codewen_mcp::auth_elicitation_completed_result;
use codewen_mcp::build_auth_elicitation_plan;
use codewen_mcp::mcp_permission_prompt_is_auto_approved;
use codewen_mcp::tool_is_model_visible;
use codewen_protocol::approvals::ElicitationRequest;
use codewen_protocol::items::McpToolCallError;
use codewen_protocol::items::McpToolCallItem;
use codewen_protocol::items::McpToolCallStatus;
use codewen_protocol::items::TurnItem;
use codewen_protocol::mcp::CallToolResult;
use codewen_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use codewen_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use codewen_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use codewen_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use codewen_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use codewen_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use codewen_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use codewen_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use codewen_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use codewen_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use codewen_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use codewen_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use codewen_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use codewen_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use codewen_protocol::request_user_input::RequestUserInputAnswer;
use codewen_protocol::request_user_input::RequestUserInputArgs;
use codewen_protocol::request_user_input::RequestUserInputQuestion;
use codewen_protocol::request_user_input::RequestUserInputQuestionOption;
...
use codewen_utils_output_truncation::TruncationPolicy;
use codewen_utils_output_truncation::truncate_text;
...
const MCP_RESULT_TELEMETRY_META_KEY: &str = "codewen/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "codewen.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "codewen.mcp.server_user_flow.triggered";
    ...
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        ...
        Self {
            ...
            action_name: trusted_mcp_app_metadata
                .and_then(|metadata| metadata.codewen_apps_meta.as_ref())
...
async fn handle_approved_mcp_tool_call(
    ...
    .await;
    maybe_track_codewen_app_used(sess, turn_context, manager, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        codewen.mcp.target.id = Empty,
        codewen.mcp.server_user_flow.triggered = Empty,
        error.type = Empty,
        codewen.mcp.error.code = Empty,
    );
...
async fn execute_mcp_tool_call(
    ...
    Ok(maybe_request_codewen_apps_auth_elicitation(
        sess,
        turn_context,
        manager,
        call_id,
...
async fn maybe_request_codewen_apps_auth_elicitation(
    sess: &Session,
    turn_context: &TurnContext,
    ...
    if !manager.is_host_owned_codewen_apps_server(server) {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        codewen_connectors::metadata::connector_install_url(
    ...
    refresh_codewen_apps_after_connector_auth(sess, turn_context, manager).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_codewen_apps_after_connector_auth(
    sess: &Session,
    turn_context: &TurnContext,
    manager: &McpConnectionManager,
) {
    let mcp_tools_result = manager.hard_refresh_codewen_apps_tools_cache().await;
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let server_environment_id = manager
        ...
        .unwrap_or(codewen_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID);
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        permission_profile,
        codewen_linux_sandbox_exe: step_context.mcp.config().codewen_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                codewen_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == codewen_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_codewen_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    manager: &McpConnectionManager,
    server: &str,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    codewen_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
fn build_mcp_tool_call_request_meta(
    ...
    if server == CODEX_APPS_MCP_SERVER_NAME {
        let mut codewen_apps_meta = metadata
            .and_then(|metadata| metadata.codewen_apps_meta.clone())
            .unwrap_or_default();
        codewen_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_CODEX_APPS_META_KEY.to_string(),
            serde_json::Value::Object(codewen_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__codewen_mcp_decline__";
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let codewen_apps_meta = tool_info
        .tool
        .meta
        .as_ref()
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == CODEX_APPS_MCP_SERVER_NAME {
        ...
        persist_codewen_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_codewen_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, codewen_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
