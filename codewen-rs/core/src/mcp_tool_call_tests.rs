...
use codewen_config::types::AppConfig;
use codewen_config::types::AppToolConfig;
use codewen_config::types::AppToolsConfig;
use codewen_config::types::ApprovalsReviewer;
use codewen_config::types::AppsConfigToml;
...
use codewen_rollout_trace::ToolDispatchInvocation;
use codewen_rollout_trace::ToolDispatchPayload;
use codewen_rollout_trace::ToolDispatchRequester;
    ...
    let dispatch_trace = session
        ...
        .start_tool_dispatch_trace(|| {
            Some(ToolDispatchInvocation {
                ...
                codewen_turn_id: turn_context.sub_id.clone(),
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.codewen_home)
        .expect("create codewen home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.codewen_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = codewen_hooks::list_hooks(HooksConfig {
    ...
    let trusted_config_layer_stack = trusted_config_layer_stack(
        ...
        &turn_context.config.codewen_home,
        hook_list.hooks,
    );
...
fn attach_trace_bundle(
    ...
    let rollout_thread_trace =
        codewen_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_codewen_turn_started(turn_context.sub_id.as_str());
...
#[test]
fn openai_file_params_are_only_honored_for_codewen_apps() {
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    let span = mcp_tool_call_span(
        ...
        McpToolCallSpanFields {
            ...
            call_id: "call-123",
            server_origin: Some("https://chatgpt.com/api/codewen/ps/mcp"),
            connector_id: Some("calendar"),
            connector_name: Some("Calendar"),
        },
    );
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("codewen.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("codewen.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("codewen.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("codewen.mcp.target.id=")
            && !invalid_logs.contains("codewen.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("codewen.mcp.target.id=")
            && !missing_logs.contains("codewen.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("codewen.mcp.target.id=")
            && !no_meta_logs.contains("codewen.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("codewen.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
#[test]
fn codewen_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        CODEX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_codewen_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        CODEX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn codewen_apps_tool_question_without_elicitation_omits_always_allow() {
...
#[test]
fn codewen_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: CODEX_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_codewen_apps_identity() {
    let mut metadata = approval_metadata(
...
#[tokio::test]
async fn codewen_apps_tool_call_request_meta_includes_turn_metadata_and_codewen_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
...
#[tokio::test]
async fn codewen_apps_tool_call_request_meta_includes_call_id_without_existing_codewen_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
...
fn codewen_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn codewen_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
async fn host_owned_codewen_apps_manager(
    session: &Session,
    turn_context: &TurnContext,
) -> Arc<codewen_mcp::McpConnectionManager> {
    let auth = session.services.auth_manager.auth().await;
    let startup_cancellation_token = CancellationToken::new();
    startup_cancellation_token.cancel();
    let mcp_servers = HashMap::from([(
        CODEX_APPS_MCP_SERVER_NAME.to_string(),
        codewen_mcp::EffectiveMcpServer::configured(codewen_mcp::codewen_apps_mcp_server_config(
            "https://chatgpt.com",
            /*apps_mcp_product_sku*/ None,
            Some(&turn_context.originator),
        )),
    )]);
    let manager = codewen_mcp::McpConnectionManager::new(
        &mcp_servers,
        ...
        codewen_mcp::McpRuntimeContext::new(
        ...
        turn_context.config.codewen_home.to_path_buf(),
        session.services.mcp_manager.codewen_apps_tools_cache(),
        session.services.mcp_manager.tool_catalog_cache(),
        codewen_connectors::connector_runtime_context_key(auth.as_ref()),
        turn_context.config.prefix_mcp_tool_names(),
        rmcp::model::ElicitationCapability::default(),
        /*supports_openai_form_elicitation*/ false,
        codewen_mcp::ToolPluginProvenance::default(),
        auth.as_ref(),
        /*codewen_apps_auth_manager*/ None,
        /*elicitation_reviewer*/ None,
        /*elicitation_lifecycle*/ None,
        codewen_mcp::ElicitationRequestRouter::default(),
    )
    .await;
    Arc::new(manager)
}
...
#[tokio::test]
async fn codewen_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let manager = host_owned_codewen_apps_manager(&session, &turn_context).await;
    let result = codewen_apps_auth_failure_result();
    let metadata = codewen_apps_auth_failure_metadata();

    let returned = maybe_request_codewen_apps_auth_elicitation(
        &session,
        &turn_context,
        manager.as_ref(),
        "call_123",
...
#[tokio::test]
async fn codewen_apps_auth_elicitation_non_host_owned_server_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn codewen_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_codewen_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn codewen_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_codewen_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn codewen_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_codewen_apps_manager(&session, &turn_context).await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_codewen_apps_auth_elicitation(
                &session,
                &turn_context,
                manager.as_ref(),
                "call_123",
    ...
    assert_eq!(
        request.id,
        codewen_protocol::mcp::RequestId::String("codewen_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        codewen_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            CODEX_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("codewen_apps_auth_call_123".into()),
            ElicitationResponse {
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_codewen_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            CODEX_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_codewen_app_tool_approval_writes_tool_override() {
    ...
    persist_codewen_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(codewen_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .codewen_home(codewen_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_codewen_dir = project_dir.path().join(".codewen");
    std::fs::create_dir_all(&project_codewen_dir).expect("create project .codewen dir");
    std::fs::write(
        project_codewen_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&codewen_home)
        .set_project_trust_level(
            project_dir.path(),
            codewen_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_codewen_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_skips_guardian_in_every_permission_mode() {
    ...
    for approval_policy in [
        ...
        turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
            codewen_login::CodewenAuth::create_dummy_chatgpt_auth_for_testing(),
        ));
        turn_context
            .approval_policy
            .set(approval_policy)
