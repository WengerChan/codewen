...
use codewen_exec_server::SelectedCapabilityRootsStatus;
...
use codewen_thread_store::ThreadStoreResult;
...
/// Explains why `CodewenThread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
pub struct CodewenThreadSettingsOverrides {
...
impl CodewenThread {
    ...
    pub async fn submit(&self, op: Op) -> CodewenResult<String> {
    ...
    pub async fn shutdown_and_wait(&self) -> CodewenResult<()> {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(codewen_extension_api::ThreadResumeInput {
    ...
    /// callers that only hold a `CodewenThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: CodewenThreadSettingsOverrides,
    ...
    async fn thread_settings_update(
        ...
        let CodewenThreadSettingsOverrides {
            environments,
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> CodewenResult<()> {
        self.io.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> CodewenResult<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> CodewenResult<()> {
        if items.is_empty() {
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> CodewenResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            CodewenErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> CodewenResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(CodewenErr::InvalidRequest(
