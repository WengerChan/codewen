...
use codewen_execpolicy::AmendError;
use codewen_execpolicy::Decision;
use codewen_execpolicy::Error as ExecPolicyRuleError;
...
use codewen_execpolicy::blocking_append_allow_prefix_rule;
use codewen_execpolicy::blocking_append_network_rule;
...
use codewen_shell_command::is_dangerous_command::DangerousCommandMatch;
use codewen_shell_command::is_dangerous_command::dangerous_command_match;
use codewen_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(codewen_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: codewen_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(codewen_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &codewen_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            codewen_shell_command::is_dangerous_command::dangerous_powershell_words_match(command)
        }
    }
}
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            codewen_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
...
pub(crate) fn default_policy_path(codewen_home: &Path) -> PathBuf {
    codewen_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            codewen_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and codewen prompts the user to run outside the sandbox
