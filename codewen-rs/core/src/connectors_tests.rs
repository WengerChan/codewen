...
use codewen_config::AppRequirementToml;
use codewen_config::AppsRequirementsToml;
...
use codewen_connectors::merge::plugin_connector_to_app_info;
use codewen_connectors::metadata::connector_install_url;
use codewen_connectors::metadata::sanitize_name;
...
fn synthetic_links_are_exposed_to_the_agent_but_not_accessible_in_app_list() {
    ...
    let tools = vec![
        synthetic_tool,
        codewen_app_tool("calendar_list_events", "calendar", Some("Calendar"), &[]),
    ];
...
fn accessible_connectors_from_mcp_tools_preserves_description() {
    let mcp_tools = vec![ToolInfo {
        ...
        callable_namespace: "mcp__codewen_apps__calendar".to_string(),
...
"#,
    ...
    let plugins_manager = PluginsManager::new(config.codewen_home.to_path_buf());
