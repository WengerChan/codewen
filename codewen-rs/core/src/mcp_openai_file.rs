...
//!
//! The model-facing local-path schema is owned by `codewen-mcp` alongside MCP tool inventory, so this
//! module only handles uploading the files and rewriting the execution-time arguments.

use crate::session::session::Session;
use crate::session::turn_context::TurnContext;
use codewen_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use codewen_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let file_name = resolved_path
        ...
        .unwrap_or("file")
        .to_string();
    let upload_auth = codewen_model_provider::auth_provider_from_auth(auth);
...
mod tests {
    ...
    fn set_primary_environment_cwd(turn_context: &mut TurnContext, cwd: &Path) {
        ...
        turn_context.permission_profile = codewen_protocol::models::PermissionProfile::Disabled;
    ...
    async fn openai_file_argument_rewrite_requires_declared_file_params() {
        ...
        let arguments = Some(serde_json::json!({
            "file": "/tmp/codewen-smoke-file.txt"
        }));
