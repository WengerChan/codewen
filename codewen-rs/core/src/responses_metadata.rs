...
pub(crate) enum CodewenResponsesRequestKind {
    Turn,
    Prewarm,
...
impl CodewenResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            CodewenResponsesRequestKind::Turn => ("turn", None),
            CodewenResponsesRequestKind::Prewarm => ("prewarm", None),
            CodewenResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            CodewenResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, CodewenResponsesRequestKind::Memory)
    }
}
...
/// `client_metadata["x-codewen-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct CodewenResponsesMetadata {
    pub(crate) installation_id: String,
    pub(crate) session_id: String,
    pub(crate) thread_id: String,
    pub(crate) turn_id: Option<String>,
    pub(crate) window_id: String,
    pub(crate) request_kind: Option<CodewenResponsesRequestKind>,
...
impl CodewenResponsesMetadata {
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        // Direct x-codewen-turn-metadata is compatibility output. New per-request consumers should
        // prefer client_metadata["x-codewen-turn-metadata"], which is rendered from this same object.
        if self.has_turn_metadata()
            && let Some(turn_metadata_json) = self.turn_metadata_json()
        {
            insert_header(
    ...
    fn turn_metadata_payload(&self) -> CodewenTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(CodewenResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(CodewenResponsesRequestKind::has_turn_identity);
        CodewenTurnMetadataPayload {
...
#[derive(Serialize)]
struct CodewenTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
