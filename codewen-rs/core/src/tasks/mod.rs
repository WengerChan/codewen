...
use crate::codewen_thread::BackgroundTerminalInfo;
...
use codewen_analytics::TurnProfileFact;
use codewen_analytics::TurnTokenUsageFact;
use codewen_login::AuthManager;
use codewen_models_manager::manager::SharedModelsManager;
use codewen_otel::SessionTelemetry;
use codewen_otel::TURN_E2E_DURATION_METRIC;
use codewen_otel::TURN_MEMORY_METRIC;
use codewen_otel::TURN_NETWORK_PROXY_METRIC;
use codewen_otel::TURN_TOKEN_USAGE_METRIC;
use codewen_otel::TURN_TOOL_CALL_METRIC;
...
const TASK_COMPACT_METRIC: &str = "codewen.task.compact";

pub(crate) type SessionTaskResult = CodewenResult<Option<String>>;
...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// [`CodewenErr::TurnAborted`] completes the task through the aborted-turn
    /// lifecycle instead.
    fn run(
        self: Arc<Self>,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            codewen.turn.reasoning_effort = %reasoning_effort,
            codewen.turn.token_usage.input_tokens = field::Empty,
            codewen.turn.token_usage.cached_input_tokens = field::Empty,
            codewen.turn.token_usage.cache_write_input_tokens = field::Empty,
            codewen.turn.token_usage.non_cached_input_tokens = field::Empty,
            codewen.turn.token_usage.output_tokens = field::Empty,
            codewen.turn.token_usage.reasoning_output_tokens = field::Empty,
            codewen.turn.token_usage.total_tokens = field::Empty,
        );
    ...
    pub async fn on_task_finished(
        ...
        let (last_agent_message, abort_reason) = match task_result {
            ...
            Err(CodewenErr::TurnAborted) => (None, Some(TurnAbortReason::Interrupted)),
            Err(err) => {
        ...
        // Emit token usage metrics.
        {
            ...
            current_span.record(
                "codewen.turn.token_usage.input_tokens",
                turn_token_usage.input_tokens,
            );
            current_span.record(
                "codewen.turn.token_usage.cached_input_tokens",
                turn_token_usage.cached_input(),
            );
            current_span.record(
                "codewen.turn.token_usage.cache_write_input_tokens",
                turn_token_usage.cache_write_input_tokens,
            );
            current_span.record(
                "codewen.turn.token_usage.non_cached_input_tokens",
                turn_token_usage.non_cached_input(),
            );
            current_span.record(
                "codewen.turn.token_usage.output_tokens",
                turn_token_usage.output_tokens,
            );
            current_span.record(
                "codewen.turn.token_usage.reasoning_output_tokens",
                turn_token_usage.reasoning_output_tokens,
            );
            current_span.record(
                "codewen.turn.token_usage.total_tokens",
