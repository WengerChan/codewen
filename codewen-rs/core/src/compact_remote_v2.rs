...
use codewen_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    codewen_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    if result.is_ok() {
        ...
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, codewen_error, analytics_details)
                .await;
    ...
    match result {
        Ok(()) => Ok(()),
        Err(err @ CodewenErr::TurnAborted) => Err(err),
        Err(err) => {
            sess.track_turn_codewen_error(turn_context, &err);
...
) -> CodewenResult<RemoteCompactionV2Output> {
...
async fn collect_compaction_output(
    ...
    if compaction_count != 1 {
        return Err(CodewenErr::Fatal(format!(
...
mod tests {
    ...
    fn response_stream(events: Vec<CodewenResult<ResponseEvent>>) -> ResponseStream {
