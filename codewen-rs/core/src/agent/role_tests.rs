...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.codewen_linux_sandbox_exe = Some(PathBuf::from("/tmp/codewen-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/codewen-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        config.codewen_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/codewen-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/codewen-execve-wrapper"))
    );
}
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&codewen_exec_server::LOCAL_FS)),
        )
        .await;
