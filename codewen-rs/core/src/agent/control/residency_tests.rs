...
async fn residency_slot_reservation_unloads_oldest_idle_v2_agent() {
    ...
    config.codewen_home = temp_home.path().to_path_buf().try_into().unwrap();
    ...
    match manager.get_thread(first.thread_id).await {
        Err(CodewenErr::ThreadNotFound(thread_id)) => assert_eq!(thread_id, first.thread_id),
...
async fn interrupted_v2_agent_is_lost_after_residency_eviction() {
    ...
    match err {
        CodewenErr::ThreadNotFound(thread_id) => assert_eq!(thread_id, first.thread_id),
...
async fn mark_thread_completed(thread: &CodewenThread) {
...
async fn mark_thread_interrupted(thread: &CodewenThread) {
    let turn = thread.session.new_default_turn().await;
    thread
        .session
        .send_event(
...
async fn clear_active_turn(thread: &CodewenThread) {
