...
) -> CodewenResult<Option<Vec<RolloutItem>>> {
    match history_mode {
...
impl AgentControl {
    ...
    pub(crate) async fn restore_v2_agent_metadata(
        ...
        let descendant_ids = match agent_graph_store
            .list_thread_spawn_descendants(
                root_thread_id,
                Some(codewen_agent_graph_store::ThreadSpawnEdgeStatus::Open),
            )
            .await
        {
    ...
    pub(crate) async fn ensure_v2_agent_loaded(
        ...
        if self.state.agent_metadata_for_thread(thread_id).is_none() {
            return Err(CodewenErr::ThreadNotFound(thread_id));
        }
        ...
        let history = load_agent_model_context(&state, thread_id, stored_thread.history_mode)
            .await?
            .ok_or(CodewenErr::ThreadNotFound(thread_id))?;
        ...
        if let Some(role_name) = session_source.get_agent_role() {
            ...
            apply_role_to_config(&mut config, Some(&role_name))
                .await
                .map_err(CodewenErr::InvalidRequest)?;
            config
                .permissions
                .approval_policy
                .set(runtime_approval_policy)
                .map_err(|err| {
                    CodewenErr::InvalidRequest(format!("approval_policy is invalid: {err}"))
                })?;
            ...
            config
                ...
                .map_err(|err| {
                    CodewenErr::InvalidRequest(format!("permission_profile is invalid: {err}"))
                })?;
        }
    ...
    async fn resume_single_agent_from_rollout(
        ...
        thread_id: ThreadId,
        session_source: SessionSource,
    ) -> CodewenResult<(ThreadId, MultiAgentVersion)> {
        let state = self.upgrade()?;
        let stored_thread = state
            .read_stored_thread(ReadThreadParams {
                thread_id,
