...
impl AgentControl {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> CodewenResult<String> {
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> CodewenResult<String> {
        let state = self.upgrade()?;
        ...
        match state.get_thread(agent_id).await {
                ...
                if !thread.config_snapshot().await.ephemeral
                    ...
                    && let Err(err) = agent_graph_store
                        .set_thread_spawn_edge_status(
                            agent_id,
                            codewen_agent_graph_store::ThreadSpawnEdgeStatus::Closed,
                        )
                        .await
                {
            ...
            Err(CodewenErr::ThreadNotFound(_)) if known_agent => {
            ...
            Err(CodewenErr::ThreadNotFound(_)) => {}
            Err(err) => {
    ...
    pub(crate) async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> CodewenResult<String> {
