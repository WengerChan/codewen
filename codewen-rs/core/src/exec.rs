...
use codewen_protocol::error::SandboxErr;
...
use codewen_protocol::protocol::ExecCommandOutputDeltaEvent;
use codewen_protocol::protocol::ExecOutputStream;
use codewen_sandboxing::SandboxCommand;
use codewen_sandboxing::SandboxManager;
use codewen_sandboxing::SandboxTransformRequest;
use codewen_sandboxing::SandboxType;
use codewen_sandboxing::SandboxablePreference;
use codewen_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use codewen_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use codewen_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use codewen_sandboxing::resolve_windows_elevated_filesystem_overrides;
use codewen_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use codewen_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use codewen_sandboxing::windows_sandbox_uses_elevated_backend;
...
use codewen_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: codewen_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: codewen_protocol::config_types::WindowsSandboxLevel,
...
fn network_proxy_environment_error(
    ...
    CodewenErr::Io(io::Error::other(format!(
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            codewen_linux_sandbox_exe: codewen_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(CodewenErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| CodewenErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let windows_sandbox_policy_cwd = windows_sandbox_policy_cwd
        .to_abs_path()
        .map_err(|err| CodewenErr::InvalidRequest(format!("invalid sandbox cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        codewen_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = codewen_otel::global() {
        let _ = metrics.counter(
            "codewen.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_codewen_home;
    use codewen_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use codewen_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            codewen_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let codewen_home = find_codewen_home().map_err(|err| {
        CodewenErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve codewen_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                codewen_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    codewen_home: codewen_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(CodewenErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, CodewenErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(CodewenErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(CodewenErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(CodewenErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        codewen_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                codewen_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
