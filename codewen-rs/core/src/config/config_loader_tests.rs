...
use codewen_config::ConfigError;
...
use codewen_config::ConfigLoadOptions;
use codewen_config::ConfigRequirements;
use codewen_config::ConfigRequirementsToml;
use codewen_config::ConfigRequirementsWithSources;
use codewen_config::FilesystemDenyReadPattern;
use codewen_config::LoaderOverrides;
use codewen_config::RequirementSource;
use codewen_config::RequirementsLayerEntry;
use codewen_config::SessionThreadConfig;
use codewen_config::StaticThreadConfigLoader;
use codewen_config::ThreadConfigSource;
use codewen_config::compose_requirements;
use codewen_config::config_error_from_ignored_toml_fields;
use codewen_config::config_error_from_toml;
use codewen_config::config_error_from_typed_toml;
...
use codewen_config::loader::load_requirements_toml;
use codewen_config::test_support::CloudConfigBundleFixture;
use codewen_exec_server::LOCAL_FS;
use codewen_protocol::config_types::EnvironmentVariablePattern;
...
async fn write_project_hook_config(
    dot_codewen_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_codewen_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_codewen_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let codewen_home = tempdir().expect("tempdir");
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .codewen_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = codewen_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        codewen_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = codewen_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = codewen_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn managed_preferences_requirements_resolve_paths_against_codewen_home() -> anyhow::Result<()> {
    ...
    let tmp = tempdir()?;
    let codewen_home = tmp.path().join("codewen-home");
...
"#
    ...
    assert_eq!(
        ...
        Some(codewen_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(codewen_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![codewen_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(codewen_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(codewen_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.openai.codewen".to_string(),
...
async fn load_config_layers_includes_cloud_config_bundle() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let codewen_home = tmp.path().join("home");
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(codewen_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_codewen_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let codewen_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&codewen_home).await?;
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let codewen_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&codewen_home)
        .await
        .expect("create codewen home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            codewen_config::SandboxModeRequirement::ReadOnly,
            codewen_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    codewen_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".codewen")).await?;
    tokio::fs::create_dir_all(project_root.join(".codewen")).await?;
...
"#,
    ...
    .await?;
    tokio::fs::write(
        nested.join(".codewen").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_codewen_folder } => Some(dot_codewen_folder),
            _ => None,
        })
        .collect();
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".codewen")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".codewen")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".codewen"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".codewen"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".codewen"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".codewen"),
        Some("worktree-child"),
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_codewen_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(
        nested.join(".codewen").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let codewen_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&codewen_home).await?;
    tokio::fs::write(codewen_home.join(CONFIG_TOML_FILE), "").await?;
...
async fn project_layer_is_added_when_dot_codewen_exists_without_config_toml() -> std::io::Result<()> {
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_codewen_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".codewen"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn codewen_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let codewen_home = home_dir.join(".codewen");
    tokio::fs::create_dir_all(&codewen_home).await?;
    tokio::fs::write(
        codewen_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn codewen_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".codewen");
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_codewen_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_codex)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let codewen_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&codewen_home_untrusted).await?;
    make_config_for_test(
        &codewen_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = codewen_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let codewen_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&codewen_home_unknown).await?;
    tokio::fs::write(
        codewen_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    // `.codewen` folder. The malformed profile value below would fail typed path
...
"#,
    ...
    // still use the project-local `.codewen` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".codewen")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".codewen").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let codewen_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&codewen_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let codewen_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&codewen_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &codewen_home,
...
mod requirements_exec_policy_tests {
    ...
    use codewen_config::ConfigRequirementsWithSources;
    use codewen_config::RequirementSource;
    use codewen_config::RequirementsExecPolicyDecisionToml;
    use codewen_config::RequirementsExecPolicyParseError;
    use codewen_config::RequirementsExecPolicyPatternTokenToml;
    use codewen_config::RequirementsExecPolicyPrefixRuleToml;
    use codewen_config::RequirementsExecPolicyToml;
    use codewen_execpolicy::Decision;
    use codewen_execpolicy::Evaluation;
    use codewen_execpolicy::RuleMatch;
    use codewen_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_codewen_folder_with_requirements(
        dot_codewen_folder: &Path,
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_codewen_folder_with_requirements(temp_dir.path(), requirements);
