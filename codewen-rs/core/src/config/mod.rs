...
use codewen_config::CloudConfigBundleLoader;
use codewen_config::ConfigLayerSource;
use codewen_config::ConfigLayerStack;
use codewen_config::ConfigLayerStackOrdering;
use codewen_config::ConfigRequirements;
use codewen_config::ConfigRequirementsToml;
use codewen_config::ConstrainedWithSource;
use codewen_config::FeatureRequirementsToml;
use codewen_config::McpServerRequirement;
use codewen_config::PluginRequirementsToml;
use codewen_config::ProfileV2Name;
use codewen_config::ResidencyRequirement;
use codewen_config::SandboxModeRequirement;
use codewen_config::Sourced;
use codewen_config::ThreadConfigLoader;
use codewen_config::config_toml::ConfigLockfileToml;
use codewen_config::config_toml::ConfigToml;
use codewen_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use codewen_config::config_toml::ProjectConfig;
use codewen_config::config_toml::RealtimeAudioConfig;
use codewen_config::config_toml::RealtimeConfig;
use codewen_config::config_toml::ThreadStoreToml;
use codewen_config::config_toml::validate_model_providers;
use codewen_config::loader::load_config_layers_state;
use codewen_config::loader::project_trust_key;
use codewen_config::permissions_toml::PermissionsToml;
use codewen_config::sandbox_mode_requirement_for_permission_profile;
use codewen_config::types::ApprovalsReviewer;
use codewen_config::types::AuthCredentialsStoreMode;
use codewen_config::types::AuthKeyringBackendKind;
use codewen_config::types::History;
use codewen_config::types::McpServerConfig;
use codewen_config::types::McpServerDisabledReason;
...
use codewen_config::types::ToolSuggestConfig;
use codewen_config::types::ToolSuggestDisabledTool;
use codewen_config::types::ToolSuggestDiscoverable;
use codewen_config::types::TuiKeymap;
use codewen_config::types::TuiNotificationSettings;
use codewen_config::types::TuiPetAnchor;
use codewen_config::types::UriBasedFileOpener;
use codewen_config::types::WindowsSandboxModeToml;
use codewen_core_plugins::PluginLoadOutcome;
use codewen_core_plugins::PluginsConfigInput;
use codewen_exec_server::ExecutorFileSystem;
use codewen_exec_server::LOCAL_FS;
use codewen_features::CodeModeConfigToml;
use codewen_features::CurrentTimeReminderConfigToml;
use codewen_features::CurrentTimeReminderDeliveryMode;
use codewen_features::CurrentTimeSource;
use codewen_features::Feature;
use codewen_features::FeatureConfigSource;
use codewen_features::FeatureOverrides;
use codewen_features::FeatureToml;
use codewen_features::Features;
use codewen_features::FeaturesToml;
use codewen_features::MultiAgentV2ConfigToml;
use codewen_features::NetworkProxyConfigToml;
use codewen_features::TokenBudgetConfigToml;
...
use codewen_install_context::InstallContext;
use codewen_login::AuthManagerConfig;
use codewen_login::AuthRouteConfig;
use codewen_mcp::McpConfig;
use codewen_mcp::McpPluginAttribution;
use codewen_mcp::McpServerRegistration;
use codewen_mcp::ResolvedMcpCatalog;
use codewen_memories_read::memory_root;
use codewen_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use codewen_model_provider_info::ModelProviderInfo;
use codewen_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use codewen_model_provider_info::built_in_model_providers;
use codewen_model_provider_info::merge_configured_model_providers;
use codewen_models_manager::ModelsManagerConfig;
use codewen_protocol::config_types::AltScreenMode;
...
use codewen_protocol::config_types::SandboxMode;
use codewen_protocol::config_types::ServiceTier;
use codewen_protocol::config_types::ShellEnvironmentPolicy;
use codewen_protocol::config_types::TrustLevel;
use codewen_protocol::config_types::Verbosity;
use codewen_protocol::config_types::WebSearchConfig;
...
use codewen_protocol::openai_models::ModelsResponse;
...
pub use codewen_thread_store::ExtraConfig;
use codewen_utils_absolute_path::AbsolutePathBuf;
use codewen_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_codewen_runtime;
...
pub use codewen_config::ConfigLoadOptions;
pub use codewen_config::Constrained;
pub use codewen_config::ConstraintError;
pub use codewen_config::ConstraintResult;
pub use codewen_config::LoaderOverrides;
pub use codewen_network_proxy::NetworkProxyAuditMetadata;
use codewen_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use codewen_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(codewen_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(codewen_home.path()).expect("temp dir should resolve"),
    )
    .await
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<codewen_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.codewen/config.toml` snippet:
    ///
    /// ```toml
    ...
    ///          https://github.com/yourname/codewen/blob/main/codewen-rs/rmcp-client/src/oauth.rs#L2
    ...
    /// Directory containing all Codex state (defaults to `~/.codewen` but can be
    /// overridden by the `CODEX_HOME` environment variable).
    pub codewen_home: AbsolutePathBuf,
    ...
    pub config_lock_allow_codewen_version_mismatch: bool,
    ...
    /// Settings that govern if and what will be written to `~/.codewen/history.jsonl`.
    pub history: History,
    ...
    pub codewen_self_exe: Option<PathBuf>,

    /// Path to the `codewen-linux-sandbox` executable. This must be set if
    /// [`codewen_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `codewen-linux-sandbox`.
    pub codewen_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `codewen-execve-wrapper` executable used for shell
    ...
    pub otel: codewen_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn codewen_home(&self) -> PathBuf {
        self.codewen_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn codewen_home(mut self, codewen_home: PathBuf) -> Self {
        self.codewen_home = Some(codewen_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let codewen_home = match codewen_home {
            Some(codewen_home) => AbsolutePathBuf::from_absolute_path(codewen_home)?,
            None => find_codewen_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&codewen_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = codewen_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    codewen_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(codewen_config::io_error_from_config_error(
        ...
        {
            let allow_codewen_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_codewen_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_codewen_version_mismatch = allow_codewen_version_mismatch;
...
impl Config {
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &codewen_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            codewen_home: self.codewen_home.to_path_buf(),
            ...
            codewen_linux_sandbox_exe: self.codewen_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                codewen_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.codewen_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_codewen_home(
            codewen_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides_for_codewen_home(
        ...
        let cli_layer = codewen_config::build_cli_overrides_layer(&cli_overrides);
        codewen_config::merge_toml_values(&mut merged, &cli_layer);
        let codewen_home = AbsolutePathBuf::from_absolute_path_checked(codewen_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &codewen_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `codewen exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `codewen_self_exe`, `codewen_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(codewen_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, codewen_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .codewen/ folders.
...
pub fn set_default_oss_provider(codewen_home: &Path, provider: &str) -> std::io::Result<()> {
    codewen_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &codewen_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            codewen_protocol::permissions::FileSystemSandboxEntry {
                path: codewen_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: codewen_protocol::permissions::FileSystemAccessMode::Deny,
            ...
            codewen_protocol::permissions::FileSystemSandboxEntry {
                path: codewen_protocol::permissions::FileSystemPath::Path { path },
                access: codewen_protocol::permissions::FileSystemAccessMode::Deny,
                missing_path_behavior: None,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&codewen_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&codewen_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(codewen_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| codewen_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .map(AbsolutePathBuf::to_path_buf)
            .or(sqlite_home_env)
            .unwrap_or_else(|| codewen_home.to_path_buf());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_codewen_runtime(
            &codewen_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_codewen_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_codewen_version_mismatch)
...
///   directory exists.
pub fn find_codewen_home() -> std::io::Result<AbsolutePathBuf> {
    codewen_utils_home_dir::find_codewen_home()
}
