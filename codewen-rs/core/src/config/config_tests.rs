...
use codewen_config::ConfigLayerEntry;
use codewen_config::ConfigLayerSource;
use codewen_config::ConfigLayerStack;
use codewen_config::McpServerCommandMatcher;
use codewen_config::McpServerIdentity;
use codewen_config::McpServerRequirement;
use codewen_config::McpServerValueMatcher;
use codewen_config::ProfileV2Name;
use codewen_config::RequirementSource;
use codewen_config::Sourced;
use codewen_config::config_toml::AgentRoleToml;
use codewen_config::config_toml::AgentsToml;
use codewen_config::config_toml::AutoReviewToml;
use codewen_config::config_toml::ConfigToml;
use codewen_config::config_toml::ExperimentalRequestUserInput;
use codewen_config::config_toml::ProjectConfig;
use codewen_config::config_toml::RealtimeConfig;
use codewen_config::config_toml::RealtimeToml;
use codewen_config::config_toml::RealtimeTransport;
use codewen_config::config_toml::RealtimeWsMode;
use codewen_config::config_toml::RealtimeWsVersion;
use codewen_config::config_toml::ToolsToml;
...
use codewen_config::permissions_toml::NetworkDomainPermissionToml;
use codewen_config::permissions_toml::NetworkDomainPermissionsToml;
use codewen_config::permissions_toml::NetworkMitmActionToml;
use codewen_config::permissions_toml::NetworkMitmHookToml;
use codewen_config::permissions_toml::NetworkMitmToml;
...
use codewen_config::permissions_toml::WorkspaceRootsToml;
use codewen_config::types::AppToolApproval;
use codewen_config::types::ApprovalsReviewer;
use codewen_config::types::BundledSkillsConfig;
use codewen_config::types::FeedbackConfigToml;
use codewen_config::types::HistoryPersistence;
use codewen_config::types::McpServerEnvVar;
use codewen_config::types::McpServerOAuthConfig;
use codewen_config::types::McpServerToolConfig;
use codewen_config::types::McpServerTransportConfig;
use codewen_config::types::MemoriesConfig;
use codewen_config::types::MemoriesToml;
use codewen_config::types::ModelAvailabilityNuxConfig;
use codewen_config::types::Notice;
use codewen_config::types::NotificationCondition;
use codewen_config::types::NotificationMethod;
use codewen_config::types::Notifications;
use codewen_config::types::OtelConfigToml;
use codewen_config::types::OtelExporterKind;
use codewen_config::types::ResumeCwdMode;
use codewen_config::types::SandboxWorkspaceWrite;
use codewen_config::types::SessionPickerViewMode;
use codewen_config::types::SkillsConfig;
use codewen_config::types::ToolSuggestDisabledTool;
use codewen_config::types::ToolSuggestDiscoverableType;
use codewen_config::types::Tui;
use codewen_config::types::TuiKeymap;
use codewen_config::types::TuiNotificationSettings;
use codewen_config::types::TuiPetAnchor;
use codewen_config::types::WindowsSandboxModeToml;
use codewen_config::types::WindowsToml;
use codewen_core_plugins::PluginsManager;
use codewen_exec_server::LOCAL_FS;
use codewen_features::Feature;
use codewen_features::FeaturesToml;
use codewen_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use codewen_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use codewen_model_provider_info::WireApi;
use codewen_models_manager::bundled_models_response;
use codewen_network_proxy::NetworkMode;
use codewen_protocol::config_types::ModelProviderAuthInfo;
use codewen_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use codewen_protocol::config_types::ServiceTier;
use codewen_protocol::models::ActivePermissionProfile;
use codewen_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use codewen_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use codewen_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
...
use codewen_protocol::protocol::NetworkAccess;
...
use codewen_config::test_support::CloudConfigBundleFixture;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: codewen_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
[features.code_mode]
enabled = true
excluded_tool_namespaces = ["mcp__codewen_apps", "multi_agent_v1"]
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        codewen_home.path().join(CONFIG_TOML_FILE),
        r#"
...
"#,
    ...
    assert_eq!(
        ...
        codewen_core_plugins::remote::RemotePluginServiceConfig::new(
...
async fn experimental_network_requirements_enable_proxy_without_feature() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .codewen_home(codewen_home.path().to_path_buf())
        .fallback_cwd(Some(codewen_home.path().to_path_buf()))
        .cloud_config_bundle(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        codewen_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = codewen_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let codewen_home = temp_dir.path().join("codewen-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".codewen"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".codewen".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".codewen"), cwd.as_path()),
            "expected .codewen carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".codewen"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".codewen"] {
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    codewen_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            codewen_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    ...
    let requirements_toml = codewen_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = codewen_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_codewen_folder: project_dot_codex.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(codewen_home.path().to_path_buf()),
            ..Default::default()
        },
        codewen_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_codewen_folder: project_dot_codex,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            codewen_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let default_zsh_path = codewen_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_codewen_home_for_workspace_write() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, codewen_home.path().to_path_buf());
...
-> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let memories_root = codewen_home.path().join("memories");
    let writable_root = codewen_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".codewen");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let codewen_home = TempDir::new()?;
    let managed_path = codewen_home.path().join("managed_config.toml");
    let config_path = codewen_home.path().join(CONFIG_TOML_FILE);
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(codewen_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let codewen_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(codewen_home.path());
    ...
    let loaded = load_global_mcp_servers(codewen_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(codewen_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/codewen-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/codewen-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-codewen-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-codewen-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-codewen-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(codewen_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(codewen_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let codewen_home = TempDir::new()?;
    let base_config = codewen_home.path().join(CONFIG_TOML_FILE);
    let selected_config = codewen_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(codewen_home.path(), &profile_name),
        codewen_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn codewen_home(&self) -> AbsolutePathBuf {
        self.codewen_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let workspace = codewen_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, codewen_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        codewen_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let missing_path = codewen_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let role_config_path = codewen_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = codewen_config::ConfigLayerStack::new(
        vec![codewen_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: codewen_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        codewen_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".codewen").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let researcher_path = codewen_home.path().join("agents").join("researcher.toml");
    let reviewer_path = codewen_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = codewen_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let catalog_path = codewen_home.path().join("catalog.json");
...
"#;
    ...
    let codewen_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        codewen_home: codewen_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: codewen_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let codewen_home = fixture.codewen_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = codewen_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![codewen_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = codewen_config::RequirementSource::Unknown;
    ...
    let requirements = codewen_config::ConfigRequirements {
        web_search_mode: codewen_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        codewen_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let codewen_home = temp_dir.path();
    let config_path = codewen_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(codewen_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(codewen_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(codewen_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(codewen_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(codewen_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let cwd = codewen_home.path().join("workspace");
    let outside_root = codewen_home.path().join("outside-write");
...
export_dir = "locks"
allow_codewen_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_codewen_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let lock_path = codewen_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
codewen_version = "older-version"
...
-> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let selected_config = codewen_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let catalog_path = codewen_home.path().join("required-models.json");
    let mut catalog = bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = codewen_home.path().join("required-state");
    let required_log_dir = codewen_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&codewen_home, requirements).await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let codewen_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(codewen_home.path().join("required-state"))?;
    let environment = codewen_home.path().join("environment-state");
