...
use codewen_mcp::ToolInfo as McpToolInfo;
...
pub(crate) fn build_mcp_tool_runtimes(
    ...
    let mut exposed_tools = filter_non_codewen_apps_mcp_tools_only(all_mcp_tools);
    if let Some(connectors) = connectors {
        exposed_tools.extend(filter_codewen_apps_mcp_tools(
            all_mcp_tools,
            connectors,
            config,
        ));
...
fn filter_non_codewen_apps_mcp_tools_only(mcp_tools: &[McpToolInfo]) -> Vec<McpToolInfo> {
    mcp_tools
        .iter()
        .filter(|tool| {
...
fn filter_codewen_apps_mcp_tools(
