...
fn test_responses_metadata_json(
    ...
    window_id: &str,
    request_kind: CodewenResponsesRequestKind,
) -> String {
    state
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, CodewenResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        CodewenResponsesRequestKind::Compaction(compaction),
    )
}
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        ...
        (
            "x-codewen-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-codewen-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert!(json.get("x-codewen-installation-id").is_none());
    assert!(json.get("x-codewen-parent-thread-id").is_none());
