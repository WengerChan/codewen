...
use codewen_analytics::CodewenCompactionEvent;
...
use codewen_analytics::CompactionStatus;
use codewen_analytics::CompactionStrategy;
use codewen_analytics::CompactionTrigger;
use codewen_analytics::now_unix_seconds;
use codewen_protocol::error::CodewenErr;
use codewen_protocol::error::Result as CodewenResult;
use codewen_protocol::items::ContextCompactionItem;
...
use codewen_utils_output_truncation::approx_token_count;
...
pub use codewen_prompts::SUMMARIZATION_PROMPT;
pub use codewen_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = CodewenErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
    ...
    let codewen_error = result.as_ref().err();
    if result.is_ok() {
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
        ...
        window_id,
        CodewenResponsesRequestKind::Compaction(compaction_metadata),
    );
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(err @ (CodewenErr::Interrupted | CodewenErr::TurnAborted)) => {
                return Err(err);
            }
            Err(e @ CodewenErr::SessionBudgetExceeded) => {
                sess.track_turn_codewen_error(turn_context.as_ref(), &e);
                let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                sess.send_event(&turn_context, event).await;
                return Err(e);
            }
            Err(e @ CodewenErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        codewen_error: Option<&CodewenErr>,
        ...
        sess.services
            ...
            .track_compaction(CodewenCompactionEvent {
                ...
                status,
                codewen_error_kind: codewen_error.map(Into::into),
                codewen_error_http_status_code: codewen_error
                    .and_then(CodewenErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &CodewenResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(CodewenErr::Interrupted | CodewenErr::TurnAborted) => CompactionStatus::Interrupted,
