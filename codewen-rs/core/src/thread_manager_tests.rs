...
impl codewen_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: codewen_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> codewen_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<codewen_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> codewen_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<codewen_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> codewen_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
fn effective_originator_prefers_thread_scoped_sources_before_env_originator() {
    for (metrics_service_name, persisted_originator, inherited_originator, expected_originator) in [
        (
            Some("codewen_work_desktop"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "codewen_work_desktop",
        ),
        (
            Some("codewen_work_web"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "codewen_work_web",
        ),
        (
            Some("codewen_work_mobile"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "codewen_work_mobile",
        ),
        (
            Some("codewen_work_cca"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "codewen_work_cca",
        ),
...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.codewen_home = temp_dir.path().join("codewen-home").abs();
    config.cwd = config.codewen_home.abs();
    std::fs::create_dir_all(&config.codewen_home).expect("create codewen home");
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl codewen_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
    ...
    impl codewen_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        ...
        ) -> codewen_extension_api::ExtensionFuture<'a, Vec<codewen_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = codewen_mcp::codewen_apps_mcp_server_config(
    ...
    let mut extensions = codewen_extension_api::ExtensionRegistryBuilder::new();
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = codewen_extension_api::ExtensionDataInit::new();
    ...
    let first_thread = manager
        .start_thread_with_options(StartThreadOptions {
            ...
            thread_source: None,
            dynamic_tools: Vec::new(),
            metrics_service_name: Some("codewen_work_desktop".to_string()),
            parent_trace: None,
            environments: Vec::new(),
    ...
    let selected_servers = |config: &codewen_mcp::McpConfig| {
        codewen_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let codewen_apps_server = codewen_mcp::configured_mcp_servers(&first_resolved.config)
    ...
    assert_eq!(
        codewen_apps_headers
            .expect("Codex Apps headers should be configured")
            .get("originator"),
        Some(&"codewen_work_desktop".to_string())
    );
}
...
#[tokio::test]
async fn explicit_installation_id_skips_codewen_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.codewen_home = temp_dir.path().join("codewen-home").abs();
    config.cwd = config.codewen_home.abs();
    ...
    assert!(!config.codewen_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.codewen_home.join("models_cache.json").exists());
}
