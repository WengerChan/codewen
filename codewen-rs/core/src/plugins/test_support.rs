...
pub(crate) fn write_curated_plugin_sha(codewen_home: &Path) {
    write_curated_plugin_sha_with(codewen_home, TEST_CURATED_PLUGIN_SHA);
}

pub(crate) fn write_curated_plugin_sha_with(codewen_home: &Path, sha: &str) {
    write_file(&codewen_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) fn write_plugins_feature_config(codewen_home: &Path) {
    write_file(
...
pub(crate) async fn load_plugins_config(codewen_home: &Path) -> crate::config::Config {
