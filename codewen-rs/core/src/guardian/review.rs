...
use codewen_analytics::GuardianReviewDecision;
use codewen_analytics::GuardianReviewFailureReason;
use codewen_analytics::GuardianReviewTerminalStatus;
use codewen_analytics::GuardianReviewTrackContext;
use codewen_analytics::GuardianReviewedAction;
...
use codewen_protocol::protocol::GuardianAssessmentDecisionSource;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: CodewenErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &codewen_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        codewen_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("codewen-approval-review".to_string())
        .spawn(move || {
            ...
            GuardianReviewError::Session {
                error_info: Some(
                    CodewenErrorInfo::ServerOverloaded
                        | CodewenErrorInfo::HttpConnectionFailed { .. }
                        | CodewenErrorInfo::ResponseStreamConnectionFailed { .. }
                        | CodewenErrorInfo::InternalServerError
                        | CodewenErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )
