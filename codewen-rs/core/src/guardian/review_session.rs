...
use anyhow::anyhow;
use codewen_analytics::GuardianReviewAnalyticsResult;
use codewen_analytics::GuardianReviewSessionAnalyticsParams;
use codewen_analytics::GuardianReviewSessionKind;
...
use codewen_protocol::openai_models::ModelMessages;
...
use crate::codewen_delegate::run_codewen_thread_interactive;
...
pub(crate) enum GuardianReviewSessionOutcome {
    ...
    SessionFailed {
        error: anyhow::Error,
        error_info: Option<CodewenErrorInfo>,
    },
    TimedOut,
    Aborted,
}
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(
        ...
        Self {
            ...
            codewen_linux_sandbox_exe: spawn_config.codewen_linux_sandbox_exe.clone(),
...
async fn spawn_guardian_review_session(
    ...
    let (session, io) = Box::pin(run_codewen_thread_interactive(
        spawn_config,
                                ...
                                && let Some(error) = last_error
                            {
                                return (
                                    GuardianReviewSessionOutcome::SessionFailed {
                                        ...
                                        error_info: error.codewen_error_info,
                                    },
                                    true,
                                    true,
                                );
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<codewen_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
mod tests {
    use super::*;
    use codewen_protocol::openai_models::AutoReviewMessages;
    ...
    async fn test_review_params() -> GuardianReviewSessionParams {
        ...
        GuardianReviewSessionParams {
            ...
            model,
            reasoning_effort,
            guardian_default_review_model_id: "codewen-auto-review".to_string(),
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::CodewenHooks));
    }
    ...
    async fn wait_for_guardian_review_preserves_structured_session_error() {
        ...
        tx_event
            .send(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    codewen_error_info: Some(CodewenErrorInfo::ServerOverloaded),
                }),
            })
            .await
        ...
        assert_eq!(error_info, Some(CodewenErrorInfo::ServerOverloaded));
        assert!(keep_review_session);
        assert!(capture_token_usage);
    }
