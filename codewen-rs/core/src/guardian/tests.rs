...
use codewen_analytics::GuardianApprovalRequestSource;
...
use codewen_model_provider_info::AMAZON_BEDROCK_GPT_5_4_MODEL_ID;
...
use codewen_models_manager::manager::StaticModelsManager;
...
// The memories extension depends on codewen-core, so this probe verifies the nested Guardian config
...
impl codewen_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
    fn on_thread_start<'a>(
        &'a self,
        input: codewen_extension_api::ThreadStartInput<'a, Config>,
...
impl codewen_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        ...
        Box::pin(async move {
            ...
            {
                vec![codewen_extension_api::PromptFragment::developer_policy(
...
fn guardian_shell_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::Shell {
        ...
        cwd: test_path_buf("/repo/codewen-rs/core").abs(),
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"openai/codewen\"}".to_string(),
                ...
                ResponseItem::FunctionCallOutput {
                    id: None,
                    call_id: "call-1".to_string(),
                    output: codewen_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/codewen-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[codewen_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            codewen_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: codewen_protocol::permissions::FileSystemSpecialPath::Root,
                },
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "codewen_core__guardian__tests__network_access_guardian_prompt_layout",
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    codewen_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    let mut extensions = codewen_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    assert!(matches!(
        ...
        Some(codewen_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    ...
    settings.bind(|| {
        assert_snapshot!(
            "codewen_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            codewen_protocol::user_input::UserInput::Text { text, .. } => text,
            codewen_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(codewen_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "codewen_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(codewen_protocol::openai_models::ReasoningEffort::Low),
        /*model_messages*/ None,
    )
