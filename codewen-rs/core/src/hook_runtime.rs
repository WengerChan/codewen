...
use codewen_analytics::CompactionTrigger;
use codewen_analytics::HookRunFact;
use codewen_analytics::build_track_events_context;
use codewen_hooks::PermissionRequestDecision;
use codewen_hooks::PermissionRequestOutcome;
use codewen_hooks::PermissionRequestRequest;
use codewen_hooks::PostToolUseOutcome;
use codewen_hooks::PostToolUseRequest;
use codewen_hooks::PreToolUseOutcome;
use codewen_hooks::PreToolUseRequest;
use codewen_hooks::SessionStartOutcome;
use codewen_hooks::StartHookTarget;
use codewen_hooks::StopHookTarget;
use codewen_hooks::StopOutcome;
use codewen_hooks::SubagentHookContext;
use codewen_hooks::UserPromptSubmitOutcome;
use codewen_hooks::UserPromptSubmitRequest;
use codewen_otel::HOOK_RUN_DURATION_METRIC;
use codewen_otel::HOOK_RUN_METRIC;
...
use codewen_protocol::protocol::HookCompletedEvent;
use codewen_protocol::protocol::HookEventName;
use codewen_protocol::protocol::HookRunStatus;
use codewen_protocol::protocol::HookRunSummary;
use codewen_protocol::protocol::HookSource;
use codewen_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    codewen_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = codewen_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = codewen_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = codewen_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = codewen_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(codewen_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: codewen_hooks::HookEvent::AfterAgent {
                event: codewen_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            codewen_hooks::HookResult::Success => continue,
            codewen_hooks::HookResult::FailedContinue(error) => (error, false),
            codewen_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(codewen_protocol::protocol::ErrorEvent {
        message,
        codewen_error_info: Some(CodewenErrorInfo::Other),
    });
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (codewen_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use codewen_protocol::protocol::HookEventName;
    use codewen_protocol::protocol::HookExecutionMode;
    use codewen_protocol::protocol::HookHandlerType;
    use codewen_protocol::protocol::HookRunStatus;
    use codewen_protocol::protocol::HookScope;
    use codewen_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    codewen_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
