...
use codewen_api::RealtimeAudioFrame;
use codewen_api::RealtimeContextAppendChannel;
use codewen_api::RealtimeEvent;
use codewen_api::RealtimeEventParser;
use codewen_api::RealtimeSessionConfig;
use codewen_api::RealtimeSessionMode;
use codewen_api::RealtimeWebsocketClient;
use codewen_api::RealtimeWebsocketEvents;
use codewen_api::RealtimeWebsocketWriter;
use codewen_api::build_session_headers;
use codewen_api::map_api_error;
...
use codewen_login::read_openai_api_key_from_env;
...
use codewen_protocol::protocol::CodewenResponseHandoffMode;
...
use codewen_protocol::protocol::RealtimeConversationClosedEvent;
use codewen_protocol::protocol::RealtimeConversationRealtimeEvent;
use codewen_protocol::protocol::RealtimeConversationSdpEvent;
use codewen_protocol::protocol::RealtimeConversationStartedEvent;
use codewen_protocol::protocol::RealtimeHandoffRequested;
use codewen_protocol::protocol::RealtimeOutputModality;
use codewen_protocol::protocol::RealtimeTranscriptEntry;
use codewen_protocol::protocol::RealtimeVoice;
use codewen_protocol::protocol::RealtimeVoicesList;
use codewen_utils_output_truncation::approx_bytes_for_tokens;
use codewen_utils_string::approx_token_count;
use codewen_utils_string::take_bytes_at_char_boundary;
use http::HeaderMap;
...
const STANDALONE_HANDOFF_ID: &str = "codewen";
...
struct RealtimeHandoffState {
    ...
    codewen_responses_as_items: bool,
    codewen_response_item_prefix: Option<String>,
    codewen_response_handoff_mode: CodewenResponseHandoffMode,
...
impl RealtimeHandoffState {
    fn new(
        ...
        Self {
            ...
            codewen_responses_as_items,
            codewen_response_item_prefix,
    ...
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.codewen_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.codewen_response_handoff_mode == CodewenResponseHandoffMode::BemTags
    }
}
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> CodewenResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> CodewenResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            client_managed_handoffs,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> CodewenResult<()> {
        let sender = {
        ...
        let Some(sender) = sender else {
            return Err(CodewenErr::InvalidRequest(
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(CodewenErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> CodewenResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| CodewenErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.codewen_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.codewen_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> CodewenResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> CodewenResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.codewen_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> CodewenResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> CodewenResult<PreparedRealtimeConversationStart> {
    ...
    Ok(PreparedRealtimeConversationStart {
        ...
        codewen_responses_as_items: params.codewen_responses_as_items,
        codewen_response_item_prefix: params.codewen_response_item_prefix,
        codewen_response_handoff_mode: params.codewen_response_handoff_mode,
        realtime_call_api_provider,
...
) -> CodewenResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> CodewenResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), CodewenErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&CodewenAuth>, provider: &ModelProviderInfo) -> CodewenResult<String> {
    ...
    if let Some(api_key) = auth.and_then(CodewenAuth::api_key) {
...
fn realtime_request_headers(
    ...
    event_parser: RealtimeEventParser,
    originator: &str,
) -> CodewenResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            CodewenErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: CodewenResponseHandoffMode,
) -> RealtimeWebsocketWriter {
    let channel = match handoff_mode {
        CodewenResponseHandoffMode::Thinking => None,
        CodewenResponseHandoffMode::Commentary => Some(RealtimeContextAppendChannel::Commentary),
        CodewenResponseHandoffMode::BemTags => match phase {
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.codewen_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    codewen_error_info: CodewenErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
