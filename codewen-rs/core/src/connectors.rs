...
pub use codewen_connectors::AppBranding;
pub use codewen_connectors::AppInfo;
pub use codewen_connectors::AppMetadata;
use codewen_connectors::ConnectorDirectoryCacheContext;
use codewen_connectors::ConnectorDirectoryCacheKey;
use codewen_connectors::app_is_enabled;
use codewen_connectors::apps_config_from_layer_stack;
...
use codewen_mcp::ToolInfo;
use codewen_mcp::ToolPluginProvenance;
use codewen_mcp::effective_mcp_servers;
use codewen_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = codewen_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        codewen_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config
        .features
        .apps_enabled_for_auth(auth.as_ref().is_some_and(CodewenAuth::uses_codewen_backend))
    {
        return Some(Vec::new());
    }
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let environment_manager = EnvironmentManager::from_codewen_home(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    let mcp_connection_manager = McpConnectionManager::new(
        ...
        mcp_manager.codewen_apps_tools_cache(),
    ...
    let codewen_apps_ready = if refreshed_tools_succeeded {
        true
    ...
    if codewen_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let chatgpt_user_id = auth.and_then(CodewenAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(CodewenAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + codewen_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    codewen_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        ...
        Some(codewen_connectors::accessible::AccessibleConnectorTool {
    ...
    codewen_connectors::accessible::collect_accessible_connectors(tools)
}
