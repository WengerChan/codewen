...
pub(crate) fn map_session_init_error(err: &anyhow::Error, codewen_home: &Path) -> CodewenErr {
    if let Some(ThreadStoreError::Unsupported { operation }) = err
        .chain()
        .find_map(|cause| cause.downcast_ref::<ThreadStoreError>())
    {
        return CodewenErr::UnsupportedOperation(format!("{operation} is not supported yet"));
    }

    if let Some(mapped) = err
        .chain()
        .filter_map(|cause| cause.downcast_ref::<std::io::Error>())
        .find_map(|io_err| map_rollout_io_error(io_err, codewen_home))
    {
        return mapped;
    }

    CodewenErr::Fatal(format!("Failed to initialize session: {err:#}"))
}

fn map_rollout_io_error(io_err: &std::io::Error, codewen_home: &Path) -> Option<CodewenErr> {
    let sessions_dir = codewen_home.join(SESSIONS_SUBDIR);
