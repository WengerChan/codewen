...
This crate implements the business logic for Codewen. It is designed to be used by the various Codewen UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //codewen-rs/core:core-all-wine-exec-test`.
...
Note that `codewen-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.codewen` read-only.
...
Expects the binary containing `codewen-core` to run the equivalent of `codewen sandbox` when `arg0` is `codewen-linux-sandbox`. See the `codewen-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `codewen-resources/bwrap`
binary shipped with Codewen and Codewen surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Codewen also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Codewen rejects sandboxed shell commands that would enter the
...
Expects the binary containing `codewen-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--codewen-run-as-apply-patch`. See the
`codewen-arg0` crate for details.
