...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let codewen_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-codewen-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-codewen-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-codewen-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(
        request.body_json()["client_metadata"]["x-codewen-window-id"].as_str(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-codewen-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    assert!(
        ...
        "turn_id should not be empty in x-codewen-turn-metadata"
    );
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/codewen.git",
    ]);
