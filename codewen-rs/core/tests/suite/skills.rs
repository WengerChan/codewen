...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_codewen().with_workspace_setup(move |cwd, fs| async move {
    ...
    core_test_support::wait_for_event(test.codewen.as_ref(), |event| {
        matches!(event, codewen_protocol::protocol::EventMsg::TurnComplete(_))
    })
    .await;
