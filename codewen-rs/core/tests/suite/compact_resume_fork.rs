...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<CodewenThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.codewen)
}

async fn user_turn(conversation: &Arc<CodewenThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<CodewenThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<CodewenThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<CodewenThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = codewen_core::test_support::auth_manager_from_auth(
        codewen_login::CodewenAuth::from_api_key("dummy"),
    );
