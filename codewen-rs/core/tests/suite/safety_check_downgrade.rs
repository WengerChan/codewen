...
const REQUESTED_MODEL: &str = "gpt-5.3-codewen";
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let mut builder = test_codewen().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.codewen, |event| {
    ...
    let _ = wait_for_event(&test.codewen, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.codewen, |event| matches!(event, EventMsg::Error(_))).await;
...
async fn openai_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    ...
    loop {
        let event = wait_for_event(&test.codewen, |_| true).await;
        match event {
            EventMsg::Warning(warning)
                if warning
                    .message
...
async fn model_verification_emits_structured_event_without_reroute_or_warning() -> Result<()> {
    ...
    let mut builder = test_codewen().with_model(SERVER_MODEL);
    let test = builder.build(&server).await?;
