...
const FIXED_CWD: &str = "/tmp/codewen_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> CodewenAuth {
        match self {
            AuthCase::ChatGpt => CodewenAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => CodewenAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&codewen, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().codewen_home_path();
...
async fn build_harness_inner(
    ...
    TestCodexHarness::with_builder(builder.with_config(move |config| {
        config.cwd = codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(
            FIXED_CWD,
        ))
