...
async fn codewen_apps_auth_failure_requests_elicitation_by_default() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let apps_server = AppsTestServer::mount_searchable(&server).await?;
    Mock::given(method("POST"))
        .and(path_regex("^/api/codewen/ps/mcp/?$"))
...
"#,
    ...
    assert_eq!(
        request.id,
        codewen_protocol::mcp::RequestId::String(format!("codewen_apps_auth_{call_id}"))
    );
    assert_eq!(
        request.request,
        ElicitationRequest::Url {
            ...
            elicitation_id: format!("codewen_apps_auth_{call_id}"),
        }
    );
