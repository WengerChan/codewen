...
async fn request_body_is_zstd_compressed_for_codewen_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/codewen/v1", server.uri());
    ...
    let codewen = builder.build(&server).await?.codewen;
