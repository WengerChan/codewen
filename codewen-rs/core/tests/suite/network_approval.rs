...
use codewen_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "codewen-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://codewen-network-test.invalid:80";
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    test.codewen.submit(Op::Interrupt).await?;
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert_eq!(
        approval.call_id,
        "network#local#http#codewen-network-test.invalid#80"
    );
    ...
    assert_eq!(
        ...
        Some("codewen-network-test.invalid is not in the allowed_domains")
    );
    test.codewen
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="codewen-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): codewen-network-test.invalid",
        )
    }));
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    .await;
    test.codewen.submit(Op::CleanBackgroundTerminals).await?;
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.codewen.list_background_terminals().await.is_empty() {
                break;
            }
