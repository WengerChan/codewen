...
fn configure_multi_agent_v2_with_role(
    ...
    config
        ...
        .expect("test config should allow feature update");
    let role_path = config.codewen_home.join("durable-worker-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    let mut initial_builder = test_codewen().with_config(move |config| {
    ...
    initial.codewen.flush_rollout().await?;
    drop(worker_thread);
    drop(initial);
