...
use codewen_mcp::CodewenAppsToolsCache;
use codewen_mcp::EffectiveMcpServer;
use codewen_mcp::ElicitationRequestRouter;
...
async fn hosted_plugin_runtime_ps_mcp_tool_calls_use_current_auth_manager_token() -> Result<()> {
    ...
    let expected_auth = CodewenAuth::from_external_chatgpt_tokens(
        "header.e30.first",
        "test-account",
    ...
    let mut hosted_plugin_runtime_config = codewen_mcp::hosted_plugin_runtime_mcp_server_config(
    ...
    *bearer_token_env_var = None;
    ...
    let manager = McpConnectionManager::new(
        ...
        McpRuntimeContext::new(
            Arc::new(codewen_exec_server::EnvironmentManager::without_environments()),
            home.path().to_path_buf(),
        ),
        home.path().to_path_buf(),
        CodewenAppsToolsCache::default(),
        McpToolCatalogCache::default(),
        codewen_mcp::codewen_apps_tools_cache_key(Some(&expected_auth)),
    ...
    auth_manager
        .set_external_auth(Arc::new(StaticExternalAuth(
            CodewenAuth::from_external_chatgpt_tokens(
                "header.e30.reloaded",
                "test-account",
                /*chatgpt_plan_type*/ None,
            )?,
