...
use core_test_support::test_codewen::ApplyPatchModelOutput;
...
async fn apply_patch_harness_with(
    ...
    let builder = configure(test_codewen());
...
async fn apply_patch_cli_uses_codewen_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let codewen_linux_sandbox_exe = harness
        .test()
        .config
        .codewen_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include codewen-linux-sandbox helper");
    assert_eq!(
        codewen_linux_sandbox_exe
            .file_name()
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&codewen_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
