...
async fn session_resource_client_follows_published_mcp_runtime() -> Result<()> {
    ...
    let test = core_test_support::test_codewen::test_codewen()
...
async fn code_mode_only_exposes_direct_model_only_mcp_namespaces() -> Result<()> {
    ...
    assert!(
        exec_description.is_some_and(|description| {
            !description.contains("mcp__codewen_apps__calendar_create_event(args:")
        }),
...
async fn apps_guidance_appears_after_background_recovery_within_a_turn() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let mcp_runtime = test.codewen.current_mcp_runtime().await;
...
-> Result<()> {
    ...
    let test = builder.build(&server).await?;
    wait_for_mcp_server(&test.codewen, CODEX_APPS_MCP_SERVER_NAME).await?;
    ...
    tokio::fs::remove_dir_all(test.codewen_home_path().join("cache/codewen_apps_tools")).await?;
