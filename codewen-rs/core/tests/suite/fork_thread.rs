...
use codewen_core::parse_turn_item;
...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    for text in ["first", "second", "third"] {
        codewen
            ...
            .await
            .unwrap();
        let _ = wait_for_event(&codewen, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    }

    // Request history from the base conversation to obtain rollout path.
    let base_path = codewen.rollout_path().expect("rollout path");
    ...
    let fork1_path = codewen_fork1.rollout_path().expect("rollout path");
    expected_after_first.push(thread_settings_applied_item(
        codewen_fork1.config_snapshot().await,
    ));
    ...
    let fork2_path = codewen_fork2.rollout_path().expect("rollout path");
    ...
    expected_after_second.push(thread_settings_applied_item(
        codewen_fork2.config_snapshot().await,
    ));
...
async fn fork_thread_from_history_does_not_require_source_rollout_path() {
    ...
    let source_path = codewen.rollout_path().expect("source rollout path");
