...
use codewen_exec_server::EnvironmentReadyInfo;
use codewen_exec_server::ExecServerError;
use codewen_exec_server::FileSystemSandboxContext;
use codewen_exec_server::LOCAL_ENVIRONMENT_ID;
use codewen_exec_server::NoiseChannelPublicKey;
use codewen_exec_server::NoiseRendezvousConnectBundle;
use codewen_exec_server::NoiseRendezvousConnectProvider;
use codewen_exec_server::REMOTE_ENVIRONMENT_ID;
...
use codewen_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = codewen_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.codewen, |event| {
        matches!(
            event,
...
async fn remote_test_env_exposes_target_shell_to_model() -> Result<()> {
    ...
    .await;
    let test = test_codewen().build_with_auto_env(&server).await?;
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/codewen-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\codewen-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    test.codewen
        ...
        .await?;
    let request = wait_for_event_match(&test.codewen, |event| match event {
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    assert_eq!(test.codewen.instruction_sources().await, vec![agents_path]);
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    test.codewen.ensure_rollout_materialized().await;
    test.codewen.flush_rollout().await?;
    let rollout_path = test.codewen.rollout_path().context("rollout path")?;
...
async fn remote_exec_materializes_target_roots_before_sandbox_selection() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/codewen-remote-target-roots-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/codewen-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/codewen-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/codewen-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/codewen-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/codewen-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/codewen-remote-dotdot-{}", std::process::id()));
...
async fn remote_test_env_remove_removes_symlink_not_target() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/codewen-remote-remove-link-{}",
        std::process::id()
    ));
    let allowed_dir = root.join("allowed");
