...
use codewen_protocol::approvals::ElicitationAction;
...
const CALENDAR_NAMESPACE: &str = "mcp__codewen_apps__calendar";
...
async fn start_install_turn(test: &TestCodex, prompt: &str) -> Result<ElicitationRequestEvent> {
    ...
    Ok(wait_for_event_match(&test.codewen, |event| match event {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "codewen_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": codewen_login::default_client::originator().value,
            }
        })
    );
