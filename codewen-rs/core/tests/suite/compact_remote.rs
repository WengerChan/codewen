...
use codewen_core::compact::SUMMARY_PREFIX;
use codewen_features::Feature;
use codewen_login::CodewenAuth;
use codewen_login::auth::AgentIdentityAuth;
use codewen_login::auth::AgentIdentityAuthRecord;
use codewen_protocol::account::PlanType as AccountPlanType;
use codewen_protocol::config_types::ServiceTier;
use codewen_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use codewen_protocol::dynamic_tools::DynamicToolFunctionSpec;
use codewen_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use codewen_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use core_test_support::test_codewen::TestCodexBuilder;
use core_test_support::test_codewen::TestCodexHarness;
use core_test_support::test_codewen::test_codewen as base_test_codex;
...
const TURN_STATE_HEADER: &str = "x-codewen-turn-state";
...
fn test_codewen() -> TestCodexBuilder {
    base_test_codex().with_config(|config| {
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    })
}

fn remote_realtime_test_codewen_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestCodexBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_codewen()
        .with_auth(CodewenAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(codewen: &codewen_core::CodewenThread) -> Result<()> {
    codewen
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            codewen_responses_as_items: false,
            codewen_response_item_prefix: None,
            codewen_response_handoff_mode:
                codewen_protocol::protocol::CodewenResponseHandoffMode::Thinking,
            model: None,
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    .await?;
    let codewen = harness.test().codewen.clone();
    ...
    codewen
        ...
        .await?;
    wait_for_turn_complete(&codewen).await;
    ...
    assert_eq!(
        ...
        compact_request.header("x-codewen-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-codewen-window-id").as_deref()
    );
    assert_ne!(
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestCodexHarness::with_builder(
        test_codewen().with_auth(CodewenAuth::AgentIdentity(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let mut builder = test_codewen().with_auth(auth);
...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        CodewenAuth::from_api_key("dummy"),
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-codewen-beta-features")
            .as_deref()
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-codewen-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let new_thread = test
        ...
        .await?;
    test.codewen = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "codewen_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "codewen_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    let responses_mock = responses::mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ...
                responses::ev_function_call_with_namespace(call_id, "codewen_app", tool_name, "{}"),
    ...
    test.codewen
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.codewen, |event| {
    ...
    test.codewen
        ...
        .await?;
    wait_for_turn_complete(&test.codewen).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&codewen, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&codewen, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    .await?;
    let baseline_codex = baseline_harness.test().codewen.clone();
    ...
    .await?;
    let override_codex = override_harness.test().codewen.clone();
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = codewen.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.codewen
        ...
        .await?;
    wait_for_event(&test.codewen, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    start_realtime_conversation(test.codewen.as_ref()).await?;
    ...
    close_realtime_conversation(test.codewen.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_codewen_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_codewen_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.codewen.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.codewen.as_ref()).await?;
    initial.codewen.submit(Op::Shutdown).await?;
    wait_for_event(&initial.codewen, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    let mut resume_builder = remote_realtime_test_codewen_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.codewen.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.codewen.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_codewen_builder(&realtime_server);
    let test = builder.build(&server).await?;
