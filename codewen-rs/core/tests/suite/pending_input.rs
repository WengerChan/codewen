...
use codewen_extension_items::sleep::SleepItem;
...
async fn build_codex(server: &StreamingSseServer) -> Arc<CodewenThread> {
    test_codewen()
        .with_model("gpt-5.4")
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&codewen, INITIAL_PROMPT).await;
    wait_for_event(&codewen, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&codewen, STEER_PROMPT).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&codewen, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&codewen, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&codewen, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&codewen, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&codewen, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&codewen, "new mailbox input").await;
    wait_for_sleep_item_completed(&codewen, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&codewen).await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let codewen = build_codex(&server).await;

    submit_user_input(&codewen, "first prompt").await;

    wait_for_reasoning_item_started(&codewen).await;

    submit_queue_only_agent_mail(&codewen, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&codewen, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&codewen, "second prompt").await;
    wait_for_turn_complete(&codewen).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&codewen, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&codewen, "resumed old task").await;
    wait_for_turn_complete(&codewen).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&codewen, "processed steered prompt").await;
    wait_for_turn_complete(&codewen).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&codewen, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&codewen, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
