use anyhow::Result;
use anyhow::anyhow;
use codewen_core::compact::SUMMARIZATION_PROMPT;
...
use core_test_support::test_codewen::turn_permission_fields;
...
async fn assert_compaction_uses_turn_lifecycle_id(
    ...
    while turn_completed_id.is_none() {
        let event = codewen.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    codewen.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&codewen, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    codewen.submit(Op::Shutdown).await.unwrap();
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    codewen.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&codewen, |ev| match ev {
    ...
    wait_for_event(&codewen, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&codewen, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let codewen = builder.build(&server).await.unwrap().codewen;
    ...
    let first = wait_for_event_match(&codewen, |ev| match ev {
    ...
    let last = wait_for_event_match(&codewen, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        codewen_protocol::models::ResponseItem::Message {
            id: None,
        ...
        codewen_protocol::models::ResponseItem::Compaction {
            id: None,
    ...
    let mut resume_builder = test_codewen().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    });
    ...
    wait_for_event(&resumed.codewen, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let test = builder.build(&server).await.expect("build test codewen");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.codewen).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let mut initial_builder = test_codewen()
        .with_auth(CodewenAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test codewen");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    let model_provider = openai_model_provider(&server);
    let mut resumed_builder = test_codewen()
        .with_auth(CodewenAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(retired_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.codewen).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&codewen, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&codewen, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-codewen-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-codewen-window-id").as_deref()
    );
    assert_ne!(
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let codewen = builder.build(&server).await.unwrap();

    codewen.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    codewen.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    codewen.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&codewen, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compaction_keeps_the_creation_time_global_instructions() -> Result<()> {
    ...
    assert_eq!(
        test.codewen.instruction_sources().await,
...
-> Result<()> {
    ...
    test.codewen.flush_rollout().await?;
    ...
    let mut resume_builder = test_codewen()
        .with_home(Arc::clone(&home))
        .with_auth(CodewenAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
    ...
    assert_eq!(
        resumed.codewen.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
