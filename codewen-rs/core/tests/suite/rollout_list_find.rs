...
use chrono::Utc;
use codewen_core::RolloutRecorder;
use codewen_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(codewen_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = codewen_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(codewen_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(codewen_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(codewen_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = codewen_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_codewen_home_directory() {
    let repo = TempDir::new().unwrap();
    let codewen_home = repo.path().join(".codewen");
    std::fs::create_dir_all(&codewen_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".codewen/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&codewen_home, id);

    let found =
        find_thread_path_by_id_str(&codewen_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
