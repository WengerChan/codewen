...
use codewen_core::ModelClient;
use codewen_core::NewThread;
use codewen_core::Prompt;
use codewen_core::ResponseEvent;
use codewen_core::ThreadManager;
use codewen_core::resolve_installation_id;
use codewen_core::thread_store_from_config;
use codewen_extension_api::empty_extension_registry;
use codewen_features::Feature;
use codewen_login::AuthKeyringBackendKind;
...
use codewen_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use codewen_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use codewen_protocol::models::LocalShellAction;
use codewen_protocol::models::LocalShellExecAction;
use codewen_protocol::models::LocalShellStatus;
use codewen_protocol::models::MessagePhase;
use codewen_protocol::models::ReasoningItemContent;
use codewen_protocol::models::ReasoningItemReasoningSummary;
use codewen_protocol::models::ResponseItem;
use codewen_protocol::models::WebSearchAction;
use codewen_protocol::openai_models::InputModality;
...
use codewen_protocol::protocol::RolloutLine;
...
use core_test_support::test_codewen::TestCodex;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> codewen_core::CodewenResponsesMetadata {
...
fn assert_codewen_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-codewen-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let turn_metadata_str = client_metadata["x-codewen-turn-metadata"]
        .as_str()
        .expect("missing x-codewen-turn-metadata client metadata");
    let turn_metadata = serde_json::from_str::<serde_json::Value>(turn_metadata_str)
        .expect("invalid x-codewen-turn-metadata json");
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-codewen-window-id"].as_str(),
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_codewen().build(&server).await.unwrap();
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    wait_for_event(&initial.codewen, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let codewen_home = Arc::new(TempDir::new()?);
    let mut builder = test_codewen();
    let first = builder
        .resume(&server, Arc::clone(&codewen_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.codewen.submit(Op::Shutdown).await?;
    wait_for_event(&first.codewen, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder.resume(&server, codewen_home, session_path).await?;
...
async fn response_item_ids_are_sent_for_all_remote_v2_compaction_requests() -> anyhow::Result<()> {
    ...
    let test = test_codewen()
        .with_auth(CodewenAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
    ...
    test.codewen.submit(Op::Compact).await?;
    wait_for_event(&test.codewen, |event| {
...
/// Writes an `auth.json` into the provided `codewen_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        codewen_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: codewen_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = codewen_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = codewen_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![codewen_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = codewen_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let codewen_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codewen()
        .with_home(codewen_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, codewen_home, session_path.clone())
        .await
        .expect("resume conversation");
    let codewen = test.codewen.clone();
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let codewen_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codewen().with_model("gpt-5.4");
    let test = builder
        .resume(&server, codewen_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_codewen().with_auth(CodewenAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.codewen_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_codewen_client_metadata(
        &request_body,
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    let codewen_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&codewen_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = codewen_core::test_support::get_model_offline(config.model.as_deref());
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &codewen_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_codewen()
        .with_auth(CodewenAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/codewen", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_codewen()
        .with_auth(create_dummy_codewen_auth())
        .with_config(move |config| {
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/codewen/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = CodewenAuth::from_auth_storage(
        codewen_home.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
        AuthKeyringBackendKind::default(),
        &codewen_login::test_support::transport_default_auth_route_config(),
    )
    .await
    .expect("Failed to load CodewenAuth")
    .expect("No CodewenAuth found in codewen_home");
    let auth_manager = codewen_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.codewen_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        codewen_core::build_models_manager(&config, auth_manager),
        codewen_core::CodewenAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(codewen_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(codewen_core::test_support::EmptyUserInstructionsProvider),
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__codewen_apps"),
        "did not expect codewen_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect codewen_apps resource calls: {resource_methods:?}"
    );
}
...
async fn skills_append_to_developer_message() {
    ...
    let codewen_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = codewen_home.path().join("skills/demo");
    ...
    let codewen_home_path = codewen_home.path().to_path_buf();
    let mut builder = test_codewen()
        .with_home(codewen_home.clone())
        .with_auth(CodewenAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = codewen_home_path.abs();
        });
    let codewen = builder
        .build(&server)
        .await
    ...
    let _codewen_home_guard = codewen_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let codewen_home_parent = TempDir::new().unwrap();
    let long_home_parent = codewen_home_parent
        .path()
        .join("codewen-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let codewen_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = codewen_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_codewen()
        ...
        .with_config(move |config| {
            config.cwd = codewen_home_path.abs();
            let user_config_path = codewen_home_path.join("config.toml").abs();
    ...
    let _codewen_home_guard = codewen_home;
    let _codewen_home_parent_guard = codewen_home_parent;
}
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestCodex { codewen, .. } = test_codewen()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestCodex { codewen, .. } = test_codewen().with_model("gpt-5.4").build(&server).await?;
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-codewen-primary-used-percent", "12.5")
        .insert_header("x-codewen-secondary-used-percent", "40.0")
        .insert_header("x-codewen-primary-window-minutes", "10")
        .insert_header("x-codewen-secondary-window-minutes", "60")
        .insert_header("x-codewen-primary-reset-at", "1704069000")
        .insert_header("x-codewen-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_codewen()
        .with_auth(CodewenAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let codewen = builder
    ...
    wait_for_event(&codewen, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-codewen-primary-used-percent", "100.0")
        .insert_header("x-codewen-secondary-used-percent", "87.5")
        .insert_header("x-codewen-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-codewen-primary-window-minutes", "15")
        .insert_header("x-codewen-secondary-window-minutes", "60")
        .insert_header("x-codewen-credits-has-credits", "true")
        .insert_header("x-codewen-credits-unlimited", "false")
        .insert_header("x-codewen-credits-balance", "")
        .insert_header(
            "x-codewen-rate-limit-reached-type",
    ...
    let mut builder = test_codewen();
    let codewen_fixture = builder.build(&server).await?;
    let codewen = codewen_fixture.codewen.clone();
    ...
    let token_event = wait_for_event(&codewen, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&codewen, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&codewen, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&codewen, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = CodewenErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_codewen_auth() -> CodewenAuth {
