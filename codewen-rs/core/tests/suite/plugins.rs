...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__codewen_apps__google_calendar";
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    write_plugin_skill_plugin(codewen_home.as_ref());
    write_plugin_app_plugin(codewen_home.as_ref());
    let test_codewen = build_apps_enabled_plugin_test_codex(
        &server,
        Arc::clone(&codewen_home),
        apps_server.chatgpt_base_url,
    )
    .await?;
    let codewen = Arc::clone(&test_codewen.codewen);

    codewen
        .submit(Op::UserInput {
            items: vec![codewen_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins() -> Result<()> {
    ...
    write_plugin_skill_plugin(codewen_home.as_ref());
    write_plugin_mcp_plugin(codewen_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(codewen_home.as_ref());

    let test_codewen =
        build_apps_enabled_plugin_test_codex(&server, codewen_home, apps_server.chatgpt_base_url)
            .await?;
    let codewen = Arc::clone(&test_codewen.codewen);
    wait_for_mcp_server(&codewen, CODEX_APPS_MCP_SERVER_NAME).await?;

    codewen
        .submit(Op::UserInput {
            items: vec![codewen_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(codewen_home.as_ref());
    write_plugin_mcp_plugin(codewen_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(codewen_home.as_ref(), "sample_app");

    let test_codewen =
        build_apps_enabled_plugin_test_codex(&server, codewen_home, apps_server.chatgpt_base_url)
            .await?;
    let codewen = Arc::clone(&test_codewen.codewen);
    wait_for_mcp_server(&codewen, "sample").await?;
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let codewen_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(codewen_home.as_ref());
    let test_codewen = build_analytics_plugin_test_codex(&server, codewen_home).await?;
    let codewen = Arc::clone(&test_codewen.codewen);
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "codewen_plugin_used")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        serde_json::json!(codewen_login::default_client::originator().value)
    );
