...
use codewen_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use codewen_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use codewen_core::CodewenResponsesMetadata;
use codewen_core::ModelClient;
use codewen_core::ModelClientSession;
use codewen_core::Prompt;
use codewen_core::ResponseEvent;
use codewen_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
use codewen_rollout_trace::ConversationPart;
use codewen_rollout_trace::InferenceTraceContext;
use codewen_rollout_trace::RawTraceEventPayload;
use codewen_rollout_trace::TraceWriter;
...
fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> CodewenResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(codewen_login::default_client::get_codewen_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-codewen-installation-id"].as_str(),
...
async fn responses_websocket_preconnect_reuses_connection() {
    ...
    assert_eq!(
        server.single_handshake().header("x-codewen-window-id"),
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-codewen-turn-metadata"]
            .as_str()
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::CodewenTurnStarted {
            codewen_turn_id: "turn-1".to_string(),
            thread_id: harness.thread_id.to_string(),
        })
        .expect("turn started");
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server.single_handshake().header("x-codewen-turn-metadata"),
        None
    );
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-codewen-primary-used-percent": "100.0",
            "x-codewen-secondary-used-percent": "87.5",
            "x-codewen-primary-over-secondary-limit-percent": "95.0",
            "x-codewen-primary-window-minutes": "15",
            "x-codewen-secondary-window-minutes": "60",
            "x-codewen-credits-has-credits": "true",
            "x-codewen-credits-unlimited": "false",
            "x-codewen-credits-balance": "",
            "x-codewen-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let test = builder
        ...
        .await
        .expect("build websocket codewen");
    ...
    let token_event =
        wait_for_event(&test.codewen, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.codewen, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-codewen-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-codewen-turn-metadata"]
            .as_str()
...
async fn responses_websocket_sends_canonical_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        body["client_metadata"]["x-codewen-turn-metadata"]
            .as_str()
