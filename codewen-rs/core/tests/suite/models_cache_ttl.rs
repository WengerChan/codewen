...
use codewen_models_manager::client_version_to_whole;
...
const REMOTE_MODEL: &str = "codewen-test-ttl";
const VERSIONED_MODEL: &str = "codewen-test-versioned";
const MISSING_VERSION_MODEL: &str = "codewen-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "codewen-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let codewen = Arc::clone(&test.codewen);
    ...
    let cache_path = config.codewen_home.join(CACHE_FILE);
    ...
    codewen
        .submit(Op::UserInput {
            ...
            thread_settings: codewen_protocol::protocol::ThreadSettingsOverrides {
                ...
                approval_policy: Some(codewen_protocol::protocol::AskForApproval::Never),
    ...
    let _ = wait_for_event(&codewen, |event| matches!(event, EventMsg::TurnComplete(_))).await;
