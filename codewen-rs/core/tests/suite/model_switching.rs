use anyhow::Result;
use codewen_config::types::Personality;
...
async fn model_change_appends_model_instructions_developer_message() -> Result<()> {
    ...
    let mut builder = test_codewen().with_model("gpt-5.2");
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-codewen-personality";
...
async fn unsupported_configured_service_tier_warns_at_session_start() -> Result<()> {
    ...
    let warning = wait_for_event(&test.codewen, |event| {
        matches!(
            event,
...
async fn thread_rollback_after_generated_image_drops_entire_image_turn_history() -> Result<()> {
    ...
    test.codewen
        ...
        .await?;
    wait_for_event(&test.codewen, |ev| {
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.codewen, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.codewen, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.codewen, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
