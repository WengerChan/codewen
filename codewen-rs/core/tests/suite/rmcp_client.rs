...
use codewen_config::types::McpServerAuth;
...
use codewen_exec_server::HttpRedirectPolicy;
use codewen_exec_server::HttpRequestParams;
use codewen_login::CodewenAuth;
use codewen_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use codewen_utils_cargo_bin::cargo_bin;
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        codewen_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/codewen-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_structured_tool(
    ...
    .await;
    let end_event = wait_for_event(&fixture.codewen, |ev| {
...
async fn openai_form_capability_updates_for_loaded_thread() -> anyhow::Result<()> {
    ...
    let fixture = test_codewen()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&fixture.codewen, server_name).await?;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.codewen, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn shutdown_cancels_startup_prewarm_waiting_for_mcp_startup() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.codewen.shutdown_and_wait())
        .await
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            codewen_linux_sandbox_exe: fixture.config.codewen_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.codewen, |ev| {
            matches!(
                ev,
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: codewen_protocol::openai_models::ReasoningEffort::Medium,
    ...
    fixture
        ...
        .list_models(
            ...
            codewen_core::test_support::default_http_client_factory(),
        )
        .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_codewen()
        .with_auth(CodewenAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
    ...
    wait_for_mcp_server(&configured_auth_fixture.codewen, "configured_auth").await?;
...
async fn streamable_http_chatgpt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    ...
    let untrusted_mcp_url = format!("{}/api/codewen/ps/mcp", untrusted_apps.chatgpt_base_url);
    ...
    wait_for_mcp_server(&fixture.codewen, "untrusted_origin").await?;
    let observed_requests = untrusted_server
        .received_requests()
        .await
        .expect("mock server should capture MCP startup requests")
        .into_iter()
        .filter(|request| request.url.path() == "/api/codewen/ps/mcp")
        .filter_map(|request| {
...
async fn configured_chatgpt_base_url_does_not_grant_mcp_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let fixture = test_codewen()
        .with_auth(CodewenAuth::create_dummy_chatgpt_auth_for_testing())
        .with_pre_build_hook(move |codewen_home| {
            fs::write(
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    let _codewen_home_guard = EnvVarGuard::set("CODEWEN_HOME", temp_home.path().as_os_str());
