...
use codewen_core::config::CurrentTimeReminderConfig;
...
use codewen_web_search_extension::install as install_web_search_extension;
...
async fn run_code_mode_turn_with_model_and_config(
    ...
    let builder = test_codewen().with_model(model).with_config(move |config| {
...
async fn missing_process_host_falls_back_to_in_process_code_mode() -> Result<()> {
    ...
    let builder = test_codewen()
        .with_model("test-gpt-5.1-codewen")
        .with_code_mode_host_program("codewen-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
...
"#,
    ...
    let auth = CodewenAuth::from_api_key("dummy");
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-codewen")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-codewen").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let mut builder = test_codewen().with_model(model).with_config(move |config| {
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.codewen, "rmcp").await?;
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__codewen_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__codewen_apps__calendar_timezone_option_99",
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "codewen_app__hidden_dynamic_tool");
const out = await tools.codewen_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    test.codewen
        .submit(Op::UserInput {
            ...
            thread_settings: codewen_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(codewen_protocol::protocol::TurnEnvironmentSelections::new(
                    cwd,
                    Vec::new(),
                )),
    ...
    let turn_id = wait_for_event_match(&test.codewen, |event| match event {
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("codewen_app"));
    ...
    test.codewen
        ...
        .await?;
    wait_for_event(&test.codewen, |event| match event {
        ...
        parsed
            ...
            .is_some_and(|description| {
                description.contains("Codex app tools.")
                    ...
                    && description.contains("codewen_app__hidden_dynamic_tool(args:")
            })
    );
