...
async fn response_for_remote_model(
    ...
    loop {
        match wait_for_event(&test.codewen, |_| true).await {
...
async fn remote_tool_mode_selector_overrides_feature_flags() -> Result<()> {
    ...
    assert!(
        direct_tools
            .iter()
            .all(|name| name != codewen_code_mode::PUBLIC_TOOL_NAME
                && name != codewen_code_mode::WAIT_TOOL_NAME),
    ...
    assert_eq!(
        ...
        vec![
            ...
            codewen_code_mode::PUBLIC_TOOL_NAME.to_string(),
            codewen_code_mode::WAIT_TOOL_NAME.to_string(),
    ...
    assert!(
        tool_names(&unsupported_response.body)
            .iter()
            .any(|name| name == codewen_code_mode::PUBLIC_TOOL_NAME)
    );
    assert_eq!(
...
async fn remote_multi_agent_selector_uses_model_selected_before_first_turn() -> Result<()> {
    ...
    assert_eq!(
        (
            ...
            test.codewen.multi_agent_version(),
        ),
        (1, None)
    );
    ...
    .await?;
    assert_eq!(test.codewen.multi_agent_version(), None);
