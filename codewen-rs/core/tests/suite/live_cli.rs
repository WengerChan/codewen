...
//! `just test -p codewen-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let codewen_home = home.path().join(".codewen");
    ...
    let mut cmd = Command::new(codewen_utils_cargo_bin::cargo_bin("codewen-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn codewen-rs");
