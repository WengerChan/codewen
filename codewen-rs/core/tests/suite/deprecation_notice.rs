...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    let TestCodex { codewen, .. } = builder.build(&server).await?;

    let notice = wait_for_event_match(&codewen, |event| match event {
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable unified_exec` or `[features].unified_exec` in config.toml. See https://developers.openai.com/codewen/config-basic#feature-flags for details."
        ),
    );
