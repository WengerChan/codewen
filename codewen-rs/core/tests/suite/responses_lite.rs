...
const RESPONSES_LITE_HEADER: &str = "x-openai-internal-codewen-responses-lite";

fn responses_extensions(auth: &CodewenAuth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut codewen_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_prepares_images() -> Result<()> {
    ...
    let mut builder = test_codewen().with_model_info_override("gpt-5.4", |model_info| {
        model_info.use_responses_lite = true;
