...
use codewen_core::config::ThreadStoreConfig;
...
use codewen_protocol::protocol::ExecPolicyAmendment;
...
async fn wait_for_spawned_thread(test: &TestCodex) -> Result<Arc<CodewenThread>> {
...
/// same sandbox with `codewen sandbox -P`. The nested invocation must also pass
...
"#
    ...
    let approval_policy = AskForApproval::OnRequest;
    let mut builder = test_codewen().with_home(home).with_config(move |config| {
        config.permissions.approval_policy = Constrained::allow_any(approval_policy);
    });
    let test = builder.build(&server).await?;
...
/// profile names before passing one to `codewen sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def main():
    args = parse_args()
    profile_name = os.environ.get("CODEWEN_PERMISSION_PROFILE")
    if not profile_name:
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected codewen sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://codewen-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
