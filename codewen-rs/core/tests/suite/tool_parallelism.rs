...
async fn build_codewen_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestCodex> {
    let mut builder = test_codewen().with_model("test-gpt-5.1-codewen");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_codewen_with_test_tool(&server).await?;
