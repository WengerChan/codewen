...
use codewen_core::REVIEW_PROMPT;
...
use codewen_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
use codewen_protocol::protocol::ReviewCodeLocation;
use codewen_protocol::protocol::ReviewFinding;
use codewen_protocol::protocol::ReviewLineRange;
use codewen_protocol::protocol::ReviewOutputEvent;
...
use codewen_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    ...
    let codewen = new_conversation_for_server(&server, codewen_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&codewen, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&codewen, |ev| {
        matches!(
            ev,
    ...
    let entered = wait_for_event(&codewen, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    match entered {
    ...
    let exited_started = wait_for_event(&codewen, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&codewen, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let path = codewen.rollout_path().expect("rollout path");
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match codewen.next_event().await.expect("review event").msg {
    ...
    codewen.submit(Op::Interrupt).await.unwrap();
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let codewen = new_conversation_for_server(&server, codewen_home.clone(), |cfg| {
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = codewen_home.path().join("resume.jsonl");
    {
        ...
        let user = codewen_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = codewen_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let codewen =
        resume_conversation_for_server(&server, codewen_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let codewen = new_conversation_for_server(&server, codewen_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, codewen_home, resume_path)
        .await
