...
fn codewen_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestCodexHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        ...
        let codewen_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&codewen_home)
            .with_context(|| format!("create stable test CODEX_HOME {}", codewen_home.display()))?;
        return Ok(TestCodexHome::Persistent(codewen_home));
    }
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("codewen-resources");
    ...
    for helper_name in ["codewen-windows-sandbox-setup", "codewen-command-runner"] {
        let helper = codewen_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let codewen_home =
        codewen_home_for_windows_sandbox_test("windows-restricted-token-deny-read-codewen-home")?;
...
async fn windows_elevated_does_not_create_missing_workspace_metadata() -> anyhow::Result<()> {
    let codewen_home =
        codewen_home_for_windows_sandbox_test("windows-elevated-missing-metadata-codewen-home")?;
    let _codewen_home_guard = EnvVarGuard::set("CODEWEN_HOME", codewen_home.path().as_os_str());
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    ...
    assert!(
        sandbox_setup_is_complete(codewen_home.path()),
