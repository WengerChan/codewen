...
use crate::test_codewen::TestCodex;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = codewen_utils_cargo_bin::cargo_bin("codewen-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `codewen-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_codewen().with_config(move |config| {
