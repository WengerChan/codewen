...
use codewen_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
pub use codewen_core::test_support::TestCodexResponsesRequestKind;
pub use codewen_core::test_support::responses_metadata;
use codewen_utils_absolute_path::AbsolutePathBuf;
pub use codewen_utils_absolute_path::test_support::PathBufExt;
pub use codewen_utils_absolute_path::test_support::PathExt;
...
pub mod test_codewen;
pub mod test_codewen_exec;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    codewen_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    codewen_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(codewen_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = codewen_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("codewen-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\codewen\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.codewen`.
pub async fn load_default_config_for_test(codewen_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        codewen_linux_sandbox_exe: Some(
            find_codewen_linux_sandbox_exe().expect("should find binary for codewen-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_codewen_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    codewen_utils_cargo_bin::cargo_bin("codewen-linux-sandbox")
}
...
where
    F: FnMut(&codewen_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(codewen, predicate, Duration::from_secs(1)).await
}
...
{
    let ev = wait_for_event(codewen, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&codewen_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), codewen.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    codewen_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    codewen_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    codewen_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! codewen_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_codewen_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("codewen-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
