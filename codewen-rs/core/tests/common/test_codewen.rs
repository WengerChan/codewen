...
use codewen_core::TimeProvider;
use codewen_core::config::Config;
use codewen_core::resolve_installation_id;
use codewen_core::shell::Shell;
use codewen_core::shell::get_shell_by_model_provided_path;
...
use codewen_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-gpt-5.1-codewen";
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "CODEWEN_TEST_REMOTE_EXEC_SERVER_URL";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: codewen_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: codewen_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
            },
        ...
        let environment =
            codewen_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &codewen_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                codewen_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestCodexBuilder {
    ...
    pub fn with_auth(mut self, auth: CodewenAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let codewen_linux_sandbox_exe = Some(
            crate::find_codewen_linux_sandbox_exe()
                .context("should find binary for codewen-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let codewen_linux_sandbox_exe = None;
        let local_runtime_paths = codewen_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            codewen_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            codewen_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            codewen_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<codewen_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestCodex> {
        let auth = self.auth.clone();
        let state_db = codewen_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.codewen_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(CodewenHomeUserInstructionsProvider::new(
                    config.codewen_home.clone(),
                ))
            });
        let auth_manager = codewen_core::test_support::auth_manager_from_auth(auth.clone());
        let thread_manager = ThreadManager::new(
            ...
            thread_store,
            codewen_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let code_mode_host_program = self
            ...
            .take()
            .or_else(|| codewen_utils_cargo_bin::cargo_bin("codewen-code-mode-host").ok());
        let thread_manager = if config.features.enabled(Feature::CodeModeHost)
            && let Some(code_mode_host_program) = code_mode_host_program
        {
            codewen_core::test_support::with_code_mode_host_program(
                thread_manager,
                code_mode_host_program,
            )
        } else {
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    codewen_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    codewen_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = codewen_utils_cargo_bin::cargo_bin("codewen") {
            config.codewen_self_exe = Some(path);
        } else if let Ok(path) = codewen_utils_cargo_bin::cargo_bin("codewen-exec") {
            // `codewen-exec` also supports `--codewen-run-as-apply-patch`, so use it
        ...
        {
            let codewen = bin_dir.join("codewen");
            let codewen_exec = bin_dir.join("codewen-exec");
            if codewen.is_file() {
                config.codewen_self_exe = Some(codewen);
            } else if codewen_exec.is_file() {
                config.codewen_self_exe = Some(codewen_exec);
            }
        }
...
impl TestCodex {
    ...
    pub fn codewen_home_path(&self) -> &Path {
        self.config.codewen_home.as_path()
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.codewen, |event| match event {
...
pub fn test_codewen() -> TestCodexBuilder {
    TestCodexBuilder {
        ...
        auth: CodewenAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
