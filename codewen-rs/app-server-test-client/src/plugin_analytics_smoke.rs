...
pub(super) const ANALYTICS_CAPTURE_ENV_VAR: &str = "CODEWEN_ANALYTICS_EVENTS_CAPTURE_FILE";
const TEST_USER_CONFIG_ENV_VAR: &str = "CODEWEN_APP_SERVER_TEST_USER_CONFIG_FILE";
...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!("codewen-plugin-analytics-{}.jsonl", process::id()))
    });
...
fn run_plugin_turn(client: &mut CodewenClient, expected: &ExpectedPlugin) -> Result<String> {
...
fn wait_for_plugin_usage(
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "codewen_plugin_used"
...
fn plugin_installed(client: &mut CodewenClient) -> Result<PluginInstalledResponse> {
...
fn wait_for_turn_analytics(path: &Path, turn_id: &str) -> Result<Vec<Value>> {
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "codewen_turn_event"
...
fn validate_plugin_events(events: Vec<Value>, expected: &ExpectedPlugin) -> Result<Vec<Value>> {
    ...
    for event_type in required_event_types() {
        ...
        if event_type == "codewen_plugin_used" {
...
fn required_event_types() -> [&'static str; 3] {
    [
        "codewen_plugin_disabled",
        "codewen_plugin_enabled",
        "codewen_plugin_used",
    ]
}
...
fn validate_used_metadata(event: &Value) -> Result<()> {
    ...
    for field in [
        ...
        if params.get(field).is_none_or(Value::is_null) {
            bail!("codewen_plugin_used event has null or missing `{field}`");
        }
    }
...
impl TemporaryConfigFile {
    fn create() -> Result<Self> {
        let path = std::env::temp_dir().join(format!(
            "codewen-plugin-analytics-config-{}.toml",
            process::id()
        ));
        fs::write(&path, "")
