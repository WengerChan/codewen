...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!(
            "codewen-plugin-analytics-mutation-{}.jsonl",
            process::id()
        ))
    });
    prepare_capture_file(&capture_path)?;
    let mut client = spawn_client(codewen_bin, config_overrides, &capture_path)?;
    ...
    match (sequence_result, restoration) {
        ...
        (sequence_result, RestorationStatus::Unknown(cleanup_err)) => {
            ...
            print_recovery_command(codewen_bin, config_overrides, remote_plugin_id);
            Err(cleanup_err)
        }
    }
}
...
pub(super) fn run_cleanup(
    ...
    let mut client = CodewenClient::spawn_stdio(codewen_bin, &overrides)?;
    client.initialize()?;
    ...
    match restore_uninstalled_state(&mut client, remote_plugin_id) {
        ...
        RestorationStatus::Dirty(err) => {
            print_dirty_recovery(codewen_bin, config_overrides, remote_plugin_id, &err);
            Err(err)
        }
...
fn spawn_client(
    ...
    CodewenClient::spawn_stdio_with_env(codewen_bin, &overrides, &environment)
}
...
fn uninstall_remote_plugin(client: &mut CodewenClient, remote_plugin_id: &str) -> Result<()> {
    let request_id = client.request_id();
...
fn print_recovery_command(codewen_bin: &Path, config_overrides: &[String], remote_plugin_id: &str) {
    let test_client = std::env::current_exe()
        .map(|path| path.display().to_string())
        .unwrap_or_else(|_| "codewen-app-server-test-client".to_string());
