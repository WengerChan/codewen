...
use codewen_app_server_protocol::JSONRPCMessage;
use codewen_app_server_protocol::JSONRPCNotification;
use codewen_app_server_protocol::JSONRPCRequest;
...
use codewen_app_server_protocol::SandboxPolicy;
...
use codewen_otel::OtelProvider;
use codewen_otel::current_span_w3c_trace_context;
use codewen_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "codewen-app-server-test-client";
...
struct Cli {
    /// Path to the `codewen` CLI binary. When set, requests use stdio by
    /// spawning `codewen app-server` as a child process.
    #[arg(long, env = "CODEWEN_BIN", global = true)]
    codewen_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--codewen-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "CODEWEN_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `codewen` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `codewen app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/codewen-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `codewen app-server --listen`.
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let codewen_bin = codewen_bin.unwrap_or_else(|| PathBuf::from("codewen"));
            serve(&codewen_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(codewen_bin, url)?;
        ...
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --codewen-bin and does not support --url"
                );
            }
            let codewen_bin =
                codewen_bin.context("plugin-analytics-mutation-smoke requires --codewen-bin")?;
...
fn resolve_endpoint(codewen_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if codewen_bin.is_some() && url.is_some() {
        bail!("--codewen-bin and --url are mutually exclusive");
    }
    if let Some(codewen_bin) = codewen_bin {
        return Ok(Endpoint::SpawnCodex(codewen_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if codewen_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --codewen-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(codewen_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(codewen_bin);
        if let Some(codewen_bin_parent) = codewen_bin.parent() {
            let mut path = OsString::from(codewen_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", codewen_bin.display()))?;
...
fn serve(codewen_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/codewen-app-server-test-client");
    ...
    let mut cmdline = format!(
        ...
        shell_quote(&codewen_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started codewen app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnCodex(codewen_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: codewen_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut CodewenClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = CodewenClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = CodewenClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (codewen_bin, url) {
        (Some(_), Some(_)) => bail!("--codewen-bin and --url are mutually exclusive"),
        (Some(codewen_bin), None) => {
            let server = BackgroundAppServer::spawn(&codewen_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve codewen-app-server-test-client binary path")?;
...
impl CodewenClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnCodex(codewen_bin) => Self::spawn_stdio(codewen_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(codewen_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(codewen_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let codewen_bin_display = codewen_bin.display();
        let mut cmd = Command::new(codewen_bin);
        if let Some(codewen_bin_parent) = codewen_bin.parent() {
            let mut path = OsString::from(codewen_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut codewen_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{codewen_bin_display}` app-server"))?;

        let stdin = codewen_app_server
            .stdin
            .take()
            .context("codewen app-server stdin unavailable")?;
        let stdout = codewen_app_server
            .stdout
            .take()
            .context("codewen app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: codewen_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `codewen-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "codewen-toy-app-server".to_string(),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: codewen_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
                codewen_streamlined_login: false,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: codewen_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to codewen app-server")?;
                    return Ok(());
                }
                bail!("codewen app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from codewen app-server")?;
                if bytes == 0 {
                    bail!("codewen app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = codewen_core::otel_init::build_provider(
            &config,
...
impl Drop for CodewenClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[codewen app-server exited: {status}]");
            return;
        }
