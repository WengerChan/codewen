...
async fn build_refresh_config(
    ...
    let mcp_servers = codewen_mcp::configured_mcp_servers(&mcp_config);
...
mod tests {
    ...
    use codewen_config::ThreadConfigContext;
    use codewen_config::ThreadConfigLoadError;
    use codewen_config::ThreadConfigLoadErrorCode;
    ...
    use codewen_extension_api::NoopExtensionEventSink;
    ...
    async fn refresh_config_uses_latest_auth_keyring_backend() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(codewen_config::CONFIG_TOML_FILE),
...
"#,
        ...
        actual.remove(codewen_mcp::CODEX_APPS_MCP_SERVER_NAME);
...
"#,
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let executor_skill_provider: Arc<dyn codewen_skills_extension::SkillProvider> = Arc::new(
            codewen_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                &good_config,
                auth_manager.clone(),
                codewen_core::build_models_manager(&good_config, auth_manager.clone()),
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        analytics_events_client: codewen_analytics::AnalyticsEventsClient::disabled(
                        ),
                        thread_manager: thread_manager.clone(),
                        goal_service: Arc::new(codewen_goal_extension::GoalService::new()),
                ...
                Arc::new(CodewenHomeUserInstructionsProvider::new(
                    good_config.codewen_home.clone(),
                )),
                /*analytics_events_client*/ None,
                Arc::clone(&thread_store),
                codewen_core::local_agent_graph_store_from_state_db(Some(&state_db)),
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> codewen_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
