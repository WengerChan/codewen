...
use chrono::DateTime;
use codewen_model_provider::is_supported_amazon_bedrock_region;
...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "CODEWEN_APP_SERVER_LOGIN_ISSUER";
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "CODEWEN_APP_SERVER_DEV_OPEN_APP_URL";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth
                .as_ref()
                .map(CodewenAuth::api_auth_mode)
                .map(auth_mode_to_api),
            plan_type: auth.as_ref().and_then(CodewenAuth::account_plan_type),
        }
    }
    ...
    async fn maybe_refresh_plugin_caches_for_current_config(
        ...
        thread_manager
            .plugins_manager()
            .set_auth_mode(auth.as_ref().map(CodewenAuth::api_auth_mode));
        thread_manager
            .plugins_manager()
    ...
    async fn login_v2(
        ...
        match params {
            ...
            LoginAccountParams::Chatgpt {
                ...
                self.login_chatgpt_v2(request_id, codewen_streamlined_login, login_success_page)
                    .await;
            }
    ...
    async fn login_chatgpt_common(
        &self,
        codewen_streamlined_login: bool,
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(codewen_streamlined_login, login_success_page)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(codewen_streamlined_login, login_success_page)
            .await?;
    ...
    async fn logout_common(&self) -> std::result::Result<Option<AuthMode>, JSONRPCErrorError> {
        let managed_bedrock_auth = matches!(
            self.auth_manager.auth_cached(),
            Some(CodewenAuth::BedrockApiKey(_))
        );
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "codewen account authentication required to read rate limits",
            ));
        };
        ...
        let response = response
            .map_err(|err| internal_error(format!("failed to fetch codewen rate limits: {err}")))?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch codewen rate limits: no snapshots returned",
            ));
        }
        ...
        let rate_limits_by_limit_id: HashMap<_, _> = response
            ...
            .map(|snapshot| {
                let limit_id = snapshot
                    .limit_id
                    .clone()
                    .unwrap_or_else(|| "codewen".to_string());
                (limit_id, snapshot)
            })
            .collect();
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "codewen account authentication required to read token usage",
            ));
        };
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "codewen account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "codewen account authentication required to notify workspace owner",
            ));
        };
...
mod tests {
    use super::*;
    use codewen_backend_client::TokenUsageProfileDailyBucket;
    use codewen_backend_client::TokenUsageProfileStats;
    ...
    fn workspace_messages_feature_disabled_only_for_not_found() {
        ...
        for (status, expected) in cases {
            let err = BackendRequestError::UnexpectedStatus {
                ...
                url: "https://example.test/api/codewen/workspace-messages".to_string(),
                status,
