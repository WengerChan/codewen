...
use codewen_app_server_protocol::ComputerUseRequirements;
...
use codewen_app_server_protocol::ConfigRequirements;
use codewen_app_server_protocol::ConfigRequirementsReadResponse;
use codewen_app_server_protocol::ConfigValueWriteParams;
use codewen_app_server_protocol::ConfigWriteErrorCode;
use codewen_app_server_protocol::ConfigWriteResponse;
use codewen_app_server_protocol::ConfiguredHookHandler;
use codewen_app_server_protocol::ConfiguredHookMatcherGroup;
use codewen_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use codewen_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use codewen_app_server_protocol::FeedbackRequirements;
use codewen_app_server_protocol::JSONRPCErrorError;
use codewen_app_server_protocol::ManagedHooksRequirements;
use codewen_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use codewen_app_server_protocol::ModelsRequirements;
use codewen_app_server_protocol::NetworkDomainPermission;
use codewen_app_server_protocol::NetworkRequirements;
use codewen_app_server_protocol::NetworkUnixSocketPermission;
...
use codewen_config::HookEventsToml;
use codewen_config::HookHandlerConfig as CoreHookHandlerConfig;
use codewen_config::ManagedHooksRequirementsToml;
use codewen_config::MatcherGroup as CoreMatcherGroup;
use codewen_config::ResidencyRequirement as CoreResidencyRequirement;
use codewen_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use codewen_core::ThreadManager;
use codewen_features::canonical_feature_for_key;
use codewen_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<codewen_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = codewen_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(codewen_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(codewen_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            codewen_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            codewen_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: codewen_config::ComputerUseRequirementsToml,
...
) -> codewen_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => codewen_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: codewen_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(codewen_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(codewen_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(codewen_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: codewen_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        codewen_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        codewen_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}
...
mod tests {
    ...
    use codewen_app_server_protocol::FeedbackRequirements;
    use codewen_app_server_protocol::WindowsSandboxSetupMode;
    use codewen_config::ComputerUseRequirementsToml;
    use codewen_config::ConfigRequirementsToml;
    use codewen_config::ModelsRequirementsToml;
    use codewen_config::NewThreadModelDefaultsToml;
    use codewen_config::WindowsRequirementsToml;
