...
use codewen_goal_extension::GoalServiceError;
...
impl ThreadGoalRequestProcessor {
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            codewen_rollout::find_thread_path_by_id_str(
    ...
    async fn reconcile_thread_goal_rollout(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => codewen_rollout::find_thread_path_by_id_str(
                &self.config.codewen_home,
                &thread_id.to_string(),
                self.state_db.as_deref(),
            )
...
pub(super) fn api_thread_goal_from_state(goal: codewen_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn api_thread_goal_status_from_state(status: codewen_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
