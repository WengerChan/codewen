...
impl McpRequestProcessor {
    ...
    async fn mcp_server_oauth_login_response(
        ...
        let effective_servers = codewen_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        let Some(server) = effective_servers
            .get(&name)
            .and_then(codewen_mcp::EffectiveMcpServer::configured_config)
        else {
    ...
    async fn list_mcp_server_status_task(
        ...
        request_id: ConnectionRequestId,
        params: ListMcpServerStatusParams,
        mcp_config: codewen_mcp::McpConfig,
    ...
    async fn read_mcp_resource(
        ...
        let mcp_manager = self.thread_manager.mcp_manager();
        let mcp_config = mcp_manager.runtime_config(&config).await;
        let codewen_apps_tools_cache = mcp_manager.codewen_apps_tools_cache();
