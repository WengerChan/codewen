...
use codewen_app_server_protocol::SelectedCapabilityRoot;
...
const CODEX_TUI_CLIENT_NAME: &str = "codewen-tui";
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: codewen_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                codewen_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = codewen_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<CodewenThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            codewen_home: self.config.codewen_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(codewen_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<codewen_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<codewen_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                codewen_core::config::set_project_trust_level(
                    &listener_task_context.codewen_home,
        ...
        if let Ok(Some(err)) =
            codewen_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let NewThread {
            ...
            .map_err(|err| match err {
                ...
                CodewenErr::UnsupportedOperation(message) => method_not_found(message),
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<codewen_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<codewen_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(codewen_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(codewen_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            codewen_linux_sandbox_exe: self.arg0_paths.codewen_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = codewen_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.codewen_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.codewen_home.display()
                ))
            })?;
    ...
    ) -> Result<codewen_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = codewen_thread.legacy_instruction_sources().await;
                ...
                thread.thread_source = codewen_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = codewen_thread.config_snapshot().await;
                ...
                self.thread_goal_processor
                    .emit_resume_goal_snapshot_and_continue(thread_id, codewen_thread.as_ref())
                    .await;
            }
            Err(err) => {
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(codewen_core::util::normalize_thread_name);
        ...
        let NewThread {
            ...
            .await
            .map_err(|err| match err {
                CodewenErr::Io(_) | CodewenErr::Json(_) => {
...
fn deserialize_stored_thread_item(
    item: codewen_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        codewen_error_info: error.codewen_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: CodewenErr) -> JSONRPCErrorError {
    match err {
...
) -> (Thread, Option<codewen_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<codewen_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(codewen_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match codewen_core::parse_turn_item(item) {
                Some(codewen_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            codewen_protocol::config_types::SandboxMode::WorkspaceWrite
                | codewen_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &codewen_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        codewen_protocol::models::PermissionProfile::Disabled
        | codewen_protocol::models::PermissionProfile::External { .. } => true,
        codewen_protocol::models::PermissionProfile::Managed { .. } => profile
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<codewen_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
