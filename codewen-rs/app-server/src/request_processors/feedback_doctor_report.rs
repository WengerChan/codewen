...
//! `codewen doctor --json`, derives a small set of Sentry tags, and otherwise
...
use codewen_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `codewen-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `codewen doctor --json` and returns a best-effort feedback attachment.
///
