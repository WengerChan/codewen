...
use codewen_connectors::ConnectorRuntimeTool;
use codewen_connectors::connector_runtime_context_key;
use codewen_connectors::connector_tool_is_synthetic;
use codewen_connectors::installed_connector_runtime;
...
use codewen_mcp::host_owned_codewen_apps_enabled;
...
const APPS_INSTALLED_RESPONSE_BYTES_METRIC: &str = "codewen.apps.installed.response_bytes";
const APPS_INSTALLED_CONNECTOR_COUNT_METRIC: &str = "codewen.apps.installed.connector_count";
const APPS_INSTALLED_TOOL_COUNT_METRIC: &str = "codewen.apps.installed.tool_count";
const APPS_SNAPSHOT_AGE_METRIC: &str = "codewen.apps.snapshot.age_ms";
    ...
    pub(crate) async fn apps_installed(
        ...
        let result = async {
            ...
            let previous_snapshot = mcp_manager
                ...
                .current_snapshot(config.codewen_home.to_path_buf(), cache_key.clone());
            let snapshot = if force_refresh && runtime_enabled {
                let refresh_result = async {
                    ...
                    let codewen_apps_auth_manager =
                        host_owned_codewen_apps_enabled(&mcp_config, auth.as_ref())
