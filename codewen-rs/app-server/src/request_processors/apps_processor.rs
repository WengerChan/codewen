...
impl AppsRequestProcessor {
    ...
    async fn apps_list_task(
        ...
        let should_retry = result
            .as_ref()
            .is_ok_and(|(_, codewen_apps_ready)| !codewen_apps_ready);
        outgoing
        ...
        if should_retry && !retry_params.force_refetch {
            ...
            .await
            {
                warn!("failed to refresh app list after codewen-apps readiness retry: {err:?}");
            }
        }
    }
    ...
    async fn apps_list_response(
        ...
        let mut codewen_apps_ready = true;
        ...
        loop {
            ...
            match result {
                AppListLoadResult::Accessible(Ok(status)) => {
                    ...
                    codewen_apps_ready = status.codewen_apps_ready;
                }
            ...
            if accessible_loaded && all_loaded {
                ...
                return Ok((response, codewen_apps_ready));
            }
        }
    }
...
const APPS_INSTALLED_DURATION_METRIC: &str = "codewen.apps.installed.duration_ms";
