...
use codewen_analytics::PluginInstallSource;
use codewen_app_server_protocol::PluginAvailability;
use codewen_app_server_protocol::PluginInstallPolicy;
use codewen_app_server_protocol::PluginSharePrincipalRole;
use codewen_app_server_protocol::PluginShareTargetRole;
use codewen_config::types::McpServerConfig;
use codewen_core_plugins::OPENAI_CURATED_MARKETPLACE_NAME;
use codewen_core_plugins::PluginListBackgroundTaskOptions;
use codewen_core_plugins::is_openai_curated_marketplace_name;
use codewen_core_plugins::remote::REMOTE_CREATED_BY_ME_MARKETPLACE_NAME;
use codewen_core_plugins::remote::REMOTE_GLOBAL_MARKETPLACE_NAME;
use codewen_core_plugins::remote::REMOTE_WORKSPACE_MARKETPLACE_NAME;
use codewen_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_MARKETPLACE_NAME;
use codewen_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_PRIVATE_MARKETPLACE_NAME;
use codewen_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_UNLISTED_MARKETPLACE_NAME;
use codewen_core_plugins::remote::RemoteAppTemplateUnavailableReason;
use codewen_core_plugins::remote::is_valid_remote_plugin_id;
use codewen_core_plugins::remote::validate_remote_plugin_id;
use codewen_core_plugins::remote_bundle::RemotePluginBundleInstallError;
use codewen_mcp::McpOAuthLoginSupport;
use codewen_mcp::oauth_login_support;
use codewen_mcp::should_retry_without_scopes;
use codewen_plugin::PluginId;
use codewen_plugin::PluginTelemetryMetadata;
use codewen_protocol::auth::AuthMode as DomainAuthMode;
use codewen_rmcp_client::perform_oauth_login_silent;
...
pub(crate) struct PluginRequestProcessor {
    ...
    on_effective_plugins_changed:
        Arc<dyn Fn(codewen_core_plugins::EffectivePluginsChange) + Send + Sync>,
}

fn plugin_skills_to_info(
    skills: &[codewen_core::skills::SkillMetadata],
    ...
    skills
        ...
        .map(|skill| SkillSummary {
            ...
            interface: skill.interface.clone().map(|interface| {
                codewen_app_server_protocol::SkillInterface {
...
fn load_shared_plugin_ids_by_local_path(
    ...
    codewen_core_plugins::remote::load_plugin_share_remote_ids_by_local_path(
...
fn convert_configured_marketplace_plugin_to_plugin_summary(
    plugin: codewen_core_plugins::ConfiguredMarketplacePlugin,
    shared_plugin_ids_by_local_path: &std::collections::BTreeMap<AbsolutePathBuf, String>,
) -> PluginSummary {
...
) -> codewen_core_plugins::remote::RemotePluginShareDiscoverability {
    match discoverability {
        PluginShareDiscoverability::Listed => {
            codewen_core_plugins::remote::RemotePluginShareDiscoverability::Listed
        }
        PluginShareDiscoverability::Unlisted => {
            codewen_core_plugins::remote::RemotePluginShareDiscoverability::Unlisted
        }
        PluginShareDiscoverability::Private => {
            codewen_core_plugins::remote::RemotePluginShareDiscoverability::Private
        }
    }
}
...
) -> codewen_core_plugins::remote::RemotePluginShareUpdateDiscoverability {
    match discoverability {
        PluginShareUpdateDiscoverability::Listed => {
            codewen_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Listed
        }
        PluginShareUpdateDiscoverability::Unlisted => {
            codewen_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Unlisted
        }
        PluginShareUpdateDiscoverability::Private => {
            codewen_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Private
        }
    }
}
...
) -> codewen_core_plugins::remote::RemotePluginShareTargetRole {
    match role {
        PluginShareTargetRole::Reader => {
            codewen_core_plugins::remote::RemotePluginShareTargetRole::Reader
        }
        PluginShareTargetRole::Editor => {
            codewen_core_plugins::remote::RemotePluginShareTargetRole::Editor
        }
    }
}

fn plugin_share_principal_role_from_remote(
    role: codewen_core_plugins::remote::RemotePluginSharePrincipalRole,
) -> PluginSharePrincipalRole {
    match role {
        codewen_core_plugins::remote::RemotePluginSharePrincipalRole::Reader => {
            PluginSharePrincipalRole::Reader
        }
        codewen_core_plugins::remote::RemotePluginSharePrincipalRole::Editor => {
            PluginSharePrincipalRole::Editor
        }
        codewen_core_plugins::remote::RemotePluginSharePrincipalRole::Owner => {
...
) -> Vec<codewen_core_plugins::remote::RemotePluginShareTarget> {
    targets
        .into_iter()
        .map(
            |target| codewen_core_plugins::remote::RemotePluginShareTarget {
                principal_type: match target.principal_type {
                    PluginSharePrincipalType::User => {
                        codewen_core_plugins::remote::RemotePluginSharePrincipalType::User
                    }
                    PluginSharePrincipalType::Group => {
                        codewen_core_plugins::remote::RemotePluginSharePrincipalType::Group
                    }
                    PluginSharePrincipalType::Workspace => {
                        codewen_core_plugins::remote::RemotePluginSharePrincipalType::Workspace
                    }
                },
...
fn plugin_share_principal_from_remote(
    principal: codewen_core_plugins::remote::RemotePluginSharePrincipal,
...
impl PluginRequestProcessor {
    pub(crate) fn new(
        ...
        on_effective_plugins_changed: Arc<
            dyn Fn(codewen_core_plugins::EffectivePluginsChange) + Send + Sync,
        >,
    ) -> Self {
        Self {
            auth_manager,
    ...
    async fn workspace_codewen_plugins_enabled(
        &self,
        config: &Config,
        auth: Option<&CodewenAuth>,
    ) -> bool {
        match workspace_settings::codewen_plugins_enabled_for_workspace(
            config,
            auth,
    ...
    async fn plugin_list_response(
        ...
        if !self
            .workspace_codewen_plugins_enabled(&config, auth.as_ref())
            .await
        {
            return Ok(empty_response());
        }
        let auth_mode = auth.as_ref().map(CodewenAuth::api_auth_mode);
        ...
        let use_remote_global_catalog =
            include_global_remote && auth_mode.is_some_and(DomainAuthMode::uses_codewen_backend);
        ...
        let refresh_global_remote_catalog_cache = use_remote_global_catalog
            && codewen_core_plugins::remote::has_cached_global_remote_plugin_catalog(
                config.codewen_home.as_path(),
            ...
            match tokio::task::spawn_blocking(move || {
                ...
                Ok::<
                    (
                        ...
                        Vec<codewen_app_server_protocol::MarketplaceLoadErrorInfo>,
                    ),
                    MarketplaceError,
                >((
                    outcome
        ...
        if include_vertical && !config.features.enabled(Feature::RemotePlugin) {
            match codewen_core_plugins::remote::fetch_openai_curated_remote_collection_marketplace(
                &remote_plugin_service_config,
                auth.as_ref(),
            )
            .await
        ...
        if !remote_sources.is_empty() {
            match codewen_core_plugins::remote::fetch_remote_marketplaces(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_sources,
                /*global_catalog_cache_path*/ Some(config.codewen_home.as_path()),
            )
            .await
            {
    ...
    async fn plugin_installed_response(
        ...
        plugins_manager.set_auth_mode(auth.as_ref().map(CodewenAuth::api_auth_mode));
    ...
    async fn load_local_installed_and_suggested_plugins(
        &self,
        plugins_manager: Arc<codewen_core_plugins::PluginsManager>,
        config: &Config,
        plugins_input: &codewen_core_plugins::PluginsConfigInput,
        ...
        let plugin = match read_source {
            Ok(marketplace_path) => {
                ...
                let share_context = match share_context {
                    Some(context) => {
                        let remote_plugin_service_config = remote_plugin_service_config(&config);
                        match codewen_core_plugins::remote::fetch_remote_plugin_share_context(
                            &remote_plugin_service_config,
                            auth.as_ref(),
                ...
                PluginDetail {
                    ...
                    hooks: outcome
                        ...
                        .hooks
                        .into_iter()
                        .map(|hook| codewen_app_server_protocol::PluginHookSummary {
                            key: hook.key,
            ...
            Err(remote_marketplace_name) => {
                ...
                let remote_detail = codewen_core_plugins::remote::fetch_remote_plugin_detail(
                    &remote_plugin_service_config,
                    auth.as_ref(),
                    &remote_marketplace_name,
                    &plugin_name,
                ...
                let plugin_apps = remote_detail
                    ...
                    .iter()
                    .cloned()
                    .map(codewen_plugin::AppConnectorId)
                    .collect::<Vec<_>>();
    ...
    async fn plugin_skill_read_response(
        ...
        let auth = self.auth_manager.auth().await;
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let remote_skill_detail = codewen_core_plugins::remote::fetch_remote_plugin_skill_detail(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_marketplace_name,
            &remote_plugin_id,
    ...
    async fn plugin_share_save_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let access_policy = codewen_core_plugins::remote::RemotePluginShareAccessPolicy {
        ...
        let result = codewen_core_plugins::remote::save_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codewen_home.as_path(),
            &plugin_path,
    ...
    async fn plugin_share_update_targets_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let result = codewen_core_plugins::remote::update_remote_plugin_share_targets(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_plugin_id,
    ...
    async fn plugin_share_list_response(
        ...
        let (config, auth) = self.load_plugin_share_config_and_auth().await?;
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let data = codewen_core_plugins::remote::list_remote_plugin_shares(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codewen_home.as_path(),
        )
    ...
    async fn plugin_share_checkout_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let result = codewen_core_plugins::remote::checkout_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codewen_home.as_path(),
            &remote_plugin_id,
    ...
    async fn plugin_share_delete_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        codewen_core_plugins::remote::delete_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codewen_home.as_path(),
            &remote_plugin_id,
    ...
    async fn load_plugin_share_config_and_auth(
        &self,
    ) -> Result<(Config, Option<CodewenAuth>), JSONRPCErrorError> {
        let config = self.load_latest_config(/*fallback_cwd*/ None).await?;
        if !config.features.enabled(Feature::Plugins) {
    ...
    async fn plugin_install_response(
        ...
        let plugin_mcp_servers = load_plugin_mcp_servers(
            ...
            auth.as_ref().map(CodewenAuth::auth_mode),
        )
        .await;
        ...
        let plugin_apps =
            codewen_plugin::app_connector_ids_from_declarations(&plugin_app_declarations);
        let apps_needing_auth = self
            .plugin_apps_needing_auth_for_install(
                &config,
                auth.as_ref().is_some_and(CodewenAuth::is_chatgpt_auth),
    ...
    async fn remote_plugin_install_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let remote_detail =
            codewen_core_plugins::remote::fetch_remote_plugin_detail_with_download_urls(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_marketplace_name,
                &remote_plugin_id,
        ...
        let _remote_plugin_cache_mutation =
            codewen_core_plugins::remote::mark_remote_plugin_cache_mutation_in_flight(
                config.codewen_home.as_path(),
                &actual_remote_marketplace_name,
                &remote_plugin_name,
            );
        let validated_bundle = codewen_core_plugins::remote_bundle::validate_remote_plugin_bundle(
            &remote_plugin_id,
            &actual_remote_marketplace_name,
            &remote_plugin_name,
        ...
        let install_result = codewen_core_plugins::remote::install_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            &actual_remote_marketplace_name,
            &remote_plugin_id,
        ...
        let is_chatgpt_auth = auth.as_ref().is_some_and(CodewenAuth::is_chatgpt_auth);
    ...
    async fn plugin_apps_needing_auth_for_install(
        ...
        plugin_id: &str,
        plugin_apps: &[codewen_plugin::AppConnectorId],
    ) -> Vec<AppSummary> {
        ...
        let (accessible_connectors, codewen_apps_ready) = match accessible_connectors_result {
            Ok(status) => (status.connectors, status.codewen_apps_ready),
            Err(err) => {
                warn!(
                    plugin = plugin_id,
        ...
        if !codewen_apps_ready {
            warn!(
                plugin = plugin_id,
                "codewen_apps MCP not ready after plugin install; skipping appsNeedingAuth check"
            );
        }
    ...
    async fn plugin_uninstall_response(
        ...
        if codewen_plugin::PluginId::parse(&plugin_id).is_err()
    ...
    async fn remote_plugin_uninstall_response(
        ...
        let uninstall_result = codewen_core_plugins::remote::uninstall_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codewen_home.to_path_buf(),
            uninstall_target,
...
fn plugin_app_category_by_id_from_value(value: &serde_json::Value) -> HashMap<String, String> {
    codewen_core_plugins::loader::plugin_app_declarations_from_value(value)
        .into_iter()
...
fn remote_plugin_share_discoverability_to_info(
    discoverability: codewen_core_plugins::remote::RemotePluginShareDiscoverability,
...
fn remote_plugin_bundle_install_error_to_jsonrpc(
    err: codewen_core_plugins::remote_bundle::RemotePluginBundleInstallError,
) -> JSONRPCErrorError {
