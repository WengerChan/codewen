use codewen_app_server_protocol::ConfigLayer as ApiConfigLayer;
use codewen_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use codewen_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use codewen_config::ConfigLayer;
use codewen_config::ConfigLayerMetadata;
use codewen_config::ConfigLayerSource;

/// Converts a config-layer source owned by `codewen-config` into the app-server wire type owned by
/// `codewen-app-server-protocol`.
///
...
pub(crate) fn config_layer_source_to_api(source: ConfigLayerSource) -> ApiConfigLayerSource {
    match source {
        ...
        ConfigLayerSource::Project { dot_codewen_folder } => {
            ApiConfigLayerSource::Project { dot_codewen_folder }
        }
...
/// Converts config-layer metadata owned by `codewen-config` into the app-server wire type owned by
/// `codewen-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `codewen-config` into the app-server wire type owned by
/// `codewen-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
