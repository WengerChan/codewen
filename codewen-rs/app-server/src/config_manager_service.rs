...
use codewen_app_server_protocol::Config as ApiConfig;
...
use codewen_config::ShellEnvironmentPolicyFilterRepresentation;
use codewen_config::config_toml::ConfigToml;
use codewen_config::merge_toml_values;
use codewen_config::shell_environment_filter_entry;
use codewen_config::validate_shell_environment_policy_filter_config;
use codewen_core::config::deserialize_config_toml_with_base;
use codewen_core::config::edit::ConfigEdit;
use codewen_core::config::edit::ConfigEditsBuilder;
use codewen_core::config::validate_feature_requirements_for_config_toml;
use codewen_core::path_utils;
use codewen_core::path_utils::SymlinkWritePaths;
use codewen_core::path_utils::resolve_symlink_write_paths;
use codewen_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.codewen_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .codewen/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_codewen_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_codewen_folder.display(),
        ),
