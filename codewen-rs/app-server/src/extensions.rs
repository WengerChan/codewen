...
use codewen_extension_api::AgentSpawnFuture;
use codewen_extension_api::AgentSpawner;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn codewen_skills_extension::SkillProvider>,
...
where
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = CodewenErr> + 'static,
{
    ...
    if let Some(state_db) = state_db {
        codewen_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            codewen_otel::global(),
            thread_manager,
            goal_service,
            |config: &Config| config.features.enabled(codewen_features::Feature::Goals),
        );
    }
    codewen_guardian::install(&mut builder, guardian_agent_spawner);
    codewen_memories_extension::install(&mut builder, codewen_otel::global());
    codewen_mcp_extension::install(&mut builder);
    codewen_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    codewen_web_search_extension::install(&mut builder, auth_manager.clone());
    codewen_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
        Some(config.codewen_home.clone())
    });
    let skill_providers = codewen_skills_extension::SkillProviders::new()
        .with_executor_provider(executor_skill_provider)
        .with_orchestrator_provider(Arc::new(
            codewen_skills_extension::OrchestratorSkillProvider::new(),
        ))
        .with_host_provider(Arc::new(codewen_skills_extension::HostSkillProvider::new()));
    codewen_skills_extension::install_with_providers_and_metrics(
        &mut builder,
        skill_providers,
        codewen_otel::global(),
        |config: &Config| codewen_skills_extension::SkillsExtensionConfig {
            include_instructions: config.include_skill_instructions,
            bundled_skills_enabled: config.bundled_skills_enabled(),
            orchestrator_skills_enabled: config.orchestrator_skills_enabled,
            shadow_selection_enabled: config
                .features
                .enabled(codewen_features::Feature::SkillSearch),
        },
    );
...
) -> impl AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = CodewenErr> {
    move |forked_from_thread_id: ThreadId,
          options: StartThreadOptions|
          -> AgentSpawnFuture<'static, NewThread, CodewenErr> {
...
mod tests {
    ...
    use codewen_protocol::protocol::ThreadGoal as CoreThreadGoal;
    use codewen_protocol::protocol::ThreadGoalStatus;
    use codewen_protocol::protocol::ThreadGoalUpdatedEvent;
