...
use codewen_app_server_protocol::experimental_required_message;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: CodewenFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                codewen_core::build_models_manager(config.as_ref(), auth_manager.clone()),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let codewen_request = deserialize_client_request(&request);
                let result = match codewen_request {
                    Ok(codewen_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = codewen_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = codewen_request.experimental_reason()
        ...
        let serialization_scope = codewen_request.serialization_scope();
