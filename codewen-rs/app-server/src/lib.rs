...
use codewen_analytics::AppServerRpcTransport;
...
use codewen_app_server_protocol::TextPosition as AppTextPosition;
use codewen_app_server_protocol::TextRange as AppTextRange;
use codewen_config::ConfigLayerSource;
use codewen_config::ConfigLoadError;
use codewen_config::TextRange as CoreTextRange;
use codewen_core::ExecPolicyError;
...
use codewen_rollout::state_db as rollout_state_db;
...
const OTEL_SERVICE_NAME: &str = "codewen-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "CODEWEN_APP_SERVER_TEST_USER_CONFIG_FILE";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_codewen_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_codewen_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
        ...
        .await
    {
        Ok(config) => {
            ...
            let auth_manager =
                AuthManager::shared_from_config(&config, /*enable_codewen_api_key_env*/ false)
                    .await;
    ...
    let otel = codewen_core::otel_init::build_provider(
        &config,
    ...
    codewen_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    codewen_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&codewen_home)?;
    ...
    let feedback = CodewenFeedback::new();
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !codewen_state::is_sqlite_corruption_error(&err)
        ...
        let backups = codewen_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("codewen-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(
