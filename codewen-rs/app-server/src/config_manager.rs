...
use codewen_cloud_config::cloud_config_bundle_loader;
...
use codewen_utils_json_to_toml::json_to_toml;
...
impl ConfigManager {
    ...
    pub(crate) fn codewen_home(&self) -> &Path {
        self.codewen_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.codewen_home())
    }
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(codewen_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        let mut config = Config::load_default_with_cli_overrides_for_codewen_home(
            self.codewen_home.clone(),
            self.current_cli_overrides(),
        )
        .await?;
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = codewen_core::config::ConfigBuilder::default()
            .codewen_home(self.codewen_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.codewen_self_exe = self.arg0_paths.codewen_self_exe.clone();
        config.codewen_linux_sandbox_exe = self.arg0_paths.codewen_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(codewen_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(codewen_home: PathBuf) -> Self {
        Self::new_for_tests(
            codewen_home,
            Vec::new(),
