...
use codewen_app_server_protocol::CommandExecOutputDeltaNotification;
use codewen_app_server_protocol::CommandExecOutputStream;
use codewen_app_server_protocol::CommandExecResizeParams;
use codewen_app_server_protocol::CommandExecResizeResponse;
use codewen_app_server_protocol::CommandExecResponse;
use codewen_app_server_protocol::CommandExecTerminalSize;
use codewen_app_server_protocol::CommandExecTerminateParams;
use codewen_app_server_protocol::CommandExecTerminateResponse;
use codewen_app_server_protocol::CommandExecWriteParams;
use codewen_app_server_protocol::CommandExecWriteResponse;
use codewen_app_server_protocol::JSONRPCErrorError;
use codewen_app_server_protocol::ServerNotification;
use codewen_core::config::StartedNetworkProxy;
use codewen_core::exec::ExecExpiration;
use codewen_core::exec::ExecExpirationOutcome;
use codewen_core::exec::IO_DRAIN_TIMEOUT_MS;
use codewen_core::sandboxing::ExecRequest;
use codewen_protocol::exec_output::bytes_to_string_smart;
use codewen_sandboxing::SandboxType;
use codewen_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use codewen_utils_pty::ProcessHandle;
use codewen_utils_pty::SpawnedProcess;
use codewen_utils_pty::TerminalSize;
...
impl CommandExecManager {
    pub(crate) async fn start(
        ...
        let spawned = if tty {
            codewen_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            codewen_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0, &[])
                .await
        } else {
            codewen_utils_pty::spawn_pipe_process_no_stdin(
                program,
                args,
                cwd.as_path(),
                &env,
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest::new(
            ...
            codewen_core::exec::ExecCapturePolicy::ShellTool,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: codewen_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: codewen_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: codewen_app_server_protocol::RequestId::Integer(100),
        };
        let cwd = AbsolutePathBuf::current_dir().expect("current dir");
    ...
    async fn timeout_or_cancellation_reports_cancellation_without_timeout_exit_code() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: codewen_app_server_protocol::RequestId::Integer(101),
        };
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: codewen_app_server_protocol::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: codewen_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: codewen_app_server_protocol::RequestId::Integer(3),
        };
