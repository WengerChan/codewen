...
use codewen_app_server_protocol::ConfigReadParams;
use codewen_app_server_protocol::ConfigValueWriteParams;
...
use codewen_app_server_protocol::FsCopyParams;
use codewen_app_server_protocol::FsCreateDirectoryParams;
use codewen_app_server_protocol::FsGetMetadataParams;
use codewen_app_server_protocol::FsReadDirectoryParams;
use codewen_app_server_protocol::FsReadFileParams;
use codewen_app_server_protocol::FsRemoveParams;
use codewen_app_server_protocol::FsUnwatchParams;
use codewen_app_server_protocol::FsWatchParams;
use codewen_app_server_protocol::FsWriteFileParams;
...
use codewen_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use codewen_app_server_protocol::ProcessKillParams;
use codewen_app_server_protocol::ProcessSpawnParams;
use codewen_app_server_protocol::RemoteControlClientsListParams;
use codewen_app_server_protocol::RemoteControlClientsRevokeParams;
use codewen_app_server_protocol::RemoteControlPairingStartParams;
use codewen_app_server_protocol::RemoteControlPairingStatusParams;
...
use codewen_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use codewen_exec_server::CODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use codewen_exec_server::CODEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use codewen_exec_server::CODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use codewen_exec_server::CODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use codewen_exec_server::CODEX_EXEC_SERVER_URL_ENV_VAR;
...
use core_test_support::test_codewen::TestEnv;
use core_test_support::test_codewen::test_env;
...
pub struct TestAppServer {
    ...
    _owned_codewen_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "codewen-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "CODEWEN_APP_SERVER_DISABLE_MANAGED_CONFIG";
const CODE_MODE_HOST_PATH_ENV_VAR: &str = "CODEWEN_CODE_MODE_HOST_PATH";
...
impl TestAppServer {
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env(
            ...
            codewen_home.join("managed_config.toml"),
        );
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("codewen-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: codewen_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    ...
    pub fn with_codewen_home(mut self, codewen_home: &Path) -> Self {
        self.codewen_home = Some(codewen_home.to_path_buf());
        self
    }
    ...
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (codewen_home, owned_codewen_home) = match codewen_home {
            Some(codewen_home) => (codewen_home, None),
            None => {
                let owned_codewen_home = TempDir::new()?;
                (
                    owned_codewen_home.path().to_path_buf(),
                    Some(owned_codewen_home),
                )
            }
        };
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = codewen_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&codewen_home, &exec_server_program)
                                .await?;
        ...
        if !env_overrides
            ...
            && let Ok(code_mode_host_program) =
                codewen_utils_cargo_bin::cargo_bin("codewen-code-mode-host")
        {
        ...
        let program = match program {
            ...
            None => codewen_utils_cargo_bin::cargo_bin("codewen-app-server")
                .context("should find binary for codewen-app-server")?,
        };
        ...
        app_server._owned_codewen_home = owned_codewen_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `codewen-app-server` child process.
        //
