...
use codewen_app_server_protocol::AppInfo;
use codewen_app_server_protocol::AppMetadata;
use codewen_app_server_protocol::AppTemplateSummary;
use codewen_app_server_protocol::AppTemplateUnavailableReason;
use codewen_app_server_protocol::HookEventName;
...
use codewen_app_server_protocol::ScheduledTaskSchedule;
use codewen_app_server_protocol::ScheduledTaskSummary;
use codewen_app_server_protocol::ScheduledTaskWeekday;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                codewen_home.path().join("marketplace.json"),
            )?),
...
"#,
    ...
    write_installed_plugin(&codewen_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(codewen_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&codewen_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "codewen-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(codewen_home.path().join(".tmp"))?;
    std::fs::write(
        codewen_home
            .path()
...
[plugins."demo-plugin@codewen-curated"]
enabled = true

[hooks.state."demo-plugin@codewen-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&codewen_home, "codewen-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "codewen-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@codewen-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            codewen_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@codewen-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            codewen_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@codewen-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            codewen_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@codewen-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_returns_app_metadata_category() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    write_connectors_config(codewen_home.path(), &server_url)?;
    write_chatgpt_auth(
        codewen_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_hides_apps_for_api_key_auth() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .without_auto_env()
        .with_env_overrides(&[
            ("CODEWEN_ACCESS_TOKEN", None),
            ("CODEWEN_API_KEY", None),
            ("OPENAI_API_KEY", None),
        ])
        .build()
        .await?;
...
fn write_plugins_enabled_config(codewen_home: &TempDir) -> Result<()> {
    std::fs::write(
        codewen_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
fn write_connectors_config(codewen_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
