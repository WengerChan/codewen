...
use codewen_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
use codewen_app_server_protocol::ExternalAgentConfigImportHistoriesReadResponse;
use codewen_app_server_protocol::ExternalAgentConfigImportProgressNotification;
use codewen_app_server_protocol::ExternalAgentConfigImportResponse;
use codewen_app_server_protocol::ExternalAgentConfigMigrationItemType;
use codewen_app_server_protocol::ExternalAgentImportedConnectorCandidate;
use codewen_app_server_protocol::ExternalAgentImportedConnectorSource;
...
fn external_agent_home(codewen_home: &Path) -> PathBuf {
    codewen_home.join(concat!(".", "cla", "ude"))
}
...
fn secondary_external_agent_home(codewen_home: &Path) -> PathBuf {
    codewen_home.join(concat!(".", "cur", "sor"))
}
...
-> Result<()> {
    ...
    let source_home = external_agent_home(codewen_home.path());
    std::fs::create_dir_all(&source_home)?;
    std::fs::write(source_home.join("CLAUDE.md"), "project instructions")?;
    let home_dir = codewen_home.path().display().to_string();
...
async fn external_agent_config_migration_source_drives_detect_and_import() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let source_home = secondary_external_agent_home(codewen_home.path());
    std::fs::create_dir_all(&source_home)?;
    ...
    assert!(
        std::fs::read_to_string(codewen_home.path().join("config.toml"))?
...
async fn external_agent_config_import_source_remains_attribution_only() -> Result<()> {
    ...
    assert_eq!(completed.item_type_results[0].failures, Vec::new());
    assert_eq!(
        std::fs::read_to_string(codewen_home.path().join("AGENTS.md"))?,
        "Codex guidance"
    );
...
async fn external_agent_config_secondary_source_imports_session_and_plugin_end_to_end() -> Result<()>
{
    let codewen_home = TempDir::new()?;
    let source_home = secondary_external_agent_home(codewen_home.path());
    let project_root = codewen_home.path().join("workspace with.dots_and-dashes");
    ...
    let configured_marketplace_root = codewen_home.path().join("configured-marketplace");
    ...
    std::fs::create_dir_all(configured_plugin_root.join(".codewen-plugin"))?;
    std::fs::write(
  ...
  "plugins": [{
    ...
    std::fs::write(
        configured_plugin_root.join(".codewen-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(
        marketplace
            .path
            .as_ref()
            .map(codewen_config::AbsolutePathBuf::as_path),
...
-> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            ("CODEWEN_SQLITE_HOME", Some(sqlite_home_dir.as_str())),
        ])
        .build()
        .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
    ...
    assert_eq!(completed.import_id, import_id);
    let state_db =
        codewen_state::StateRuntime::init(sqlite_home.path().to_path_buf(), "mock_provider".into())
            .await?;
...
async fn external_agent_config_detects_and_imports_project_memory_files() -> Result<()> {
    ...
    let project_cwd = codewen_home.path().join("project-a");
    std::fs::create_dir_all(&source_memory)?;
    ...
    let expected_resources_root = codewen_home
        .path()
    ...
    let memory_diff = codewen_git_utils::diff_since_latest_init(&memory_root).await?;
    ...
    codewen_memories_write::workspace::reset_memory_workspace_baseline(&memory_root).await?;
    ...
    assert_eq!(
        ...
        vec![
            (
                codewen_git_utils::GitBaselineChangeStatus::Deleted,
                "extensions/external_agent_import/resources/project-a/MEMORY.md",
            ),
            (
                codewen_git_utils::GitBaselineChangeStatus::Deleted,
...
async fn external_agent_config_import_reports_failed_sync_import_in_completion() -> Result<()> {
    ...
    std::fs::write(codewen_home.path().join("config.toml"), "invalid = [")?;
    let home_dir = codewen_home.path().display().to_string();
    let analytics_capture_file = codewen_home.path().join("analytics-events.jsonl");
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ("HOME", Some(home_dir.as_str())),
            (
                "CODEWEN_ANALYTICS_EVENTS_CAPTURE_FILE",
    ...
    let events = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if captured_events.iter().any(|event| {
                event["event_type"] == "codewen_onboarding_external_agent_import_complete"
    ...
    let event = events
        .iter()
        .find(|event| {
            event["event_type"] == "codewen_onboarding_external_agent_import_failure"
...
async fn external_agent_config_import_completed_tracks_analytics_event() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    write_analytics_config(codewen_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        codewen_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    let missing_session_path =
        external_agent_home(codewen_home.path()).join("projects/repo/missing.jsonl");
    let project_root = codewen_home.path().join("repo");
    let home_dir = codewen_home.path().display().to_string();
    let mut mcp = TestAppServer::builder()
        .with_codewen_home(codewen_home.path())
        .without_auto_env()
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "codewen_onboarding_external_agent_import_complete",
    )
    .await?;
    let event_params = &event["event_params"];
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "codewen_onboarding_external_agent_import_failure",
    )
    .await?;
    let event_params = &event["event_params"];
    assert_eq!(event_params["import_id"], serde_json::json!(import_id));
...
async fn external_agent_config_import_reinstalls_plugins_from_known_marketplaces() -> Result<()> {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        Some(vec![codewen_app_server_protocol::PluginsMigration {
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let session_dir = external_agent_home(codewen_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");
    let manifest_dir = connector_metadata_root(codewen_home.path())
...
async fn external_agent_config_import_does_not_initialize_required_mcp() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    create_config_toml(codewen_home.path(), &server.uri())?;
    let mut config = std::fs::read_to_string(codewen_home.path().join("config.toml"))?;
    config.push_str(
        r#"
...
"#,
    ...
    std::fs::write(codewen_home.path().join("config.toml"), config)?;
    let project_root = codewen_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = external_agent_home(codewen_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");
...
fn write_analytics_config(codewen_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        codewen_home.join("config.toml"),
