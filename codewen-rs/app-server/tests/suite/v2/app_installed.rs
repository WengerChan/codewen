...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let codewen_home = configured_codewen_home(fixture.base_url())?;
    let mut app_server = start_app_server(codewen_home.path()).await?;
...
async fn start_app_server(codewen_home: &Path) -> Result<TestAppServer> {
...
fn configured_codewen_home(base_url: &str) -> Result<TempDir> {
