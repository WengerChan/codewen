...
async fn standalone_web_search_round_trips_output() -> Result<()> {
    ...
    let turn_metadata_header = search_request
        .headers
        .get("x-codewen-turn-metadata")
        .context("standalone search should include x-codewen-turn-metadata")?
        .to_str()
        .context("x-codewen-turn-metadata should be valid ASCII")?;
    let turn_metadata: Value = serde_json::from_str(turn_metadata_header)
        .context("x-codewen-turn-metadata should be valid JSON")?;
...
async fn mount_search_response(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/api/codewen/alpha/search"))
...
async fn search_request(server: &MockServer) -> Result<wiremock::Request> {
    ...
    requests
        .into_iter()
        .find(|request| request.url.path() == "/api/codewen/alpha/search")
...
name = "OpenAI"
base_url = "{server_uri}/api/codewen"
wire_api = "responses"
