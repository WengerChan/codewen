...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_codewen_home = AbsolutePathBuf::try_from(codewen_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        codewen_home: response_codewen_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("codewen_vscode/"));
    assert_eq!(response_codewen_home, expected_codewen_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "codewen_app_server_daemon".to_string(),
    ...
    assert!(user_agent.starts_with("codewen_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_codewen_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let codewen_home = TempDir::new()?;
    create_config_toml(codewen_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "codewen-backend".to_string(),
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[(
            ...
            Some("codewen_originator_via_env_var"),
        )])
        .build()
        .await?;
    ...
    assert!(user_agent.starts_with("codewen_originator_via_env_var/"));
    assert_eq!(response_codewen_home, expected_codewen_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        .with_codewen_home(codewen_home.path())
        .without_auto_env()
        .with_env_overrides(&[("CODEWEN_INTERNAL_ORIGINATOR_OVERRIDE", None)])
        .build()
        .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let codewen_home = TempDir::new()?;
    let notify_file = codewen_home.path().join("notify.json");
    let notify_capture = cargo_bin("codewen-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(codewen_home, server_uri, approval_policy, "")
}
