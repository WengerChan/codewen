...
use chrono::Utc;
use codewen_app_server_protocol::ApprovalsReviewer;
...
use codewen_app_server_protocol::JSONRPCError;
...
use codewen_core::ARCHIVED_SESSIONS_SUBDIR;
use codewen_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use codewen_protocol::user_input::ByteRange;
use codewen_protocol::user_input::TextElement;
use codewen_rollout::append_rollout_item_to_path;
use codewen_rollout::read_session_meta_line;
...
async fn thread_resume_paginated_metadata_only_uses_model_context() -> Result<()> {
    ...
    create_config_toml(codewen_home.path(), &server.uri())?;
...
async fn thread_resume_preserves_persisted_approvals_reviewer() -> Result<()> {
    ...
    let config_path = codewen_home.path().join("config.toml");
...
async fn thread_resume_preserves_goal_first_and_fork_approvals_reviewer() -> Result<()> {
    ...
    let (thread_id, fork_thread_id) = {
        ...
        let start_id = mcp
            .send_thread_start_request_with_auto_env(ThreadStartParams {
                model: Some("gpt-5.2-codewen".to_string()),
                approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
                ..Default::default()
            })
            .await?;
...
-> Result<()> {
    ...
    assert_eq!(model, "gpt-5.2-codewen");
    ...
    let update_id = mcp
        .send_thread_settings_update_request(ThreadSettingsUpdateParams {
            ...
            collaboration_mode: Some(CollaborationMode {
                ...
                settings: Settings {
                    model: "gpt-5.2-codewen".to_string(),
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codewen_home = TempDir::new()?;
    let codewen_home_path = normalized_existing_path(codewen_home.path())?;
    create_config_toml(&codewen_home_path, &server.uri())?;
    let config_path = codewen_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = codewen_home_path
        .as_path()
        .to_str()
        .expect("test codewen home should be utf-8");
    let mut mcp = TestAppServer::builder()
        .with_codewen_home(&codewen_home_path)
        .without_managed_config()
        .with_env_overrides(&[("CODEWEN_SQLITE_HOME", Some(sqlite_home))])
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(codewen_home.path(), &server.uri(), &server.uri())?;
    mount_analytics_capture(&server, codewen_home.path()).await?;
...
async fn thread_resume_running_thread_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let start_id = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            ...
            service_name: Some("codewen_work_desktop".to_string()),
            ..Default::default()
        })
        .await?;
    let start_resp: JSONRPCResponse = timeout(
    ...
    let event = wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "codewen_thread_initialized"
...
fn set_session_meta_on_fake_rollout(
    ...
    let path = rollout_path(codewen_home, filename_ts, thread_id);
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(codewen_home, filename_ts, conversation_id);
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(codewen_home.path(), filename_ts, &conversation_id);
    let archived_dir = codewen_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "codewen unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    let state_db =
        StateRuntime::init(codewen_home.path().to_path_buf(), "mock_provider".into()).await?;
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            codewen_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    .await??;
    let get: codewen_app_server_protocol::ThreadGoalGetResponse = to_response(get_resp)?;
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(codewen_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codewen_home = TempDir::new()?;
    let config_toml = codewen_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
...
"#,
    ...
    let repo_path = codewen_home.path().join("repo");
    ...
    let rollout_path = rollout_path(codewen_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codewen_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(codewen_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(codewen_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let resume_resp: JSONRPCResponse = timeout(
...
async fn thread_resume_rejects_mismatched_path_for_running_thread_id() -> Result<()> {
    ...
    let stale_path = rollout_path(codewen_home.path(), "2025-01-01T00-00-00", &stale_thread_id);
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let codewen_home = tmp.path().join("codewen_home");
    std::fs::create_dir(&codewen_home)?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&codewen_home, &server.uri())?;

    let mut primary = TestAppServer::builder()
        .with_codewen_home(&codewen_home)
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![codewen_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(codewen_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(codewen_home.path(), &server.uri()).await?;
    create_config_toml_with_required_broken_mcp(codewen_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(codewen_home.path(), "seed history").await?;
...
fn create_config_toml(codewen_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
...
command = "codewen-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
async fn setup_rollout_fixture(codewen_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(codewen_home, server_uri)?;
