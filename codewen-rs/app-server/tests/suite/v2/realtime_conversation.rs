...
use codewen_app_server_protocol::ThreadRealtimeAudioChunk;
...
use codewen_app_server_protocol::ThreadRealtimeInitialItem;
...
impl RealtimeE2eHarness {
    ...
    async fn start_webrtc_realtime(&mut self, offer_sdp: &str) -> Result<StartedWebrtcRealtime> {
        self.start_webrtc_realtime_with_codewen_response_routing(
            offer_sdp,
            /*client_managed_handoffs*/ None,
            /*codewen_responses_as_items*/ None,
            /*codewen_response_handoff_mode*/ None,
            RealtimeConversationVersion::V1,
        )
        .await
    }

    async fn start_webrtc_realtime_with_codewen_response_items(
        &mut self,
        offer_sdp: &str,
        ...
        self.start_webrtc_realtime_with_codewen_response_routing(
            offer_sdp,
            /*client_managed_handoffs*/ None,
            /*codewen_responses_as_items*/ Some(true),
            /*codewen_response_handoff_mode*/ None,
            RealtimeConversationVersion::V1,
        )
        .await
    ...
    async fn start_webrtc_realtime_with_codewen_response_routing(
        &mut self,
        offer_sdp: &str,
        client_managed_handoffs: Option<bool>,
        codewen_responses_as_items: Option<bool>,
        codewen_response_handoff_mode: Option<CodewenResponseHandoffMode>,
        ...
        let start_request_id = self
            ...
            .send_thread_realtime_start_request(ThreadRealtimeStartParams {
                ...
                codewen_response_item_prefix: codewen_responses_as_items
                    .unwrap_or(false)
    ...
    async fn start_websocket_realtime(&mut self) -> Result<ThreadRealtimeStartedNotification> {
        self.start_websocket_realtime_with_codewen_responses_as_items(
            /*codewen_responses_as_items*/ None,
        )
        .await
    }

    async fn start_websocket_realtime_with_codewen_response_items(
        &mut self,
    ...
    async fn start_websocket_realtime_with_codewen_responses_as_items(
        &mut self,
        codewen_responses_as_items: Option<bool>,
    ) -> Result<ThreadRealtimeStartedNotification> {
        let start_request_id = self
...
async fn webrtc_v3_start_posts_live_session_and_joins_without_session_update() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_codewen_response_routing(
            "v=offer\r\n",
            /*client_managed_handoffs*/ None,
            /*codewen_responses_as_items*/ None,
            /*codewen_response_handoff_mode*/ None,
...
#[tokio::test]
async fn webrtc_v1_ignores_codewen_response_handoff_mode() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let started = harness
        .start_webrtc_realtime_with_codewen_response_routing(
            ...
            /*client_managed_handoffs*/ None,
            /*codewen_responses_as_items*/ None,
            /*codewen_response_handoff_mode*/ Some(CodewenResponseHandoffMode::BemTags),
            RealtimeConversationVersion::V1,
        )
        .await?;
    assert_eq!(started.started.version, RealtimeConversationVersion::V1);
...
async fn webrtc_v1_handoff_request_delegates_context_and_manual_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_codewen_response_items("v=offer\r\n")
        .await?;
    assert_eq!(started.started.version, RealtimeConversationVersion::V1);
    assert_call_create_multipart(
        harness.call_capture.single_request(),
...
async fn realtime_automatic_standalone_output_is_item_and_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_websocket_realtime_with_codewen_response_items()
        .await?;
    assert_eq!(started.version, RealtimeConversationVersion::V2);
    assert_eq!(
...
async fn websocket_v3_routes_handoffs_by_session_mode() -> Result<()> {
    ...
    for (mode, expected_channels) in [
        ...
        (
            Some(CodewenResponseHandoffMode::Commentary),
            [
                Some("commentary"),
                Some("commentary"),
                Some("commentary"),
        ...
        (
            Some(CodewenResponseHandoffMode::BemTags),
            [
                Some("commentary"),
                Some("commentary"),
                Some("speakable"),
