...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    let codewen_bin = toml::Value::String(
        codewen_utils_cargo_bin::cargo_bin("codewen")?
            .to_string_lossy()
            .into_owned(),
    );
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".codewen-plugin"))?;
    std::fs::write(
        plugin.path().join(".codewen-plugin/plugin.json"),
