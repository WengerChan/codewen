...
use codewen_login::CLIENT_ID_OVERRIDE_ENV_VAR;
...
const LOGIN_ISSUER_ENV_VAR: &str = "CODEWEN_APP_SERVER_LOGIN_ISSUER";
const LOGIN_OPEN_APP_URL_ENV_VAR: &str = "CODEWEN_APP_SERVER_DEV_OPEN_APP_URL";
...
fn create_config_toml(codewen_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
fn read_config_toml(codewen_home: &Path) -> Result<toml::Value> {
    Ok(toml::from_str(&std::fs::read_to_string(
        codewen_home.join("config.toml"),
    )?)?)
}

fn load_file_auth(codewen_home: &Path) -> Result<Option<AuthDotJson>> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(codewen_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(codewen_home.path().join("auth.json").exists());
    ...
    assert!(
        !codewen_home.path().join("auth.json").exists(),
...
async fn logout_account_succeeds_when_config_reload_fails() -> Result<()> {
    ...
    assert_eq!(load_file_auth(codewen_home.path())?, None);
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(codewen_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<codewen_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(codewen_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![codewen_app_server_protocol::UserInput::Text {
...
async fn login_amazon_bedrock_replaces_primary_auth_and_persists_provider() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    let mut expected_config = read_config_toml(codewen_home.path())?;
    expected_config
        .as_table_mut()
    ...
    assert_eq!(
        load_file_auth(codewen_home.path())?,
        Some(AuthDotJson {
    ...
    assert_eq!(read_config_toml(codewen_home.path())?, expected_config);
...
async fn login_amazon_bedrock_rejects_non_bedrock_provider_override_without_changes() -> Result<()>
{
    ...
    let expected_auth = load_file_auth(codewen_home.path())?;
    let expected_config = read_config_toml(codewen_home.path())?;
    ...
    assert_eq!(load_file_auth(codewen_home.path())?, expected_auth);
    assert_eq!(read_config_toml(codewen_home.path())?, expected_config);
    ...
    assert_eq!(
        ...
        GetAccountResponse {
            account: Some(Account::AmazonBedrock {
                uses_codewen_managed_credentials: true,
            }),
...
async fn logout_aws_managed_bedrock_errors_without_changing_auth_or_config() -> Result<()> {
    let codewen_home = TempDir::new()?;
    create_config_toml(codewen_home.path(), aws_managed_bedrock_config())?;
    login_with_api_key(
        codewen_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/codewen/device"));
...
async fn login_account_chatgpt_redirects_to_hosted_success_page() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            (
                ...
                Some("http://localhost:3000/codewen/open-app"),
            ),
        ])
        .build()
        .await?;
    ...
    assert_eq!(
        ...
        "http://localhost:3000/codewen/open-app?source=login&app_brand=chatgpt"
    );
    Ok(())
}
...
"#
    ...
    let expected = GetAccountResponse {
        account: Some(Account::AmazonBedrock {
            uses_codewen_managed_credentials: false,
        }),
        requires_openai_auth: false,
    };
    assert_eq!(received, expected);
