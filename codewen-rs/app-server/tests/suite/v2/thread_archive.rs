...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    // Verify file moved.
    let archived_directory = codewen_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let child_rollout_path =
        find_thread_path_by_id_str(codewen_home.path(), &child_id, /*state_db_ctx*/ None)
            .await?
            .expect("child rollout path");
    let archived_child_path = codewen_home
        .path()
...
async fn thread_archive_succeeds_when_spawned_descendant_is_missing() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(codewen_home.path(), &parent_id, /*state_db_ctx*/ None)
            .await?
            .is_none(),
