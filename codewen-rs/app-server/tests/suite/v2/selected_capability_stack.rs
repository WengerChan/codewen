...
use codewen_app_server_protocol::CapabilityRootLocation;
...
async fn selected_capability_stack_tracks_environment_availability_and_resume() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        .with_codewen_home(fixture.codewen_home.path())
    ...
    let mut exec_server =
        spawn_exec_server(fixture.codewen_home.path(), &fixture.exec_server_url).await?;
    ...
    exec_server = spawn_exec_server(fixture.codewen_home.path(), &fixture.exec_server_url).await?;
    add_environment(&mut app_server, &fixture.exec_server_url).await?;
    wait_for_selected_mcp_server(&mut app_server, &thread_id).await?;
...
fn selected_capability_fixture(
    ...
    drop(listener);
    std::fs::write(
        codewen_home.path().join("environments.toml"),
        format!(
            "default = \"{EXECUTOR_ID}\"\ninclude_local = true\n\n[[environments]]\nid = \"{EXECUTOR_ID}\"\nurl = \"{exec_server_url}\"\nconnect_timeout_sec = 0.05\n"
        ),
    )?;

    let local_skill_dir = codewen_home.path().join("skills/local-deploy");
    ...
    let manifest_dir = plugin.path().join(".codewen-plugin");
...
fn assert_selected_plugin_tools_absent(request: &ResponsesRequest) {
    ...
    let connector = request
        .tool_by_name("mcp__codewen_apps__calendar", "connector_calendar")
...
async fn spawn_exec_server(codewen_home: &std::path::Path, url: &str) -> Result<Child> {
    let mut child = Command::new(codewen_utils_cargo_bin::cargo_bin("codewen")?)
