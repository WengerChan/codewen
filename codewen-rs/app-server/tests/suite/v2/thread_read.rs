...
use codewen_app_server::in_process;
...
use codewen_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use codewen_thread_store::AppendThreadItemsParams;
...
async fn thread_turns_list_can_page_backward_and_forward() -> Result<()> {
    ...
    let rollout_path = rollout_path(codewen_home.path(), filename_ts, &conversation_id);
...
async fn thread_search_occurrences_reads_paginated_projection() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    create_config_toml(codewen_home.path(), &server.uri())?;
    let thread_id = codewen_protocol::ThreadId::default();
    ...
    let store = LocalThreadStore::new(
        LocalThreadStoreConfig {
            ...
            sqlite_home: codewen_home.path().to_path_buf(),
    ...
    store
        .create_thread(CreateThreadParams {
            ...
            history_mode: codewen_protocol::protocol::ThreadHistoryMode::Paginated,
            ...
            items: vec![
                ...
                paginated_completed_item(
                    ...
                    CoreTurnItem::UserMessage(UserMessageItem {
                        ...
                        client_id: None,
                        content: vec![
                            codewen_protocol::user_input::UserInput::Text {
                                text: "Nee".to_string(),
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let client = in_process::start(InProcessStartArgs {
        ...
        thread_config_loader: Arc::new(codewen_config::NoopThreadConfigLoader),
        ...
        initialize: InitializeParams {
            client_info: ClientInfo {
                name: "codewen-app-server-tests".to_string(),
                title: None,
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = codewen_protocol::ThreadId::from_string(&thread.id)?;
    store
        .append_items(AppendThreadItemsParams {
            thread_id,
...
async fn thread_resume_initial_turns_page_matches_requested_turns_list_page() -> Result<()> {
    ...
    assert_eq!(
        initial_turns_page,
        Some(codewen_app_server_protocol::TurnsPage::from(expected_page))
    );
...
async fn paginated_history_lists_use_projected_turns_and_items() -> Result<()> {
    ...
    let rejoin_id = mcp
        ...
        .await?;
    let rejoin_resp: JSONRPCResponse = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(rejoin_id)),
    )
    .await??;
    let ThreadResumeResponse {
        ...
        ..
    } = to_response::<ThreadResumeResponse>(rejoin_resp)?;
    assert_eq!(
...
) -> anyhow::Result<codewen_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
async fn read_turns_page(
    mcp: &mut TestAppServer,
    thread_id: codewen_protocol::ThreadId,
...
fn turn_user_texts(turns: &[codewen_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[codewen_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn create_config_toml(codewen_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = codewen_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
