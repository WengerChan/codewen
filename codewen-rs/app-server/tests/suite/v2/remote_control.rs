...
use codewen_app_server::AppServerRuntimeOptions;
use codewen_app_server::AppServerTransport;
use codewen_app_server::AppServerWebsocketAuthSettings;
use codewen_app_server::PluginStartupTasks;
use codewen_app_server::RemoteControlStartupMode;
use codewen_app_server::run_main_with_transport_options;
use codewen_app_server_protocol::JSONRPCError;
use codewen_app_server_protocol::JSONRPCResponse;
use codewen_app_server_protocol::RemoteControlClient;
use codewen_app_server_protocol::RemoteControlClientsListOrder;
use codewen_app_server_protocol::RemoteControlClientsListParams;
use codewen_app_server_protocol::RemoteControlClientsListResponse;
use codewen_app_server_protocol::RemoteControlClientsRevokeParams;
use codewen_app_server_protocol::RemoteControlClientsRevokeResponse;
use codewen_app_server_protocol::RemoteControlConnectionStatus;
use codewen_app_server_protocol::RemoteControlDisableResponse;
use codewen_app_server_protocol::RemoteControlEnableResponse;
use codewen_app_server_protocol::RemoteControlPairingStartParams;
use codewen_app_server_protocol::RemoteControlPairingStartResponse;
use codewen_app_server_protocol::RemoteControlPairingStatusParams;
use codewen_app_server_protocol::RemoteControlPairingStatusResponse;
use codewen_app_server_protocol::RemoteControlStatusChangedNotification;
use codewen_app_server_protocol::RemoteControlStatusReadResponse;
...
use codewen_state::RemoteControlEnrollmentRecord;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = codewen_home.path().join("managed_config.toml");
    let socket_path = codewen_home.path().join("app-server.sock");
    let transport =
    ...
    let result = timeout(
        ...
        run_main_with_transport_options(
            Arg0DispatchPaths {
                codewen_self_exe: Some(std::env::current_exe()?),
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let listener = configured_remote_control_listener(codewen_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(codewen_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_codewen_home(codewen_home.path())
        .without_auto_env()
        .build()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(codewen_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_codewen_home(codewen_home.path())
        .without_auto_env()
        .build()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(codewen_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_codewen_home(codewen_home.path())
        .without_auto_env()
        .build()
...
impl ClientManagementRemoteControlBackend {
    async fn start(codewen_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(codewen_home).await?;
...
async fn configured_remote_control_listener(codewen_home: &std::path::Path) -> Result<TcpListener> {
