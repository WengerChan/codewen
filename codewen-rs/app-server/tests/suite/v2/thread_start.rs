...
use codewen_app_server_protocol::TextPosition;
use codewen_app_server_protocol::TextRange;
...
use codewen_app_server_protocol::ThreadStatusChangedNotification;
...
async fn thread_start_warns_for_exec_policy_parse_failure_after_initialize() -> Result<()> {
    ...
    let rules_dir = codewen_home.path().join("rules");
...
async fn thread_start_provider_model_fallback_ignores_dynamic_catalog() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    create_config_toml_without_approval_policy(codewen_home.path(), &server.uri())?;
    let mut mcp = TestAppServer::builder()
        .with_codewen_home(codewen_home.path())
        .build()
        .await?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    create_config_toml_without_approval_policy(codewen_home.path(), &server.uri())?;
    let global_agents_path = codewen_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(codewen_home.path(), &server.uri())?;
...
"#,
    ...
    set_project_trust_level(codewen_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let config_before = std::fs::read_to_string(codewen_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(codewen_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
