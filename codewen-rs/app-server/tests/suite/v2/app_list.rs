...
use codewen_app_server_protocol::AppBranding;
...
use codewen_app_server_protocol::AppReview;
use codewen_app_server_protocol::AppScreenshot;
...
#[tokio::test]
async fn list_apps_returns_empty_when_workspace_codewen_plugins_disabled() -> Result<()> {
    let connectors = vec![AppInfo {
        id: "beta".to_string(),
        name: "Beta".to_string(),
        description: Some("Beta connector".to_string()),
...
async fn list_apps_includes_plugin_apps_for_chatgpt_auth() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    write_connectors_and_plugins_config(codewen_home.path(), &server_url)?;
    write_plugin_app_fixture(codewen_home.path(), "sample", "connector_sample")?;
    write_chatgpt_auth(
        codewen_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_connectors_and_plugins_config(codewen_home: &Path, base_url: &str) -> std::io::Result<()> {
    let config_toml = codewen_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
fn write_plugin_app_fixture(codewen_home: &Path, plugin_name: &str, app_id: &str) -> Result<()> {
