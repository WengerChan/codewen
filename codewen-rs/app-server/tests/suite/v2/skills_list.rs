...
use codewen_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = codewen_home.join("plugins/cache/openai-curated-remote/linear/local");
    std::fs::create_dir_all(plugin_root.join(".codewen-plugin"))?;
    std::fs::write(
        plugin_root.join(".codewen-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
fn write_cached_local_curated_plugin_with_skill(codewen_home: &std::path::Path) -> Result<()> {
    let plugin_root = codewen_home.join("plugins/cache/openai-curated/google-calendar/local");
    std::fs::create_dir_all(plugin_root.join(".codewen-plugin"))?;
    std::fs::write(
        plugin_root.join(".codewen-plugin/plugin.json"),
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(codewen_home.path())?;
    std::fs::write(
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(codewen_home.path())?)?;
...
async fn skills_list_excludes_plugin_skills_when_workspace_codewen_plugins_disabled() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_skill(&codewen_home, "home-skill")?;
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_skill(&codewen_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".codewen/skills/repo-skill");
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(codewen_home.path().join(&relative_cwd))?;
...
async fn skills_list_uses_cached_result_until_force_reload() -> Result<()> {
    ...
    let skill_dir = cwd.path().join(".codewen/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
