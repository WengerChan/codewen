...
//! the temporary `codewen_home` must not contain rollout session files or sqlite
...
use codewen_app_server::in_process::InProcessClientHandle;
use codewen_app_server::in_process::InProcessServerEvent;
...
use codewen_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_start_rejects_paginated_history_without_list_support() -> Result<()> {
    ...
    create_config_toml_with_thread_store(codewen_home.path(), &server.uri(), &store_id)?;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    let codewen_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    // Plugin startup warmups may create `.tmp` under codewen_home. Disable them
    ...
    let mut client = start_in_process_server(codewen_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(codewen_home.path())?;
...
async fn start_in_process_server(codewen_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
            .codewen_home(codewen_home.to_path_buf())
            .fallback_cwd(Some(codewen_home.to_path_buf()))
            .loader_overrides(loader_overrides.clone())
            .build()
            .await?,
    );
...
fn assert_no_local_persistence_artifacts(codewen_home: &Path) -> Result<()> {
    ...
    let mut entries = codewen_home_entries(codewen_home)?;
    // Bazel test runs may initialize shell snapshot storage under codewen_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in codewen_home"
    );

    Ok(())
}

fn codewen_home_entries(codewen_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(codewen_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
