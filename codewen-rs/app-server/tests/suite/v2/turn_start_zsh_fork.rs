...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&codewen_home, &workspace, &zsh_path).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(codewen_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(codewen_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
    ...
    assert!(command.starts_with(&command_packaged_zsh_path(&codewen_home).display().to_string()));
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(codewen_app_server_protocol::AskForApproval::UnlessTrusted),
    ...
    while target_decision_index < target_decisions.len() || !saw_parent_approval {
        ...
        let is_parent_approval = approval_command
            .contains(&command_packaged_zsh_path(&codewen_home).display().to_string())
...
async fn create_zsh_test_mcp_process(
    ...
    let app_server = create_test_package_app_server(codewen_home, zsh_path)?;
...
fn create_test_package_app_server(codewen_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    let package_dir = codewen_home.join("test-package");
    let bin_dir = package_dir.join("bin");
    let package_zsh_path = packaged_zsh_path(codewen_home);
    ...
    std::fs::write(package_dir.join("codewen-package.json"), "{}")?;

    let app_server = bin_dir.join("codewen-app-server");
    copy_with_permissions(
        &codewen_utils_cargo_bin::cargo_bin("codewen-app-server")?,
        &app_server,
    )?;
...
fn packaged_zsh_path(codewen_home: &Path) -> PathBuf {
    codewen_home
        .join("test-package")
        .join("codewen-resources")
        .join("zsh")
        .join("bin")
        .join("zsh")
}

fn command_packaged_zsh_path(codewen_home: &Path) -> PathBuf {
    let path = packaged_zsh_path(codewen_home);
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = codewen_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("codewen-rs/app-server/tests/suite/zsh");
