...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "codewen@example.com"])?;
...
fn disable_plugin_startup_tasks(codewen_home: &Path) -> Result<()> {
...
fn marketplace_install_root(codewen_home: &Path) -> std::path::PathBuf {
    codewen_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(codewen_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&codewen_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(codewen_home.path())?;
    ...
    let debug_root = expected_installed_root(codewen_home.path(), "debug")?;
    let tools_root = expected_installed_root(codewen_home.path(), "tools")?;
