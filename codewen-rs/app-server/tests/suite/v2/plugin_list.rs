...
use codewen_app_server_protocol::HookTrustStatus;
use codewen_app_server_protocol::HooksListParams;
use codewen_app_server_protocol::HooksListResponse;
use codewen_app_server_protocol::JSONRPCResponse;
use codewen_app_server_protocol::PluginAuthPolicy;
use codewen_app_server_protocol::PluginInstallPolicy;
use codewen_app_server_protocol::PluginInstallPolicySource;
...
use codewen_core::config::set_project_trust_level;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "CODEWEN_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
const ALTERNATE_MARKETPLACE_RELATIVE_PATH: &str = ".claude-plugin/marketplace.json";
const ALTERNATE_PLUGIN_MANIFEST_RELATIVE_PATH: &str = ".claude-plugin/plugin.json";

fn write_plugins_enabled_config(codewen_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(codewen_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = codewen_home.path().to_string_lossy().into_owned();
...
async fn plugin_installed_includes_installed_plugins_and_explicit_install_suggestions() -> Result<()>
{
    ...
    write_installed_plugin(&codewen_home, "openai-curated", "linear")?;
    std::fs::write(
...
async fn plugin_installed_prefers_remote_curated_conflicts_when_remote_plugin_enabled() -> Result<()>
{
    ...
    write_openai_curated_marketplace(codewen_home.path(), &["linear", "calendar"])?;
    write_installed_plugin(&codewen_home, "openai-curated", "linear")?;
    write_installed_plugin(&codewen_home, "openai-curated", "calendar")?;
    std::fs::write(
        codewen_home.path().join("config.toml"),
        format!(
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.codewen-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.codewen-plugin/plugin.json"),
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_codewen_plugins_disabled() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_codewen_plugins_setting() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.codewen-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        codewen_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.codewen-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    auth_policy: PluginAuthPolicy::OnInstall,
                    availability: codewen_app_server_protocol::PluginAvailability::Available,
                    interface: Some(codewen_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let codewen_home = TempDir::new()?;
    std::fs::create_dir_all(codewen_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(codewen_home.path())?;
    std::fs::write(
        codewen_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "codewen-curated",
  "plugins": [
    {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&codewen_home, "codewen-curated", "enabled-plugin")?;
    write_installed_plugin(&codewen_home, "codewen-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "codewen-curated",
...
[plugins."enabled-plugin@codewen-curated"]
enabled = true

[plugins."disabled-plugin@codewen-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "codewen-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@codewen-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@codewen-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@codewen-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let codewen_home = TempDir::new()?;
    std::fs::create_dir_all(codewen_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&codewen_home, "codewen-curated", "shared-plugin")?;
    std::fs::write(
        codewen_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "codewen-curated",
...
[plugins."shared-plugin@codewen-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".codewen"))?;
    std::fs::write(
        workspace_enabled.path().join(".codewen/config.toml"),
        r#"[plugins."shared-plugin@codewen-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@codewen-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@codewen-curated");
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let installed_path = codewen_home
        .path()
    ...
    wait_for_path_exists(&installed_path.join(".codewen-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".codewen-plugin/plugin.json"))?,
    )?;
    assert_eq!(
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    write_installed_plugin_with_version(&codewen_home, "openai-curated-remote", "linear", "1.0.0")?;
    write_installed_plugin_with_version(&codewen_home, "openai-curated-remote", "stale", "1.0.0")?;
    ...
    let old_path = codewen_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.0.0");
    let new_path = codewen_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.2.3");
    let stale_path = codewen_home
        .path()
    ...
    wait_for_path_exists(&new_path.join(".codewen-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".codewen-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
    ...
    write_installed_plugin_with_version(&codewen_home, "openai-curated-remote", "linear", "1.2.3")?;
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        codewen_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
    ...
    let cache_files = std::fs::read_dir(codewen_home.path().join("cache/remote_plugin_catalog"))?
...
async fn plugin_list_uses_cached_global_remote_catalog_and_refreshes_it() -> Result<()> {
    ...
    wait_for_cached_remote_catalog_plugin_ids(codewen_home.path(), &[cached_remote_plugin_id])
        .await?;
    ...
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
    wait_for_cached_remote_catalog_plugin_ids(codewen_home.path(), &[refreshed_remote_plugin_id])
        .await?;
...
-> Result<()> {
    ...
    write_openai_api_curated_marketplace(codewen_home.path(), &["api-plugin"])?;
    login_with_api_key(
        codewen_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
...
"#,
    ...
    write_remote_plugin_test_auth(codewen_home.path())?;
    ...
    wait_for_path_exists(
        &codewen_home
            .path()
            .join("plugins/cache/workspace-directory/no-hooks/1.2.3/.codewen-plugin/plugin.json"),
    )
    .await?;
...
async fn plugin_installed_hook_trust_write_failure_stays_untrusted() -> Result<()> {
    ...
    symlink(&config_target, codewen_home.path().join("config.toml"))?;
    write_remote_plugin_test_auth(codewen_home.path())?;
...
async fn plugin_list_marks_remote_plugin_disabled_by_admin() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        plugin.availability,
        codewen_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
    Ok(())
}
...
async fn wait_for_cached_remote_catalog_plugin_ids(
    ...
    timeout(DEFAULT_TIMEOUT, async {
        loop {
            let plugin_ids = cached_remote_catalog_plugin_ids(codewen_home)?;
...
fn cached_remote_catalog_plugin_ids(codewen_home: &std::path::Path) -> Result<Vec<String>> {
    let cache_dir = codewen_home.join("cache/remote_plugin_catalog");
    if !cache_dir.exists() {
        return Ok(Vec::new());
    }
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(codewen_home, marketplace_name, plugin_name, "local")
}
...
fn write_plugin_sync_config(codewen_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        codewen_home.join("config.toml"),
        format!(
            r#"
...
fn write_remote_plugin_test_auth(codewen_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        codewen_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_curated_marketplace(
    ...
    let curated_root = codewen_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.codewen-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(codewen_home.join(".tmp"))?;
    std::fs::write(
        codewen_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(codewen_home.join(".tmp"))?;
    std::fs::write(
        codewen_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
