...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(codewen_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".codewen-plugin/plugin.json"),
    )
    .await;
...
async fn plugin_install_uses_remote_apps_needing_auth_response() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_with_apps_test(codewen_home.path(), &server)?;
    mount_remote_plugin_detail_with_app_manifest(
        &server,
        REMOTE_PLUGIN_ID,
        "1.2.3",
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_codewen_plugins_disabled() -> Result<()> {
    let codewen_home = TempDir::new()?;
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                codewen_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_skips_mcp_oauth_for_chatgpt_dual_surface_plugin() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    write_connectors_config(codewen_home.path(), &apps_server_url)?;
    write_chatgpt_auth(
        codewen_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/codewen/ps/mcp", mcp_service);
...
async fn mount_backend_analytics_events(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/backend-api/codewen/analytics-events/events"))
...
fn configure_remote_plugin_test(codewen_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(codewen_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        codewen_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
