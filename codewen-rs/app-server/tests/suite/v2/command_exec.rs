...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", codewen_home.path().display()),
            stderr: String::new(),
        }
    );
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let codewen_home = TempDir::new()?;
    create_config_toml(codewen_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(codewen_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::builder()
        .with_codewen_home(codewen_home.path())
        .without_auto_env()
        .build()
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let codewen_home = TempDir::new()?;
    let command_dir = codewen_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !codewen_home.path().join("parent.txt").exists(),
...
"#
    ...
    insert_command_exec_config(codewen_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(codewen_home: &Path, inserted_config: &str) -> Result<()> {
    let config_path = codewen_home.join("config.toml");
