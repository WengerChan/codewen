...
async fn personal_access_token_without_email_supports_auth_status_and_account_read() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ("OPENAI_API_KEY", None),
            ("CODEWEN_ACCESS_TOKEN", Some("at-test-token")),
            ("CODEWEN_AUTHAPI_BASE_URL", Some(authapi_base_url.as_str())),
        ])
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    let codewen_home = TempDir::new()?;
    create_config_toml_custom_provider(codewen_home.path(), /*requires_openai_auth*/ false)?;
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let codewen_home = TempDir::new()?;
    create_config_toml_forced_login(codewen_home.path(), "chatgpt")?;
