...
fn main() {
    ...
    if target_os != "linux" || env::var_os("CODEWEN_SKIP_BWRAP_BUILD").is_some() {
        return;
    }
...
/// 2. The vendored bubblewrap tree under `codewen-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("CODEWEN_BWRAP_SOURCE_DIR") {
...
Set CODEX_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under codewen-rs/vendor.",
        vendor_dir.display()
    ))
}
