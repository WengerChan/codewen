//! Implements the `codewen doctor` diagnostic report.
//!
...
use codewen_api::ResponsesWebsocketClient;
use codewen_api::is_azure_responses_provider;
...
use codewen_install_context::CodewenPackageLayout;
use codewen_install_context::InstallContext;
use codewen_install_context::InstallMethod;
use codewen_install_context::StandalonePlatform;
use codewen_login::AuthDotJson;
use codewen_login::AuthManager;
use codewen_login::CODEX_ACCESS_TOKEN_ENV_VAR;
use codewen_login::CODEX_API_KEY_ENV_VAR;
use codewen_login::CodewenAuth;
use codewen_login::OPENAI_API_KEY_ENV_VAR;
use codewen_login::default_client::create_client_without_request_logging;
use codewen_login::default_client::default_headers;
use codewen_login::load_auth_dot_json;
...
use codewen_terminal_detection::Multiplexer;
use codewen_terminal_detection::TerminalInfo;
use codewen_terminal_detection::TerminalName;
use codewen_terminal_detection::terminal_info;
...
///
/// This is the CLI entry point for codewen doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun codewen doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        codewen_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
/// JSON support report emitted by `codewen doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        codewen_version: report.codewen_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("CODEWEN_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("CODEWEN_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "CODEWEN_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = codewen_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH codewen entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH codewen #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @openai/codewen would update a different install".to_string();
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("CODEWEN_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("CODEWEN_MANAGED_BY_NPM").is_none()
        && env::var_os("CODEWEN_MANAGED_BY_BUN").is_none()
        && env::var_os("CODEWEN_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&CodewenPackageLayout>,
) -> String {
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("CODEWEN_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@openai").join("codewen");
...
fn codewen_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["codewen"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "codewen"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("CODEX_HOME: {}", config.codewen_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.codewen_home.join(codewen_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.codewen_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    match load_auth_dot_json(
        ...
        .details(details)
        .remediation("Run codewen login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run codewen login again."),
    }
}
...
fn stored_auth_mode(auth: &codewen_login::AuthDotJson) -> &'static str {
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["CODEWEN_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "codewen-linux-sandbox helper",
        arg0_paths.codewen_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.codewen_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "CODEWEN_HOME", &config.codewen_home);
    ...
    for db in codewen_state::runtime_db_paths(&config.sqlite_home) {
    ...
    rollout_stats_details(&mut details, &config.codewen_home);
...
async fn sqlite_integrity_detail(
    ...
    match codewen_state::sqlite_integrity_check(path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, codewen_home: &Path) {
    let active = collect_rollout_stats(&codewen_home.join("sessions"));
    let archived = collect_rollout_stats(&codewen_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &CodewenAuth) -> &'static str {
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@openai/codewen");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@openai").join("codewen")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@openai/codewen");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@openai").join("codewen"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].codewen_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            codewen_self_exe: Some(PathBuf::from("/bin/codewen")),
            codewen_linux_sandbox_exe: Some(PathBuf::from("/bin/codewen-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/codewen-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.codewen_self_exe, arg0_paths.codewen_self_exe);
        assert_eq!(
            overrides.codewen_linux_sandbox_exe,
            arg0_paths.codewen_linux_sandbox_exe
        );
        assert_eq!(
    ...
    fn redacted_json_report_structures_and_sanitizes_details() {
        let report = DoctorReport {
            ...
            codewen_version: "0.0.0".to_string(),
            checks: vec![
                DoctorCheck::new(
                    "system.environment",
                    "system",
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "CODEWEN_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("CODEWEN_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-codewen-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-codewen-doctor-mcp\" is not resolvable",
            )
        }));
    }
