...
use codewen_state::ThreadStateAuditRow;
use codewen_utils_path::normalize_for_path_comparison;
...
async fn thread_inventory_check_for_roots(
    ...
    let scan = scan_rollout_files(codewen_home).await;
    let state_db_path = codewen_state::state_db_path(sqlite_home);
    ...
    let rows = match codewen_state::read_thread_state_audit_rows(&state_db_path).await {
        Ok(rows) => rows,
        Err(err) => {
    ...
    parity_check_from_scan_and_rows(codewen_home, scan, rows, details)
}
                ...
                let expected_archived = rollout_by_key
                    ...
                    .or_else(|| {
                        (scan
                            ...
                            .contains(&rollout_path_key(&row.rollout_path))
                            || row.rollout_path.is_file())
                        .then(|| archived_from_rollout_path(codewen_home, &row.rollout_path))
                        .flatten()
                    })?;
...
async fn scan_rollout_files(codewen_home: &Path) -> RolloutScan {
    let mut scan = RolloutScan::default();
    scan_rollout_root(
        &codewen_home.join("sessions"),
        /*archived*/ false,
        &mut scan,
    )
    .await;
    scan_rollout_root(
        &codewen_home.join("archived_sessions"),
        /*archived*/ true,
        &mut scan,
    )
    .await;
...
async fn scan_rollout_root(root: &Path, archived: bool, scan: &mut RolloutScan) {
    ...
    while let Some(dir) = dirs.pop() {
        ...
        for entry in entries {
            ...
            let logical_path = codewen_rollout::plain_rollout_path(&path);
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    let mut lines = match codewen_rollout::open_rollout_line_reader(path).await {
        Ok(lines) => lines,
    ...
    let logical_path = codewen_rollout::plain_rollout_path(path);
    codewen_rollout::builder_from_items(&[], &logical_path)
...
fn rollout_path_key(path: &Path) -> PathBuf {
    path_key(&codewen_rollout::plain_rollout_path(path))
}

fn archived_from_rollout_path(codewen_home: &Path, path: &Path) -> Option<bool> {
    let key = path_key(path);
    if key.starts_with(path_key(&codewen_home.join("archived_sessions"))) {
        return Some(true);
    }
    if key.starts_with(path_key(&codewen_home.join("sessions"))) {
        return Some(false);
    }
    None
}
...
mod tests {
    ...
    async fn thread_inventory_check_uses_compressed_metadata_id_and_legacy_filename_fallback() {
        ...
        let legacy_path = fixture.codewen_home.path().join(format!(
    ...
    async fn thread_inventory_check_ignores_compression_temp_files() {
        let fixture = Fixture::new().await;
        let temp_path = fixture.codewen_home.path().join(
    ...
    impl Fixture {
        async fn new() -> Self {
            let codewen_home = TempDir::new().expect("codewen home");
            let sqlite_home = TempDir::new().expect("sqlite home");
            let _runtime = codewen_state::StateRuntime::init(
        ...
        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            let root = if archived {
                self.codewen_home.path().join("archived_sessions")
            } else {
                self.codewen_home.path().join("sessions/2025/01/02")
            };
            ...
            let rollout_line = RolloutLine {
                ...
                ordinal: None,
                item: RolloutItem::SessionMeta(codewen_protocol::protocol::SessionMetaLine {
                    meta: codewen_protocol::protocol::SessionMeta {
                        session_id: parsed_thread_id.into(),
                        id: parsed_thread_id,
                        timestamp: timestamp.to_string(),
                        cwd: self.codewen_home.path().to_path_buf(),
        ...
        async fn insert_thread_row(&self, id: &str, rollout_path: &Path, archived: bool) {
            let state_db_path = codewen_state::state_db_path(self.sqlite_home.path());
            let pool = codewen_state::SqliteConfig::new_for_testing(self.sqlite_home.path().abs())
...
INSERT INTO threads (
    ...
    archived_at
            ...
            .bind("cli")
            .bind("test-provider")
            .bind(self.codewen_home.path().display().to_string())
            .bind("test title")
            .bind("read-only")
            .bind("on-request")
