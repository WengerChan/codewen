...
use codewen_core_plugins::ConfiguredMarketplace;
use codewen_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use codewen_core_plugins::PluginInstallOutcome;
...
use codewen_core_plugins::allowed_configured_marketplace_names;
use codewen_core_plugins::installed_marketplaces::marketplace_install_root;
use codewen_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use codewen_core_plugins::marketplace::MarketplaceListError;
use codewen_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use codewen_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use codewen_core_plugins::marketplace::MarketplacePluginSource;
use codewen_core_plugins::marketplace::find_marketplace_manifest_path;
use codewen_login::CodewenAuth;
use codewen_login::auth::read_codewen_api_key_from_env;
use codewen_plugin::PluginId;
use codewen_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(bin_name = "codewen plugin")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "codewen plugin add",
    after_help = "Examples:\n  codewen plugin add sample@debug\n  codewen plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "codewen plugin list",
    after_help = "Examples:\n  codewen plugin list\n  codewen plugin list --marketplace debug\n  codewen plugin list --json\n  codewen plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "codewen plugin remove",
    after_help = "Examples:\n  codewen plugin remove sample@debug\n  codewen plugin remove sample --marketplace debug"
)]
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    codewen_core_plugins::marketplace::MarketplacePluginSource::Local { path } => {
                        path.as_path().display().to_string()
                    }
                    codewen_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
                    ...
                    codewen_core_plugins::marketplace::MarketplacePluginSource::Npm {
                        package,
                        version,
                        registry,
                    } => {
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<codewen_core_plugins::ConfiguredMarketplace>,
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, codewen_home);
...
async fn load_plugin_command_context(
    ...
    let codewen_home = find_codewen_home().context("failed to resolve CODEX_HOME")?;
    let config = Config::load_with_cli_overrides(overrides)
        .await
        .context("failed to load configuration")?;
    let plugins_input = config.plugins_config_input();
    let manager = PluginsManager::new(codewen_home.to_path_buf());
...
pub(crate) async fn load_cli_auth_mode(config: &Config) -> Option<AuthMode> {
    if let Some(api_key) = read_codewen_api_key_from_env() {
        return Some(CodewenAuth::from_api_key(&api_key).api_auth_mode());
    }

    let auth_route_config = config.auth_route_config();
    CodewenAuth::from_auth_storage(
        &config.codewen_home,
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(codewen_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, codewen_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
