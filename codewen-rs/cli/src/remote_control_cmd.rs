...
use codewen_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use codewen_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use codewen_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use codewen_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use codewen_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = codewen_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = codewen_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
            print_remote_control_stop_output(&output, command.json)?;
        }
        Some(RemoteControlSubcommand::Pair) => {
            let output = codewen_app_server_daemon::start_remote_control_pairing().await?;
...
async fn run_foreground_remote_control(
    ...
    let runtime_options = AppServerRuntimeOptions {
        remote_control_startup_mode: codewen_app_server::RemoteControlStartupMode::EnabledEphemeral,
        install_shutdown_signal_handler: false,
        ..Default::default()
    };
    let (stop_rx, stop_signal_task) = foreground_stop_signal();
    let mut app_server_task = tokio::spawn(codewen_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    codewen_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
...
fn daemon_app_server_human_lines(output: &AppServerRemoteControlStartOutput) -> Vec<String> {
    let (managed_codewen_path, managed_codewen_version) = daemon_app_server_identity(output);
    vec![
...
fn daemon_app_server_identity(
    ...
    match output {
        AppServerRemoteControlStartOutput::Bootstrap(output) => (
            &output.managed_codewen_path,
            output.managed_codewen_version.as_deref(),
        ),
...
mod tests {
    ...
    fn daemon_ready_output(
        ...
        AppServerRemoteControlReadyOutput {
            daemon: AppServerRemoteControlStartOutput::Start(AppServerLifecycleOutput {
                ...
                backend: None,
                pid: Some(42),
                managed_codewen_path: PathBuf::from("/opt/codewen/bin/codewen"),
                managed_codewen_version: Some("1.0.0".to_string()),
    ...
    fn daemon_app_server_human_lines_include_path_and_version() {
        assert_eq!(
            ...
            vec![
                "Daemon used app-server:".to_string(),
                "  path: /opt/codewen/bin/codewen".to_string(),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "daemon": {
                    "status": "started",
                    "pid": 42,
                    "managedCodewenPath": "/opt/codewen/bin/codewen",
                    "managedCodewenVersion": "1.0.0",
