...
use codewen_state::RuntimeDbBackup;
use codewen_tui::LocalStateDbStartupError;
...
pub(crate) fn is_locked(detail: &str) -> bool {
    codewen_state::sqlite_error_detail_is_lock(detail)
}

pub(crate) fn is_corruption(detail: &str) -> bool {
    codewen_state::sqlite_error_detail_is_corruption(detail)
}
...
pub(crate) async fn backup_files_for_fresh_start(
    ...
    codewen_state::backup_runtime_db_for_fresh_start(startup_error.database_path()).await
}
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Codex couldn't start because its local database appears to be damaged.");
    eprintln!("Run `codewen doctor` to check your setup and get next-step guidance.");
...
mod tests {
    ...
    async fn backup_backs_up_only_failed_database_file() -> std::io::Result<()> {
        ...
        let state_path = codewen_state::state_db_path(temp_dir.path());
        let failed_db_path = codewen_state::logs_db_path(temp_dir.path());
    ...
    async fn backup_replaces_blocking_sqlite_home_file() -> std::io::Result<()> {
        ...
        let startup_error = LocalStateDbStartupError::new(
            codewen_state::state_db_path(sqlite_home.as_path()),
