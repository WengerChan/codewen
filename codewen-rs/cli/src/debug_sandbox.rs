...
#[cfg(target_os = "macos")]
use codewen_core::spawn::CODEX_SANDBOX_ENV_VAR;
use codewen_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
use codewen_sandboxing::landlock::allow_network_for_proxy;
use codewen_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use codewen_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use codewen_sandboxing::seatbelt::create_seatbelt_command_args;
use codewen_sandboxing::with_managed_mitm_ca_readable_root;
...
pub async fn run_command_under_seatbelt(
    ...
    _codewen_linux_sandbox_exe: Option<PathBuf>,
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<codewen_mcp::SandboxState>)
        .transpose()
    ...
    let codewen_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .codewen_linux_sandbox_exe
                .clone()
                .or(codewen_linux_sandbox_exe)
        }
        None => codewen_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(codewen_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Landlock => {
            ...
            let codewen_linux_sandbox_exe = config
                .codewen_linux_sandbox_exe
                .expect("codewen-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use codewen_windows_sandbox::WindowsSandboxProxySettingsMode;
    use codewen_windows_sandbox::WindowsSandboxSessionRequest;
    use codewen_windows_sandbox::spawn_windows_sandbox_session_for_level;

    let empty_paths: &[AbsolutePathBuf] = &[];
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        permission_profile,
        workspace_roots: workspace_roots.as_slice(),
        codewen_home: config.codewen_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = codewen_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_codewen_home(
        cli_overrides,
        codewen_linux_sandbox_exe,
        options,
        /*codewen_home*/ None,
...
async fn load_debug_sandbox_config_with_codewen_home(
    cli_overrides: Vec<(String, TomlValue)>,
    codewen_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `codewen sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            codewen_linux_sandbox_exe: codewen_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        codewen_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(codewen_home) = codewen_home {
        builder = builder
            .codewen_home(codewen_home.clone())
            .fallback_cwd(Some(codewen_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_codewen_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = codewen_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_codewen_home(
            ...
            Some(codewen_home.path().to_path_buf()),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = codewen_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
