...
const CODEX_BUNDLE_IDENTIFIER: &str = "com.openai.codewen";
const CODEX_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/codewen-app-prod/Codex.dmg";
const CODEX_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/codewen-app-prod/Codex-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_codewen_app_path(&codewen_app_search_dirs()) {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_codewen_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let installed_app = download_and_install_codewen_to_user_applications(&download_url)
        .await
    ...
    open_codewen_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_codewen_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "Codex.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_codewen_app_bundle(candidate))
}

fn codewen_app_search_dirs() -> Vec<PathBuf> {
...
fn is_codewen_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
...
async fn open_codewen_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = codewen_new_thread_url(workspace);
...
fn codewen_new_thread_url(workspace: &Path) -> String {
...
async fn download_and_install_codewen_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("codewen-app-installer-")
        .tempdir()
    ...
    let result = async {
        let app_in_volume = find_codewen_app_in_mount(&mount_point)
            .context("failed to locate Codex.app in mounted dmg")?;
        install_codewen_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_codewen_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
...
fn find_codewen_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
...
mod tests {
    ...
    use super::find_existing_codewen_app_path;
    ...
    #[test]
    fn finds_chatgpt_app_with_codewen_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("ChatGPT.app");
        write_app_bundle(&app_path, "com.openai.codewen");

        assert_eq!(
            find_existing_codewen_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let codewen_app_path = temp_dir.path().join("Codex.app");
        write_app_bundle(&codewen_app_path, "com.openai.codewen");

        assert_eq!(
            find_existing_codewen_app_path(&[temp_dir.path().to_path_buf()]),
            Some(codewen_app_path)
        );
    }
    ...
    #[test]
    fn codewen_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&codewen_new_thread_url(Path::new("/tmp/codewen workspace/#1")))
        ...
        assert_eq!(
            ...
            (
                ...
                "/new".to_string(),
                vec![("path".to_string(), "/tmp/codewen workspace/#1".to_string())],
            )
        );
    }
}
