...
pub(crate) struct SandboxSetupCommand {
    ...
    #[arg(long = "codewen-home", value_name = "DIR")]
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`codewen sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        .codewen_home(identity.codewen_home.clone())
        .fallback_cwd(Some(identity.codewen_home.clone()))
        .loader_overrides(super::loader_overrides_for_profile_at_codewen_home(
            config_profile.as_ref(),
            &identity.codewen_home,
        ))
    ...
    codewen_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.codewen_home.as_path(),
        identity.real_user.as_str(),
        config.permissions.network.as_ref(),
    )?;
    ConfigEditsBuilder::new(identity.codewen_home.as_path())
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let codewen_home = match cmd.codewen_home.clone() {
            Some(codewen_home) => codewen_home,
            None => find_codewen_home()?.to_path_buf(),
        };
    ...
    let codewen_home = cmd
        .codewen_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--codewen-home is required with --user"))?;
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            command.codewen_home.as_deref(),
            Some(std::path::Path::new(r"C:\Users\alice\.codewen"))
        );
    }
    ...
    #[test]
    fn requires_codewen_home_for_managed_user() {
        let err =
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            "--codewen-home".to_string(),
            r"C:\Users\alice\.codewen".to_string(),
        ])
        .expect("parse")
