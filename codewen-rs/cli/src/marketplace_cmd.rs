...
use codewen_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use codewen_core_plugins::marketplace::marketplace_root_dir;
use codewen_core_plugins::marketplace_add::MarketplaceAddOutcome;
use codewen_core_plugins::marketplace_add::MarketplaceAddRequest;
use codewen_core_plugins::marketplace_add::add_marketplace;
use codewen_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use codewen_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "codewen plugin marketplace")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "codewen plugin marketplace add",
    after_help = "Examples:\n  codewen plugin marketplace add ./path/to/marketplace\n  codewen plugin marketplace add owner/repo --ref main\n  codewen plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "codewen plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "codewen plugin marketplace upgrade",
    after_help = "Examples:\n  codewen plugin marketplace upgrade\n  codewen plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "codewen plugin marketplace remove",
    after_help = "Example:\n  codewen plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    let config = Config::load_with_cli_overrides(overrides)
        .await
        .context("failed to load configuration")?;
    let manager = PluginsManager::new(config.codewen_home.to_path_buf());
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.codewen_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<codewen_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, codewen_home);
