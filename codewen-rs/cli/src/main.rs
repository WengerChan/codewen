...
use codewen_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use codewen_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use codewen_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use codewen_arg0::Arg0DispatchPaths;
use codewen_arg0::arg0_dispatch_or_else;
use codewen_chatgpt::apply_command::ApplyCommand;
use codewen_chatgpt::apply_command::run_apply_command;
use codewen_cli::read_access_token_from_stdin;
use codewen_cli::read_api_key_from_stdin;
use codewen_cli::run_login_status;
use codewen_cli::run_login_with_access_token;
use codewen_cli::run_login_with_api_key;
use codewen_cli::run_login_with_chatgpt;
use codewen_cli::run_login_with_device_code;
use codewen_cli::run_logout;
use codewen_cloud_tasks::Cli as CloudTasksCli;
use codewen_exec::Cli as ExecCli;
use codewen_exec::Command as ExecCommand;
use codewen_exec::ReviewArgs;
use codewen_execpolicy::ExecPolicyCheckCommand;
use codewen_responses_api_proxy::Args as ResponsesApiProxyArgs;
use codewen_rollout_trace::REDUCED_STATE_FILE_NAME;
use codewen_rollout_trace::replay_bundle;
use codewen_state::StateRuntime;
use codewen_state::memories_db_path;
use codewen_tui::AppExitInfo;
use codewen_tui::Cli as TuiCli;
use codewen_tui::ExitReason;
use codewen_tui::UpdateAction;
use codewen_utils_absolute_path::AbsolutePathBuf;
use codewen_utils_cli::CliConfigOverrides;
use codewen_utils_cli::ProfileV2Name;
use codewen_utils_cli::SharedCliOptions;
...
use codewen_config::LoaderOverrides;
use codewen_core::build_models_manager;
use codewen_core::config::ConfigBuilder;
use codewen_core::config::ConfigOverrides;
use codewen_core::config::edit::ConfigEditsBuilder;
use codewen_core::config::find_codewen_home;
use codewen_core::config::resolve_profile_v2_config_path;
use codewen_features::FEATURES;
use codewen_features::Stage;
use codewen_features::is_known_feature_key;
use codewen_home::CodewenHomeUserInstructionsProvider;
use codewen_login::AuthManager;
use codewen_login::CodewenAuth;
use codewen_login::read_codewen_access_token_from_env;
use codewen_memories_write::clear_memory_roots_contents;
use codewen_models_manager::bundled_models_response;
use codewen_models_manager::manager::RefreshStrategy;
use codewen_protocol::protocol::AskForApproval;
use codewen_protocol::user_input::UserInput;
use codewen_terminal_detection::TerminalName;
...
#[clap(
    ...
    // `codewen-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `codewen` command name that users run.
    bin_name = "codewen",
    override_usage = "codewen [OPTIONS] [PROMPT]\n       codewen [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Codewen.
    Mcp(McpCli),

    /// Manage Codewen plugins.
    Plugin(PluginCli),

    /// Start Codewen as an MCP server (stdio).
    ...
    /// Update Codewen to the latest version.
    Update,

    /// Diagnose local Codewen installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Codewen-provided sandbox.
    ...
    /// Apply the latest diff produced by Codewen agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Codewen Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Codewen.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = codewen_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = codewen_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = codewen_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $CODEWEN_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | codewen login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv CODEWEN_ACCESS_TOKEN | codewen login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = codewen_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: codewen_app_server::AppServerTransport,
    ...
    #[command(flatten)]
    auth: codewen_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    /// Use Agent Identity auth from CODEWEN_ACCESS_TOKEN for remote registration.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Codewen tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Codewen via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Codewen.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`codewen update` is not available in debug builds. Install a release build of Codewen to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = codewen_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Codewen installation method. Please update manually: https://developers.openai.com/codewen/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn run_session_archive_cli_command(
    action: codewen_tui::SessionArchiveAction,
    ...
    codewen_tui::run_session_archive_command(
        action,
        target,
        codewen_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<codewen_tui::SessionArchiveAction> {
    if force && codewen_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => codewen_tui::DeleteConfirmation::Skip,
        false => codewen_tui::DeleteConfirmation::Prompt,
    };
    Ok(codewen_tui::SessionArchiveAction::Delete(confirmation))
}
...
fn main() -> anyhow::Result<()> {
    let remote_control_disabled = codewen_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            codewen_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["codewen", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
            ...
            match subcommand {
                None => {
                    let transport = if stdio {
                        codewen_app_server::AppServerTransport::Stdio
                    } else {
                        listen
                    };
                    let auth = auth.try_into_settings()?;
                    let runtime_options = codewen_app_server::AppServerRuntimeOptions {
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            (true, _) => {
                                codewen_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                codewen_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                codewen_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    codewen_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        ...
                        analytics_default_enabled,
                        transport,
                        codewen_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            codewen_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        ...
                        codewen_app_server_daemon::run_pid_update_loop(http_client_factory).await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            let codewen_home = find_codewen_home()?;
                            codewen_app_server::app_server_control_socket_path(&codewen_home)?
                        }
                    };
                    codewen_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = codewen_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    codewen_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    codewen_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    codewen_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                codewen_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                codewen_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | codewen login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            codewen_cloud_tasks::run_main(cloud_cli, arg0_paths.codewen_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            codewen_cli::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.codewen_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
            codewen_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.codewen_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            codewen_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.codewen_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`codewen sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || codewen_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `codewen mcp`: `codewen`, `codewen exec`, `codewen review`, `codewen resume`, `codewen archive`, `codewen delete`, `codewen unarchive`, `codewen fork`, `codewen mcp`, `codewen sandbox`, and `codewen debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let codewen_self_exe = arg0_paths
        .codewen_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Codewen executable path is not configured"))?;
    let runtime_paths = codewen_exec_server::ExecServerRuntimePaths::new(
        codewen_self_exe,
        arg0_paths.codewen_linux_sandbox_exe.clone(),
    )?;
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = codewen_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
        )?;
        ...
        exec_server_telemetry::run_until_shutdown(async move {
            codewen_exec_server::run_remote_environment(remote_config, runtime_paths).await
        })
        .await?;
        Ok(())
    } else {
...
async fn load_exec_server_remote_auth_provider(
    config: &codewen_core::config::Config,
    base_url: &str,
    use_agent_identity_auth: bool,
    ...
    if use_agent_identity_auth {
        ...
        .await?;
        return Ok(codewen_model_provider::auth_provider_from_auth(&auth));
    }

    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `codewen login` or set CODEWEN_API_KEY",
    )
    .await?;
    ...
    Ok(codewen_model_provider::auth_provider_from_auth(&auth))
}

fn is_supported_exec_server_remote_auth(auth: &CodewenAuth) -> bool {
...
async fn load_exec_server_config(
    root_config_overrides: &CliConfigOverrides,
    strict_config: bool,
) -> anyhow::Result<codewen_core::config::Config> {
...
) -> anyhow::Result<codewen_login::CodewenAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_codewen_api_key_env*/ true).await;
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    let codewen_home = find_codewen_home()?;
    ConfigEditsBuilder::new(&codewen_home)
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&codewen_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let codewen_home = find_codewen_home()?;
            Ok(loader_overrides_for_profile_at_codewen_home(
                Some(profile_v2),
                &codewen_home,
            ))
        }
...
fn loader_overrides_for_profile_at_codewen_home(
    profile_v2: Option<&ProfileV2Name>,
    codewen_home: &std::path::Path,
) -> LoaderOverrides {
    match profile_v2 {
        Some(profile_v2) => LoaderOverrides {
            user_config_path: Some(resolve_profile_v2_config_path(codewen_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(codewen_home: &std::path::Path, feature: &str) {
    ...
    let config_path = codewen_home.join(codewen_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(codewen_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        codewen_self_exe: arg0_paths.codewen_self_exe,
        codewen_linux_sandbox_exe: arg0_paths.codewen_linux_sandbox_exe,
    ...
    let user_instructions_provider = Arc::new(CodewenHomeUserInstructionsProvider::new(
        config.codewen_home.clone(),
    ));
    let prompt_input = codewen_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.codewen_home).await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `codewen {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `codewen {subcommand}`"
        );
    }
    Ok(())
}
...
/// `codewen --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `codewen {subcommand}`");
    }
    Ok(())
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = codewen_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<codewen_core::config::Config>,
) -> codewen_http_client::HttpClientFactory {
    match config {
        Ok(config) => config.http_client_factory(),
        Err(error) => {
            eprintln!("warning: failed to load updater network configuration: {error}");
            codewen_http_client::HttpClientFactory::new(
                codewen_http_client::OutboundProxyPolicy::ReqwestDefault,
            )
        }
    }
}
...
async fn print_app_server_remote_control_output(
    ...
    let output = codewen_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = codewen_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Codewen's interactive TUI may not work in this terminal."
        );
    ...
    let start_tui = || {
        codewen_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            codewen_config::LoaderOverrides::default(),
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Codewen local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<codewen_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(codewen_tui::resolve_remote_addr)
        .transpose()
    ...
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !codewen_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let codewen_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `codewen resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `codewen` without additional flags.
...
/// Build the final `TuiCli` for a `codewen fork` invocation.
...
mod tests {
    ...
    use codewen_tui::TokenUsage;
    use pretty_assertions::assert_eq;

    #[tokio::test]
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let codewen_home = tempfile::tempdir().expect("temporary Codewen home");
        let config = ConfigBuilder::default()
            .codewen_home(codewen_home.path().to_path_buf())
            .cli_overrides(vec![(
        ...
        assert_eq!(
            ...
            codewen_http_client::OutboundProxyPolicy::RespectSystemProxy
        );
    }
    ...
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = CodewenAuth::from_api_key("sk-test");
    ...
    #[test]
    fn profile_loader_overrides_use_explicit_codewen_home() -> anyhow::Result<()> {
        let codewen_home = tempfile::tempdir()?;
        let profile: ProfileV2Name = "work".parse()?;

        let overrides =
            loader_overrides_for_profile_at_codewen_home(Some(&profile), codewen_home.path());
    ...
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["codewen", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["codewen", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["codewen", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["codewen", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["codewen", "import"]).expect("parse");
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["codewen", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(codewen_exec::Command::Resume(args)) = exec.command else {
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let codewen_home = find_codewen_home().expect("codewen home");
        codewen_app_server::app_server_control_socket_path(&codewen_home)
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["codewen", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: codewen plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: codewen plugin marketplace add"),
            ("list", "Usage: codewen plugin marketplace list"),
            ("upgrade", "Usage: codewen plugin marketplace upgrade"),
            ("remove", "Usage: codewen plugin marketplace remove"),
        ] {
            let help = help_from_args(&["codewen", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_add_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["codewen", "plugin", "marketplace", "add", "owner/repo"])
                .expect("parse");
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["codewen", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["codewen", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["codewen", "update"]).expect("parse");
    ...
    fn sandbox_help_only_shows_singular_permission_profile() {
        let help = help_from_args(&["codewen", "sandbox", "--help"]);
    ...
    fn sandbox_parses_permissions_profile_short_alias() {
        let cli =
            MultitoolCli::try_parse_from(["codewen", "sandbox", "-P", ":workspace", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_parses_config_profile() {
        let cli =
            MultitoolCli::try_parse_from(["codewen", "sandbox", "--profile", "work", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["codewen", "sandbox", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["codewen", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["codewen", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["codewen", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["codewen", "marketplace", "remove", "debug"]);
        assert!(remove_result.is_err());
    }

    #[test]
    fn full_auto_no_longer_parses_at_top_level() {
        let result = MultitoolCli::try_parse_from(["codewen", "--full-auto"]);
    ...
    fn exec_full_auto_reports_migration_path() {
        let cli = MultitoolCli::try_parse_from(["codewen", "exec", "--full-auto", "summarize"])
    ...
    fn sandbox_full_auto_no_longer_parses() {
        let result = MultitoolCli::try_parse_from(["codewen", "sandbox", "--full-auto", "--"]);
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: codewen_utils_cli::resume_hint(thread_name, thread_id),
            update_action: None,
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run codewen resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run codewen resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["codewen", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["codewen", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["codewen", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["codewen", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["codewen", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["codewen", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["codewen", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["codewen", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["codewen", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(codewen_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(codewen_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["codewen", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["codewen", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["codewen", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["codewen", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["codewen", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["codewen", "fork", "1234", "continue here"].as_ref());
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["codewen", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["codewen", "app-server"].as_ref());
    ...
    fn app_server_remote_control_startup_flag_enables_remote_control() {
        let enabled = app_server_from_args(["codewen", "app-server", "--remote-control"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["codewen", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["codewen", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli = MultitoolCli::try_parse_from(["codewen", "mcp-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
        ...
        let cli =
            MultitoolCli::try_parse_from(["codewen", "review", "--strict-config", "--uncommitted"])
                .expect("parse");
        assert_matches!(
            cli.subcommand,
            Some(Subcommand::Review(ReviewCommand {
        ...
        let cli = MultitoolCli::try_parse_from(["codewen", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["codewen", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["codewen", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `codewen mcp`"
        );

        let cli = MultitoolCli::try_parse_from(["codewen", "--strict-config", "remote-control"])
            .expect("parse");
        let err = reject_root_strict_config_for_subcommand(
            cli.interactive.strict_config,
            &cli.subcommand,
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `codewen remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["codewen", "app-server", "--strict-config", "proxy"].as_ref());
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `codewen app-server proxy`"
        );
    }
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["codewen", "--remote", "unix://codewen.sock"])
            .expect("parse");
        assert_eq!(cli.remote.remote.as_deref(), Some("unix://codewen.sock"));
    }
    ...
    fn remote_auth_token_env_flag_parses_for_interactive_root() {
        ...
        assert_eq!(
            cli.remote.remote_auth_token_env.as_deref(),
            Some("CODEWEN_REMOTE_AUTH_TOKEN")
        );
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["codewen", "resume", "--remote", "unix://codewen.sock"])
                .expect("parse");
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://codewen.sock"));
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("CODEWEN_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("CODEWEN_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["codewen", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            codewen_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["codewen", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            codewen_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["codewen", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["codewen", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
            codewen_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        ...
        assert_eq!(
            app_server.listen,
            codewen_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/codewen.sock")
    ...
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["codewen", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["codewen", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        assert!(matches!(
            app_server_from_args(["codewen", "app-server", "daemon", "start"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["codewen", "app-server", "daemon", "enable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["codewen", "app-server", "daemon", "disable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["codewen", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("codewen.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(codewen_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/codewen-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(codewen_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/codewen-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["codewen", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["codewen", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    fn strict_config_feature_toggle_error(args: &[&str]) -> anyhow::Error {
        let cli_args = std::iter::once("codewen")
