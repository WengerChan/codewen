...
fn write_file_auth_config(codewen_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(codewen_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(codewen_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(codewen_home.path())?;
    ...
    let auth = read_auth_json(codewen_home.path())?;
...
async fn device_login_revokes_existing_auth_before_requesting_new_tokens() -> Result<()> {
    ...
    .env("no_proxy", "127.0.0.1,localhost")
    .env_remove("CODEWEN_ACCESS_TOKEN")
