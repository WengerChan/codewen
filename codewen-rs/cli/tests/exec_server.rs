...
"#
    ...
    let codewen_bin = codewen_utils_cargo_bin::cargo_bin("codewen")?;
    let codewen_home = codewen_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(codewen_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let codewen_home = TempDir::new()?;
    let mut child = std::process::Command::new(codewen_utils_cargo_bin::cargo_bin("codewen")?)
