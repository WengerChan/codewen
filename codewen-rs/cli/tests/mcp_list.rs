...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = codewen_command(codewen_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(codewen_home.path()).await?;
    ...
    let mut list_json_cmd = codewen_command(codewen_home.path())?;
    ...
    let mut get_cmd = codewen_command(codewen_home.path())?;
    ...
    assert!(stdout.contains("remove: codewen mcp remove docs"));

    let mut get_json_cmd = codewen_command(codewen_home.path())?;
    get_json_cmd
