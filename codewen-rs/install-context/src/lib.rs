...
const PACKAGE_METADATA_FILENAME: &str = "codewen-package.json";
const PATH_DIRNAME: &str = "codewen-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "codewen-resources";
...
pub struct CodewenPackageLayout {
...
pub struct InstallContext {
    ...
    pub package_layout: Option<CodewenPackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.codewen/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `codewen-resources/`, and `codewen-path/`.
    ...
    /// A Codex binary launched through the npm-managed `codewen.js` shim.
    Npm,
    /// A Codex binary launched through the bun-managed `codewen.js` shim.
    Bun,
    /// A Codex binary launched through the pnpm-managed `codewen.js` shim.
    Pnpm,
...
impl InstallContext {
    pub fn from_exe(
        ...
        let codewen_home = codewen_utils_home_dir::find_codewen_home().ok();
        Self::from_exe_with_codewen_home(
            is_macos,
            current_exe,
            method_override,
    ...
    fn from_exe_with_codewen_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        codewen_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(CodewenPackageLayout::from_exe);
        let method = if let Some(method) = method_override {
            method
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, codewen_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("CODEWEN_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("CODEWEN_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("CODEWEN_MANAGED_BY_BUN").is_some() {
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) = standalone_install_method(exe_path, codewen_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_codewen_home = canonical_absolute_path(codewen_home?)?;
    ...
    let releases_root = canonical_codewen_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let context = InstallContext::from_exe_with_codewen_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*codewen_home*/ Some(codewen_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
    ...
    fn detects_package_layout_independently_from_install_method() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "codewen.exe" } else { "codewen" });
        fs::write(&exe_path, "")?;
        fs::write(resources_dir.join(TEST_RESOURCE_NAME), "")?;
        ...
        let package_layout = CodewenPackageLayout {
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_codewen_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn package_manager_method_overrides_take_precedence() {
        let pnpm_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/codewen")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_codewen_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/codewen")),
            /*method_override*/ None,
            /*codewen_home*/ None,
        );
        assert_eq!(
