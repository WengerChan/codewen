...
use codewen_connectors::ConnectorMetadata;
use codewen_connectors::ConnectorMetadataStore;
use codewen_connectors::ConnectorToolSummary;
use codewen_connectors::DirectoryListResponse;
use codewen_connectors::merge::merge_connectors;
use codewen_connectors::merge::merge_plugin_connectors;
use codewen_core::config::Config;
pub use codewen_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use codewen_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use codewen_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use codewen_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use codewen_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use codewen_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
pub use codewen_core::connectors::with_app_enabled_state;
...
const DEFAULT_APPS_PRODUCT_SKU: &str = "codewen";
...
async fn connector_auth(config: &Config) -> anyhow::Result<CodewenAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_codewen_api_key_env*/ false).await;
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = codewen_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = codewen_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        ...
        "ChatGPT account ID not available, please re-run codewen login"
    );
...
mod tests {
    use super::*;
    use codewen_connectors::metadata::connector_install_url;
    use codewen_plugin::AppConnectorId;
