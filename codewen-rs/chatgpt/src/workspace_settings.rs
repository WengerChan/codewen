...
struct CachedWorkspaceSettings {
    ...
    expires_at: Instant,
    codewen_plugins_enabled: bool,
}

impl WorkspaceSettingsCache {
    fn get_codewen_plugins_enabled(&self, key: &WorkspaceSettingsCacheKey) -> Option<bool> {
        {
            ...
            if let Some(cached) = entry.as_ref()
                ...
                && cached.key == *key
            {
                return Some(cached.codewen_plugins_enabled);
            }
        }
    ...
    fn set_codewen_plugins_enabled(&self, key: WorkspaceSettingsCacheKey, enabled: bool) {
        let mut entry = match self.entry.write() {
            Ok(entry) => entry,
            Err(err) => err.into_inner(),
        };
        *entry = Some(CachedWorkspaceSettings {
            ...
            codewen_plugins_enabled: enabled,
        });
    }
}

pub async fn codewen_plugins_enabled_for_workspace(
    config: &Config,
    ...
    if let Some(cache) = cache
        && let Some(enabled) = cache.get_codewen_plugins_enabled(&cache_key)
    {
        return Ok(enabled);
    }
    ...
    let codewen_plugins_enabled = settings
        .beta_settings
    ...
    if let Some(cache) = cache {
        cache.set_codewen_plugins_enabled(cache_key, codewen_plugins_enabled);
    }

    Ok(codewen_plugins_enabled)
}
