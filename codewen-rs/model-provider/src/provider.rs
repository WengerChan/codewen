...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "codewen-auto-review";
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    fn auth(&self) -> ModelProviderFuture<'_, Option<CodewenAuth>>;

    /// Returns the current app-visible account state for this provider.
    fn account_state(&self) -> ProviderAccountResult;

    /// Maps an API client error into the provider's user-facing error representation.
    fn map_api_error(&self, error: ApiError) -> CodewenErr {
        codewen_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, codewen_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            self.info()
                .to_api_provider(auth.as_ref().map(CodewenAuth::auth_mode))
        })
    }
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, codewen_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, codewen_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    ) -> ModelProviderFuture<'_, codewen_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| codewen_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
        ...
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .and_then(|auth_manager| {
                    ...
                    if matches!(auth, CodewenAuth::Headers(_)) {
                        return None;
                    }
                    Some(auth)
                })
                .map(|auth| match &auth {
                    CodewenAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    CodewenAuth::BedrockApiKey(_) => {
...
mod tests {
    ...
    use codewen_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    fn bedrock_api_key_auth() -> CodewenAuth {
        CodewenAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_codewen_home(), /*config_model_catalog*/ None);
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = codewen_models_manager::bundled_models_response()
