...
impl AuthProvider for AuthManagerAuthProvider {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        let Some(auth) = self
            .auth_manager
            .auth_cached()
            .filter(CodewenAuth::uses_codewen_backend)
        else {
            return;
        };
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> codewen_protocol::error::Result<SharedAuthProvider> {
    if matches!(auth, Some(CodewenAuth::BedrockApiKey(_))) {
        return Err(CodewenErr::UnsupportedOperation(
...
) -> codewen_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(CodewenAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(CodewenAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> codewen_protocol::error::Result<Option<BearerAuthProvider>> {
...
pub fn auth_provider_from_auth(auth: &CodewenAuth) -> SharedAuthProvider {
    match auth {
        CodewenAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        CodewenAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        CodewenAuth::BedrockApiKey(_) => unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}"),
        CodewenAuth::ApiKey(_)
        | CodewenAuth::Chatgpt(_)
        | CodewenAuth::ChatgptAuthTokens(_)
        | CodewenAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
#[cfg(test)]
mod tests {
    use codewen_agent_identity::generate_agent_key_material;
    use codewen_login::AuthCredentialsStoreMode;
    use codewen_login::AuthKeyringBackendKind;
    use codewen_login::auth::AgentIdentityAuthRecord;
    use codewen_login::auth::BedrockApiKeyAuth;
    use codewen_login::auth::login_with_chatgpt_auth_tokens;
    use codewen_model_provider_info::WireApi;
    use codewen_model_provider_info::create_oss_provider_with_base_url;
    ...
    fn test_codewen_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "codewen-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp codewen home");
        path
    }

    fn write_chatgpt_auth_json(codewen_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    async fn chatgpt_auth_manager(
    ...
    ) -> (PathBuf, Arc<AuthManager>, CodewenAuth) {
        let codewen_home = test_codewen_home();
        write_chatgpt_auth_json(&codewen_home);
        ...
        (codewen_home, auth_manager, auth)
    }
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = CodewenAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    fn openai_provider_rejects_bedrock_api_key_auth() {
        ...
        let auth = CodewenAuth::BedrockApiKey(BedrockApiKeyAuth {
            api_key: "bedrock-api-key-test".to_string(),
            region: "us-east-1".to_string(),
        });

        match resolve_provider_auth(Some(&auth), &provider) {
            Err(CodewenErr::UnsupportedOperation(message)) => {
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = CodewenAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&CodewenAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_codewen_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
