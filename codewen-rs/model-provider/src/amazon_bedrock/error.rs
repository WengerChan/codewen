...
pub(super) fn map_api_error(error: ApiError) -> CodewenErr {
    let mut error = codewen_api::map_api_error(error);
    if let CodewenErr::UnexpectedStatus(response) = &mut error
