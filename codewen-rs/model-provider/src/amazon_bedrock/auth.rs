...
use codewen_aws_auth::AwsAuthContext;
use codewen_aws_auth::AwsAuthError;
use codewen_aws_auth::AwsRequestToSign;
use codewen_http_client::Request;
use codewen_http_client::RequestBody;
use codewen_http_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_codewen_error)?;
...
fn aws_auth_error_to_codewen_error(error: AwsAuthError) -> CodewenErr {
    CodewenErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
