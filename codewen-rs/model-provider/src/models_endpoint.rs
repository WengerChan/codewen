...
use codewen_login::AuthEnvTelemetry;
use codewen_login::AuthManager;
use codewen_login::CodewenAuth;
use codewen_login::collect_auth_env_telemetry;
use codewen_login::default_client::create_client_for_route_async;
use codewen_model_provider_info::ModelProviderInfo;
use codewen_models_manager::manager::ModelsEndpointClient;
use codewen_models_manager::manager::ModelsEndpointFuture;
use codewen_otel::TelemetryAuthMode;
use codewen_protocol::error::CodewenErr;
use codewen_protocol::error::Result as CoreResult;
...
impl OpenAiModelsEndpoint {
    ...
    async fn auth(&self) -> Option<CodewenAuth> {
    ...
    async fn list_models(
        ...
        let _timer =
            codewen_otel::start_global_timer("codewen.remote_models.fetch_update.duration_ms", &[]);
        let auth = self.auth().await;
        let auth_mode = auth.as_ref().map(CodewenAuth::auth_mode);
        ...
        .await
        .map_err(|_| CodewenErr::Timeout)?
    }

    fn auth_env(&self) -> AuthEnvTelemetry {
        let codewen_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.codewen_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, codewen_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_codewen_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_codewen_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "codewen_otel.log_only",
            tracing::Level::INFO,
            event.name = "codewen.api_request",
            ...
            auth.env_codewen_api_key_present = self.auth_env.codewen_api_key_env_present,
            auth.env_codewen_api_key_enabled = self.auth_env.codewen_api_key_env_enabled,
        ...
        tracing::event!(
            target: "codewen_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "codewen.api_request",
            duration_ms = %duration.as_millis(),
            http.response.status_code = status,
