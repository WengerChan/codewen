...
Runtime orchestration for Phase 1 and Phase 2 still lives in `codewen-core` under
`codewen-rs/core/src/memories/`.

## Crates

- `codewen-rs/memories/read` (`codewen-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `codewen-rs/memories/write` (`codewen-memories-write`) owns the write path:
...
- In `codewen`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.codewen/memories/.git` by `codewen-git-utils`
