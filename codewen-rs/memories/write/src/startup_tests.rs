...
use codewen_git_utils::diff_since_latest_init;
use codewen_git_utils::reset_git_repository;
use codewen_login::AuthManager;
use codewen_login::CodewenAuth;
use codewen_model_provider::ModelProvider;
use codewen_model_provider::ModelProviderFuture;
use codewen_model_provider::ProviderAccountResult;
...
use codewen_state::Phase2JobClaimOutcome;
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test = build_test_codewen_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_codewen_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.codewen_home);
...
async fn build_test_codex(
    ...
    build_test_codewen_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_codewen_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestCodex> {
...
async fn trigger_memories_startup(test: &TestCodex) {
    let config_snapshot = test.codewen.config_snapshot().await;
...
) -> (Arc<MemoryStartupContext>, Arc<codewen_core::config::Config>) {
    let config_snapshot = test.codewen.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> codewen_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(codewen_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &codewen_state::StateRuntime,
    codewen_home: &Path,
    ...
    let mut metadata_builder = codewen_state::ThreadMetadataBuilder::new(
        thread_id,
        codewen_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = codewen_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = codewen_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<codewen_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.codewen.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        codewen_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
