...
//!
//! This crate wraps [`codewen_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](codewen_app_server::in_process::InProcessClientHandle),
...
pub use codewen_app_server::app_server_control_socket_path;
pub use codewen_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use codewen_app_server::in_process::InProcessServerEvent;
use codewen_app_server::in_process::InProcessStartArgs;
use codewen_app_server::in_process::LogDbLayer;
pub use codewen_app_server::in_process::StateDbHandle;
use codewen_app_server_protocol::ClientInfo;
use codewen_app_server_protocol::ClientNotification;
...
use codewen_app_server_protocol::Result as JsonRpcResult;
...
use codewen_config::NoopThreadConfigLoader;
use codewen_config::RemoteThreadConfigLoader;
use codewen_config::ThreadConfigLoader;
use codewen_core::config::Config;
pub use codewen_core::otel_init::build_provider as build_otel_provider;
...
/// module exists so clients can remove a direct `codewen-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub mod config {
        pub use codewen_core::config::*;

        pub mod edit {
            pub use codewen_core::config::edit::*;
        }
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    pub enable_codewen_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_codewen_api_key_env: self.enable_codewen_api_key_env,
            initialize,
...
/// - [`codewen_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            codewen_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    pub fn codewen_home(&self, local_codewen_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_codewen_home.display().to_string(),
            )),
            Self::Remote(client) => client.codewen_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use codewen_app_server_protocol::ServerNotification;
    use codewen_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use codewen_core::init_state_db;
    use codewen_uds::UnixListener;
    use codewen_utils_absolute_path::AbsolutePathBuf;
    use futures::SinkExt;
    ...
    async fn build_test_config_for_codewen_home(codewen_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_codewen_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let codewen_home = TempDir::new().expect("temp dir");
        let config = Arc::new(build_test_config_for_codewen_home(codewen_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "codewen-app-server-client-test".to_string(),
        ...
        write_websocket_message(
            ...
            JSONRPCMessage::Response(JSONRPCResponse {
                ...
                result: serde_json::json!({
                    "userAgent": "codewen_cli_rs/9.8.7-test (Test OS; x86_64) rust",
                    "codexHome": "/server/.codewen",
                }),
            }),
        )
        .await;
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            codewen_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            codewen_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(codewen_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: codewen_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(codewen_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: codewen_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: codewen_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: codewen_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: codewen_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<codewen_app_server_protocol::ThreadReadResponse>(
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        assert!(matches!(
            ...
            InProcessServerEvent::ServerNotification(ServerNotification::ItemCompleted(
                ...
                &notification.item,
                codewen_app_server_protocol::ThreadItem::AgentMessage { text, .. } if text == "hello"
            )
        ));
        assert!(matches!(
            &events[4],
            InProcessServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                notification
            )) if notification.turn.status == codewen_app_server_protocol::TurnStatus::Completed
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.codewen_home(), Some("/server/.codewen"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: codewen_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
        ...
        for event in &remaining_events {
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    == codewen_app_server_protocol::TurnStatus::Completed =>
                {
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                codewen_app_server_protocol::ServerNotification::TurnCompleted(
                    codewen_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: codewen_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items_view: codewen_app_server_protocol::TurnItemsView::Full,
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                codewen_app_server_protocol::ServerNotification::AgentMessageDelta(
                    codewen_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                codewen_app_server_protocol::ServerNotification::ItemCompleted(
                    codewen_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        completed_at_ms: 0,
                        item: codewen_app_server_protocol::ThreadItem::AgentMessage {
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                codewen_app_server_protocol::ServerNotification::ExternalAgentConfigImportCompleted(
                    codewen_app_server_protocol::ExternalAgentConfigImportCompletedNotification {
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                codewen_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    codewen_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            codewen_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }
