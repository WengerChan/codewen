...
use codewen_extension_items::image_generation::ImageGenerationItem;
use codewen_protocol::items::parse_hook_prompt_message;
use codewen_protocol::models::MessagePhase;
use codewen_protocol::protocol::AgentReasoningEvent;
use codewen_protocol::protocol::AgentReasoningRawContentEvent;
...
use codewen_protocol::protocol::ContextCompactedEvent;
use codewen_protocol::protocol::DynamicToolCallResponseEvent;
use codewen_protocol::protocol::ErrorEvent;
use codewen_protocol::protocol::EventMsg;
use codewen_protocol::protocol::ExecCommandBeginEvent;
use codewen_protocol::protocol::ExecCommandEndEvent;
use codewen_protocol::protocol::GuardianAssessmentEvent;
use codewen_protocol::protocol::GuardianAssessmentStatus;
use codewen_protocol::protocol::ImageGenerationBeginEvent;
use codewen_protocol::protocol::ImageGenerationEndEvent;
use codewen_protocol::protocol::ItemCompletedEvent;
use codewen_protocol::protocol::ItemStartedEvent;
use codewen_protocol::protocol::McpToolCallBeginEvent;
use codewen_protocol::protocol::McpToolCallEndEvent;
use codewen_protocol::protocol::PatchApplyBeginEvent;
use codewen_protocol::protocol::PatchApplyEndEvent;
...
use codewen_protocol::protocol::ViewImageToolCallEvent;
use codewen_protocol::protocol::WebSearchBeginEvent;
use codewen_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use codewen_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
...
#[cfg(test)]
use codewen_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use codewen_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `codewen-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &codewen_protocol::models::ResponseItem) {
        let codewen_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &codewen_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            codewen_protocol::items::TurnItem::EnteredReviewMode(_)
                | codewen_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            codewen_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            codewen_protocol::items::TurnItem::HookPrompt(_)
            | codewen_protocol::items::TurnItem::CommandExecution(_)
            | codewen_protocol::items::TurnItem::DynamicToolCall(_)
            | codewen_protocol::items::TurnItem::CollabAgentToolCall(_)
            | codewen_protocol::items::TurnItem::SubAgentActivity(_)
            | codewen_protocol::items::TurnItem::Extension(_)
            | codewen_protocol::items::TurnItem::EnteredReviewMode(_)
            | codewen_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            codewen_protocol::items::TurnItem::UserMessage(_)
            | codewen_protocol::items::TurnItem::AgentMessage(_)
            | codewen_protocol::items::TurnItem::Reasoning(_)
            | codewen_protocol::items::TurnItem::WebSearch(_)
            | codewen_protocol::items::TurnItem::ImageView(_)
            | codewen_protocol::items::TurnItem::ImageGeneration(_)
            | codewen_protocol::items::TurnItem::FileChange(_)
            | codewen_protocol::items::TurnItem::McpToolCall(_)
            | codewen_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &codewen_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &codewen_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &codewen_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &codewen_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &codewen_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &codewen_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &codewen_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &codewen_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &codewen_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &codewen_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &codewen_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &codewen_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &codewen_protocol::protocol::ExitedReviewModeEvent,
    ) {
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                codewen_error_info: payload.codewen_error_info.clone().map(Into::into),
...
fn convert_dynamic_tool_content_items(
    items: &[codewen_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
    ...
    items
        ...
        .map(|item| match item {
            ...
            codewen_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use codewen_extension_items::ExtensionItem as CoreExtensionItem;
    use codewen_extension_items::sleep::SleepItem as CoreSleepItem;
    use codewen_protocol::ThreadId;
    use codewen_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use codewen_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use codewen_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use codewen_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use codewen_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use codewen_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use codewen_protocol::items::TurnItem as CoreTurnItem;
    use codewen_protocol::items::UserMessageItem as CoreUserMessageItem;
    use codewen_protocol::items::build_hook_prompt_message;
    use codewen_protocol::mcp::CallToolResult;
    use codewen_protocol::models::ImageDetail;
    use codewen_protocol::models::MessagePhase as CoreMessagePhase;
    use codewen_protocol::models::WebSearchAction as CoreWebSearchAction;
    use codewen_protocol::parse_command::ParsedCommand;
    ...
    use codewen_protocol::protocol::CompactedItem;
    use codewen_protocol::protocol::DynamicToolCallResponseEvent;
    use codewen_protocol::protocol::EnteredReviewModeEvent;
    use codewen_protocol::protocol::ExecCommandEndEvent;
    use codewen_protocol::protocol::ExecCommandSource;
    use codewen_protocol::protocol::ExitedReviewModeEvent;
    use codewen_protocol::protocol::ItemStartedEvent;
    use codewen_protocol::protocol::McpInvocation;
    use codewen_protocol::protocol::McpToolCallEndEvent;
    use codewen_protocol::protocol::PatchApplyBeginEvent;
    use codewen_protocol::protocol::ReviewTarget;
    use codewen_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![codewen_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
        ...
        let events = vec![
            ...
            EventMsg::WebSearchEnd(WebSearchEndEvent {
                ...
                action: CoreWebSearchAction::Search {
                    query: Some("codewen".into()),
                    queries: None,
                },
                results: Some(vec![serde_json::json!({
                    "type": "text_result",
                    "ref_id": "turn0search0",
                    "url": "https://example.com/codewen",
                })]),
            }),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                codewen_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("codewen_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    codewen_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(codewen_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(codewen_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: codewen_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(codewen_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                codewen_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                codewen_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                codewen_error_info: Some(CodewenErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                codewen_error_info: Some(CodewenErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                message: "stream failure".into(),
                codewen_error_info: Some(
                    crate::protocol::v2::CodewenErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(codewen_protocol::items::HookPromptItem {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(codewen_protocol::models::ResponseItem::Message {
                id: Some(codewen_protocol::ResponseItemId::with_suffix("msg", "1")),
                role: "user".into(),
                content: vec![codewen_protocol::models::ContentItem::InputText {
