...
use codewen_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use codewen_protocol::config_types::SandboxMode as CoreSandboxMode;
use codewen_protocol::protocol::AskForApproval as CoreAskForApproval;
use codewen_protocol::protocol::CodewenErrorInfo as CoreCodewenErrorInfo;
use codewen_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use codewen_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
...
/// This translation layer make sure that we expose codewen error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `codewenErrorInfo` variant.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub enum CodewenErrorInfo {
...
impl From<CoreCodewenErrorInfo> for CodewenErrorInfo {
    fn from(value: CoreCodewenErrorInfo) -> Self {
        match value {
            CoreCodewenErrorInfo::ContextWindowExceeded => CodewenErrorInfo::ContextWindowExceeded,
            CoreCodewenErrorInfo::SessionBudgetExceeded => CodewenErrorInfo::SessionBudgetExceeded,
            CoreCodewenErrorInfo::UsageLimitExceeded => CodewenErrorInfo::UsageLimitExceeded,
            CoreCodewenErrorInfo::ServerOverloaded => CodewenErrorInfo::ServerOverloaded,
            CoreCodewenErrorInfo::CyberPolicy => CodewenErrorInfo::CyberPolicy,
            CoreCodewenErrorInfo::HttpConnectionFailed { http_status_code } => {
                CodewenErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreCodewenErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                CodewenErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreCodewenErrorInfo::InternalServerError => CodewenErrorInfo::InternalServerError,
            CoreCodewenErrorInfo::Unauthorized => CodewenErrorInfo::Unauthorized,
            CoreCodewenErrorInfo::BadRequest => CodewenErrorInfo::BadRequest,
            CoreCodewenErrorInfo::ThreadRollbackFailed => CodewenErrorInfo::ThreadRollbackFailed,
            CoreCodewenErrorInfo::SandboxError => CodewenErrorInfo::SandboxError,
            CoreCodewenErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                CodewenErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreCodewenErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                CodewenErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreCodewenErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                CodewenErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreCodewenErrorInfo::Other => CodewenErrorInfo::Other,
        }
    }
}
