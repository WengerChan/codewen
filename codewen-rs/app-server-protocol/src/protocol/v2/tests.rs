...
use codewen_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use codewen_protocol::config_types::MultiAgentMode;
use codewen_protocol::items::AgentMessageContent;
use codewen_protocol::items::AgentMessageItem;
use codewen_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use codewen_protocol::items::CollabAgentToolCallItem;
use codewen_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use codewen_protocol::items::CommandExecutionItem;
...
use codewen_protocol::items::FileChangeItem;
use codewen_protocol::items::ImageViewItem;
use codewen_protocol::items::McpToolCallItem;
use codewen_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use codewen_protocol::items::ReasoningItem;
...
use codewen_protocol::items::WebSearchItem as CoreWebSearchItem;
use codewen_protocol::mcp::CallToolResult;
use codewen_protocol::mcp::McpServerInfo;
use codewen_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use codewen_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use codewen_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use codewen_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use codewen_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use codewen_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use codewen_protocol::protocol::AgentStatus as CoreAgentStatus;
use codewen_protocol::protocol::AskForApproval as CoreAskForApproval;
use codewen_protocol::protocol::ConversationTextRole;
use codewen_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use codewen_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use codewen_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use codewen_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use codewen_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use codewen_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: codewen_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        codewen_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        codewen_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.codewen/state",
            "logDir": "file:///C:/Users/alice/.codewen/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.codewen/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.codewen/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.codewen/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.codewen/models.json").expect("valid URI"))
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        codewen_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.codewen/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![codewen_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let sender_thread_id = codewen_protocol::ThreadId::default();
    let receiver_thread_id = codewen_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: codewen_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            codewen_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(codewen_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@codewen-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "codewen-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@codewen-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "codewen-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn codewen_error_info_serializes_http_status_code_in_camel_case() {
    let value = CodewenErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn codewen_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(CodewenErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn codewen_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = CodewenErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(codewen_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(codewen_protocol::config_types::CollaborationMode {
            mode: codewen_protocol::config_types::ModeKind::Plan,
