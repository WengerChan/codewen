...
use codewen_code_mode_protocol::CellId;
use codewen_code_mode_protocol::CodeModeNestedToolCall;
use codewen_code_mode_protocol::CodeModeSessionDelegate;
use codewen_code_mode_protocol::ExecuteRequest;
use codewen_code_mode_protocol::NotificationFuture;
use codewen_code_mode_protocol::ToolInvocationFuture;
use codewen_code_mode_protocol::WaitRequest;
use codewen_code_mode_protocol::host::DelegateRequest;
use codewen_code_mode_protocol::host::DelegateRequestId;
use codewen_code_mode_protocol::host::HostResponse;
use codewen_code_mode_protocol::host::HostToClient;
use codewen_code_mode_protocol::host::RequestId;
use codewen_code_mode_protocol::host::SessionId;
use codewen_code_mode_protocol::host::WireNestedToolCall;
use codewen_code_mode_protocol::host::WireResult;
use codewen_code_mode_protocol::host::WireRuntimeResponse;
use codewen_code_mode_protocol::host::WireWaitOutcome;
...
struct DriverHarness {
    ...
    outgoing_rx: mpsc::Receiver<codewen_code_mode_protocol::host::EncodedFrame>,
...
impl DriverHarness {
    ...
    async fn start_cell(
        ...
        request_id: i64,
        cell_id: &str,
    ) -> codewen_code_mode_protocol::StartedCell {
        let (response_tx, response_rx) = oneshot::channel();
        self.command_tx
            ...
            .send(DriverEvent::HostMessage(HostToClient::DelegateRequest {
                ...
                request: DelegateRequest::InvokeTool {
                    invocation: WireNestedToolCall {
                        ...
                        tool_kind: codewen_code_mode_protocol::CodeModeToolKind::Function.into(),
                        input: None,
                    },
                },
            }))
...
async fn terminate_closes_cell_without_waiting_for_delegate_cleanup() {
    ...
    assert_eq!(
        ...
        Ok(codewen_code_mode_protocol::WaitOutcome::LiveCell(
            codewen_code_mode_protocol::RuntimeResponse::Terminated {
                cell_id: CellId::new("1".to_string()),
                content_items: Vec::new(),
            }
        ))
...
async fn remote_wait_accepts_durations_longer_than_five_minutes() {
    ...
    assert_eq!(
        ...
        Ok(codewen_code_mode_protocol::WaitOutcome::LiveCell(
            codewen_code_mode_protocol::RuntimeResponse::Yielded {
                cell_id: CellId::new("1".to_string()),
                content_items: Vec::new(),
            }
        ))
