...
use crate::loader::configured_curated_plugin_ids_from_codewen_home;
...
use codewen_config::clear_user_plugin;
use codewen_config::set_user_plugin_enabled;
use codewen_config::types::PluginConfig;
...
use codewen_core_skills::config_rules::skill_config_rules_from_stack;
use codewen_core_skills::loader::MAX_CONCURRENT_ROOT_SCANS;
use codewen_hooks::plugin_hook_declarations;
...
use codewen_plugin::PluginIdError;
use codewen_plugin::PluginTelemetryMetadata;
use codewen_plugin::app_connector_ids_from_declarations;
use codewen_plugin::prompt_safe_plugin_description;
...
use codewen_skills::SkillMetadata;
use codewen_tools::DiscoverablePluginInfo;
use codewen_tools::DiscoverableTool;
...
use codewen_utils_plugins::PluginSkillRoot;
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        ...
        account_id: auth.and_then(CodewenAuth::get_account_id),
        chatgpt_user_id: auth.and_then(CodewenAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(CodewenAuth::is_workspace_account),
    }
}
...
impl PluginsManager {
    pub fn new(codewen_home: PathBuf) -> Self {
        Self::new_with_options(codewen_home, Some(Product::Codex), /*auth_mode*/ None)
    }
    ...
    pub fn new_with_options(
        ...
        Self {
            ...
            store: PluginStore::new(codewen_home),
    ...
    fn remote_global_catalog_active(&self, config: &PluginsConfigInput) -> bool {
        config.remote_plugin_enabled && self.auth_mode().is_some_and(AuthMode::uses_codewen_backend)
    }
    ...
    pub fn cached_global_remote_discoverable_plugins_for_config(
        ...
        let Some(auth) = auth.filter(|auth| auth.uses_codewen_backend()) else {
            return Vec::new();
        };
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.codewen_home.as_path())
                    .ok_or_else(|| {
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(codewen_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.codewen_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(codewen_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let manifest =
            if codewen_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest,
            self.restriction_product,
            &codewen_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
            /*plugin_skill_snapshots*/ None,
            Arc::clone(&self.skill_root_scan_slots),
    ...
    pub fn maybe_start_plugin_startup_tasks_for_config(
        ...
        if config.plugins_enabled {
            let use_remote_global_catalog =
                config.remote_plugin_enabled && auth_manager.current_auth_uses_codewen_backend();
    ...
    fn schedule_non_curated_plugin_cache_refresh(
        ...
        let mut configured_plugin_keys =
            configured_plugins_from_stack(&config.config_layer_stack, self.codewen_home.as_path())
                .into_keys()
                .collect::<Vec<_>>();
        configured_plugin_keys.sort_unstable();
    ...
    fn start_curated_repo_sync(self: &Arc<Self>, http_client_factory: HttpClientFactory) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(move || {
                match sync_openai_plugins_repo(codewen_home.as_path(), http_client_factory) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_codewen_home(codewen_home.as_path());
        ...
        let curated_marketplace_path = if include_openai_curated {
            if matches!(
                ...
                let api_marketplace_path =
                    curated_plugins_api_marketplace_path(self.codewen_home.as_path());
                api_marketplace_path
                    .is_file()
                    .then_some(api_marketplace_path)
            } else {
                let curated_repo_root = curated_plugins_repo_path(self.codewen_home.as_path());
