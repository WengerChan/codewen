...
use codewen_config::HooksFile;
use codewen_config::types::McpServerConfig;
use codewen_config::types::PluginConfig;
use codewen_config::types::PluginMcpServerConfig;
use codewen_connectors::parse_plugin_app_config;
use codewen_connectors::parse_plugin_app_config_value;
use codewen_core_skills::PluginSkillSnapshots;
use codewen_core_skills::config_rules::resolve_disabled_skill_paths;
use codewen_core_skills::config_rules::skill_config_rules_from_stack;
use codewen_core_skills::loader::SkillRoot;
use codewen_core_skills::loader::load_skills_from_roots;
use codewen_exec_server::LOCAL_FS;
use codewen_mcp::parse_plugin_mcp_config;
use codewen_plugin::AppDeclaration;
use codewen_plugin::LoadedPlugin;
...
use codewen_utils_plugins::find_plugin_manifest_path;
...
async fn load_plugins_from_layer_stack_with_scope(
    ...
    let configured_plugins = merge_configured_plugins_with_remote_installed(
        configured_plugins_from_stack(config_layer_stack, store.codewen_home().as_path()),
        extra_plugins,
        store,
        remote_global_catalog_active,
    );
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(codewen_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(codewen_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = codewen_home
        .join(".tmp/plugins")
...
fn configured_plugins_from_codewen_home(
    codewen_home: &Path,
...
pub fn configured_curated_plugin_ids_from_codewen_home(codewen_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_codewen_home(
        codewen_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".codewen-plugin/plugin.json"));
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = codewen_home.join("plugins/.marketplace-plugin-source-staging");
