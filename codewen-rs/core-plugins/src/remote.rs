...
use codewen_app_server_protocol::SkillInterface;
...
use codewen_http_client::RouteAwareRequestError;
...
const CODEX_PRODUCT_SKU: &str = "codewen";
    ...
    for source in sources {
        match source {
            RemoteMarketplaceSource::Global => {
                ...
                if let (Some(codewen_home), Some(directory_plugins)) =
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_global_directory_plugins(codewen_home, config, auth, &plugins);
    Ok(())
}
...
pub fn has_cached_global_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_global_directory_plugins(codewen_home, config, auth).is_some()
}
...
pub fn cached_global_remote_discoverable_plugins(
    ...
    catalog_cache::load_cached_global_directory_plugins(codewen_home, config, auth)
        .unwrap_or_default()
        .into_iter()
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(codewen_home.clone())
    ...
    let legacy_remote_plugin_cache_root = codewen_home
...
fn authenticated_request(
    ...
    request
        ...
        .headers(codewen_model_provider::auth_provider_from_auth(auth).to_auth_headers())
