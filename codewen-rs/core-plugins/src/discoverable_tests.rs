...
#[tokio::test]
async fn returns_fallback_plugins_when_remote_disabled_for_codewen_auth() {
    let codewen_home = tempdir().expect("tempdir should succeed");
    write_file(
...
"#,
    ...
    let curated_root = curated_plugins_repo_path(codewen_home.path());
...
async fn returns_api_curated_fallback_plugins_for_direct_provider_auth() {
    ...
    let auth = CodewenAuth::from_api_key("test-api-key");
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        Some(&auth),
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(codewen_home.path(), curated_root.as_path(), "teams").await;
...
#[tokio::test]
async fn omits_openai_curated_but_keeps_configured_marketplaces_for_remote_codewen_auth() {
    let codewen_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(codewen_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);

    let bundled_marketplace_name = OPENAI_BUNDLED_MARKETPLACE_NAME;
    let bundled_marketplace_root = codewen_home
        .path()
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(codewen_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.codewen-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
"#
    ...
    install_marketplace_plugin(codewen_home.path(), curated_root.as_path(), "installed").await;
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.codewen-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(codewen_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(codewen_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.codewen-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(logs.matches("gmail/.codewen-plugin/plugin.json").count(), 4);
    assert_eq!(
        logs.matches("openai-developers/.codewen-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn does_not_expand_local_sales_apps() {
    ...
    let sales_marketplace_root = codewen_home
        .path()
...
"#,
    ...
    let auth = CodewenAuth::create_dummy_chatgpt_auth_for_testing();
    let mut plugins = load_plugins_config(codewen_home.path(), codewen_home.path()).await;
