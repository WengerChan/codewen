...
use codewen_config::AppToolApproval;
...
use codewen_config::McpServerConfig;
use codewen_config::McpServerOAuthConfig;
use codewen_config::McpServerToolConfig;
use codewen_config::RequirementSource;
use codewen_config::RequirementsLayerEntry;
use codewen_config::compose_requirements;
use codewen_config::types::McpServerTransportConfig;
use codewen_core_skills::PluginSkillSnapshots;
use codewen_core_skills::SkillsLoadInput;
use codewen_core_skills::SkillsService;
use codewen_login::CodewenAuth;
use codewen_plugin::AppDeclaration;
use codewen_plugin::PluginId;
use codewen_protocol::auth::AuthMode;
use codewen_protocol::protocol::HookEventName;
use codewen_protocol::protocol::Product;
use codewen_skills::SkillConfigRules;
...
fn plugins_config_input_with_requirements(
    ...
    PluginsConfigInput::new(
        config_layer_stack_with_requirements(codewen_home, user_config, requirements),
...
async fn marketplace_policy_projection_disables_installed_plugin_and_invalidates_cache() {
    let codewen_home = TempDir::new().expect("create Codex home");
    write_plugin(
        &codewen_home.path().join("plugins/cache/company"),
        "sample/local",
        "sample",
    );
    let user_config = r#"
...
"#,
    ...
    let manager = PluginsManager::new(codewen_home.path().to_path_buf());
...
async fn plugin_read_rejects_marketplace_blocked_by_requirements() {
    let codewen_home = TempDir::new().expect("create Codex home");
    let marketplace_root = codewen_home.path().join("marketplace");
...
"#,
    ...
    let err = PluginsManager::new(codewen_home.path().to_path_buf())
...
fn marketplace_policy_filters_discovered_marketplaces_by_configured_name() {
    let codewen_home = TempDir::new().expect("create Codex home");
    let repo_root = codewen_home.path().join("repo");
...
fn write_auth_projection_plugin(codewen_home: &Path, name: &str, include_app: bool) {
  ...
  "mcpServers": {{
    ...
    if include_app {
        write_auth_projection_app(codewen_home, name, name);
    }
}

fn write_auth_projection_app(codewen_home: &Path, plugin_name: &str, app_name: &str) {
    let plugin_root = codewen_home
        .join("plugins/cache")
        .join("test")
        .join(plugin_name)
...
async fn auth_projection_config(codewen_home: &Path) -> PluginsConfigInput {
...
"#
    .to_string();
    write_file(&codewen_home.join(CONFIG_TOML_FILE), &config_toml);
    load_config(codewen_home, codewen_home).await
}
...
async fn plugin_auth_projection_hides_apps_without_chatgpt_auth() {
    ...
    write_auth_projection_plugin(codewen_home.path(), "sample", /*include_app*/ true);
    write_auth_projection_plugin(codewen_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(codewen_home.path()).await;
...
async fn plugin_auth_projection_keeps_non_conflicting_mcp_with_chatgpt_apps_route() {
    let codewen_home = TempDir::new().unwrap();
    write_auth_projection_plugin(codewen_home.path(), "sample", /*include_app*/ false);
    write_auth_projection_app(codewen_home.path(), "sample", "sample_app");
    write_auth_projection_plugin(codewen_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(codewen_home.path()).await;
    let manager = PluginsManager::new_with_options(
        codewen_home.path().to_path_buf(),
...
"#,
    ...
    let config = load_config(codewen_home.path(), codewen_home.path()).await;
    let manager = PluginsManager::new_with_options(
        codewen_home.path().to_path_buf(),
        Some(Product::Codex),
        Some(AuthMode::Chatgpt),
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".codewen-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "codewen-test@example.com"]);
...
async fn load_plugins_from_config(
    ...
    write_file(&codewen_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(codewen_home, codewen_home).await;
    PluginsManager::new_with_options(codewen_home.to_path_buf(), Some(Product::Codex), auth_mode)
        .plugins_for_config(&config)
        .await
}

async fn load_config(codewen_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(codewen_home, cwd).await
}
...
fn remote_installed_plugin_in_marketplace(
    ...
    RemoteInstalledPlugin {
        ...
        name: name.to_string(),
        enabled: true,
        install_policy: codewen_app_server_protocol::PluginInstallPolicy::Available,
        install_policy_source: None,
        must_show_installation_interstitial: None,
        auth_policy: codewen_app_server_protocol::PluginAuthPolicy::OnUse,
        availability: codewen_app_server_protocol::PluginAvailability::Available,
        interface: None,
        keywords: Vec::new(),
    }
}

fn write_cached_plugin(codewen_home: &Path, marketplace_name: &str, plugin_name: &str) {
    write_plugin_with_version(
        &codewen_home
            .join("plugins/cache")
...
"#;
    let outcome =
        load_plugins_from_config(config_toml, codewen_home.path(), /*auth_mode*/ None).await;
...
async fn installed_plugin_telemetry_metadata_collects_capabilities() {
    let codewen_home = TempDir::new().unwrap();
    write_cached_plugin(codewen_home.path(), "test", "sample");
    let manager = PluginsManager::new(codewen_home.path().to_path_buf());
...
async fn installed_plugin_telemetry_metadata_resolves_persisted_remote_identity() {
    let codewen_home = TempDir::new().unwrap();
    write_cached_plugin(codewen_home.path(), "openai-curated-remote", "linear");
    let plugin_id =
        PluginId::parse("linear@openai-curated-remote").expect("plugin id should parse");
    PluginStore::new(codewen_home.path().to_path_buf())
...
"#,
    ...
    write_cached_plugin(codewen_home.path(), "openai-curated", "linear");
    write_cached_plugin(codewen_home.path(), "openai-curated", "calendar");
    write_cached_plugin(codewen_home.path(), "openai-curated-remote", "linear");
    write_cached_plugin(codewen_home.path(), "openai-curated-remote", "remote-only");
...
"#,
    ...
    write_cached_plugin(codewen_home.path(), "openai-curated", "linear");
    write_cached_plugin(codewen_home.path(), "openai-api-curated", "linear");
    write_cached_plugin(codewen_home.path(), "openai-curated", "calendar");
    write_cached_plugin(codewen_home.path(), "openai-curated-remote", "linear");
    write_cached_plugin(codewen_home.path(), "openai-curated-remote", "remote-only");
...
#[tokio::test]
async fn remote_plugin_feature_keeps_local_curated_without_codewen_backend() {
    let codewen_home = TempDir::new().unwrap();
    write_file(
        &codewen_home.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy = codewen_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = codewen_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(codewen_app_server_protocol::PluginInterface {
    ...
    assert_eq!(
        ...
        codewen_app_server_protocol::PluginInstallPolicy::InstalledByDefault
    );
    assert_eq!(
        plugin.auth_policy,
        codewen_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn install_plugin_materializes_default_command_skills() {
    let codewen_home = TempDir::new().unwrap();
    let source_root = codewen_home.path().join("source/sample");

    write_file(
        &source_root.join(".codewen-plugin/plugin.json"),
        r#"{
  "name": "sample",
  "skills": "./custom-skills/"
    ...
    write_file(
        &source_root.join(".codewen-plugin/migrated-command-skills/undeclared-command/SKILL.md"),
        "---\nname: undeclared-command\ndescription: undeclared command\n---\n",
    );
    let result = PluginStore::new(codewen_home.path().to_path_buf())
        .install(
            source_root.abs(),
            PluginId::parse("sample@test").expect("plugin id should parse"),
        )
    ...
    let migrated_skill = result
        .installed_path
        .join(".codewen-plugin/migrated-command-skills/source-command-pr-review/SKILL.md");
    ...
    assert!(
        !result
            .installed_path
            .join(".codewen-plugin/migrated-command-skills/undeclared-command")
            .exists()
    );
    assert!(
        !result
            .installed_path
            .join(".codewen-plugin/migrated-command-skills/source-command-oversized")
            .exists()
    );
        ...
        vec![
            ...
            AbsolutePathBuf::from_absolute_path_checked(
                fs::canonicalize(result.installed_path.join(
                    ".codewen-plugin/migrated-command-skills/source-command-summarize/SKILL.md"
                ))
                .unwrap()
            )
            .unwrap()
...
fn install_plugin_ignores_invalid_commands_manifest_field() {
    ...
    assert!(
        !result
            .installed_path
            .join(".codewen-plugin/migrated-command-skills")
            .exists()
    );
}
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let codewen_home = TempDir::new().unwrap();
    let plugin_a_root = codewen_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = codewen_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".codewen-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".codewen-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    let outcome =
        load_plugins_from_config(&config_toml, codewen_home.path(), Some(AuthMode::Chatgpt)).await;
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(codewen_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![codewen_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![codewen_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(codewen_home.path(), codewen_home.path()).await;
    let outcome = PluginsManager::new(codewen_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_ignores_unrelated_session_overrides() {
    ...
    let plugin_root = codewen_home
        ...
        .join("test/sample/local");
    write_plugin(
        codewen_home.path().join("plugins/cache/test").as_path(),
        "sample/local",
        "sample",
    );
    write_file(
  ...
  "mcpServers": {
    ...
    let user_file = codewen_home.path().join(CONFIG_TOML_FILE).abs();
...
async fn skills_service_reuses_skills_parsed_during_plugin_load() {
    let codewen_home = TempDir::new().unwrap();
    let codewen_home_abs = codewen_home.path().to_path_buf().abs();
    let plugin_root = codewen_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
...
async fn strict_install_requires_allowed_local_marketplace_to_be_added_first() {
    let codewen_home = TempDir::new().expect("create Codex home");
    let marketplace_root = codewen_home.path().join("company-marketplace");
    write_plugin(&marketplace_root, "sample", "sample");
    write_file(
        &marketplace_root.join(".agents/plugins/marketplace.json"),
        r#"{
...
"#
    ...
    let config = config_layer_stack_with_requirements(codewen_home.path(), "", &requirements);
    let marketplace_path =
        AbsolutePathBuf::try_from(marketplace_root.join(".agents/plugins/marketplace.json"))
            .expect("absolute marketplace path");
    let manager = PluginsManager::new(codewen_home.path().to_path_buf());
    ...
    assert!(!codewen_home.path().join(CONFIG_TOML_FILE).exists());
...
"#
    ...
    write_file(&codewen_home.path().join(CONFIG_TOML_FILE), &user_config);
    let config =
        config_layer_stack_with_requirements(codewen_home.path(), &user_config, &requirements);
    let outcome = manager
        .install_plugin(
            &config,
            PluginInstallRequest {
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".codewen-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".codewen-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        fs::read_to_string(
            installed_path
                .join(".codewen-plugin/migrated-command-skills/source-command-review/SKILL.md")
        )
        .unwrap(),
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".codewen-plugin/plugin.json").is_file());
}
...
#[tokio::test]
async fn read_plugin_for_config_filters_mcp_servers_for_codewen_backend_auth() {
    let tmp = tempfile::tempdir().unwrap();
    let repo_root = tmp.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).unwrap();
    fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
  ...
  "plugins": [
    ...
    write_file(
        &repo_root.join("sample-plugin/.codewen-plugin/plugin.json"),
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".codewen-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
"#,
    ...
    write_file(
        &repo_root.join(".codewen/config.toml"),
        r#"[[skills.config]]
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.codewen-plugin/plugin.json"),
...
"#,
    ...
    let featured_plugin_ids = manager
        .featured_plugin_ids_for_config(
            &config,
            Some(&CodewenAuth::create_dummy_chatgpt_auth_for_testing()),
        )
        .await
        .unwrap();
...
"#,
    ...
    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "codewen"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["codewen-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["codewen-plugin".to_string()]);
}
...
"#,
    ...
    let auth = CodewenAuth::create_dummy_chatgpt_auth_for_testing();
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .and(header("authorization", "Bearer Access Token"))
        .and(header("chatgpt-account-id", "account_id"))
        .and(header("OAI-Product-Sku", "codewen"))
        .respond_with(
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_codewen_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_codewen_home(tmp.path())
            .into_iter()
...
async fn load_plugins_ignores_project_config_files() {
    let codewen_home = TempDir::new().unwrap();
    let project_root = codewen_home.path().join("project");
    let plugin_root = codewen_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".codewen/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
