...
pub(crate) fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&codewen_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(codewen_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
                info!(
...
fn remove_stale_remote_plugin_caches(
    ...
    for marketplace_name in [
        ...
        let marketplace_root = codewen_home.join(PLUGINS_CACHE_DIR).join(marketplace_name);
...
fn remote_plugin_cache_root(codewen_home: &Path) -> PathBuf {
    codewen_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(codewen_home.path()),
        };
    ...
    async fn sync_same_version_backfills_metadata_without_materialization() {
        ...
        let codewen_home = tempfile::tempdir().expect("create codewen home");
        let cached_manifest = codewen_home
            .path()
        ...
        let metadata_path = PluginStore::new(codewen_home.path().to_path_buf())
    ...
    fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache() {
        let codewen_home = tempfile::tempdir().expect("create codewen home");
        let created_by_me_cached_manifest = codewen_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = codewen_home
            .path()
            .join(PLUGINS_CACHE_DIR)
