...
pub type UriPluginManifest = codewen_plugin::manifest::PluginManifest<PathUri>;
...
pub fn parse_plugin_manifest_uri(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = codewen_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(codewen_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: codewen_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<codewen_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| codewen_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        ...
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(codewen_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<codewen_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(codewen_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(codewen_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use codewen_plugin::PluginProvider;
    use codewen_plugin::ResolvedPlugin;
    use codewen_plugin::manifest::PluginManifest as GenericPluginManifest;
    use codewen_plugin::manifest::PluginManifestHooks;
    use codewen_plugin::manifest::PluginManifestInterface;
    use codewen_plugin::manifest::PluginManifestMcpServers;
    use codewen_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".codewen-plugin")).expect("create manifest dir");
        let version = version
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".codewen-plugin/plugin.json")).expect("manifest");
