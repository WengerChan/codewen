...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    if let Some(auth) = auth.filter(|auth| auth.uses_codewen_backend()) {
        request =
            request.headers(codewen_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_codewen_backend_auth(
    auth: Option<&CodewenAuth>,
) -> Result<&CodewenAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_codewen_backend_auth(auth)?;
