...
//!
//! The history is stored at `~/.codewen/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.codewen`.
...
impl HistoryConfig {
    pub fn new(codewen_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            codewen_home: codewen_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.codewen_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.codewen/history.jsonl` and ensure the parent directory exists.
