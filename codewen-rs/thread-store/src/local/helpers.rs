...
use codewen_rollout::ThreadItem;
use codewen_rollout::find_thread_names_by_ids;
...
pub(super) fn rollout_path_is_archived(codewen_home: &Path, path: &Path) -> bool {
    path.starts_with(codewen_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = codewen_rollout::plain_rollout_path(item.path.as_path());
...
pub(super) async fn resolve_thread_names(
    ...
    if let Ok(legacy_names) =
        find_thread_names_by_ids(store.config.codewen_home.as_path(), &legacy_thread_ids).await
    {
