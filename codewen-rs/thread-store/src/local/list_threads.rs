...
use codewen_rollout::RolloutConfig;
use codewen_rollout::RolloutRecorder;
use codewen_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => codewen_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => codewen_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => codewen_rollout::ThreadSortKey::RecencyAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => codewen_rollout::SortDirection::Asc,
        SortDirection::Desc => codewen_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        codewen_home: store.config.codewen_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.codewen_home.clone(),
...
pub(super) async fn list_rollout_threads(
    state_db: Option<codewen_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&codewen_rollout::Cursor>,
    sort_key: codewen_rollout::ThreadSortKey,
    sort_direction: codewen_rollout::SortDirection,
) -> ThreadStoreResult<codewen_rollout::ThreadsPage> {
    if let Some(relation_filter) = params.relation_filter {
        let relation_filter = match relation_filter {
            ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                codewen_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
            }
            ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                codewen_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
            }
        };
        let page = codewen_rollout::state_db::list_threads_db(
            state_db.as_deref(),
