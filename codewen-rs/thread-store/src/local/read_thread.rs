...
use codewen_rollout::RolloutRecorder;
use codewen_rollout::find_archived_thread_path_by_id_str;
use codewen_rollout::find_thread_name_by_id;
use codewen_rollout::find_thread_path_by_id_str;
use codewen_rollout::read_session_meta_line;
use codewen_rollout::read_thread_item_from_rollout;
        ...
        && (params.include_archived
            || (metadata.archived_at.is_none()
                && !rollout_path_is_archived(
                    store.config.codewen_home.as_path(),
...
async fn sqlite_rollout_path_can_load_history_for_thread(
    ...
    if codewen_rollout::existing_rollout_path(path).await.is_none() {
        return false;
    }
...
async fn resolve_requested_rollout_path(
    ...
    let path = if rollout_path.is_relative() {
        store.config.codewen_home.join(rollout_path)
    } else {
        rollout_path
    };
    ...
    let Some(path) = codewen_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
async fn read_thread_from_rollout_path(
    ...
    let archived = rollout_path_is_archived(store.config.codewen_home.as_path(), path.as_path());
    ...
    thread.rollout_path = Some(codewen_rollout::plain_rollout_path(path.as_path()));
    ...
    if thread.history_mode == ThreadHistoryMode::Legacy
        && let Ok(Some(name)) =
            find_thread_name_by_id(store.config.codewen_home.as_path(), &thread.thread_id).await
        && !name.trim().is_empty()
    {
...
pub(super) async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<codewen_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let session_meta = match read_required_session_meta_line(metadata.rollout_path.as_path()).await
    {
        ...
        Err(_)
            if codewen_rollout::existing_rollout_path(metadata.rollout_path.as_path())
                .await
                .is_none() =>
        {
            None
    ...
    let rollout_path = codewen_rollout::plain_rollout_path(metadata.rollout_path.as_path());
    ...
    match history_mode {
        ...
        ThreadHistoryMode::Legacy => {
            if let Some(title) = distinct_thread_metadata_title(metadata) {
                ...
                find_thread_name_by_id(store.config.codewen_home.as_path(), &metadata.id)
                    .await
                    .ok()
                    .flatten()
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = codewen_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_returns_rollout_path_summary() {
        ...
        let relative_path = active_path
            ...
            .expect("path should be under codewen home")
            .to_path_buf();
    ...
    async fn read_thread_by_rollout_path_preserves_legacy_name_and_sqlite_git_info() {
        ...
        let active_path =
            write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        let runtime = codewen_state::StateRuntime::init(
        ...
        codewen_rollout::append_thread_name(home.path(), thread_id, "Latest index name")
            .await
    ...
    async fn read_paginated_thread_uses_sqlite_name_over_legacy_compatibility() {
        ...
        runtime
            ...
            .await
            .expect("state db upsert should succeed");
        codewen_rollout::append_thread_name(home.path(), thread_id, "Stale index name")
            .await
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        codewen_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
            .expect("append legacy thread name");
