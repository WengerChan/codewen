...
use crate::environment_toml::environment_provider_from_codewen_home;
...
pub const CODEX_EXEC_SERVER_URL_ENV_VAR: &str = "CODEWEN_EXEC_SERVER_URL";
pub const CODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR: &str =
    "CODEWEN_EXEC_SERVER_NOISE_REGISTRY_URL";
pub const CODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR: &str =
    "CODEWEN_EXEC_SERVER_NOISE_ENVIRONMENT_ID";
pub const CODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR: &str = "CODEWEN_EXEC_SERVER_NOISE_AUTH_TOKEN";
pub const CODEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR: &str =
    "CODEWEN_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID";
...
impl EnvironmentManager {
    ...
    pub async fn from_codewen_home(
        codewen_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
        ...
        let provider = environment_provider_from_codewen_home(codewen_home.as_ref())?;
...
mod tests {
        ...
        let environment = Environment::remote_with_transport(
            ExecServerTransportParams::StdioCommand {
                command: StdioExecServerCommand {
                    program: "codewen-missing-exec-server-for-test".to_string(),
                    args: Vec::new(),
                    env: HashMap::new(),
                    cwd: None,
                },
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = codewen_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = codewen_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            codewen_protocol::models::PermissionProfile::from_runtime_permissions(
                &codewen_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                codewen_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
