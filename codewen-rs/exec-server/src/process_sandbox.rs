...
use codewen_exec_server_protocol::JSONRPCErrorError;
...
use codewen_network_proxy::NetworkProxyHandle;
use codewen_network_proxy::NetworkProxyState;
use codewen_network_proxy::RemoteNetworkProxyLaunchConfig;
use codewen_network_proxy::is_managed_mitm_ca_trust_bundle_path;
#[cfg(target_os = "windows")]
use codewen_network_proxy::strip_managed_proxy_env;
use codewen_protocol::models::PermissionProfile;
use codewen_sandboxing::SandboxCommand;
use codewen_sandboxing::SandboxDirectSpawnTransformRequest;
...
use codewen_sandboxing::WindowsSandboxProxySettingsMode;
use codewen_sandboxing::WindowsSandboxSpawnRequest;
...
pub(crate) async fn prepare_exec_request(
    ...
    let sandbox_helper_paths = params
        .arg0
        .iter()
        .map(|_| runtime_paths.codewen_self_exe.clone())
        .collect::<Vec<_>>();
    ...
    let sandbox_helper_paths = {
        ...
        if !sandbox_helper_paths.contains(&runtime_paths.codewen_self_exe) {
            sandbox_helper_paths.push(runtime_paths.codewen_self_exe.clone());
        }
        sandbox_helper_paths.extend(runtime_paths.codewen_linux_sandbox_exe.iter().cloned());
    ...
    let transform_request = SandboxDirectSpawnTransformRequest {
        ...
        windows_sandbox_proxy_settings_mode:
            codewen_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
        transform: SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd,
            codewen_linux_sandbox_exe: runtime_paths.codewen_linux_sandbox_exe.as_deref(),
