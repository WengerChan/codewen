...
use codewen_sandboxing::SandboxExecRequest;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.codewen_self_exe;
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                windows_sandbox_proxy_settings_mode:
                    codewen_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.codewen_self_exe.as_path())
        .chain(runtime_paths.codewen_linux_sandbox_exe.as_deref())
    {
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let codewen_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(codewen_self_exe, /*codewen_linux_sandbox_exe*/ None)
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/codewen".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let codewen_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(codewen_self_exe.clone(), Some(codewen_self_exe))
                .expect("runtime paths");
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let codewen_self_exe = root.path().join("bin").join("codewen");
        let codewen_linux_sandbox_exe = root.path().join("aliases").join("codewen-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(codewen_self_exe, Some(codewen_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let codewen_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
            .expect("absolute codewen parent");
        ...
        assert!(policy.can_read_path_with_cwd(codewen_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        codewen_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }
