...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    #[cfg(unix)]
    {
        ...
        if args.next().as_deref()
            == Some(OsStr::new(
                codewen_exec_server::CODEX_ARG0_EXEC_HELPER_ARG1,
            ))
        {
    ...
    let runtime_paths =
        ExecServerRuntimePaths::new(current_exe, /*codewen_linux_sandbox_exe*/ None)?;
    codewen_exec_server::run_main("ws://127.0.0.1:0", runtime_paths).await
}
