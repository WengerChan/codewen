...
pub(crate) struct TestCodexHelperPaths {
    pub(crate) codewen_exe: PathBuf,
    pub(crate) codewen_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_codewen_helper_paths() -> anyhow::Result<TestCodexHelperPaths> {
    let (helper_binary, codewen_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestCodexHelperPaths {
        codewen_exe: helper_binary,
...
{
    let helper_paths = test_codewen_helper_paths()?;
    let codewen_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.codewen_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("CODEWEN_HOME", codewen_home.path());
    child.envs(env);
