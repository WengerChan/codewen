...
use codewen_exec_server::ExecBackend;
use codewen_exec_server::ExecOutputStream;
use codewen_exec_server::ExecParams;
use codewen_exec_server::ExecProcess;
use codewen_exec_server::ExecProcessEvent;
    ...
    let session = context
        ...
        .start(ExecParams {
            ...
            env: HashMap::from([
                ...
                (
                    "CODEWEN_TEST_DENIED_FILE".to_string(),
...
async fn remote_tty_process_uses_configured_sandbox_helper_with_hostile_path() -> Result<()> {
    ...
    let file = workspace.path().join("allowed.txt");
    std::fs::write(&file, b"allowed")?;
    let hostile_helper = workspace.path().join("codewen-linux-sandbox");
