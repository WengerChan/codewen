...
#[path = "../src/proto/codewen.exec_server.relay.v1.rs"]
mod relay_proto;
...
use codewen_exec_server::EnvironmentConnectionState;
use codewen_exec_server::EnvironmentManager;
use codewen_exec_server::EnvironmentReadyInfo;
use codewen_exec_server::ExecParams;
use codewen_exec_server::ExecResponse;
use codewen_exec_server::ExecServerClient;
use codewen_exec_server::ExecServerError;
use codewen_exec_server::ExecServerRuntimePaths;
use codewen_exec_server::FsReadFileParams;
use codewen_exec_server::NoiseChannelIdentity;
use codewen_exec_server::NoiseChannelPublicKey;
use codewen_exec_server::NoiseRendezvousConnectArgs;
use codewen_exec_server::NoiseRendezvousConnectBundle;
use codewen_exec_server::NoiseRendezvousConnectProvider;
use codewen_exec_server::ProcessId;
use codewen_exec_server::RemoteEnvironmentConfig;
...
fn static_registry_auth_provider() -> codewen_api::SharedAuthProvider {
...
async fn deferred_noise_environment_connects_and_reconnects_with_fresh_bundle() -> Result<()> {
    ...
    let (codewen_exe, codewen_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(codewen_exe, codewen_linux_sandbox_exe)?;
    let config = RemoteEnvironmentConfig::new(
        registry.uri(),
        ENVIRONMENT_ID.to_string(),
        static_registry_auth_provider(),
    )?;
    let remote_environment = tokio::spawn(codewen_exec_server::run_remote_environment(
        config,
        runtime_paths,
    ));
