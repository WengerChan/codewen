...
use codewen_exec_server::CAPABILITY_ROOTS_DISCOVER_METHOD;
use codewen_exec_server::CapabilityRootDiscovery;
use codewen_exec_server::CapabilityRootsDiscoverParams;
use codewen_exec_server::CapabilityRootsDiscoverResponse;
...
async fn discovers_a_complete_capability_bundle_in_one_request() -> anyhow::Result<()> {
    ...
    write_file(
        &root.path().join(".codewen-plugin/plugin.json"),
        r#"{
  "name": "demo",
  ...
  "apps": "./config/apps.json"
    ...
    assert_eq!(
        ...
        root_uri.join(".codewen-plugin/plugin.json")?
    );
