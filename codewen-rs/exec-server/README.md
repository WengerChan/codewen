...
`codewen-exec-server` is the library backing `codewen exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`codewen-utils-pty`.

It provides:

- a CLI entrypoint: `codewen exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `codewen` binary owns hidden helper dispatch for sandboxed
filesystem operations and `codewen-linux-sandbox`.

## Transport

The server speaks the exec-specific `codewen-exec-server-protocol` message
...
must support it.
It uses the standard Codewen ChatGPT sign-in state; run `codewen login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `CODEWEN_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Codewen then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `CODEWEN_API_KEY`;
Codewen sends it as a bearer token on the registration request. For example:

```sh
CODEWEN_API_KEY="$OPENAI_API_KEY" \
codewen exec-server \
  --remote ... \
...
`codewen.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/codewen.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `codewen-utils-pty`.
...
hidden helper process launched from the top-level `codewen` executable and
...
`codewen exec-server` command builds these paths from the `codewen` arg0 dispatch
state. `RemoteEnvironmentConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from Codewen auth
