...
impl AnalyticsEventsDestination {
    ...
    fn from_base_url_and_capture_file(base_url: String, capture_file: Option<PathBuf>) -> Self {
        ...
        Self::Http {
            url: format!("{base_url}/codewen/analytics-events/events"),
        }
    }
}
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::CodewenCompactionEvent) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(
            Box::new(event),
        )));
    }

    pub fn track_goal_event(&self, event: CodewenGoalEvent) {
    ...
    pub fn track_turn_codewen_error(&self, fact: TurnCodexErrorFact) {
...
async fn send_track_events(
    ...
    } else if !auth.uses_codewen_backend() {
        return;
    }
    if events.is_empty() {
        return;
