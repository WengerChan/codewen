...
use crate::events::CodewenAcceptedLineFingerprintsEventParams;
use crate::events::CodewenAcceptedLineFingerprintsEventRequest;
use crate::events::CodewenAppMentionedEventRequest;
use crate::events::CodewenAppServerClientMetadata;
use crate::events::CodewenAppUsedEventRequest;
use crate::events::CodewenCommandExecutionEventParams;
use crate::events::CodewenCommandExecutionEventRequest;
use crate::events::CodewenCompactionEventRequest;
use crate::events::CodewenHookRunEventRequest;
use crate::events::CodewenOnboardingExternalAgentImportFailureEventRequest;
use crate::events::CodewenOnboardingExternalAgentImportFailureMetadata;
use crate::events::CodewenPluginEventRequest;
use crate::events::CodewenPluginInstallFailedEventRequest;
use crate::events::CodewenPluginInstallFailedMetadata;
use crate::events::CodewenPluginUsedEventRequest;
use crate::events::CodewenReviewEventParams;
use crate::events::CodewenReviewEventRequest;
use crate::events::CodewenRuntimeMetadata;
use crate::events::CodewenToolItemEventBase;
use crate::events::CodewenTurnEventRequest;
...
use crate::events::codewen_app_metadata;
use crate::events::codewen_hook_run_metadata;
use crate::events::codewen_plugin_metadata;
use crate::events::codewen_plugin_used_metadata;
...
use crate::facts::CodewenCompactionEvent;
use crate::facts::CodewenErrKind;
...
use codewen_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use codewen_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use codewen_app_server_protocol::CollabAgentTool;
use codewen_app_server_protocol::CollabAgentToolCallStatus;
use codewen_app_server_protocol::CommandAction;
use codewen_app_server_protocol::CommandExecutionApprovalDecision;
use codewen_app_server_protocol::CommandExecutionRequestApprovalParams;
use codewen_app_server_protocol::CommandExecutionRequestApprovalResponse;
use codewen_app_server_protocol::CommandExecutionSource;
use codewen_app_server_protocol::CommandExecutionStatus;
use codewen_app_server_protocol::DynamicToolCallStatus;
use codewen_app_server_protocol::GuardianApprovalReview;
use codewen_app_server_protocol::GuardianApprovalReviewAction;
use codewen_app_server_protocol::GuardianApprovalReviewStatus;
use codewen_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use codewen_app_server_protocol::ImageGenerationItem;
use codewen_app_server_protocol::InitializeCapabilities;
use codewen_app_server_protocol::InitializeParams;
use codewen_app_server_protocol::ItemCompletedNotification;
use codewen_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use codewen_app_server_protocol::ItemStartedNotification;
use codewen_app_server_protocol::JSONRPCErrorError;
use codewen_app_server_protocol::McpToolCallAppContext;
use codewen_app_server_protocol::McpToolCallStatus;
use codewen_app_server_protocol::NonSteerableTurnKind;
use codewen_app_server_protocol::PatchApplyStatus;
use codewen_app_server_protocol::PermissionsRequestApprovalParams;
use codewen_app_server_protocol::RequestId;
use codewen_app_server_protocol::RequestPermissionProfile;
use codewen_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
use codewen_app_server_protocol::ServerNotification;
use codewen_app_server_protocol::ServerRequest;
use codewen_app_server_protocol::ServerResponse;
use codewen_app_server_protocol::SessionSource as AppServerSessionSource;
use codewen_app_server_protocol::SubAgentActivityKind;
...
use codewen_app_server_protocol::ThreadSource as AppServerThreadSource;
use codewen_app_server_protocol::ThreadStartResponse;
use codewen_app_server_protocol::ThreadStatus as AppServerThreadStatus;
use codewen_app_server_protocol::Turn;
use codewen_app_server_protocol::TurnCompletedNotification;
use codewen_app_server_protocol::TurnDiffUpdatedNotification;
use codewen_app_server_protocol::TurnError as AppServerTurnError;
use codewen_app_server_protocol::TurnStartParams;
use codewen_app_server_protocol::TurnStartedNotification;
use codewen_app_server_protocol::TurnStatus as AppServerTurnStatus;
use codewen_app_server_protocol::TurnSteerParams;
use codewen_app_server_protocol::TurnSteerResponse;
use codewen_app_server_protocol::UserInput;
use codewen_app_server_protocol::WebSearchItem;
use codewen_login::default_client::DEFAULT_ORIGINATOR;
use codewen_login::default_client::originator;
use codewen_plugin::AppConnectorId;
use codewen_plugin::PluginCapabilitySummary;
use codewen_plugin::PluginId;
use codewen_plugin::PluginTelemetryMetadata;
use codewen_protocol::approvals::NetworkApprovalProtocol;
use codewen_protocol::config_types::ApprovalsReviewer;
use codewen_protocol::config_types::ModeKind;
use codewen_protocol::error::CodewenErr;
use codewen_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use codewen_protocol::models::PermissionProfile as CorePermissionProfile;
use codewen_protocol::protocol::AskForApproval;
use codewen_protocol::protocol::HookEventName;
use codewen_protocol::protocol::HookRunStatus;
use codewen_protocol::protocol::HookSource;
use codewen_protocol::protocol::SessionSource;
use codewen_protocol::protocol::SubAgentSource;
use codewen_protocol::protocol::ThreadSource;
use codewen_protocol::protocol::TokenUsage;
use codewen_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use codewen_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use codewen_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
use codewen_utils_absolute_path::test_support::PathBufExt;
use codewen_utils_absolute_path::test_support::test_path_buf;
...
const TEST_PRODUCT_CLIENT_ID: &str = "codewen_work_desktop";
...
fn sample_app_server_client_metadata() -> CodewenAppServerClientMetadata {
    CodewenAppServerClientMetadata {
...
fn sample_runtime_metadata() -> CodewenRuntimeMetadata {
    CodewenRuntimeMetadata {
        codewen_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(codewen_app_server_protocol::TurnStartResponse {
        turn: Turn {
            id: turn_id.to_string(),
            items_view: codewen_app_server_protocol::TurnItemsView::Full,
            items: vec![],
...
fn sample_turn_completed_notification(
    ...
    codewen_error_info: Option<codewen_app_server_protocol::CodewenErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: codewen_error_info.map(|codewen_error_info| AppServerTurnError {
                message: "turn failed".to_string(),
                codewen_error_info: Some(codewen_error_info),
    ...
    JSONRPCErrorError {
        ...
        data: Some(
            serde_json::to_value(AppServerTurnError {
                message: "cannot steer a review turn".to_string(),
                codewen_error_info: Some(CodewenErrorInfo::ActiveTurnNotSteerable {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "codewen-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "codewen-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "codewen-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "codewen-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        product_client_id: DEFAULT_ORIGINATOR.to_string(),
        runtime: CodewenRuntimeMetadata {
            codewen_rs_version: "0.99.0".to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(codewen_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: codewen_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.codewen/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".codewen/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.codewen/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/codewen/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.codewen/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(CodewenAppMentionedEventRequest {
        event_type: "codewen_app_mentioned",
        event_params: codewen_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(CodewenAppUsedEventRequest {
        event_type: "codewen_app_used",
        event_params: codewen_app_metadata(
            &tracking,
            AppInvocation {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        CodewenAcceptedLineFingerprintsEventRequest {
            event_type: "codewen_accepted_line_fingerprints",
            event_params: CodewenAcceptedLineFingerprintsEventParams {
                event_type: "codewen.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: "thread-1".to_string(),
                product_surface: Some("codewen".to_string()),
                model_slug: Some("gpt-5.1-codewen".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": "turn-1",
                "thread_id": "thread-1",
                "product_surface": "codewen",
                "model_slug": "gpt-5.1-codewen",
                "completed_at": 1710000000,
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(CodewenCompactionEventRequest {
        event_type: "codewen_compaction_event",
        event_params: crate::events::codewen_compaction_event_params(
            CodewenCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                ...
                codewen_error_kind: None,
                codewen_error_http_status_code: None,
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "codewen-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "codewen_rs_version": "0.1.0",
                    "runtime_os": "macos",
                ...
                "strategy": "memento",
                "status": "completed",
                "codewen_error_kind": null,
                "codewen_error_http_status_code": null,
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "codewen_thread_initialized",
        event_params: ThreadInitializedEventParams {
            thread_id: "thread-0".to_string(),
            session_id: "session-thread-0".to_string(),
            app_server_client: CodewenAppServerClientMetadata {
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                client_name: Some("codewen-tui".to_string()),
                client_version: Some("1.0.0".to_string()),
                rpc_transport: AppServerRpcTransport::Stdio,
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(CodewenCommandExecutionEventRequest {
        event_type: "codewen_command_execution_event",
        event_params: CodewenCommandExecutionEventParams {
            base: CodewenToolItemEventBase {
                thread_id: "thread-1".to_string(),
                session_id: "session-thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                item_id: "item-1".to_string(),
                app_server_client: CodewenAppServerClientMetadata {
                    product_client_id: "codewen_tui".to_string(),
                    client_name: Some("codewen-tui".to_string()),
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(CodewenReviewEventRequest {
        event_type: "codewen_review_event",
        event_params: CodewenReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "codewen_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["codewen_rs_version"],
        "0.99.0"
    );
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        codewen_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                CodewenCompactionEvent {
                    ...
                    codewen_error_kind: Some(CodewenErrKind::ContextWindowExceeded),
                    codewen_error_http_status_code: None,
                    active_context_tokens_before: 131_000,
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "codewen_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["codewen_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["codewen_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_default_review_model_id: Some("codewen-auto-review".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "codewen_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "codewen_command_execution_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
...
async fn command_execution_approval_response_publishes_user_review_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "codewen_review_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
    assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "codewen_review_event");
...
fn subagent_thread_started_review_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            ...
            forked_from_thread_id: None,
            product_client_id: "codewen-tui".to_string(),
            client_name: "codewen-tui".to_string(),
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        codewen_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        codewen_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        codewen_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_events_keep_thread_originator_with_explicit_turn_connection() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        codewen_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.codewen_rs_version, "0.1.0");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(CodewenPluginUsedEventRequest {
        event_type: "codewen_plugin_used",
        event_params: codewen_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(CodewenPluginEventRequest {
        event_type: "codewen_plugin_installed",
        event_params: codewen_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(CodewenPluginInstallFailedEventRequest {
        event_type: "codewen_plugin_install_failed",
        event_params: CodewenPluginInstallFailedMetadata {
            plugin: codewen_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_keeps_plugin_id_local_when_remote_id_exists() {
    ...
    let event = TrackEventRequest::PluginInstalled(CodewenPluginEventRequest {
        event_type: "codewen_plugin_installed",
        event_params: codewen_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    let tracking = test_tracking_context("thread-3", "turn-3");
    let event = TrackEventRequest::HookRun(CodewenHookRunEventRequest {
        event_type: "codewen_hook_run",
        event_params: codewen_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(codewen_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(codewen_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(codewen_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(codewen_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(codewen_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: codewen_protocol::protocol::SkillScope::User,
                    skill_path,
                    plugin_id: None,
...
async fn reducer_includes_plugin_id_for_plugin_skill_invocations() {
    ...
    let tracking = test_tracking_context("thread-1", "turn-1");
    let skill_path =
        PathBuf::from("/Users/abc/.codewen/plugins/cache/test/sample/skills/doc/SKILL.md");
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "codewen_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "codewen_app_mentioned");
    assert_eq!(payload[1]["event_type"], "codewen_app_used");
    assert_eq!(payload[2]["event_type"], "codewen_plugin_used");
    assert_eq!(
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "codewen_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "remote_plugin_id": null,
                "plugin_name": "sample",
...
async fn reducer_ingests_plugin_install_requested_fact() {
    ...
    assert_eq!(
        ...
        json!([{
            "event_type": "codewen_plugin_install_requested",
            "event_params": {
...
async fn reducer_ingests_external_agent_config_import_completed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "codewen_onboarding_external_agent_import_complete",
            "event_params": {
                "import_id": "import-1",
                "source": "app_server",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        CodewenOnboardingExternalAgentImportFailureEventRequest {
            event_type: "codewen_onboarding_external_agent_import_failure",
            event_params: CodewenOnboardingExternalAgentImportFailureMetadata {
                import_id: "import-1".to_string(),
                source: "app_server".to_string(),
                provider_id: "test-provider-42".to_string(),
                item_type: "PLUGINS".to_string(),
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(CodewenTurnEventRequest {
        event_type: "codewen_turn_event",
        event_params: crate::events::CodewenTurnEventParams {
            thread_id: "thread-2".to_string(),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "codewen_cli_rs",
                    "client_name": "codewen-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("codewen_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["codewen_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("codewen_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
...
async fn turn_event_counts_completed_tool_items() {
    ...
    assert_eq!(
        emitted_tool_events,
        vec![
            ("codewen_command_execution_event", "session-thread-2"),
            ("codewen_file_change_event", "session-thread-2"),
            ("codewen_mcp_tool_call_event", "session-thread-2"),
            ("codewen_dynamic_tool_call_event", "session-thread-2"),
            ("codewen_collab_agent_tool_call_event", "session-thread-2"),
            ("codewen_web_search_event", "session-thread-2"),
            ("codewen_image_generation_event", "session-thread-2"),
        ]
    );
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnCodexError(Box::new(
                TurnCodexErrorFact::from_codewen_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &CodewenErr::InvalidRequest("unknown turn environment id `env-2`".to_string()),
                ),
            ))),
            &mut out,
        )
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(codewen_app_server_protocol::CodewenErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["codewen_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["codewen_error_http_status_code"],
        json!(null)
    );
}
...
async fn turn_lifecycle_emits_interrupted_turn_event_without_error() {
    ...
    assert_eq!(payload["event_params"]["codewen_error_kind"], json!(null));
}
