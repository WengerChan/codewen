...
#[cfg(debug_assertions)]
use crate::events::CodewenPluginMetadata;
#[cfg(debug_assertions)]
use crate::events::CodewenPluginUsedEventRequest;
#[cfg(debug_assertions)]
use crate::events::CodewenPluginUsedMetadata;
#[cfg(debug_assertions)]
...
fn sample_mcp_tool_call_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::McpToolCall(CodewenMcpToolCallEventRequest {
        ...
        event_params: CodewenMcpToolCallEventParams {
            base: CodewenToolItemEventBase {
                ...
                app_server_client: CodewenAppServerClientMetadata {
                    product_client_id: "codewen_desktop".to_string(),
                    client_name: None,
                    client_version: None,
                ...
                runtime: CodewenRuntimeMetadata {
                    codewen_rs_version: "0.0.0".to_string(),
...
fn sample_plugin_used_track_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::PluginUsed(CodewenPluginUsedEventRequest {
        event_type: "codewen_plugin_used",
        event_params: CodewenPluginUsedMetadata {
            plugin: CodewenPluginMetadata {
                plugin_id: plugin_id.map(str::to_string),
                ...
                product_client_id: Some("codewen_desktop".to_string()),
            },
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "codewen-analytics-{name}-{}-{nonce}.jsonl",
        std::process::id()
    ))
}
...
fn analytics_destination_uses_http_without_capture_file() {
    ...
    assert_eq!(
        ...
        AnalyticsEventsDestination::Http {
            url: "https://chatgpt.com/backend-api/codewen/analytics-events/events".to_string()
        }
    );
}
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = codewen_login::CodewenAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn capture_file_writes_final_batches_as_separate_lines() {
    ...
    assert_eq!(
        ...
        "codewen_accepted_line_fingerprints"
    );
...
#[tokio::test]
#[cfg(debug_assertions)]
async fn api_key_auth_sends_only_plugin_events_to_codewen_backend() {
    let capture_path = unique_capture_path("api-key-plugin-events");
    let destination = AnalyticsEventsDestination::CaptureFile {
        path: capture_path.clone(),
    };
    let auth_manager = codewen_login::AuthManager::from_auth_for_testing(
        codewen_login::CodewenAuth::from_api_key("sk-test"),
    );
