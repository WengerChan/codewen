...
use crate::events::CodewenCollabAgentToolCallEventParams;
use crate::events::CodewenCollabAgentToolCallEventRequest;
...
use crate::events::CodewenDynamicToolCallEventParams;
use crate::events::CodewenDynamicToolCallEventRequest;
use crate::events::CodewenFileChangeEventParams;
use crate::events::CodewenFileChangeEventRequest;
use crate::events::CodewenGoalEventRequest;
use crate::events::CodewenHookRunEventRequest;
use crate::events::CodewenImageGenerationEventParams;
use crate::events::CodewenImageGenerationEventRequest;
use crate::events::CodewenMcpToolCallEventParams;
use crate::events::CodewenMcpToolCallEventRequest;
use crate::events::CodewenOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::CodewenOnboardingExternalAgentImportCompleteMetadata;
...
use crate::events::CodewenPluginInstallRequestedEventRequest;
...
use crate::events::CodewenTurnEventParams;
use crate::events::CodewenTurnEventRequest;
use crate::events::CodewenTurnSteerEventParams;
use crate::events::CodewenTurnSteerEventRequest;
use crate::events::CodewenWebSearchEventParams;
use crate::events::CodewenWebSearchEventRequest;
...
use crate::events::codewen_compaction_event_params;
use crate::events::codewen_goal_event_params;
use crate::events::codewen_hook_run_metadata;
use crate::events::codewen_plugin_install_requested_metadata;
...
use crate::facts::CodewenGoalEvent;
...
use codewen_app_server_protocol::ClientResponse;
use codewen_app_server_protocol::CodewenErrorInfo;
use codewen_app_server_protocol::CollabAgentStatus;
...
use codewen_app_server_protocol::DynamicToolCallOutputContentItem;
...
use codewen_app_server_protocol::NetworkPolicyRuleAction;
use codewen_app_server_protocol::PatchApplyStatus;
use codewen_app_server_protocol::PatchChangeKind;
...
use codewen_app_server_protocol::WebSearchAction;
use codewen_git_utils::collect_git_info;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a CodewenCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a CodewenGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a codewen_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<CodewenErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    codewen_error: Option<TurnCodexError>,
...
impl AnalyticsReducer {
        ...
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::TurnCodexError(input) => {
                    self.ingest_turn_codewen_error(*input);
                }
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "codewen_guardian_review",
    ...
    fn ingest_turn_codewen_error(&mut self, input: TurnCodexErrorFact) {
        ...
        turn_state.codewen_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = codewen_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(CodewenAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = codewen_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(CodewenAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(CodewenHookRunEventRequest {
            event_type: "codewen_hook_run",
            event_params: codewen_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            CodewenPluginInstallRequestedEventRequest {
                event_type: "codewen_plugin_install_requested",
                event_params: codewen_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = CodewenPluginEventRequest {
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            CodewenOnboardingExternalAgentImportCompleteEventRequest {
                event_type: "codewen_onboarding_external_agent_import_complete",
                event_params: CodewenOnboardingExternalAgentImportCompleteMetadata {
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.codewen_error_info),
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: codewen_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            CodewenCompactionEventRequest {
                event_type: "codewen_compaction_event",
                event_params: codewen_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: CodewenGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
            self.thread_context_or_warn(AnalyticsDropSite::goal(&input))
        else {
            return;
        };
        out.push(TrackEventRequest::Goal(Box::new(CodewenGoalEventRequest {
            event_type: "codewen_goal_event",
            event_params: codewen_goal_event_params(
                input,
                thread_metadata.session_id.clone(),
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: codewen_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(CodewenTurnSteerEventRequest {
            event_type: "codewen_turn_steer_event",
            event_params: CodewenTurnSteerEventParams {
    ...
    fn emit_review_event(
        ...
        out.push(TrackEventRequest::ReviewEvent(CodewenReviewEventRequest {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(CodewenTurnEventRequest {
            event_type: "codewen_turn_event",
            event_params: codewen_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                CodewenCommandExecutionEventRequest {
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                CodewenMcpToolCallEventRequest {
                    event_type: "codewen_mcp_tool_call_event",
                    event_params: CodewenMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                CodewenDynamicToolCallEventRequest {
                    event_type: "codewen_dynamic_tool_call_event",
                    event_params: CodewenDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                CodewenCollabAgentToolCallEventRequest {
                    event_type: "codewen_collab_agent_tool_call_event",
                    event_params: CodewenCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(CodewenWebSearchEventRequest {
                event_type: "codewen_web_search_event",
                event_params: CodewenWebSearchEventParams {
                    base,
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                CodewenImageGenerationEventRequest {
                    event_type: "codewen_image_generation_event",
                    event_params: CodewenImageGenerationEventParams {
                        base,
...
fn codewen_turn_event_params(
    app_server_client: CodewenAppServerClientMetadata,
    runtime: CodewenRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> CodewenTurnEventParams {
    let (
        Some(thread_id),
    ...
    let codewen_error = turn_state.codewen_error.as_ref();
    CodewenTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        codewen_error_kind: codewen_error.map(|error| error.kind),
        codewen_error_http_status_code: codewen_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: codewen_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        codewen_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        codewen_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        codewen_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        codewen_app_server_protocol::TurnStatus::InProgress => None,
    }
}
