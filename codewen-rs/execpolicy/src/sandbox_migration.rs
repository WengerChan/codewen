...
/// removes legacy allow rules that newer codewen versions no longer offer.
///
/// this migration is intentionally one-shot. once complete, a marker in `codewen_home` prevents
/// policies saved by newer codewen versions from being removed on later startups.
pub async fn prefix_rule_migration(
    codewen_home: &Path,
    policy_path: &Path,
    banned_prefixes: &[&[&str]],
) -> io::Result<()> {
    let marker_path = codewen_home.join(MIGRATION_MARKER_FILENAME);
    if tokio::fs::try_exists(&marker_path).await? {
        return Ok(());
    }
    clean_rules_file(policy_path, banned_prefixes).await?;

    write_migration_marker(codewen_home, &marker_path).await?;
    Ok(())
}

// atomically writes the marker after creating codewen home when needed.
async fn write_migration_marker(codewen_home: &Path, marker_path: &Path) -> io::Result<()> {
    tokio::fs::create_dir_all(codewen_home).await?;
    let codewen_home = codewen_home.to_owned();
    let marker_path = marker_path.to_owned();
    tokio::task::spawn_blocking(move || {
        let mut marker = tempfile::NamedTempFile::new_in(codewen_home)?;
