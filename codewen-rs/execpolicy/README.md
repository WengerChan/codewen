...
- From the Codewen CLI, run `codewen execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
codewen execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
codewen execpolicy check \
...
```bash
cargo run -p codewen-execpolicy -- check --rules path/to/policy.rules git status
```
