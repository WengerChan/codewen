...
const KEYRING_SERVICE: &str = "codewen";
...
impl SecretsManager {
    pub fn new(codewen_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(codewen_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn compute_keyring_account(codewen_home: &Path) -> String {
