...
const CODEX_AUTH_SECRETS_FILENAME: &str = "codewen_auth.age";
...
impl LocalSecretsBackend {
    pub fn new(codewen_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.codewen_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(codewen_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let codewen_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(codewen_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        let codewen_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let codewen_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        codewen_auth_backend.set(&scope, &name, "codewen-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            codewen_auth_backend.get(&scope, &name)?,
            Some("codewen-auth-value".to_string())
        );
        assert_eq!(
        ...
        assert!(!codewen_home.path().join("secrets").join("local.age").exists());
        Ok(())
    }
}
