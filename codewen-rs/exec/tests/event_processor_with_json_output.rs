use codewen_app_server_protocol::CollabAgentState as ApiCollabAgentState;
use codewen_app_server_protocol::CollabAgentStatus as ApiCollabAgentStatus;
use codewen_app_server_protocol::CollabAgentTool;
use codewen_app_server_protocol::CollabAgentToolCallStatus as ApiCollabAgentToolCallStatus;
use codewen_app_server_protocol::CommandAction;
use codewen_app_server_protocol::CommandExecutionSource;
use codewen_app_server_protocol::CommandExecutionStatus as ApiCommandExecutionStatus;
use codewen_app_server_protocol::ErrorNotification;
use codewen_app_server_protocol::FileUpdateChange as ApiFileUpdateChange;
use codewen_app_server_protocol::ItemCompletedNotification;
use codewen_app_server_protocol::ItemStartedNotification;
use codewen_app_server_protocol::McpToolCallError;
use codewen_app_server_protocol::McpToolCallResult;
use codewen_app_server_protocol::McpToolCallStatus as ApiMcpToolCallStatus;
use codewen_app_server_protocol::PatchApplyStatus as ApiPatchApplyStatus;
use codewen_app_server_protocol::PatchChangeKind as ApiPatchChangeKind;
...
use codewen_app_server_protocol::WebSearchAction as ApiWebSearchAction;
use codewen_app_server_protocol::WebSearchItem as ApiWebSearchItem;
...
use codewen_exec::AgentMessageItem;
use codewen_exec::CodewenStatus;
use codewen_exec::CollabAgentState;
use codewen_exec::CollabAgentStatus;
use codewen_exec::CollabTool;
use codewen_exec::CollabToolCallItem;
use codewen_exec::CollabToolCallStatus;
use codewen_exec::CollectedThreadEvents;
use codewen_exec::CommandExecutionItem;
use codewen_exec::CommandExecutionStatus;
use codewen_exec::ErrorItem;
use codewen_exec::EventProcessorWithJsonOutput;
use codewen_exec::ExecThreadItem;
use codewen_exec::FileChangeItem;
use codewen_exec::FileUpdateChange as ExecFileUpdateChange;
use codewen_exec::ItemCompletedEvent;
use codewen_exec::ItemStartedEvent;
use codewen_exec::ItemUpdatedEvent;
use codewen_exec::McpToolCallItem;
use codewen_exec::McpToolCallItemError;
use codewen_exec::McpToolCallItemResult;
use codewen_exec::McpToolCallStatus;
use codewen_exec::PatchApplyStatus;
use codewen_exec::PatchChangeKind;
use codewen_exec::ReasoningItem;
use codewen_exec::ThreadErrorEvent;
use codewen_exec::ThreadEvent;
use codewen_exec::ThreadItemDetails;
use codewen_exec::ThreadStartedEvent;
use codewen_exec::TodoItem;
use codewen_exec::TodoListItem;
use codewen_exec::TurnCompletedEvent;
use codewen_exec::TurnFailedEvent;
use codewen_exec::TurnStartedEvent;
use codewen_exec::Usage;
use codewen_exec::WebSearchItem;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_configured = SessionConfiguredEvent {
        ...
        thread_source: None,
        thread_name: None,
        model: "codewen-mini-latest".to_string(),
        model_provider_id: "test-provider".to_string(),
        service_tier: None,
        approval_policy: AskForApproval::Never,
        approvals_reviewer: codewen_protocol::config_types::ApprovalsReviewer::User,
...
fn plan_update_emits_started_then_updated_then_completed() {
    ...
    assert_eq!(
        ...
        CollectedThreadEvents {
            ...
            status: CodewenStatus::InitiateShutdown,
        }
    );
}
...
fn token_usage_update_is_emitted_on_turn_completion() {
    ...
    let usage_update =
        processor.collect_thread_events(ServerNotification::ThreadTokenUsageUpdated(
            codewen_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
...
fn failed_turn_clears_stale_final_message() {
    ...
    assert_eq!(collected.status, CodewenStatus::Running);
    ...
    assert_eq!(collected.status, CodewenStatus::InitiateShutdown);
...
fn model_reroute_surfaces_as_error_item() {
    ...
    let collected = processor.collect_thread_events(ServerNotification::ModelRerouted(
        codewen_app_server_protocol::ModelReroutedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            from_model: "gpt-5".to_string(),
            to_model: "gpt-5-mini".to_string(),
            reason: codewen_app_server_protocol::ModelRerouteReason::HighRiskCyberActivity,
        },
    ));
