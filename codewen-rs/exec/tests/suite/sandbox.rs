#![cfg(unix)]
use codewen_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use codewen_core::exec::build_exec_request;
    ...
    let codewen_linux_sandbox_exe = Some(
        core_test_support::find_codewen_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_codewen_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .codewen && echo 'sandbox_mode = \"danger-full-access\"' > .codewen/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    let dot_codewen_metadata = tokio::fs::symlink_metadata(&dot_codex).await;
    if let Ok(metadata) = dot_codewen_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_codex.display()
        );
    } else if let Err(err) = &dot_codewen_metadata {
        assert_eq!(
            err.kind(),
