...
/// While we may add an `apply-patch` subcommand to the `codewen` CLI multitool
/// at some point, we must ensure that the smaller `codewen-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(codewen_utils_cargo_bin::cargo_bin("codewen-exec")?)
