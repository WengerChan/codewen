...
#[command(
    version,
    override_usage = "codewen exec [OPTIONS] [PROMPT]\n       codewen exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
...
struct ResumeArgsRaw {
    ...
    // so "codewen resume --last <prompt>" treats the positional as a prompt, not a session id.
