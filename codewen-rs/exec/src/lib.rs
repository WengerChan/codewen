...
use codewen_app_server_client::EnvironmentManager;
use codewen_app_server_client::ExecServerRuntimePaths;
use codewen_app_server_client::InProcessAppServerClient;
use codewen_app_server_client::InProcessClientStartArgs;
use codewen_app_server_client::InProcessServerEvent;
...
use codewen_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use codewen_core::StateDbHandle;
use codewen_core::check_execpolicy_for_warnings;
use codewen_core::config::Config;
use codewen_core::config::ConfigBuilder;
use codewen_core::config::ConfigOverrides;
use codewen_core::config::ConfigTomlLoadResult;
use codewen_core::config::find_codewen_home;
use codewen_core::config::load_config_toml_with_layer_stack;
use codewen_core::config::resolve_bootstrap_auth_keyring_backend_kind;
use codewen_core::config::resolve_bootstrap_auth_route_config;
use codewen_core::config::resolve_oss_provider;
use codewen_core::config::resolve_profile_v2_config_path;
use codewen_core::find_thread_meta_by_name_str;
use codewen_core::format_exec_policy_error_with_source;
...
use codewen_login::default_client::set_default_originator;
...
use codewen_otel::set_parent_from_context;
use codewen_otel::traceparent_context_from_env;
...
pub use event_processor_with_jsonl_output::CodewenStatus;
...
enum StdinPromptBehavior {
    ...
    /// `codewen exec` behavior for `codewen exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `codewen exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<codewen_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&codewen_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        codewen_core::otel_init::build_provider(
            &config,
    ...
    codewen_core::otel_init::record_process_start(otel.as_ref(), "codewen_exec");
    codewen_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "codewen_exec");
    ...
    let state_db = codewen_core::init_state_db(&config).await;
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        feedback: CodewenFeedback::new(),
        log_db: None,
        state_db: state_db.clone(),
        environment_manager: std::sync::Arc::new(environment_manager),
        config_warnings,
        session_source: SessionSource::Exec,
        enable_codewen_api_key_env: true,
        client_name: "codewen_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = codewen_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    if !json_mode
        && let Some(message) =
            codewen_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        codewen_app_server_protocol::TurnStatus::Failed
                            | codewen_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        CodewenStatus::Running => {}
                        CodewenStatus::InitiateShutdown => {
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<codewen_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<codewen_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<codewen_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.codewen_home, session_id, Some(state_db.as_ref()))
                .await?
