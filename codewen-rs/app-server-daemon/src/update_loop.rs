...
#[cfg(unix)]
use crate::managed_install::resolved_managed_codewen_bin;
...
#[cfg(unix)]
const INSTALL_URL: &str = "https://chatgpt.com/codewen/install.sh";
...
async fn update_once(
    ...
    let managed_codewen_bin = resolved_managed_codewen_bin(&daemon.managed_codewen_bin).await?;
    let managed_identity = executable_identity(&managed_codewen_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_codewen_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_codewen_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_codewen_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Codex binary {}",
            managed_codewen_bin.display()
        )
    })
}
