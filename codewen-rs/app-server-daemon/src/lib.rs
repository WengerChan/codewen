...
use codewen_app_server_transport::app_server_control_socket_path;
use codewen_utils_home_dir::find_codewen_home;
use managed_install::managed_codewen_bin;
#[cfg(unix)]
use managed_install::managed_codewen_version;
use serde::Serialize;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_codewen_path: PathBuf,
    pub managed_codewen_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "codewen app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
struct Daemon {
    ...
    managed_codewen_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let codewen_home = find_codewen_home().context("failed to resolve CODEX_HOME")?;
        let socket_path = app_server_control_socket_path(codewen_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = codewen_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_codewen_bin: managed_codewen_bin(codewen_home.as_path()),
        })
    }
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        self.ensure_managed_codewen_bin()?;
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by codewen app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_codewen_version(managed_codewen_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_codewen_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_codewen_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_codewen_version = self
            .managed_codewen_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_codewen_version}",
            self.managed_codewen_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_codewen_version = self.managed_codewen_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_codewen_path: self.managed_codewen_bin.clone(),
            managed_codewen_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_codewen_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_codewen_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_codewen_bin(&self) -> Result<()> {
        if self.managed_codewen_bin.is_file() {
            return Ok(());
        }

        let managed_codewen_path = self.managed_codewen_bin.display();
        Err(anyhow!(
            "managed standalone Codex install not found at {managed_codewen_path}\n\n\
             ...
             Install it with:\n  curl -fsSL https://chatgpt.com/codewen/install.sh | sh\n\n\
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_codewen_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            codewen_bin: managed_codewen_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_codewen_path: "codewen".into(),
            managed_codewen_version: Some("1.2.3".to_string()),
            socket_path: "codewen.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedCodewenPath": "codewen",
                "managedCodewenVersion": "1.2.3",
                "socketPath": "codewen.sock",
                "cliVersion": "1.2.3",
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_codewen_bin: temp_dir.path().join("missing-codewen"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_codewen_bin.display(),
                stderr_log.display()
            )
        );
    }
