...
pub(crate) fn managed_codewen_bin(codewen_home: &Path) -> PathBuf {
    codewen_home
        .join("packages")
        .join("standalone")
        .join("current")
        .join(managed_codewen_file_name())
}

#[cfg(unix)]
pub(crate) async fn resolved_managed_codewen_bin(codewen_bin: &Path) -> Result<PathBuf> {
    fs::canonicalize(codewen_bin).await.with_context(|| {
        format!(
...
#[cfg(unix)]
pub(crate) async fn managed_codewen_version(codewen_bin: &Path) -> Result<String> {
    let output = Command::new(codewen_bin)
        .arg("--version")
        .output()
        .await
        .with_context(|| {
...
#[cfg(unix)]
fn parse_codewen_version(output: &str) -> Result<String> {
    let version = output
        .split_whitespace()
        .nth(1)
