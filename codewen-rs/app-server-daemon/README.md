# codewen-app-server-daemon

> `codewen-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`codewen-app-server-daemon` backs the machine-readable `codewen app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Codewen instances launched over SSH, including fresh developer
...
```sh
codewen app-server daemon start
codewen app-server daemon restart
codewen app-server daemon enable-remote-control
codewen app-server daemon disable-remote-control
codewen app-server daemon stop
codewen app-server daemon version
codewen app-server daemon bootstrap --remote-control
```
...
$HOME/.codewen/packages/standalone/current/codewen app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `CODEWEN_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Codewen is installed through `install.sh` and always launches
the standalone managed binary under `CODEWEN_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `CODEWEN_HOME/packages/standalone/current/codewen` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `CODEWEN_HOME/packages/standalone/current/codewen` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `codewen remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `CODEWEN_HOME`, so a concurrent
...
The daemon stores its local state under `CODEWEN_HOME/app-server-daemon/`:
