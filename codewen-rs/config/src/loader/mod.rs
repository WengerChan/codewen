...
use codewen_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/codewen/config.toml";
...
///
/// - system    `/etc/codewen/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Codewen\requirements.toml` (Windows)
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/codewen/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Codewen\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${CODEWEN_HOME}/config.toml`
/// - profile   `${CODEWEN_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.codewen/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.codewen/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(codewen_home)?;
        ...
        #[cfg(target_os = "macos")]
        {
            let managed_preferences_base_dir = AbsolutePathBuf::from_absolute_path(codewen_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, codewen_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(codewen_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, codewen_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                ...
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.openai.com/codewen/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(codewen_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/codewen/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_codewen_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("Codewen")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_codewen_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_codewen_system_dir().join("config.toml");
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Codewen to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".codewen"))
    }
}

fn project_layer_entry(
    dot_codewen_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_codewen_folder: dot_codewen_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_codewen_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    for dir in dirs {
        let dot_codewen_abs = dir.join(".codewen");
        let dot_codewen_uri = PathUri::from_abs_path(&dot_codewen_abs);
        if !fs
            .get_metadata(&dot_codewen_uri, /*sandbox*/ None)
            .await
            ...
            Ok(contents) => {
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_codewen_abs.as_path(),
                    )?;
                }
                let ignored_project_config_keys = sanitize_project_config(&mut config);
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_codewen_abs.as_path())?;
...
/// from a file like `/etc/codewen/managed_config.toml` that has the same
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Codewen").join("config.toml"))
        );
    }
}
