...
pub fn remove_user_marketplace(codewen_home: &Path, marketplace_name: &str) -> std::io::Result<bool> {
    let outcome = remove_user_marketplace_config(codewen_home, marketplace_name)?;
...
mod tests {
    ...
    fn remove_user_marketplace_removes_requested_entry() {
        ...
        record_user_marketplace(codewen_home.path(), "debug", &update).unwrap();
        record_user_marketplace(codewen_home.path(), "other", &update).unwrap();

        let removed = remove_user_marketplace(codewen_home.path(), "debug").unwrap();

        assert!(removed);
        let config: toml::Value =
            toml::from_str(&fs::read_to_string(codewen_home.path().join(CONFIG_TOML_FILE)).unwrap())
                .unwrap();
    ...
    fn remove_user_marketplace_config_reports_case_mismatch() {
        ...
        let outcome = remove_user_marketplace_config(codewen_home.path(), "Debug").unwrap();
