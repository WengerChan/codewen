...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("codewen-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, codewen_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.codewen/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_codewen_folder } => Some(dot_codewen_folder.clone()),
    ...
    /// Returns the `.codewen/` folder that should be used for hook declarations.
    ///
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_codewen_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_codewen_folder: current_project_dot_codewen_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_codewen_folder {
                ...
                if previous == current_project_dot_codewen_folder
                    || !current_project_dot_codewen_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_codewen_folder = Some(current_project_dot_codewen_folder);
        }
    }
