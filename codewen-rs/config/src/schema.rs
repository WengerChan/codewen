...
use codewen_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == codewen_features::Feature::Artifact {
            continue;
        }
        if feature.id == codewen_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<codewen_features::FeatureToml<
                    codewen_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == codewen_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<codewen_features::FeatureToml<
                    codewen_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == codewen_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<codewen_features::FeatureToml<
                    codewen_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == codewen_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<codewen_features::FeatureToml<
                    codewen_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == codewen_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<codewen_features::FeatureToml<
                    codewen_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == codewen_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == codewen_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<codewen_features::FeatureToml<
                    codewen_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
