...
use codewen_config::McpServerAuth;
...
use codewen_protocol::protocol::McpStartupCompleteEvent;
use codewen_protocol::protocol::McpStartupFailure;
use codewen_protocol::protocol::McpStartupFailureReason;
...
use codewen_rmcp_client::McpAuthState;
use codewen_rmcp_client::McpLoginRequirement;
use codewen_rmcp_client::determine_streamable_http_auth_status_from_credentials;
...
pub struct McpConnectionManager {
    ...
    codewen_apps_tools_override: RwLock<Option<Vec<ToolInfo>>>,
    codewen_apps_refresh_lock: Mutex<()>,
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        codewen_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
        tool_catalog_cache: McpToolCatalogCache,
        codewen_apps_tools_cache_key: ConnectorRuntimeContextKey,
        prefix_mcp_tool_names: bool,
        client_elicitation_capability: ElicitationCapability,
        supports_openai_form_elicitation: bool,
        tool_plugin_provenance: ToolPluginProvenance,
        auth: Option<&CodewenAuth>,
        codewen_apps_auth_manager: Option<Arc<AuthManager>>,
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_codewen_backend())
            .map(codewen_model_provider::auth_provider_from_auth);
        let codewen_apps_auth_provider = codewen_apps_auth_manager.and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_codewen_backend()).map(|auth| {
                codewen_model_provider::auth_provider_from_auth_manager(auth_manager, auth)
            })
        });
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            let shares_codewen_apps_tools_cache =
                should_share_codewen_apps_tools_cache(&server_name, uses_env_bearer_token);
            let codewen_apps_tools_cache_context = shares_codewen_apps_tools_cache.then(|| {
                codewen_apps_tools_cache
                    .context(codewen_home.clone(), codewen_apps_tools_cache_key.clone())
            });
            ...
            // not also attach the ambient CodewenAuth provider.
        ...
        let manager = Self {
            ...
            codewen_apps_tools_override: RwLock::new(None),
            codewen_apps_refresh_lock: Mutex::new(()),
    ...
    pub fn tool_approval_mode(
        ...
        server_name: &str,
        tool_name: &str,
    ) -> codewen_config::AppToolApproval {
        self.server_metadata
            .get(server_name)
    ...
    pub fn is_host_owned_codewen_apps_server(&self, server_name: &str) -> bool {
...
fn should_share_codewen_apps_tools_cache(server_name: &str, uses_env_bearer_token: bool) -> bool {
