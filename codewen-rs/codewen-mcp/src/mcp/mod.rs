...
use codewen_connectors::ConnectorSnapshot;
use codewen_connectors::connector_runtime_context_key;
use codewen_login::CodewenAuth;
use codewen_model_provider::CHATGPT_CODEX_BASE_URL;
use codewen_protocol::mcp::McpServerInfo;
use codewen_protocol::mcp::Resource;
use codewen_protocol::mcp::ResourceTemplate;
use codewen_protocol::mcp::Tool;
use codewen_protocol::models::PermissionProfile;
use codewen_protocol::protocol::AskForApproval;
use codewen_protocol::protocol::McpAuthStatus;
...
pub const CODEX_APPS_MCP_SERVER_NAME: &str = "codewen_apps";
const DEFAULT_CODEX_APPS_MCP_PRODUCT_SKU: &str = "codewen";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const CODEX_CONNECTORS_TOKEN_ENV_VAR: &str = "CODEWEN_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `codewen_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `codewen-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    ...
    /// Optional path to `codewen-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_codewen_apps_enabled(config: &McpConfig, auth: Option<&CodewenAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(CodewenAuth::uses_codewen_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_codewen_apps_enabled(config, auth) {
...
fn codewen_apps_mcp_bearer_token_env_var() -> Option<String> {
...
fn normalize_codewen_apps_base_url(base_url: &str) -> String {
...
fn codewen_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_codewen_apps_base_url(base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/codewen") {
        base_url
    } else {
        format!("{base_url}/api/codewen")
    };
    format!("{base_url}/ps/mcp")
}

pub fn codewen_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        codewen_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    codewen_apps_mcp_server_config(chatgpt_base_url, apps_mcp_product_sku, originator)
}
...
fn mcp_server_config_for_url(
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: codewen_apps_mcp_bearer_token_env_var(),
