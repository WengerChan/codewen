...
pub(crate) fn test_mcp_config(codewen_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        connector_snapshot: codewen_connectors::ConnectorSnapshot::default(),
    }
}
...
fn tool_plugin_provenance_collects_app_and_mcp_sources() {
    ...
    catalog.register(McpServerRegistration::from_plugin(
        ...
        /*plugin_order*/ 0,
        codewen_apps_mcp_server_config(
            "https://alpha.example",
            /*apps_mcp_product_sku*/ None,
            /*originator*/ None,
        ),
...
#[test]
fn codewen_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    assert_eq!(
        codewen_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/ps/mcp"
    );
    assert_eq!(
        codewen_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/ps/mcp"
    );
    assert_eq!(
        codewen_apps_mcp_url_for_base_url("http://localhost:8080/api/codewen"),
        "http://localhost:8080/api/codewen/ps/mcp"
    );
    assert_eq!(
        codewen_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/codewen/ps/mcp"
    );
}

#[test]
fn codewen_apps_server_config_uses_plugin_service_path() {
    let config = codewen_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for codewen apps"),
    };
...
#[test]
fn codewen_apps_server_config_forwards_thread_originator_header() {
    let config = codewen_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        ...
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ...
                    ("X-OpenAI-Product-Sku".to_string(), "codewen".to_string()),
                ]))
            );
...
#[test]
fn codewen_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in [(None, "codewen"), (Some("tpp"), "tpp")]
    {
        let config = codewen_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn codewen_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = codewen_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(codewen_home.path().to_path_buf());
    ...
    let codewen_apps = effective
        .get(CODEX_APPS_MCP_SERVER_NAME)
        .expect("codewen apps server should exist");
    ...
    let docs = docs
        .configured_config()
        .expect("configured server should retain transport");
    let codewen_apps = codewen_apps
        .configured_config()
        .expect("codewen apps should use configured transport");
