...
use crate::rmcp_client::CodewenAppsStartupReconnect;
...
use codewen_config::McpServerEnvVar;
...
use codewen_connectors::ConnectorRuntimeContextKey;
use codewen_connectors::ConnectorRuntimeFetchSource;
use codewen_connectors::ConnectorRuntimeManager;
use codewen_exec_server::EnvironmentManager;
use codewen_protocol::ToolName;
...
use codewen_rmcp_client::InProcessTransportFactory;
...
async fn create_test_managed_client(tools: Vec<ToolInfo>) -> ManagedClient {
    ManagedClient {
        ...
        codewen_apps_tools_cache_context: None,
    }
}
...
async fn create_ready_async_managed_client(tools: Vec<ToolInfo>) -> AsyncManagedClient {
    AsyncManagedClient {
        ...
        .boxed()
        .shared(),
        is_codewen_apps_mcp_server: false,
...
async fn create_test_manager_with_ready_apps_client(
    ...
    client
        .initialize(
            InitializeRequestParams::new(
                ...
                Implementation::new("codewen-test", "0.0.0-test"),
            )
    ...
    let managed_client = ManagedClient {
        ...
        server_instructions: None,
        server_supports_sandbox_state_meta_capability: false,
        codewen_apps_tools_cache_context: Some(cache_context.clone()),
    };
    ...
    manager.clients.insert(
        ...
        AsyncManagedClient {
            ...
            codewen_apps_tools_cache_context: Some(cache_context),
            tool_catalog_cache_context: None,
            tool_filter: ToolFilter::default(),
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: None,
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        codewen_rmcp_client::Elicitation::Mcp(
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let elicitation = codewen_rmcp_client::Elicitation::Mcp(
        CreateElicitationRequestParams::FormElicitationParams {
            meta: None,
    ...
    let (
        codewen_protocol::mcp::RequestId::String(request_a_id),
        codewen_protocol::mcp::RequestId::String(request_b_id),
...
#[test]
fn codewen_apps_env_bearer_token_bypasses_shared_tools_cache() {
    assert!(!should_share_codewen_apps_tools_cache(
...
#[tokio::test]
async fn list_all_tools_uses_shared_codewen_apps_cache_while_client_is_pending() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__codewen_apps", "calendar_create_event")
        })
...
async fn capture_binding_uses_the_ready_clients_own_tools() {
    ...
    ready_client.tool_filter = tool_filter.clone();
    ready_client.codewen_apps_tools_cache_context = Some(cache_context.clone());
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionManager::new_uninitialized(
        &approval_policy,
...
-> anyhow::Result<()> {
    ...
    let refresh_a = tokio::spawn(async move {
        manager_a_for_refresh
            .hard_refresh_codewen_apps_tools_cache()
            .await
    });
    list_started.notified().await;
    let tools_b = manager_b.hard_refresh_codewen_apps_tools_cache().await?;
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<codewen_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
#[tokio::test]
async fn list_all_tools_does_not_block_when_shared_codewen_apps_cache_is_empty() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_uses_shared_codewen_apps_cache_when_client_startup_fails() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_reconnects_failed_codewen_apps_startup_and_reuses_client() {
...
#[tokio::test]
async fn tool_lists_do_not_block_and_share_codewen_apps_startup_reconnect() {
    let recovered_client = create_test_managed_client(vec![create_test_tool(
        CODEX_APPS_MCP_SERVER_NAME,
        "drive_search",
    )])
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::codewen_apps_mcp_server_config(
        "https://docs.example",
...
#[test]
fn host_owned_codewen_apps_requires_server_metadata() {
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    ...
    assert!(!manager.is_host_owned_codewen_apps_server(CODEX_APPS_MCP_SERVER_NAME));
}

#[test]
fn host_owned_codewen_apps_matches_reserved_name_with_server_metadata() {
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionManager::new_uninitialized(
        &approval_policy,
    ...
    let server = EffectiveMcpServer::configured(crate::codewen_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    ));
    ...
    assert!(manager.is_host_owned_codewen_apps_server(CODEX_APPS_MCP_SERVER_NAME));
    assert!(!manager.is_host_owned_codewen_apps_server("docs"));
}
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `codewen mcp login {server_name}`."
    );
