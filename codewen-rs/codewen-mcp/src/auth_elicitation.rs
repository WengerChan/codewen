...
//! Session orchestration stays in `codewen-core`.

use codewen_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_CODEX_APPS_META_KEY: &str = "_codewen_apps";
...
pub struct CodewenAppsConnectorAuthFailure {
...
pub struct CodewenAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct CodewenAppsAuthElicitationPlan {
    pub auth_failure: CodewenAppsConnectorAuthFailure,
    pub elicitation: CodewenAppsAuthElicitation,
}

#[derive(Serialize)]
struct CodewenAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<CodewenAppsConnectorAuthFailure> {
    ...
    Some(CodewenAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<CodewenAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(CodewenAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &CodewenAppsConnectorAuthFailure,
) -> CodewenAppsAuthElicitation {
    CodewenAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_CODEX_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: CodewenAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("codewen_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &CodewenAppsConnectorAuthFailure) -> String {
...
mod tests {
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "codewen_apps_auth_call_123");
    }
}
