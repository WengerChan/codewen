...
use crate::codewen_apps::normalize_codewen_apps_callable_name;
use crate::codewen_apps::normalize_codewen_apps_callable_namespace;
use crate::codewen_apps::normalize_codewen_apps_tool_title;
use crate::codewen_apps::prepare_openai_file_params_for_model;
...
use codewen_async_utils::CancelErr;
use codewen_async_utils::OrCancelExt;
use codewen_config::McpServerConfig;
use codewen_config::McpServerTransportConfig;
use codewen_config::types::AuthKeyringBackendKind;
use codewen_config::types::OAuthCredentialsStoreMode;
use codewen_connectors::ConnectorRuntimeContext;
use codewen_connectors::ConnectorRuntimeFetchSource;
use codewen_exec_server::Environment;
use codewen_exec_server::HttpClient;
use codewen_exec_server::ReqwestHttpClient;
use codewen_protocol::mcp::McpServerInfo;
use codewen_protocol::protocol::Event;
use codewen_protocol::protocol::EventMsg;
use codewen_protocol::protocol::McpStartupStatus;
use codewen_protocol::protocol::McpStartupUpdateEvent;
use codewen_rmcp_client::ExecutorStdioServerLauncher;
use codewen_rmcp_client::LocalStdioServerLauncher;
use codewen_rmcp_client::RmcpClient;
use codewen_rmcp_client::StdioServerLauncher;
use codewen_rmcp_client::ToolWithConnectorId;
use codewen_rmcp_client::is_authentication_required_error;
...
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "codewen/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "codewen/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub const OPENAI_FORM_CAPABILITY: &str = "openai/form";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "codewen.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "codewen.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const CODEX_APPS_REFRESH_DURATION_METRIC: &str = "codewen.apps.refresh.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) codewen_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if self.codewen_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct CodewenAppsStartupReconnectState {
...
#[derive(Clone)]
struct CodewenAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct CodewenAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<CodewenAppsStartupReconnectState>,
    startup_status_context: Option<CodewenAppsStartupStatusContext>,
}

impl CodewenAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(CodewenAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = tx_event.map(|tx_event| CodewenAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
...
fn codewen_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
...
struct ManagedClientStartup {
    ...
    codewen_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_codewen_apps_mcp_server = server_name == CODEX_APPS_MCP_SERVER_NAME;
        ...
        async move {
            let refresh_start = is_codewen_apps_mcp_server.then(Instant::now);
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_codewen_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<CodewenAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let tool_filter = server
            ...
            .map(ToolFilter::from_config)
            .unwrap_or_default();
        let cached_server_info = if is_codewen_apps_mcp_server {
            codewen_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            codewen_apps_tools_cache_context: codewen_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_codewen_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                CodewenAppsStartupReconnect::new(Arc::new(move || startup.start()))
        ...
        if codewen_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.codewen_apps_tools_cache_context
            .as_ref()
            .is_some_and(ConnectorRuntimeContext::has_current_tools)
            || self
                .tool_catalog_cache_context
    ...
    pub(crate) fn prepare_tools(&self, tools: Vec<ToolInfo>) -> Vec<ToolInfo> {
        if self.is_codewen_apps_mcp_server {
            prepare_codewen_apps_tools_for_model(tools, &self.tool_plugin_provenance)
        } else {
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_codewen_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }?;
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_codewen_apps_mcp_server: bool,
    codewen_apps_refresh_trigger: &'static str,
    ...
    let tools = resp
        ...
        .collect();
    if is_codewen_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", codewen_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// server when absent.
fn prepare_codewen_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_codewen_apps_mcp_server {
        codewen_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
...
fn codewen_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_codewen_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_codewen_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_codewen_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = codewen_apps_tools_cache_context
        .as_ref()
    ...
    let client_tools = list_tools_for_client_uncached(
        &server_name,
        is_codewen_apps_mcp_server,
        /*codewen_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
    ...
    let shared_tools = match (codewen_apps_tools_cache_context.as_ref(), fetch_ticket) {
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("codewen-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Codex"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(codewen_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    ...
    #[test]
    fn codewen_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            ...
            callable_namespace: "codewen_apps__gmail".to_string(),
