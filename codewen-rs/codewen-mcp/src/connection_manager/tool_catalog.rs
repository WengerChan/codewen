...
impl McpConnectionManager {
    ...
    pub async fn capture_binding_with_metadata(
        ...
        for (server_name, managed_client) in &self.clients {
            let catalog_override = if server_name == CODEX_APPS_MCP_SERVER_NAME {
                self.codewen_apps_tools_override.read().await.clone()
            } else {
                None
            };
    ...
    /// Force-refresh codewen apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_codewen_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let _refresh = self.codewen_apps_refresh_lock.lock().await;
        ...
        let client_tools = list_tools_for_client_uncached(
            ...
            /*codewen_apps_refresh_trigger*/ "explicit",
        ...
        let tools = match (
            managed_client.codewen_apps_tools_cache_context.as_ref(),
            fetch_ticket,
        ) {
        ...
        *self.codewen_apps_tools_override.write().await = Some(client_tools);
