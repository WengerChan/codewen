...
impl McpProcess {
    pub async fn new(codewen_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(codewen_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = codewen_utils_cargo_bin::cargo_bin("codewen-mcp-server")
            .context("should find binary for codewen-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = codewen_login::default_client::originator().value;
    ...
    pub async fn send_codewen_tool_call(
        &mut self,
        params: CodewenToolCallParam,
    ) -> anyhow::Result<i64> {
        let codewen_tool_call_params = CallToolRequestParams::new("codewen").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(codewen_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "codewen/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "codewen/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `codewen-mcp-server` child process.
        //
