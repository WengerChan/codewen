...
use codewen_mcp_server::CodewenToolCallParam;
use codewen_mcp_server::ExecApprovalElicitRequestParams;
use codewen_mcp_server::ExecApprovalResponse;
use codewen_mcp_server::PatchApprovalElicitRequestParams;
use codewen_mcp_server::PatchApprovalResponse;
use codewen_protocol::protocol::FileChange;
use codewen_protocol::protocol::ReviewDecision;
use codewen_shell_command::parse_command;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "codewen" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let codewen_request_id = mcp_process
        .send_codewen_tool_call(CodewenToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            codewen_request_id.to_string(),
            params.codewen_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `codewen` tool call completes and that the file was created.
    let codewen_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(codewen_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(codewen_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    codewen_mcp_tool_call_id: String,
    codewen_event_id: String,
    ...
    let codewen_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        codewen_elicitation: "exec-approval".to_string(),
        codewen_mcp_tool_call_id,
        codewen_event_id,
        codewen_command: command,
        codewen_cwd: workdir.to_path_buf(),
        codewen_call_id: "call1234".to_string(),
        codewen_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "codewen" tool request that will trigger the apply_patch command
    let codewen_request_id = mcp_process
        .send_codewen_tool_call(CodewenToolCallParam {
    ...
    // Verify the original `codewen` tool call completes
    let codewen_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(codewen_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_codewen_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    codewen_tool_passes_base_instructions()
        .await
        .expect("codewen tool should pass base instructions");
}

async fn codewen_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `codewen mcp` with a specific config.toml.
    let codewen_home = TempDir::new()?;
    create_config_toml(codewen_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(codewen_home.path()).await?;
    ...
    .await??;
    assert_eq!(codewen_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(codewen_response.id, RequestId::Number(codewen_request_id));
    assert_eq!(
        codewen_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": codewen_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("codewen tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
