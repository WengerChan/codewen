...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the codewen tool call.
    ...
    pub codewen_command: Vec<String>,
    pub codewen_cwd: PathBuf,
    pub codewen_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    codewen_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, codewen).await;
        });
    }
}
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `codewen`.
