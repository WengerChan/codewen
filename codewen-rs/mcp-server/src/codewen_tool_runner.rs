...
use codewen_core::config::Config as CodewenConfig;
...
pub async fn run_codewen_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: CodewenConfig,
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_codewen_uuid.lock().await.remove(&id);
        return;
    }

    run_codewen_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_codewen_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_codewen_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<CodewenThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(codewen_event_to_notification(&event)) above has
