...
use codewen_login::default_client::USER_AGENT_SUFFIX;
...
use crate::codewen_tool_config::CodewenToolCallParam;
use crate::codewen_tool_config::CodewenToolCallReplyParam;
use crate::codewen_tool_config::create_tool_for_codewen_tool_call_param;
use crate::codewen_tool_config::create_tool_for_codewen_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_codewen_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        codewen_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.codewen_home.clone()),
        );
        let thread_manager = Arc::new(ThreadManager::new(
            config.as_ref(),
            Arc::clone(&auth_manager),
            codewen_core::build_models_manager(config.as_ref(), auth_manager),
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_codewen_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("codewen-mcp-server", env!("CARGO_PKG_VERSION")).with_title("Codex");
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_codewen_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_codewen_tool_call_param(),
                create_tool_for_codewen_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "codewen" => self.handle_tool_call_codex(id, arguments).await,
            "codewen-reply" => {
                self.handle_tool_call_codewen_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_codex(
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodewenToolCallParam>(json_val) {
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for codewen tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result).await;
                return;
            }
        ...
        let running_requests_id_to_codewen_uuid = self.running_requests_id_to_codewen_uuid.clone();

        // Spawn an async task to handle the Codex session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Codex session and stream events back to the client.
            crate::codewen_tool_runner::run_codewen_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_codewen_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_codewen_session_reply(
        &self,
        ...
        // parse arguments
        let codewen_tool_call_reply_param: CodewenToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<CodewenToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
            ...
            None => {
                tracing::error!(
                    "Missing arguments for codewen-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for codewen-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match codewen_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let codewen = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::codewen_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = codewen_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_codewen_uuid = running_requests_id_to_codewen_uuid.clone();

            async move {
                crate::codewen_tool_runner::run_codewen_tool_session_reply(
                    thread_id,
                    codewen,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_codewen_uuid.lock().await;
        ...
        let codewen_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        if let Err(e) = codewen_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: codewen_protocol::protocol::Op::Interrupt,
        ...
        self.running_requests_id_to_codewen_uuid
            .lock()
            .await
            .remove(&request_id);
    }
