...
use crate::protocol::CodewenErrorInfo;
...
use codewen_utils_string::truncate_middle_chars;
use codewen_utils_string::truncate_middle_with_token_budget;
...
pub type Result<T> = std::result::Result<T, CodewenErr>;
...
pub enum CodewenErr {
    ...
    /// could not be captured). Analogous to the previous `CodewenError::Spawn` variant.
    ...
    #[error("codewen-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for CodewenErr {
    fn from(_: CancelErr) -> Self {
        CodewenErr::TurnAborted
    }
}
...
impl CodewenErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            CodewenErr::TurnAborted
            | CodewenErr::SessionBudgetExceeded
            | CodewenErr::Interrupted
            | CodewenErr::EnvVar(_)
            | CodewenErr::Fatal(_)
            | CodewenErr::UsageNotIncluded
            | CodewenErr::QuotaExceeded
            | CodewenErr::InvalidImageRequest()
            | CodewenErr::InvalidRequest(_)
            | CodewenErr::RefreshTokenFailed(_)
            | CodewenErr::UnsupportedOperation(_)
            | CodewenErr::Sandbox(_)
            | CodewenErr::LandlockSandboxExecutableNotProvided
            | CodewenErr::RetryLimit(_)
            | CodewenErr::ContextWindowExceeded
            | CodewenErr::ThreadNotFound(_)
            | CodewenErr::AgentLimitReached { .. }
            | CodewenErr::Spawn
            | CodewenErr::SessionConfiguredNotFirstEvent
            | CodewenErr::UsageLimitReached(_)
            | CodewenErr::ServerOverloaded
            | CodewenErr::CyberPolicy { .. } => false,
            CodewenErr::Stream(..)
            | CodewenErr::Timeout
            | CodewenErr::RequestTimeout
            | CodewenErr::UnexpectedStatus(_)
            | CodewenErr::ResponseStreamFailed(_)
            | CodewenErr::ConnectionFailed(_)
            | CodewenErr::InternalServerError
            | CodewenErr::InternalAgentDied
            | CodewenErr::Io(_)
            | CodewenErr::Json(_)
            | CodewenErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            CodewenErr::LandlockRuleset(_) | CodewenErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<CodewenErr>()` checks continue to compile
    ...
    pub fn to_codewen_protocol_error(&self) -> CodewenErrorInfo {
        match self {
            CodewenErr::ContextWindowExceeded => CodewenErrorInfo::ContextWindowExceeded,
            CodewenErr::SessionBudgetExceeded => CodewenErrorInfo::SessionBudgetExceeded,
            CodewenErr::UsageLimitReached(_)
            | CodewenErr::QuotaExceeded
            | CodewenErr::UsageNotIncluded => CodewenErrorInfo::UsageLimitExceeded,
            CodewenErr::ServerOverloaded => CodewenErrorInfo::ServerOverloaded,
            CodewenErr::CyberPolicy { .. } => CodewenErrorInfo::CyberPolicy,
            CodewenErr::RetryLimit(_) => CodewenErrorInfo::ResponseTooManyFailedAttempts {
            ...
            CodewenErr::SessionConfiguredNotFirstEvent
            | CodewenErr::InternalServerError
            | CodewenErr::InternalAgentDied => CodewenErrorInfo::InternalServerError,
            CodewenErr::UnsupportedOperation(_)
            | CodewenErr::ThreadNotFound(_)
            | CodewenErr::AgentLimitReached { .. } => CodewenErrorInfo::BadRequest,
            CodewenErr::Sandbox(_) => CodewenErrorInfo::SandboxError,
            _ => CodewenErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            codewen_error_info: Some(self.to_codewen_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            CodewenErr::RetryLimit(err) => Some(err.status),
            CodewenErr::UnexpectedStatus(err) => Some(err.status),
            CodewenErr::ConnectionFailed(err) => err.source.status(),
            CodewenErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("codewen")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/codewen/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/codewen/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &CodewenErr) -> String {
    let message = match e {
        CodewenErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        CodewenErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
