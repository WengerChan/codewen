...
pub struct ConversationStartParams {
    ...
    pub codewen_responses_as_items: bool,
    /// Optional prefix added to automatic Codex response items when `codewen_responses_as_items` is set.
    pub codewen_response_item_prefix: Option<String>,
    /// Selects how automatic Codex handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub codewen_response_handoff_mode: CodewenResponseHandoffMode,
...
pub enum Op {
    ...
    /// Request to shut down codewen instance.
    Shutdown,
...
/// privileges of the agent (e.g. `.codewen`, `.git`, notably `.git/hooks`) under
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum CodewenResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub codewen_error_info: Option<CodewenErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.codewen_error_info
            .as_ref()
            .is_none_or(CodewenErrorInfo::affects_turn_status)
    }
}
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(codewen_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                codewen_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("codewen".to_string()).restriction_product(),
            Some(Product::Codex)
        );
        assert_eq!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        let expected_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".codewen"))
