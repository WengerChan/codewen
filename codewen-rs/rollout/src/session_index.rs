...
pub async fn append_thread_name(
    ...
    append_session_index_entry(codewen_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(codewen_home);
...
fn session_index_path(codewen_home: &Path) -> PathBuf {
    codewen_home.join(SESSION_INDEX_FILE)
}
