...
use codewen_state::BackfillState;
use codewen_state::BackfillStats;
use codewen_state::BackfillStatus;
use codewen_state::DB_ERROR_METRIC;
use codewen_state::DB_METRIC_BACKFILL;
use codewen_state::DB_METRIC_BACKFILL_DURATION_MS;
use codewen_state::ExtractionOutcome;
use codewen_state::ThreadMetadataBuilder;
use codewen_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = codewen_otel::global();
    ...
    let sessions_root = codewen_home.join(SESSIONS_SUBDIR);
    let archived_root = codewen_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(codewen_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(codewen_home: &Path, path: &Path) -> String {
    path.strip_prefix(codewen_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
