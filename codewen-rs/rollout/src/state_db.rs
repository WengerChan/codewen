...
pub use codewen_state::LogEntry;
...
pub type StateDbHandle = Arc<codewen_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        codewen_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    codewen_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == codewen_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, codewen_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = codewen_state::state_db_path(config.sqlite_home());
    ...
    let runtime = match codewen_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    metrics: codewen_otel::MetricsClient,
    originator: &str,
) -> codewen_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == codewen_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<codewen_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&codewen_state::StateRuntime>,
    codewen_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.codewen_home() != codewen_home {
        warn!(
            "state db codewen_home mismatch: expected {}, got {}",
            ctx.codewen_home().display(),
            codewen_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => codewen_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => codewen_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => codewen_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<codewen_state::ThreadRelationFilter>,
    archived: bool,
    search_term: Option<&str>,
) -> Option<codewen_state::ThreadsPage> {
    let ctx = context?;
    if ctx.codewen_home() != codewen_home {
        warn!(
            "state db codewen_home mismatch: expected {}, got {}",
    ...
    let filters = codewen_state::ThreadFilterOptions {
        ...
        sort_direction: match sort_direction {
            SortDirection::Asc => codewen_state::SortDirection::Asc,
            SortDirection::Desc => codewen_state::SortDirection::Desc,
        },
        search_term,
    };
