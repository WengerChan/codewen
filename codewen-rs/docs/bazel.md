...
This repository uses Bazel to build the Rust workspace under `codewen-rs`.
...
- `rules_rs` imports third-party crates from `codewen-rs/Cargo.toml` and
  `codewen-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `codewen_rust_crate`, which wraps `rust_library`,
...
- Each crate in `codewen-rs/*/BUILD.bazel` typically uses `codewen_rust_crate` and
...
Codewen uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `openai/codewen` | Yes | Yes |
| `workflow_dispatch` in `openai/codewen` | Yes | Yes |
| Same-repository pull request in `openai/codewen` | Yes | Yes |
| Fork pull request into `openai/codewen` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/codewen \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //codewen-rs/cli:codewen
```

The wrapper selects the OpenAI host only inside GitHub Actions for a trusted
run in `openai/codewen`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `codewen_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `codewen_rust_crate` to configure it.
