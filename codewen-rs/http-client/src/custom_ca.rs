...
pub const CODEX_CA_CERT_ENV: &str = "CODEWEN_CA_CERTIFICATE";
...
fn build_reqwest_client_with_env(
    ...
    info!(
        codewen_ca_certificate_configured = false,
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_codewen_env() {
        let env = map_env(&[
            (CODEX_CA_CERT_ENV, "/tmp/codewen.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/codewen.pem"))
        );
    }
