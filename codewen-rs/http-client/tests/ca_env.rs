...
//! detailed explanation of what "hermetic" means here lives in `codewen_http_client::custom_ca`; these
...
const CODEX_CA_CERT_ENV: &str = "CODEWEN_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "CODEWEN_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "CODEX_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "CODEWEN_CUSTOM_CA_PROBE_URL";
...
fn spawn_tls13_test_server() -> Tls13TestServer {
    codewen_utils_rustls_provider::ensure_rustls_crypto_provider();
...
fn generate_tls13_material() -> Tls13Material {
    ...
    ca_distinguished_name.push(DnType::CommonName, "codewen test CA");
...
#[test]
fn prefers_codewen_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("CODEWEN_CA_CERTIFICATE"));
