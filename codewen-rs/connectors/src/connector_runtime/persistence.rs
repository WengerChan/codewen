...
const MCP_TOOLS_CACHE_WRITE_DURATION_METRIC: &str = "codewen.mcp.tools.cache_write.duration_ms";
const CODEX_APPS_TOOLS_CACHE_DIR: &str = "cache/codewen_apps_tools";
pub(crate) const CODEX_APPS_TOOLS_CACHE_SCHEMA_VERSION: u8 = 4;
const CODEX_APPS_SERVER_INFO_CACHE_DIR: &str = "cache/codewen_apps_server_info";
...
fn cache_path_in(identity: &ConnectorRuntimeIdentity, cache_dir: &str) -> PathBuf {
    // `codewen_home` is already the parent directory. Keep it out of the
...
pub(crate) fn load_cached_connector_runtime_for_identity<T: ConnectorRuntimePayload>(
    ...
    let cache: CodewenAppsToolsDiskCache<T> = serde_json::from_slice(&bytes).ok()?;
    ...
    T: ConnectorRuntimePayload,
{
    ...
    let bytes = serde_json::to_vec_pretty(&CodewenAppsToolsDiskCache {
    ...
    write_codewen_apps_cache_file(&cache_path, "runtime", bytes)
}

#[instrument(level = "trace", skip_all)]
pub(crate) fn load_cached_codewen_apps_server_info<T: ConnectorRuntimePayload>(
    cache_context: &ConnectorRuntimeContext<T>,
) -> Option<McpServerInfo> {
    let (bytes, _) = read_bounded_cache_file(&cache_context.server_info_cache_path()).ok()?;
    let cache: CodewenAppsServerInfoDiskCache = serde_json::from_slice(&bytes).ok()?;
    (cache.schema_version == CODEX_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION)
        .then_some(cache.server_info)
}

fn write_cached_codewen_apps_server_info<T: ConnectorRuntimePayload>(
    cache_context: &ConnectorRuntimeContext<T>,
    server_info: &McpServerInfo,
) -> anyhow::Result<()> {
    let cache_path = cache_context.server_info_cache_path();
    let bytes = serde_json::to_vec_pretty(&CodewenAppsServerInfoDiskCache {
        schema_version: CODEX_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION,
        server_info: server_info.clone(),
    })
    .context("failed to serialize Codex Apps server info cache")?;
    write_codewen_apps_cache_file(&cache_path, "server info", bytes)
}

pub(crate) fn persist_codewen_apps_cache<T>(
    cache_context: &ConnectorRuntimeContext<T>,
    server_info: &McpServerInfo,
    snapshot: &ConnectorRuntimeSnapshot<T>,
) where
    T: ConnectorRuntimePayload,
{
    ...
    let server_info_result = write_cached_codewen_apps_server_info(cache_context, server_info);
...
fn write_codewen_apps_cache_file(
    cache_path: &Path,
    cache_name: &str,
    bytes: Vec<u8>,
) -> anyhow::Result<()> {
...
struct CodewenAppsToolsDiskCache<T> {
    schema_version: u8,
    tools: Vec<T>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct CodewenAppsServerInfoDiskCache {
    schema_version: u8,
    server_info: McpServerInfo,
}
...
#[cfg(test)]
pub(crate) fn write_cached_codewen_apps_tools_for_test<T>(
    cache_context: &ConnectorRuntimeContext<T>,
    server_info: &McpServerInfo,
    tools: &[T],
) where
    T: ConnectorRuntimePayload,
{
    ...
    persist_codewen_apps_cache(cache_context, server_info, &snapshot);
}

#[cfg(test)]
pub(crate) fn read_cached_codewen_apps_tools<T>(
    cache_context: &ConnectorRuntimeContext<T>,
) -> Option<Vec<T>>
where
    T: ConnectorRuntimePayload,
...
#[cfg(test)]
pub(crate) fn write_cached_codewen_apps_tools<T>(
    cache_context: &ConnectorRuntimeContext<T>,
    tools: &[T],
) -> anyhow::Result<()>
where
