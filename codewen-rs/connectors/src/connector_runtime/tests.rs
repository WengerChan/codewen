...
use super::persistence::read_cached_codewen_apps_tools;
use super::persistence::write_cached_codewen_apps_tools;
use super::persistence::write_cached_codewen_apps_tools_for_test;
use super::*;
...
const CODEX_APPS_MCP_SERVER_NAME: &str = "codewen_apps";
...
fn create_codewen_apps_tools_cache_context(
...
#[test]
fn codewen_apps_tools_cache_is_overwritten_by_last_write() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context = create_codewen_apps_tools_cache_context(
    ...
    write_cached_codewen_apps_tools(&cache_context, &tools_gateway_1).expect("write first cache");
    let cached_gateway_1 =
        read_cached_codewen_apps_tools(&cache_context).expect("cache entry exists for first write");
...
#[test]
fn codewen_apps_tools_cache_is_scoped_per_user() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    ...
    write_cached_codewen_apps_tools(&cache_context_user_1, &tools_user_1)
        .expect("write user one cache");
    write_cached_codewen_apps_tools(&cache_context_user_2, &tools_user_2)
        .expect("write user two cache");

    let read_user_1 =
        read_cached_codewen_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_codewen_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn codewen_apps_tools_cache_preserves_formerly_disallowed_connectors() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn codewen_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
    ...
    assert!(read_cached_codewen_apps_tools(&cache_context).is_none());
}

#[test]
fn codewen_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_codewen_apps_tools_loads_from_disk_cache() {
    let codewen_home = tempdir().expect("tempdir");
    let writer_cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    ...
    write_cached_codewen_apps_tools_for_test(&writer_cache_context, &server_info, &cached_tools);
    let cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
...
#[test]
fn startup_cached_codewen_apps_tools_loads_without_server_info_cache() {
    let codewen_home = tempdir().expect("tempdir");
    let writer_cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn codewen_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let codewen_home = tempdir().expect("tempdir");
    let cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let server_info = create_test_server_info("Codex Apps");
    write_cached_codewen_apps_tools_for_test(
        &cache_context,
        &server_info,
        &[create_test_tool(
            CODEX_APPS_MCP_SERVER_NAME,
    ...
    let startup_cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
#[test]
fn codewen_apps_tools_cache_context_does_not_reread_disk_after_creation() {
    let codewen_home = tempdir().expect("tempdir");
    let writer_cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_codewen_apps_tools(&writer_cache_context, &cached_tools).expect("write cache");
    let reader_cache_context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let updated_tools = vec![create_test_tool(CODEX_APPS_MCP_SERVER_NAME, "updated")];
    write_cached_codewen_apps_tools(&writer_cache_context, &updated_tools).expect("rewrite cache");
    ...
    assert_eq!(
        read_cached_codewen_apps_tools(&writer_cache_context).expect("disk tools")[0].callable_name,
        "updated"
    );
}

#[test]
fn codewen_apps_tools_cache_publishes_newest_shared_snapshot() {
    let codewen_home = tempdir().expect("tempdir");
    ...
    assert_eq!(
        read_cached_codewen_apps_tools(&cache_context_1).expect("persisted snapshot")[0]
            .callable_name,
        "newer"
    );
}
...
#[test]
fn codewen_apps_tools_cache_keeps_live_publish_when_disk_persistence_fails() {
    let codewen_home = tempdir().expect("tempdir");
    let codewen_home_file = codewen_home.path().join("not-a-directory");
    std::fs::write(&codewen_home_file, b"occupied").expect("create codewen home file");
...
fn connector_runtime_without_cache_ignores_disk_state() {
    let codewen_home = tempdir().expect("tempdir");
    let writer = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    ...
    let server_info = create_test_server_info("Codex Apps");
    write_cached_codewen_apps_tools_for_test(&writer, &server_info, &tools);
...
fn connector_runtime_without_cache_publishes_without_writing() {
    ...
    assert!(!codewen_home.exists());
}

#[cfg(unix)]
#[test]
fn codewen_apps_tools_cache_scopes_non_utf8_home_disk_paths() {
    let codewen_home = PathBuf::from(std::ffi::OsString::from_vec(
        b"/tmp/codewen-home-\xff".to_vec(),
    ));
    let cache = ConnectorRuntimeManager::<TestTool>::default();
...
fn oversized_tools_cache_is_ignored_during_initial_load() {
    let codewen_home = tempdir().expect("tempdir");
    let context = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    ...
    let reloaded = create_codewen_apps_tools_cache_context(
        codewen_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
fn cold_loaded_snapshot_uses_cache_modification_time() {
    ...
    let tools = vec![create_test_tool(CODEX_APPS_MCP_SERVER_NAME, "cached")];
    write_cached_codewen_apps_tools(&writer, &tools).expect("write tools cache");
