...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        let thread_id = self.tool_thread_id(thread_id, codewen_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, codewen_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_codewen_turn_id: codewen_turn_id,
    ...
    fn tool_thread_id(
        &self,
        thread_id: Option<String>,
        codewen_turn_id: Option<&str>,
    ) -> Result<String> {
        if let Some(thread_id) = thread_id {
            return Ok(thread_id);
        }
        let Some(codewen_turn_id) = codewen_turn_id else {
        ...
        self.rollout
            ...
            .with_context(|| {
                format!("tool call start referenced unknown Codex turn {codewen_turn_id}")
            })
    }

    fn validate_tool_turn(&self, thread_id: &str, codewen_turn_id: Option<&str>) -> Result<()> {
        if !self.rollout.threads.contains_key(thread_id) {
            bail!("tool call start referenced unknown thread {thread_id}");
        }
        if let Some(codewen_turn_id) = codewen_turn_id {
            let Some(turn) = self.rollout.codewen_turns.get(codewen_turn_id) else {
                bail!("tool call start referenced unknown Codex turn {codewen_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but Codex turn {codewen_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
