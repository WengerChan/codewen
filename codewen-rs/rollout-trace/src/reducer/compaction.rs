...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.codewen_turns.get(&started.codewen_turn_id) else {
            bail!(
                "compaction request {} referenced unknown codewen turn {}",
                started.compaction_request_id,
                started.codewen_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but codewen turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.codewen_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                codewen_turn_id: started.codewen_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.codewen_turns.get(&codewen_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown codewen turn {codewen_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but codewen turn {codewen_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) codewen_turn_id: String,
