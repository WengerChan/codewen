...
pub(super) struct PendingCodeCellStart {
    ...
    pub(super) codewen_turn_id: Option<String>,
...
impl TraceReducer {
    ...
    fn start_code_cell(&mut self, pending: PendingCodeCellStart) -> Result<()> {
        ...
        self.validate_code_cell_turn(&thread_id, &codewen_turn_id)?;
    ...
    pub(super) fn terminate_running_code_cells_for_turn_end(
        ...
        let code_cell_ids: Vec<_> = self
            ...
            .values()
            .filter(|cell| {
                cell.codewen_turn_id == codewen_turn_id
    ...
    pub(super) fn code_cell_event_thread_id(
        ...
        self.rollout
            ...
            .with_context(|| {
                format!(
                    "{event_name} {runtime_cell_id} referenced unknown Codex turn {codewen_turn_id}"
                )
            })
    }
    ...
    fn validate_code_cell_turn(&self, thread_id: &str, codewen_turn_id: &str) -> Result<()> {
        ...
        let Some(turn) = self.rollout.codewen_turns.get(codewen_turn_id) else {
            bail!("code cell start referenced unknown Codex turn {codewen_turn_id}");
        };
        if turn.thread_id != thread_id {
            bail!(
                "code cell start used thread {thread_id}, but Codex turn {codewen_turn_id} belongs \
                 to {}",
                turn.thread_id
            );
        }
