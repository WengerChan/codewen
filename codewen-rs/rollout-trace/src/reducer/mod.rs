...
impl TraceReducer {
    ...
    fn apply_event(&mut self, event: RawTraceEvent) -> Result<()> {
        ...
        match event.payload {
            ...
            RawTraceEventPayload::CodewenTurnEnded {
                codewen_turn_id,
                status,
            } => {
                self.end_codewen_turn(
                    event.seq,
                    event.wall_time_unix_ms,
                    event.thread_id,
                    codewen_turn_id,
            ...
            RawTraceEventPayload::CodeCellStarted {
                ...
                let thread_id = self.code_cell_event_thread_id(
                    event.thread_id,
                    event.codewen_turn_id.as_deref(),
                    &runtime_cell_id,
                    "code cell start",
                )?;
                ...
                self.start_or_queue_code_cell(PendingCodeCellStart {
                    ...
                    thread_id,
                    codewen_turn_id: event.codewen_turn_id,
            ...
            RawTraceEventPayload::CompactionInstalled {
                ...
                let Some(codewen_turn_id) = event.codewen_turn_id else {
                    bail!(
                        "compaction installed event {compaction_id} did not include a codewen turn id"
                    );
                };
