...
impl TraceReducer {
    ...
    pub(super) fn start_codewen_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.codewen_turns.contains_key(&codewen_turn_id) {
            bail!("duplicate codewen turn start for {codewen_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.codewen_turns.insert(
    ...
    pub(super) fn end_codewen_turn(
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.codewen_turns.get(&codewen_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "codewen turn end for {codewen_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.codewen_turns.get_mut(&codewen_turn_id) else {
            bail!("codewen turn end referenced unknown turn {codewen_turn_id}");
        };
