...
//! `codewen-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `codewen-rollout-trace`.
//!
...
use codewen_protocol::protocol::PatchApplyStatus;
use codewen_protocol::protocol::SubAgentActivityEvent;
use codewen_protocol::protocol::TurnAbortReason;
use serde::Serialize;
use std::time::Duration;

use crate::AgentThreadId;
use crate::CodewenTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct CodewenTurnTraceEvent {
    pub context_turn_id: CodewenTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn codewen_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<CodewenTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let codewen_turn_id = event.turn_id.clone();
            Some(CodewenTurnTraceEvent {
                context_turn_id: codewen_turn_id.clone(),
                payload: RawTraceEventPayload::CodewenTurnStarted {
                    codewen_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(CodewenTurnTraceEvent {
                context_turn_id: codewen_turn_id.clone(),
                payload: RawTraceEventPayload::CodewenTurnEnded {
                    codewen_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a codewen_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a codewen_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a codewen_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a codewen_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a codewen_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a codewen_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a codewen_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a codewen_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [codewen_protocol::parse_command::ParsedCommand],
