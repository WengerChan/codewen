...
use crate::protocol_event::codewen_turn_trace_event;
...
pub const CODEX_ROLLOUT_TRACE_ROOT_ENV: &str = "CODEWEN_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    pub fn record_codewen_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            codewen_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    pub fn record_tool_call_event(&self, codewen_turn_id: impl Into<CodewenTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(codewen_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_codewen_turn_id: impl Into<CodewenTurnId>,
    ...
    pub fn record_codewen_turn_started(&self, codewen_turn_id: impl Into<CodewenTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let codewen_turn_id = codewen_turn_id.into();
        context.append_with_context_best_effort(
            codewen_turn_id.clone(),
            RawTraceEventPayload::CodewenTurnStarted {
                codewen_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        codewen_turn_id: CodewenTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            codewen_turn_id: Some(codewen_turn_id),
        };
