...
use clap::Parser;
use clap::ValueEnum;
use codewen_state::LogQuery;
use codewen_state::LogRow;
use codewen_state::StateRuntime;
use dirs::home_dir;
use owo_colors::OwoColorize;

#[derive(Debug, Parser)]
#[command(name = "codewen-state-logs")]
#[command(about = "Tail Codex logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to CODEX_HOME. Defaults to $CODEX_HOME or ~/.codewen.
    #[arg(long, env = "CODEWEN_HOME")]
    codewen_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --codewen-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let codewen_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(codewen_home, "logs-client".to_string()).await?;
...
fn default_codewen_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".codewen");
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["codewen-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["codewen-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["codewen-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["codewen-state-logs", "--level", "WARN"])
