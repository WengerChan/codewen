...
//!
//! # async fn example(state_db: std::sync::Arc<codewen_state::StateRuntime>) {
...
pub fn default_filter() -> Targets {
    Targets::new()
        ...
        .with_target("codewen_otel.log_only", LevelFilter::OFF)
        .with_target("codewen_otel.trace_safe", LevelFilter::OFF)
...
mod tests {
    ...
    fn temp_codewen_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("codewen-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let codewen_home = temp_codewen_home();
