...
async fn sqlite_sink_drops_low_level_opentelemetry_sdk_logs() {
    let codewen_home =
        std::env::temp_dir().join(format!("codewen-state-log-db-filter-{}", Uuid::new_v4()));
    ...
    tracing::trace!(target: "codewen_state", "retained-trace");
    tracing::trace!(
        target: "codewen_api::responses_websocket_timing",
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("TRACE", "codewen_state", Some("retained-trace")),
        ]
    );
