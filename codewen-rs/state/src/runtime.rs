...
impl RuntimeDbSpec {
    fn path(self, codewen_home: &Path) -> PathBuf {
        codewen_home.join(self.filename)
    }
}
...
impl StateRuntime {
    ...
    pub(crate) async fn init_with_telemetry_for_tests(
        ...
        Self::init_inner(codewen_home, default_provider, Some(telemetry_override)).await
    }
    ...
    async fn init_inner(
        ...
        let state_path = STATE_DB.path(codewen_home.as_path());
        let logs_path = LOGS_DB.path(codewen_home.as_path());
        let goals_path = GOALS_DB.path(codewen_home.as_path());
        let memories_path = MEMORIES_DB.path(codewen_home.as_path());
...
pub fn state_db_path(codewen_home: &Path) -> PathBuf {
    STATE_DB.path(codewen_home)
}
...
pub fn logs_db_path(codewen_home: &Path) -> PathBuf {
    LOGS_DB.path(codewen_home)
}
...
pub fn goals_db_path(codewen_home: &Path) -> PathBuf {
    GOALS_DB.path(codewen_home)
}
...
pub fn memories_db_path(codewen_home: &Path) -> PathBuf {
    MEMORIES_DB.path(codewen_home)
}
...
pub fn thread_history_db_path(codewen_home: &Path) -> PathBuf {
    THREAD_HISTORY_DB.path(codewen_home)
}

pub fn runtime_db_paths(codewen_home: &Path) -> Vec<RuntimeDbPath> {
    RUNTIME_DBS
        .iter()
        .map(|spec| RuntimeDbPath {
            label: spec.label,
            path: spec.path(codewen_home),
        })
        .collect()
}
...
mod tests {
    ...
    async fn sqlite_integrity_check_reports_ok_for_valid_db() {
        ...
        let path = state_db_path(codewen_home.as_path());
        let pool = crate::SqliteConfig::new_for_testing(codewen_home.as_path().abs())
            .open_read_write_pool(&path)
            .await
    ...
    async fn open_state_sqlite_tolerates_newer_applied_migrations() {
        ...
        tokio::fs::create_dir_all(&codewen_home)
            .await
            .expect("create codewen home");
        let state_path = state_db_path(codewen_home.as_path());
        let pool = crate::SqliteConfig::new_for_testing(codewen_home.as_path().abs())
        ...
        let tolerant_pool = open_state_sqlite(
            &crate::SqliteConfig::new_for_testing(codewen_home.as_path().abs()),
            state_path.as_path(),
            &tolerant_migrator,
            /*telemetry_override*/ None,
        )
