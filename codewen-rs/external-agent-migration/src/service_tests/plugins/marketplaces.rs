...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, codewen_home)
        .import_plugins(
            /*cwd*/ None,
...
async fn import_plugins_reuses_configured_marketplace_with_different_source() {
    ...
    fs::create_dir_all(configured_plugin_root.join(".codewen-plugin"))
    ...
    fs::create_dir_all(source_plugin_root.join(".codewen-plugin"))
...
"#
    ...
    fs::write(
        source_plugin_root.join(".codewen-plugin/plugin.json"),
...
async fn detect_home_supports_relative_external_agent_plugin_marketplace_path() {
    ...
    let items = service_for_paths(external_agent_home.clone(), codewen_home)
...
fn import_skills_returns_only_new_skill_directory_names() {
    ...
    let copied_names = service_for_paths(external_agent_home, codewen_home)
