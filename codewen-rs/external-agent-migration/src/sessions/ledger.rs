...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(codewen_home)?.contains_current_source(source_path)
}
...
pub fn record_completed_session_imports(
    ...
    let mut ledger = load_import_ledger(codewen_home)?;
...
pub(crate) fn load_import_ledger(
    ...
    let path = import_ledger_path(codewen_home);
...
fn import_ledger_path(codewen_home: &Path) -> PathBuf {
    codewen_home.join(SESSION_IMPORT_LEDGER_FILE)
}
