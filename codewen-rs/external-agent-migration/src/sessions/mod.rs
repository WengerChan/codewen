...
pub fn prepare_validated_session_import_with_metadata_mode(
    ...
    let has_been_imported = has_current_session_been_imported(codewen_home, &session.path)?;
...
mod tests {
    ...
    fn skips_session_that_was_already_imported() {
        ...
        let codewen_home = root.path().join("codewen-home");
        let source_path = root.path().join("session.jsonl");
        std::fs::write(&source_path, "{}\n").expect("session");
        ledger::record_imported_session(&codewen_home, &source_path, ThreadId::new())
            .expect("record import");

        let pending =
            prepare_validated_session_import(&codewen_home, session_migration(&source_path))
