...
pub(super) async fn import(
    ...
    let memory_root = codewen_home.join("memories");
    codewen_memories_write::workspace::prepare_memory_workspace(&memory_root)
        .await
        .map_err(io::Error::other)?;
    let memory_files = discover_external_memory_files(external_agent_home)?;
    let copy_outcome = copy_resources(codewen_home, &memory_files, &selected_memory)?;
...
pub(crate) fn projects_needing_import(
    ...
    for (&project_key, project_files) in &files_by_project {
        let Some(project_cwd) = project_cwd(project_files) else {
            if project_has_unscoped_target(codewen_home, project_key)? {
                projects.insert(project_key.to_string());
            }
            continue;
        };
        if project_needs_import(codewen_home, project_key, project_cwd, project_files)? {
            projects.insert(project_key.to_string());
        }
    }
    projects.extend(
        owned_project_keys(codewen_home)?
...
fn copy_resources(
        ...
        let sync_result = match files_by_project.get(project_key) {
            Some(project_files) => {
                if let Some(project_cwd) = project_cwd(project_files) {
                    replace_project_resources(codewen_home, project_key, project_cwd, project_files)
                        .map(|()| true)
                } else if project_has_unscoped_target(codewen_home, project_key)? {
                    remove_project_resources(codewen_home, project_key)
                } else {
            ...
            None => remove_project_resources(codewen_home, project_key),
        };
        match sync_result {
            Ok(true) => {
    ...
    if !synchronized_projects.is_empty() {
        let instructions_path = extension_root(codewen_home).join("instructions.md");
...
fn owned_project_keys(codewen_home: &Path) -> io::Result<BTreeSet<String>> {
    let entries = match fs::read_dir(resources_root(codewen_home)) {
        Ok(entries) => entries,
...
fn project_has_unscoped_target(codewen_home: &Path, project_key: &str) -> io::Result<bool> {
    let target_root = resources_root(codewen_home).join(project_key);
...
fn project_needs_import(
    ...
    for memory_file in memory_files {
        ...
        if fs::read(resource_path(codewen_home, memory_file))
            .ok()
            .as_deref()
    ...
    if fs::read(project_scope_path(codewen_home, project_key))
        .ok()
        .as_deref()
...
fn remove_project_resources(codewen_home: &Path, project_key: &str) -> io::Result<bool> {
    let target_root = resources_root(codewen_home).join(project_key);
...
fn resource_path(codewen_home: &Path, memory_file: &ExternalMemoryFile) -> PathBuf {
    resources_root(codewen_home)
        .join(&memory_file.project_key)
        .join(&memory_file.relative_path)
}

fn project_scope_path(codewen_home: &Path, project_key: &str) -> PathBuf {
    resources_root(codewen_home)
        .join(project_key)
...
fn extension_root(codewen_home: &Path) -> PathBuf {
    codewen_home
        .join("memories")
        .join("extensions")
        .join(EXTENSION_NAME)
...
pub(super) fn resources_root(codewen_home: &Path) -> PathBuf {
    extension_root(codewen_home).join("resources")
}
