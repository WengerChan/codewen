...
const LOCK_CONTENTION_EVENT_TARGET: &str = "codewen_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    pub(super) fn acquire(store: OAuthStore) -> Result<Self, OAuthStoreLockFailure> {
        ...
        // the default home (`~/.codewen`), and if an embedder has no local home/filesystem authority
        ...
        let codewen_home = find_codewen_home()
            .map_err(|source| OAuthStoreLockFailure::CodewenHome { store, source })?;
        Self::acquire_in(&codewen_home, store, STORE_LOCK_ACQUIRE_TIMEOUT)
    }
    ...
    pub(super) fn acquire_in(
        ...
        let path = oauth_store_lock_path(codewen_home, store);
