...
const MEMO_URI: &str = "memo://codewen/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "codewen/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// - Build this binary: `cargo build -p codewen-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `codewen mcp add mcpimg -- /abs/path/to/test_stdio_server`
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://codewen/{slug}".to_string(),
            name: "codewen-memo".to_string(),
            title: Some("Codex Memo".to_string()),
            description: Some(
                "Template for memo://codewen/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("codewen-rmcp-test-server", env!("CARGO_PKG_VERSION"))
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            ...
            "encrypted_output" => {
                ...
                meta.insert("codewen/encryptedContent".to_string(), json!(true));
