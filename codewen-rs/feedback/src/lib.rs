...
/// Filename used for the redacted `codewen doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "codewen-doctor-report.json";
/// Filename used for the raw Codex Apps MCP tools cache feedback attachment.
pub const CODEX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str = "codewen-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const CODEX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str = "codewen-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_codewen_api_key_present = tracing::field::debug(auth_env.codewen_api_key_env_present),
        auth_env_codewen_api_key_enabled = tracing::field::debug(auth_env.codewen_api_key_env_enabled),
...
#[derive(Clone)]
pub struct CodewenFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for CodewenFeedback {
    fn default() -> Self {
        Self::new()
    }
}

impl CodewenFeedback {
    pub fn new() -> Self {
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("codewen_api::responses_websocket_timing", LevelFilter::OFF),
            )
    }
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `codewen-logs.log` and before path-backed rollout
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("codewen-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = CodewenFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_excludes_responses_websocket_timing_payloads() {
        let fb = CodewenFeedback::new();
        let _guard = tracing_subscriber::registry()
            .with(fb.logger_layer())
            .set_default();

        tracing::trace!(target: "codewen_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "codewen_feedback_test", "retained");
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("codewen-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = CodewenFeedback::new()
        ...
        let attachments_without_diagnostics = CodewenFeedback::new()
            .snapshot(/*session_id*/ None)
    ...
    fn path_backed_attachments_use_binary_content_types() {
        ...
        let gzip_filename = format!("codewen-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("codewen-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = CodewenFeedback::new()
            .snapshot(/*session_id*/ None)
