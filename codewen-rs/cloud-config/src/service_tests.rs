...
use codewen_backend_client::ConfigBundleResponse;
use codewen_backend_client::DeliveredTomlFragment;
use codewen_config::AbsolutePathBuf;
use codewen_config::CloudConfigFragment;
use codewen_config::CloudConfigTomlBundle;
use codewen_config::CloudRequirementsFragment;
use codewen_config::CloudRequirementsTomlBundle;
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(CodewenAuth::AgentIdentity(
...
struct TestExternalChatgptAuth {
    current: RwLock<CodewenAuth>,
    refreshed: CodewenAuth,
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let codewen_home = tempdir().expect("tempdir");
    let cache = create_test_cache(codewen_home.path());
    cache
        .save(
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = CodewenAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = CodewenAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
