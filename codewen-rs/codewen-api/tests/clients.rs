...
use codewen_api::AuthError;
use codewen_api::AuthProvider;
use codewen_api::Compression;
use codewen_api::Provider;
use codewen_api::ResponsesApiRequest;
use codewen_api::ResponsesClient;
use codewen_api::ResponsesOptions;
use codewen_client::HttpTransport;
use codewen_client::Request;
use codewen_client::RequestBody;
use codewen_client::Response;
use codewen_client::StreamResponse;
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, codewen_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, codewen_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
impl AuthProvider for FailsOnceAuth {
    ...
    fn apply_auth(&self, request: Request) -> codewen_api::AuthProviderFuture<'_> {
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, codewen_client::RequestCompression::None);
    Ok(())
}
