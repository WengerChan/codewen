...
use codewen_client::TransportError;
use codewen_http_client::HttpClientFactory;
use codewen_websocket_client::WebSocketConnection;
use codewen_websocket_client::WebSocketConnector;
use futures::SinkExt;
...
const X_CODEX_TURN_STATE_HEADER: &str = "x-codewen-turn-state";
...
const RESPONSES_WEBSOCKET_TIMING_EVENT_TARGET: &str = "codewen_api::responses_websocket_timing";
...
const WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str = "x-codewen-ws-stream-request-start-ms";
...
fn emit_responses_websocket_timing_event(
    ...
    // `RUST_LOG='codewen_api::responses_websocket_timing=trace'`.
    tracing::event!(
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-codewen-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-codewen-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
    ...
    fn websocket_safety_buffering_uses_event_before_header_fallback() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "codewen.response.metadata",
            "headers": {
                "x-codewen-safety-buffering-enabled": "true",
                "x-codewen-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
    ...
    fn websocket_safety_buffering_event_controls_visibility_when_header_disables_it() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "codewen.response.metadata",
            "headers": {
                "x-codewen-safety-buffering-enabled": "false",
                "x-codewen-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
        .expect("deserialize treatment metadata");
