...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, CodewenErr::ServerOverloaded));
}
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let CodewenErr::UnexpectedStatus(err) = err else {
        panic!("expected CodewenErr::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let CodewenErr::CyberPolicy { message } = err else {
        panic!("expected CodewenErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let CodewenErr::InvalidRequest(message) = err else {
        panic!("expected CodewenErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("codewen_other"),
    );
    headers.insert(
        "x-codewen-other-limit-name",
        http::HeaderValue::from_static("codewen_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let CodewenErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected CodewenErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_copies_rate_limit_reached_type_to_usage_limit_snapshot() {
    for (active_limit, expected_limit_id) in [(None, "codewen"), (Some("codewen_other"), "codewen_other")]
    {
        let mut headers = HeaderMap::new();
...
fn map_api_error_ignores_unparseable_rate_limit_reached_type_headers() {
    ...
    for value in values {
        let mut headers = HeaderMap::new();
        headers.insert("x-codewen-rate-limit-reached-type", value);
        let body = serde_json::json!({
            "error": {
                "type": "usage_limit_reached",
                "plan_type": "pro",
