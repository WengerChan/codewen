...
use chrono::DateTime;
use chrono::Utc;
use codewen_protocol::auth::PlanType;
use codewen_protocol::error::CodewenErr;
use codewen_protocol::error::RetryLimitReachedError;
use codewen_protocol::error::UnexpectedResponseError;
use codewen_protocol::error::UsageLimitReachedError;
use http::HeaderMap;
use serde::Deserialize;
use serde_json::Value;

pub fn map_api_error(err: ApiError) -> CodewenErr {
    match err {
        ApiError::ContextWindowExceeded => CodewenErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => CodewenErr::QuotaExceeded,
        ApiError::UsageNotIncluded => CodewenErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => CodewenErr::Stream(message, delay),
        ApiError::Stream(msg) => CodewenErr::Stream(msg, None),
        ApiError::ServerOverloaded => CodewenErr::ServerOverloaded,
        ApiError::Api { status, message } => {
            let user_message = api_error_user_message(status, &message);
            CodewenErr::UnexpectedStatus(UnexpectedResponseError {
                status,
                body: message,
                user_message,
                url: None,
        ...
        ApiError::InvalidRequest { message } => CodewenErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => CodewenErr::CyberPolicy { message },
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return CodewenErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                            ...
                            == Some(CYBER_POLICY_ERROR_CODE)
                    {
                        ...
                        CodewenErr::CyberPolicy { message }
                    } else if body_text
                        .contains("The image data you provided does not represent a valid image")
                    {
                        CodewenErr::InvalidImageRequest()
                    } else {
                        CodewenErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return CodewenErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return CodewenErr::UsageNotIncluded;
                        }
                    }

                    CodewenErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => CodewenErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => CodewenErr::RequestTimeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                CodewenErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => CodewenErr::Stream(msg, None),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-codewen-active-limit";
