...
use codewen_file_system::find_nearest_native_ancestor_with_markers;
...
mod tests {
    ...
    async fn git_metadata_commands_do_not_inherit_stdin() {
        const CHILD_ENV: &str = "CODEWEN_GIT_UTILS_STDIN_CHILD";
    ...
    fn canonicalize_git_remote_url_normalizes_github_variants() {
        for remote in [
            ...
            "ssh://git@github.com/openai/codewen.git",
            "ssh://git@github.com:22/OpenAI/Codex.git",
            "https://github.com/openai/codewen.git",
            "https://github.com:443/openai/codewen.git",
            "https://token@github.com/openai/codewen/",
            "github.com/OpenAI/Codex.git",
        ] {
            assert_eq!(
                canonicalize_git_remote_url(remote),
                Some("github.com/openai/codewen".to_string())
            );
        }
    }
    ...
    async fn local_git_branches_excludes_detached_head_entry() {
        ...
        run_git(&[
            ...
            "-c",
            "user.email=codewen-tests@example.com",
            "commit",
            "--allow-empty",
            "-q",
            "-m",
