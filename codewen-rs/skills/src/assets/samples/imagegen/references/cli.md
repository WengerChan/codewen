...
Set a stable path to the skill CLI (default `CODEWEN_HOME` is `~/.codewen`):

```
export CODEWEN_HOME="${CODEWEN_HOME:-$HOME/.codewen}"
export IMAGE_GEN="$CODEWEN_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/codewen-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$CODEWEN_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
