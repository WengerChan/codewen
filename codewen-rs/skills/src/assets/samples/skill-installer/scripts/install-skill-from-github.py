#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $CODEWEN_HOME/skills."""
...
def _codewen_home() -> str:
    return os.environ.get("CODEWEN_HOME", os.path.expanduser("~/.codewen"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "codewen")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "codewen-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_codewen_home(), "skills")
