...
const DEFAULT_MANUAL_URL = "https://developers.openai.com/codex/codewen-manual.md";
const DEFAULT_CACHE_DIR_NAME = "openai-docs-cache";
const CACHE_FILE_NAME = "codewen-manual.md";
const OUTLINE_FILE_NAME = "codewen-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "codewen-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-codewen-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# Codewen Manual Outline\n\n${outline.text}\n`;
