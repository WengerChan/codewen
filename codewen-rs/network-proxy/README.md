# codewen-network-proxy

`codewen-network-proxy` is Codewen's local network policy enforcement proxy. It runs:
...
`codewen-network-proxy` reads from Codewen's merged `config.toml` (via `codewen-core` config loading).
...
# bundle env vars pointing at immutable public files under $CODEWEN_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p codewen-network-proxy --
```
...
`codewen-network-proxy` can be embedded as a library with a thin API:

```rust
use codewen_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `codewen-network-proxy` is embedded in managed Codewen runtime, policy decisions emit structured
OTEL-compatible events with `target=codewen_otel.network_proxy`.

Event name:

- `codewen.network_proxy.policy_decision`
...
This section documents the protections implemented by `codewen-network-proxy`, and the boundaries of
