...
#[command(name = "codewen-network-proxy", about = "Codex network sandbox proxy")]
pub struct Args {}
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "CODEWEN_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "CODEWEN_NETWORK_ALLOW_LOCAL_BINDING";
...
#[cfg(target_os = "macos")]
fn codewen_proxy_git_ssh_command(socks_addr: SocketAddr) -> String {
    format!("{CODEX_PROXY_GIT_SSH_COMMAND_PREFIX}{socks_addr}{CODEX_PROXY_GIT_SSH_COMMAND_SUFFIX}")
}

#[cfg(target_os = "macos")]
fn is_codewen_proxy_git_ssh_command(command: &str) -> bool {
...
fn apply_proxy_env_overrides(
    ...
    if socks_enabled {
        ...
        match env.get(GIT_SSH_COMMAND_ENV_KEY) {
            Some(command) if !is_codewen_proxy_git_ssh_command(command) => {}
            _ => {
                env.insert(
                    GIT_SSH_COMMAND_ENV_KEY.to_string(),
                    codewen_proxy_git_ssh_command(socks_addr),
                );
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn non_codewen_managed_proxy_builder_uses_configured_ports() {
    ...
    fn apply_proxy_env_overrides_sets_mitm_ca_trust_bundle_vars() {
        let mut env = HashMap::new();
        let mitm_ca_trust_bundle_path = Path::new("/tmp/codewen-proxy/ca-bundle.pem");
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn apply_proxy_env_overrides_refreshes_previous_codewen_proxy_git_ssh_command() {
        let mut env = HashMap::new();
        env.insert(
            GIT_SSH_COMMAND_ENV_KEY.to_string(),
            codewen_proxy_git_ssh_command(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8081)),
        );
        ...
        assert_eq!(
            env.get(GIT_SSH_COMMAND_ENV_KEY),
            Some(&codewen_proxy_git_ssh_command(SocketAddr::new(
