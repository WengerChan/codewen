...
mod tests {
    ...
    use codewen_protocol::approvals::EscalationPermissions;
    ...
    /// executable paths.
    ///
    /// The `/bin/zsh` and `/tmp/codewen-execve-wrapper` values here are
    ...
    async fn start_session_exposes_wrapper_env_overlay() -> anyhow::Result<()> {
        ...
        let execve_wrapper = PathBuf::from("/tmp/codewen-execve-wrapper");
    ...
    async fn exec_closes_parent_socket_after_shell_spawn() -> anyhow::Result<()> {
        ...
        let server = EscalateServer::new(
            ...
            PathBuf::from("/tmp/codewen-execve-wrapper"),
            DeterministicEscalationPolicy {
                decision: EscalationDecision::run(),
            },
        );
