...
use codewen_agent_identity::AgentIdentityJwtClaims;
use codewen_agent_identity::decode_agent_identity_jwt;
use codewen_config::types::AuthCredentialsStoreMode;
pub use codewen_config::types::AuthKeyringBackendKind;
use codewen_keyring_store::DefaultKeyringStore;
use codewen_keyring_store::KeyringStore;
use codewen_protocol::account::PlanType as AccountPlanType;
use codewen_protocol::auth::AuthMode;
use codewen_secrets::LocalSecretsNamespace;
use codewen_secrets::SecretName;
use codewen_secrets::SecretScope;
use codewen_secrets::SecretsBackendKind;
use codewen_secrets::SecretsManager;
...
pub(super) fn get_auth_file(codewen_home: &Path) -> PathBuf {
    codewen_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(codewen_home: &Path) -> std::io::Result<bool> {
    let auth_file = get_auth_file(codewen_home);
...
impl FileAuthStorage {
    pub(super) fn new(codewen_home: PathBuf) -> Self {
        Self { codewen_home }
    }
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.codewen_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.codewen_home)
    }
}

static CODEX_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("CODEWEN_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("CODEX_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Codex Auth";

// turns codewen_home path into a stable, short key string
fn compute_store_key(codewen_home: &Path) -> std::io::Result<String> {
    let canonical = codewen_home
        .canonicalize()
        .unwrap_or_else(|_| codewen_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(codewen_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            codewen_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.codewen_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.codewen_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.codewen_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(codewen_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(codewen_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            codewen_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::CodewenAuth,
        );
        Self {
            codewen_home,
            direct_storage,
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(codewen_home)),
        }
    }
}
...
// A global in-memory store for mapping codewen_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(codewen_home: PathBuf) -> Self {
        Self { codewen_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(codewen_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(codewen_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(codewen_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(codewen_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(codewen_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(codewen_home, keyring_store))
        }
    }
}
