...
use codewen_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn refresh_without_id_token() {
    let codewen_home = tempdir().unwrap();
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let CodewenAuth::AgentIdentity(agent_identity_auth) = auth else {
        panic!("stored JWT should load as agent identity auth");
    };
    assert_eq!(agent_identity_auth.run_task_id(), "task-id");
    let storage = FileAuthStorage::new(codewen_home.path().to_path_buf());
    let auth = storage
        .try_read_auth_json(&get_auth_file(codewen_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn login_with_access_token_writes_only_personal_access_token() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let _authapi_guard = EnvVarGuard::set("CODEWEN_AUTHAPI_BASE_URL", &server.uri());
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(codewen_home.path().to_path_buf());
    let auth_path = get_auth_file(codewen_home.path());
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    let err = CodewenAuth::from_agent_identity_jwt_with_authapi_base_url(
        &agent_identity,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = CodewenAuth::from_auth_dot_json(
        codewen_home.path(),
...
#[derive(Clone)]
struct StaticExternalAuth(CodewenAuth);
...
impl ExternalAuth for StaticExternalAuth {
    ...
    fn refresh(&self, _context: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, CodewenAuth> {
        Box::pin(async { Ok(self.0.clone()) })
    }
}
...
async fn external_auth_provider_can_install_headers() {
    ...
    let auth = CodewenAuth::Headers(AuthHeaders::new(headers));
    ...
    assert!(
        manager
            .auth_cached()
            .is_some_and(|auth| auth.uses_codewen_backend())
    );
    assert!(
        !manager
            .auth_cached()
...
fn write_auth_file(params: AuthFileParams, codewen_home: &Path) -> std::io::Result<String> {
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let CodewenAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(codewen_home.path()).exists(),
...
#[tokio::test]
#[serial(codewen_auth_env)]
async fn load_auth_keeps_codewen_api_key_env_precedence() {
    let codewen_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(CODEX_ACCESS_TOKEN_ENV_VAR, &agent_identity);
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        codewen_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/codewen-backend/agent-identity",
        "aud": "codewen-app-server",
...
async fn assert_agent_identity_plan_alias(
    ...
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    let auth = CodewenAuth::from_agent_identity_jwt_with_authapi_base_url(
        &jwt,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),
