...
use codewen_utils_template::Template;
use rand::RngCore;
...
pub struct ServerOptions {
    ...
    pub codewen_streamlined_login: bool,
    ...
    match path.as_str() {
            ...
            .await
            {
                Ok(tokens) => {
                    ...
                    let redirect = compose_success_url(
                        ...
                        &tokens.id_token,
                        &tokens.access_token,
                        opts.codewen_streamlined_login,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "codewen_streamlined_login" && value == "true");
...
fn redact_sensitive_error_url(
    mut err: codewen_http_client::HttpError,
) -> codewen_http_client::HttpError {
...
fn is_missing_codewen_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_codewen_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_codewen_entitlement_error(error_code, error_description) {
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_codewen_entitlement_error(code, error_description) {
            (
...
mod tests {
    ...
    use super::is_missing_codewen_entitlement_error;
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_codewen_entitlement");
        assert!(is_missing_codewen_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(!body.contains("missing_codewen_entitlement"));
    }
}
