...
use codewen_http_client::HttpClientBuilder;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = codewen_home.clone();

    let opts = ServerOptions {
        codewen_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: codewen_login::test_support::transport_default_auth_route_config(),
        client_id: codewen_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = codewen_home.join("auth.json");
...
async fn hosted_login_redirects_to_configured_open_app_url() -> Result<()> {
    ...
    let server = run_login_server(ServerOptions {
        ...
        login_success_page: LoginSuccessPage::Hosted {
            url: Url::parse("http://localhost:3000/codewen/open-app?source=old")?,
...
#[tokio::test]
async fn creates_missing_codewen_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let codewen_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = HttpClientBuilder::new().build_direct()?;
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_codewen_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    ...
    assert!(
        !body.contains("missing_codewen_entitlement"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not codewen"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        codewen_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_codewen_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        codewen_home: first_codewen_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: codewen_login::test_support::transport_default_auth_route_config(),
        client_id: codewen_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_codewen_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        codewen_home: second_codewen_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: codewen_login::test_support::transport_default_auth_route_config(),
        client_id: codewen_login::CLIENT_ID.to_string(),
        issuer,
