...
use codewen_app_server_client::AppServerEvent;
use codewen_app_server_client::AppServerPath;
use codewen_app_server_client::AppServerRequestHandle;
use codewen_app_server_client::TypedRequestError;
...
use codewen_app_server_protocol::ConfigBatchWriteParams;
use codewen_app_server_protocol::ConfigRequirementsReadResponse;
use codewen_app_server_protocol::ConfigWriteResponse;
use codewen_app_server_protocol::ExternalAgentConfigDetectParams;
use codewen_app_server_protocol::ExternalAgentConfigDetectResponse;
use codewen_app_server_protocol::ExternalAgentConfigImportParams;
use codewen_app_server_protocol::ExternalAgentConfigImportResponse;
use codewen_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use codewen_app_server_protocol::Model as ApiModel;
use codewen_app_server_protocol::ModelListParams;
use codewen_app_server_protocol::ModelListResponse;
use codewen_app_server_protocol::NewThreadModelDefaults;
use codewen_app_server_protocol::RateLimitSnapshot;
use codewen_app_server_protocol::RequestId;
use codewen_app_server_protocol::ReviewDelivery;
use codewen_app_server_protocol::ReviewStartParams;
use codewen_app_server_protocol::ReviewStartResponse;
use codewen_app_server_protocol::ReviewTarget;
use codewen_app_server_protocol::SessionSource;
...
use codewen_app_server_protocol::ThreadMemoryMode;
...
use codewen_protocol::approvals::GuardianAssessmentEvent;
...
use codewen_protocol::openai_models::ModelAvailabilityNux;
use codewen_protocol::openai_models::ModelPreset;
use codewen_protocol::openai_models::ModelServiceTier;
use codewen_protocol::openai_models::ModelUpgrade;
use codewen_protocol::openai_models::ReasoningEffortPreset;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<codewen_protocol::account::PlanType>,
...
impl AppServerSession {
    ...
    pub(crate) fn codewen_home_path(
        &self,
        local_codewen_home: &AbsolutePathBuf,
    ) -> Option<AppServerPath> {
        self.client.codewen_home(local_codewen_home)
    }
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: codewen_protocol::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<codewen_protocol::openai_models::ReasoningEffort>,
        summary: Option<codewen_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<codewen_protocol::config_types::CollaborationMode>,
        personality: Option<codewen_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<codewen_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<codewen_app_server_protocol::ApprovalsReviewer> {
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &std::path::Path,
) -> Option<codewen_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(codewen_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(codewen_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(codewen_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(codewen_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn turn_permissions_overrides(
    ...
    Option<codewen_app_server_protocol::SandboxPolicy>,
    Option<String>,
) {
...
fn display_permission_profile_from_thread_response(
    sandbox: &codewen_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<codewen_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let (log_id, entry_count) = codewen_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(codewen_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn app_server_rate_limit_snapshots_deduplicates_top_level_limit_from_map() {
        let response = GetAccountRateLimitsResponse {
            rate_limits: rate_limit_snapshot("codewen"),
            rate_limits_by_limit_id: Some(HashMap::from([
                ("codewen".to_string(), rate_limit_snapshot("codewen")),
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(codewen_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn persisted_resume_does_not_forward_implicit_service_tier() -> Result<()> {
        ...
        let mut config = build_config(&codewen_home).await;
    ...
    {
        let codewen_home = tempfile::tempdir().expect("tempdir");
        let config = build_config(&codewen_home).await;
    ...
    async fn side_fork_excludes_turns_without_clearing_regular_ephemeral_fork() -> Result<()> {
        ...
        assert!(matches!(
            ...
            [codewen_app_server_protocol::ThreadItem::UserMessage { content, .. }]
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: codewen_app_server_protocol::Thread {
                ...
                source: codewen_app_server_protocol::SessionSource::Cli,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        codewen_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            client_id: None,
                            content: vec![codewen_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        codewen_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: codewen_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: codewen_app_server_protocol::ApprovalsReviewer::User,
        ...
        let embedded_config = ConfigBuilder::default()
            .codewen_home(temp_dir.path().join("embedded-codewen-home"))
            .harness_overrides(ConfigOverrides {
                default_permissions: Some(BUILT_IN_PERMISSION_PROFILE_WORKSPACE.to_string()),
                ..ConfigOverrides::default()
            })
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &codewen_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        codewen_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            codewen_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(codewen_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(codewen_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
