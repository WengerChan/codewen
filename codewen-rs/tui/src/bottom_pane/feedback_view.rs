...
use codewen_feedback::DOCTOR_REPORT_ATTACHMENT_FILENAME;
use codewen_feedback::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use codewen_feedback::FeedbackDiagnostics;
...
const BASE_CLI_BUG_ISSUE_URL: &str =
    "https://github.com/openai/codewen/issues/new?template=3-cli.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const CODEX_FEEDBACK_INTERNAL_URL: &str = "http://go/codewen-feedback-internal";
...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            format!("{prefix} Please report this in #codewen-feedback:")
        }
    ...
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            lines.extend([
                ...
                Line::from(vec![
                    "    ".into(),
                    format!("https://go/codewen-feedback/{thread_id}").bold(),
                ]),
            ]);
        }
        Some(url) => {
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "codewen-logs.log".into()]).into(),
        Line::from(vec![
            "  • ".into(),
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use codewen_feedback::FeedbackDiagnostic;
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/codewen-feedback-internal".to_string();
        ...
        let expected_external_url = "https://github.com/openai/codewen/issues/new?template=3-cli.yml&steps=Uploaded%20thread:%20t";
    ...
    fn feedback_success_cell_matches_employee_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please report this in #codewen-feedback:\n\n  http://go/codewen-feedback-internal\n\n  Share this and add some info about your problem:\n    https://go/codewen-feedback/thread-2"
        );
    }
