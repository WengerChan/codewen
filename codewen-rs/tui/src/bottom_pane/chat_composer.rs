...
//!
//! For the detailed burst state machine, see `codewen-rs/tui/src/bottom_pane/paste_burst.rs`.
//!
...
use codewen_message_history::HistoryBatchCursor;
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let display_name = codewen_connectors::metadata::connector_display_label(connector);
...
mod tests {
    ...
    use crate::bottom_pane::textarea::TextArea;
    use codewen_protocol::models::local_image_label_text;
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                composer.set_status_line(Some(Line::from(
                    "gpt-5.4 high fast · ~/code/codewen-1 · Context 0% used",
                )));
    ...
    fn status_line_hyperlink_marks_pr_number_cells() {
        ...
        let url = "https://github.com/openai/codewen/pull/20252";
        composer.set_status_line_enabled(/*enabled*/ true);
    ...
    fn set_skill_mentions_refreshes_open_mention_popup() {
        ...
        assert_eq!(mention.insert_text, "$codewen".to_string());
...
程序员 与 Unicode 同行";
    ...
    fn file_completion_preserves_large_paste_placeholder_elements() {
        ...
        composer.on_file_search_result(
            ...
            vec![FileMatch {
                ...
                match_type: codewen_file_search::MatchType::File,
    ...
    fn pasting_filepath_attaches_image() {
        let tmp = tempdir().expect("create TempDir");
        let tmp_path: PathBuf = tmp.path().join("codewen_tui_test_paste_image.png");
