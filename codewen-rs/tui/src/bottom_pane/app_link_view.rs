...
const MCP_CODEX_APPS_SERVER_NAME: &str = "codewen_apps";
const MCP_TOOL_CODEX_APPS_META_KEY: &str = "_codewen_apps";
...
impl AppLinkViewParams {
    pub(crate) fn from_url_app_server_request(
        ...
        request: &codewen_app_server_protocol::McpServerElicitationRequest,
    ) -> Option<Self> {
        let codewen_app_server_protocol::McpServerElicitationRequest::Url {
            meta,
            message,
            url,
            elicitation_id,
        ...
        if server_name == MCP_CODEX_APPS_SERVER_NAME {
            ...
            return Self::from_codewen_apps_auth_url_parts(
                thread_id,
                server_name,
                request_id,
                meta.as_ref(),
    ...
    fn from_codewen_apps_auth_url_parts(
        thread_id: ThreadId,
        server_name: &str,
        request_id: AppServerRequestId,
...
impl AppLinkView {
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let is_codewen_apps_auth = is_auth_suggestion
            && self
                .elicitation_target
                .as_ref()
...
mod tests {
    ...
    fn auth_url_request(url: &str) -> codewen_app_server_protocol::McpServerElicitationRequest {
        codewen_app_server_protocol::McpServerElicitationRequest::Url {
            ...
            url: url.to_string(),
            elicitation_id: "codewen_apps_auth_call_123".to_string(),
        }
    }

    #[test]
    fn codewen_apps_auth_url_elicitation_builds_auth_app_link_params() {
    ...
    #[test]
    fn non_codewen_apps_url_elicitation_builds_generic_app_link_params() {
        let target = generic_url_target();
        let request = codewen_app_server_protocol::McpServerElicitationRequest::Url {
            meta: None,
    ...
    #[test]
    fn codewen_apps_auth_url_elicitation_rejects_untrusted_urls() {
        let target = suggestion_target();
        for url in [
