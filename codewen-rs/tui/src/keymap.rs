...
use codewen_config::types::KeybindingsSpec;
use codewen_config::types::MAX_FUNCTION_KEY;
...
Set unique keys in `~/.codewen/config.toml` and retry. \
...
Set a different key in `~/.codewen/config.toml` and retry. \
See the Codex keymap documentation for supported actions and examples."
                ));
            }
        }
...
mod tests {
    use super::*;
    use codewen_config::types::KeybindingSpec;
