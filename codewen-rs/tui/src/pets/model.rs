...
impl Pet {
    ...
    /// first use.
    pub(super) fn load_with_codewen_home(value: &str, codewen_home: Option<&Path>) -> Result<Self> {
        if path_like(value) {
            return load_pet_path(value);
        }

        if let Some(custom_id) = value.strip_prefix(CUSTOM_PET_PREFIX) {
            return load_custom_pet(custom_id, codewen_home);
        }

        if let Some(builtin) = catalog::builtin_pet(value) {
            return load_builtin_pet(builtin, codewen_home);
        }

        load_custom_pet(value, codewen_home)
    }
...
fn load_builtin_pet(pet: catalog::BuiltinPet, codewen_home: Option<&Path>) -> Result<Pet> {
    let codewen_home = codewen_home.context("CODEX_HOME is not available")?;
    let spritesheet_path = super::builtin_spritesheet_path(codewen_home, pet.spritesheet_file);
...
fn load_custom_pet(value: &str, codewen_home: Option<&Path>) -> Result<Pet> {
    let codewen_home = codewen_home.context("CODEX_HOME is not available")?;
    let pet_dir = codewen_home.join("pets").join(value);
    if pet_dir.join("pet.json").is_file() {
        return load_pet_manifest(&pet_dir, "pet.json", value, &custom_pet_cache_id(value));
    }

    let avatar_dir = codewen_home.join("avatars").join(value);
...
mod tests {
    ...
    fn load_pet_from_dir(dir: &tempfile::TempDir) -> Pet {
        Pet::load_with_codewen_home(dir.path().to_str().unwrap(), /*codewen_home*/ None).unwrap()
    }
    ...
    fn load_builtin_pet_uses_app_catalog_storage() {
        let codewen_home = tempfile::tempdir().unwrap();
    ...
    fn load_pet_json_path_uses_containing_directory() {
        ...
        let pet = Pet::load_with_codewen_home(
    ...
    #[test]
    fn custom_pet_selector_loads_codewen_home_pet_manifest() {
        let dir = write_minimal_pet();
        let codewen_home = tempfile::tempdir().unwrap();
        let pet_dir = codewen_home.path().join("pets").join("chefito");
    ...
    fn custom_pet_selector_falls_back_to_legacy_avatar_manifest() {
        let dir = write_minimal_pet();
        let codewen_home = tempfile::tempdir().unwrap();
        let avatar_dir = codewen_home.path().join("avatars").join("legacy");
    ...
    fn custom_pet_rejects_spritesheet_path_escape() {
        let codewen_home = tempfile::tempdir().unwrap();
        let pet_dir = codewen_home.path().join("pets").join("escape");
        fs::create_dir_all(&pet_dir).unwrap();
        fs::write(
        ...
        let err = Pet::load_with_codewen_home(
