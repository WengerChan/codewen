...
pub(crate) fn build_pet_picker_params(
    ...
    let mut entries = available_pet_entries(codewen_home);
...
fn available_pet_entries(codewen_home: &Path) -> Vec<PetPickerEntry> {
    ...
    entries.extend(custom_pet_entries(codewen_home));
    entries
}

fn custom_pet_entries(codewen_home: &Path) -> Vec<PetPickerEntry> {
    let mut entries_by_selector = HashMap::new();
    for (directory_name, manifest_file) in [("avatars", "avatar.json"), ("pets", "pet.json")] {
        let Ok(children) = fs::read_dir(codewen_home.join(directory_name)) else {
            continue;
        };
        for child in children.flatten() {
            ...
            let Ok(pet) =
                Pet::load_with_codewen_home(&selector, /*codewen_home*/ Some(codewen_home))
            else {
                continue;
            };
...
mod tests {
    ...
    fn picker_lists_app_bundled_and_custom_pets() {
        ...
        write_pet(codewen_home.path(), "chefito", "Chefito");
    ...
    #[test]
    fn picker_preselects_codewen_without_marking_it_current_when_no_pet_is_configured() {
        let codewen_home = tempfile::tempdir().unwrap();
        let params = build_pet_picker_params(
            /*current_pet*/ None,
            codewen_home.path(),
    ...
    fn picker_imports_legacy_avatar_manifests() {
        let codewen_home = tempfile::tempdir().unwrap();
        write_legacy_avatar(codewen_home.path(), "legacy", "Legacy");
