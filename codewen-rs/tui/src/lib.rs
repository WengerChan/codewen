...
// The standalone `codewen-tui` binary prints a short help message before the
...
use codewen_app_server_client::AppServerClient;
use codewen_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use codewen_app_server_client::InProcessAppServerClient;
use codewen_app_server_client::InProcessClientStartArgs;
use codewen_app_server_client::RemoteAppServerClient;
use codewen_app_server_client::RemoteAppServerConnectArgs;
pub use codewen_app_server_client::RemoteAppServerEndpoint;
use codewen_app_server_protocol::Account as AppServerAccount;
use codewen_app_server_protocol::AskForApproval;
use codewen_app_server_protocol::ConfigWarningNotification;
use codewen_app_server_protocol::Thread as AppServerThread;
use codewen_app_server_protocol::ThreadListCwdFilter;
use codewen_app_server_protocol::ThreadListParams;
use codewen_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
use codewen_app_server_protocol::ThreadSourceKind;
use codewen_cloud_config::cloud_config_bundle_loader_for_storage;
use codewen_config::CloudConfigBundleLoader;
use codewen_config::ConfigLoadError;
use codewen_config::LoaderOverrides;
use codewen_config::format_config_error_with_source;
use codewen_config::types::ResumeCwdMode;
use codewen_exec_server::EnvironmentManager;
use codewen_exec_server::ExecServerRuntimePaths;
use codewen_login::AuthConfig;
use codewen_login::default_client::originator;
use codewen_login::default_client::set_default_client_residency_requirement;
use codewen_login::enforce_login_restrictions;
...
use codewen_rollout::StateDbHandle;
use codewen_rollout::state_db;
use codewen_state::log_db;
use codewen_utils_absolute_path::AbsolutePathBuf;
use codewen_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use codewen_utils_home_dir::find_codewen_home;
use codewen_utils_oss::ensure_oss_provider_ready;
use codewen_utils_oss::get_default_model_for_oss_provider;
...
pub(crate) use codewen_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "codewen-tui.log";
...
async fn start_embedded_app_server(
    ...
    feedback: codewen_feedback::CodewenFeedback,
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = codewen_state::runtime_db_path_for_corruption_error(&err)
                .unwrap_or_else(|| codewen_state::state_db_path(config.sqlite_home.as_path()));
...
fn remove_legacy_tui_log_file(codewen_home: &Path) {
    ...
    let _ = std::fs::remove_file(codewen_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let codewen_home = find_codewen_home().wrap_err("failed to resolve CODEX_HOME")?;
            codewen_app_server_client::app_server_control_socket_path(&codewen_home)
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_codewen_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        codewen_feedback::CodewenFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
    ...
    Fut: Future<Output = std::io::Result<InProcessAppServerClient>>,
{
    ...
    let client = start_client(InProcessClientStartArgs {
        ...
        enable_codewen_api_key_env: false,
        client_name: "codewen-tui".to_string(),
        client_version: env!("CARGO_PKG_VERSION").to_string(),
        experimental_api: true,
        mcp_server_openai_form_elicitation: false,
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&codewen_state::StateRuntime>,
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = codewen_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let codewen_home = match find_codewen_home() {
        Ok(codewen_home) => codewen_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding codewen home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&codewen_home, profile_v2);
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        maybe_probe_default_daemon_socket(&codewen_home).await
    } else {
        None
    };
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.codewen_self_exe.clone(),
    ...
    let overrides = ConfigOverrides {
        ...
        codewen_self_exe: arg0_paths.codewen_self_exe.clone(),
        codewen_linux_sandbox_exe: arg0_paths.codewen_linux_sandbox_exe.clone(),
    ...
    remove_legacy_tui_log_file(config.codewen_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        codewen_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(codewen_otel::OtelProvider::metrics) {
        let _ = codewen_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            codewen_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = codewen_state::install_process_db_telemetry(telemetry);
    }
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("codewen_core=info,codewen_tui=info,codewen_rmcp_client=info")
        });
    ...
    let feedback = codewen_feedback::CodewenFeedback::new();
...
async fn run_ratatui_app(
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !uses_remote_workspace {
            cloud_config_bundle = cloud_config_bundle_loader_for_storage(
                initial_config.codewen_home.to_path_buf(),
                /*enable_codewen_api_key_env*/ false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `codewen {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_codewen_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.codewen_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, codewen_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        codewen_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .codewen_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
    ...
    fn write_session_rollout(
        ...
        let session_meta = codewen_protocol::protocol::SessionMeta {
            session_id: thread_id.into(),
            id: thread_id,
            timestamp: meta_rfc3339.to_string(),
            cwd: cwd.to_path_buf(),
            originator: "codewen".to_string(),
            cli_version: "0.0.0".to_string(),
            source: codewen_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(codewen_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://codewen.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("codewen.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("codewen.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_probe_default_daemon_socket(codewen_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let codewen_home = TempDir::new()?;
        let socket_path =
            codewen_app_server_client::app_server_control_socket_path(codewen_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(codewen_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("codewen.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            codewen_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = codewen_state::StateRuntime::init(
            ...
            let mut builder = codewen_state::ThreadMetadataBuilder::new(
                thread_id,
                rollout_path.clone(),
                created_at,
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            codewen_state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
    ...
    -> color_eyre::Result<()> {
        ...
        let logs_db_path = codewen_state::logs_db_path(&sqlite_home);
        ...
        assert!(
            codewen_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let codewen_home = temp_dir.path().to_path_buf();
