...
#[cfg(any(unix, windows))]
const TUI_SOURCE_CLIENT_ID: &str = "codewen-tui";
#[cfg(any(unix, windows))]
...
pub(crate) fn fetch_ide_context(
    ...
    let primary_socket_path = primary_ipc_socket_path(codewen_home);
...
#[cfg(unix)]
fn primary_ipc_socket_path(codewen_home: &Path) -> PathBuf {
    codewen_home.join("ipc").join("ipc.sock")
}

#[cfg(unix)]
fn legacy_ipc_socket_paths(temp_dir: &Path, uid: libc::uid_t) -> Vec<PathBuf> {
    let ipc_dir = temp_dir.join("codewen-ipc");
    if uid == 0 {
...
fn default_ipc_socket_path() -> PathBuf {
    PathBuf::from(r"\\.\pipe\codewen-ipc")
}
...
mod tests {
    ...
    #[test]
    fn primary_ipc_socket_path_uses_codewen_home() {
        let codewen_home = Path::new("/home/test/.codewen");

        assert_eq!(
            primary_ipc_socket_path(codewen_home),
            codewen_home.join("ipc").join("ipc.sock")
        );
    }
    ...
    fn validate_unix_socket_path_rejects_unsafe_parent_directory() {
        ...
        let socket_path = tempdir.path().join("codewen-ipc.sock");
