...
use codewen_app_server_protocol::CodewenErrorInfo as AppServerCodexErrorInfo;
...
use codewen_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use codewen_app_server_protocol::CreditsSnapshot;
...
use codewen_app_server_protocol::ModelVerification as AppServerModelVerification;
use codewen_app_server_protocol::RateLimitReachedType;
...
use codewen_app_server_protocol::SkillMetadata;
use codewen_app_server_protocol::SkillsListResponse;
use codewen_app_server_protocol::ThreadGoal as AppThreadGoal;
use codewen_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use codewen_config::Constrained;
use codewen_config::ConstraintResult;
...
#[cfg(test)]
use codewen_git_utils::CommitLogEntry;
use codewen_git_utils::current_branch_name;
use codewen_git_utils::get_git_repo_root;
use codewen_git_utils::local_git_branches;
use codewen_git_utils::recent_commits;
use codewen_otel::RuntimeMetricsSummary;
use codewen_otel::SessionTelemetry;
use codewen_plugin::PluginCapabilitySummary;
use codewen_protocol::ThreadId;
use codewen_protocol::account::PlanType;
use codewen_protocol::approvals::GuardianAssessmentAction;
use codewen_protocol::approvals::GuardianAssessmentDecisionSource;
use codewen_protocol::approvals::GuardianAssessmentEvent;
use codewen_protocol::approvals::GuardianAssessmentStatus;
...
use codewen_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use codewen_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
use codewen_utils_cli::resume_hint;
...
const MEMORIES_DOC_URL: &str = "https://developers.openai.com/codewen/memories";
...
use codewen_file_search::FileMatch;
...
use codewen_utils_approval_presets::ApprovalPreset;
use codewen_utils_approval_presets::builtin_approval_presets;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_codewen_backend_auth: bool,
    pub(crate) model_catalog: Arc<ModelCatalog>,
    pub(crate) feedback: codewen_feedback::CodewenFeedback,
...
/// and by sending requests back to codewen-core.
///
...
pub(crate) struct ChatWidget {
    ...
    codewen_op_target: CodewenOpTarget,
    ...
    codewen_rate_limit_reached_type: Option<RateLimitReachedType>,
    codewen_spend_control_reached: Option<bool>,
...
fn request_permissions_from_params(
    params: codewen_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    /// Forward a command directly to codewen.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.codewen_op_target {
            CodewenOpTarget::Direct(codewen_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = codewen_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            CodewenOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::CodewenOp(op));
            }
        }
        true
    }
