...
use clap::Parser;
use codewen_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `codewen resume` subcommand.
    // These are not exposed as user flags on the base `codewen` command.
    #[clap(skip)]
    ...
    /// top-level `codewen resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `codewen fork` subcommand.
    // These are not exposed as user flags on the base `codewen` command.
    #[clap(skip)]
    ...
    /// top-level `codewen fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
