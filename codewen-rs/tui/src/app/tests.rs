...
use codewen_app_server_client::AppServerPath;
use codewen_app_server_protocol::AdditionalFileSystemPermissions;
use codewen_app_server_protocol::AdditionalNetworkPermissions;
use codewen_app_server_protocol::AdditionalPermissionProfile;
use codewen_app_server_protocol::AgentMessageDeltaNotification;
...
use codewen_app_server_protocol::FileUpdateChange;
use codewen_app_server_protocol::ItemStartedNotification;
use codewen_app_server_protocol::JSONRPCErrorError;
use codewen_app_server_protocol::McpServerElicitationRequest;
...
use codewen_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use codewen_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use codewen_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use codewen_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use codewen_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use codewen_app_server_protocol::PatchChangeKind;
use codewen_app_server_protocol::PermissionsRequestApprovalParams;
use codewen_app_server_protocol::RequestId as AppServerRequestId;
...
use codewen_app_server_protocol::TokenUsageBreakdown;
...
use codewen_app_server_protocol::UserInput as AppServerUserInput;
...
use codewen_protocol::models::NetworkPermissions;
use codewen_protocol::models::PermissionProfile;
use codewen_protocol::protocol::EventMsg;
use codewen_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
...
use codewen_protocol::protocol::SessionSource as RolloutSessionSource;
...
use codewen_protocol::user_input::TextElement;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let codewen_app_server_client::AppServerEvent::ServerNotification(
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: codewen_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_codewen_backend_auth: false,
        model_catalog: app.model_catalog.clone(),
        feedback: codewen_feedback::CodewenFeedback::new(),
        is_first_run: false,
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::CodewenOp(Op::UserTurn { items, .. }) => {
                assert!(
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    let cursor = codewen_message_history::HistoryBatchCursor::new(/*end_offset*/ 10);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let cursor = codewen_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(codewen_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: codewen_app_server_protocol::CollabAgentTool::Wait,
                status: codewen_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(codewen_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: codewen_app_server_protocol::CollabAgentTool::SendInput,
                status: codewen_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    codewen_app_server_protocol::CollabAgentState {
                        status: codewen_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
    ...
    runtime.block_on(async {
            ...
            .enumerate()
        {
            ...
            let session_meta = SessionMeta {
                ...
                originator: "codewen-tui-test".to_string(),
        ...
        assert!(
            std::iter::from_fn(|| app_event_rx.try_recv().ok())
                .any(|event| matches!(event, AppEvent::CodewenOp(Op::UserTurn { .. })))
        );
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.codewen_home = codewen_home.path().to_path_buf().abs();
    ...
    let config = std::fs::read_to_string(codewen_home.path().join("config.toml"))?;
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let codewen_home = tempdir()?;
        app.config.codewen_home = codewen_home.path().to_path_buf().abs();
        app.config.sqlite_home = codewen_home.path().to_path_buf();
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = codewen_home.path().join("memories");
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::CodewenOp(op)) => op,
        other => panic!("expected CodewenOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let codewen_home = tempdir()?;
    app.config.codewen_home = codewen_home.path().to_path_buf().abs();
    let config_toml_path = codewen_home.path().join("config.toml").abs();
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            codewen_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            codewen_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            codewen_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            codewen_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: codewen_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: codewen_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: codewen_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: codewen_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: codewen_app_server_protocol::SessionSource::Unknown,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            codewen_app_server_protocol::ServerRequestResolvedNotification {
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        codewen_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            codewen_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        codewen_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let codewen_home = app_server
        .codewen_home_path(&app.chat_widget.config_ref().codewen_home)
        .expect("codewen home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&codewen_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&codewen_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{codewen_home}/../other/{suffix}"),
        format!("{codewen_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&codewen_home, &path));
    }
    assert!(!is_managed(
        &AppServerPath::from_app_server("/tmp/codewen\\home"),
        &format!("/tmp/codewen/home/{suffix}")
    ));
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        codewen_error_info: Some(AppServerCodexErrorInfo::ActiveTurnNotSteerable {
...
fn session_start_error_surfaces_archived_guidance_without_rollout_path() {
    ...
    let target_session = SessionTarget {
        path: Some(std::path::PathBuf::from(
            "/Users/me/.codewen/archived_sessions/rollout.jsonl",
        )),
        thread_id,
    };
    let expected = format!(
        "session {thread_id} is archived. Run `codewen unarchive {thread_id}` to unarchive it first."
    );
...
async fn remote_resume_current_cwd_rejection_snapshot() -> Result<()> {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    std::fs::write(
        app.config.codewen_home.join("config.toml"),
        "[tui]\nresume_cwd = \"current\"\n",
    )?;
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(codewen_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
                /*codewen_linux_sandbox_exe*/ None,
            )?),
        )
        .await,
    );
...
async fn remote_resume_keeps_server_only_cwd_out_of_local_config() -> Result<()> {
    ...
    let thread_id = app_test_support::create_fake_rollout(
        app.config.codewen_home.as_path(),
        filename_timestamp,
        "2025-01-05T12:00:00Z",
        "Saved user message",
...
async fn in_app_resume_uses_configured_or_explicit_cwd() -> Result<()> {
    for (configured_mode, has_explicit_cwd, has_remote_exec, expected_directory) in [
        ...
        let config = ConfigBuilder::default()
            .codewen_home(codewen_home.clone())
            .loader_overrides(LoaderOverrides::without_managed_config_for_tests())
        ...
        .expect("materialized rollout should be created");
        let rollout_path =
            app_test_support::rollout_path(&codewen_home, filename_timestamp, &thread_id);
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(codewen_config::types::ResumeCwdMode::Current)
        .apply()
        .await
...
async fn prompt_edit_forks_before_selected_prompt_and_preserves_source() -> Result<()> {
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            codewen_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_codewen_backend_auth: app.chat_widget.has_codewen_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    codewen_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: codewen_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                codewen_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                codewen_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: codewen_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: codewen_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                codewen_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.codewen/skills/abc/SKILL.md"),
