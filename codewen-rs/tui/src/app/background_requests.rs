...
pub(super) async fn fetch_account_token_activity(
    request_handle: AppServerRequestHandle,
) -> Result<codewen_app_server_protocol::GetAccountTokenUsageResponse> {
...
pub(super) async fn fetch_workspace_messages(
    request_handle: AppServerRequestHandle,
) -> Result<codewen_app_server_protocol::GetWorkspaceMessagesResponse> {
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<codewen_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: codewen_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
fn plugin_remote_section_error_next_step(label: &str, err: &str) -> &'static str {
    ...
    } else if err.contains("codewen plugins are disabled")
    ...
    } else if err.contains("old build") || err.contains("update codewen") || err.contains("stale") {
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![codewen_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, codewen_protocol::mcp::Tool>,
    HashMap<String, Vec<codewen_protocol::mcp::Resource>>,
    HashMap<String, Vec<codewen_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                codewen_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                codewen_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                codewen_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                codewen_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
