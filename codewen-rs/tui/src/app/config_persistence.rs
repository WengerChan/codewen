...
impl App {
    ...
    pub(super) async fn apply_permission_profile_selection(
        ...
        self.sync_active_thread_permission_settings_to_cached_session()
            .await;
        self.app_event_tx
            .send(AppEvent::CodewenOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                approval_policy,
                approvals_reviewer,
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("codewen-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
    ...
    pub(super) fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) =
                crate::render::highlight::resolve_theme_by_name(name, Some(&self.config.codewen_home))
        {
...
fn windows_sandbox_mode_from_effective_config(
    effective_config: &ConfigReadResponse,
) -> Option<codewen_config::types::WindowsSandboxModeToml> {
...
mod tests {
    ...
    async fn resume_model_settings_preserves_only_explicit_model_overrides() {
        ...
        let profile = "work"
            .parse::<codewen_config::ProfileV2Name>()
    ...
    -> Result<()> {
        let mut app = make_test_app().await;
        let codewen_home = tempdir()?;
        let required_policy = codewen_protocol::protocol::AskForApproval::Never;
        let cloud_config_bundle =
            codewen_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
...
"#
    ...
    -> Result<()> {
        ...
        let codewen_home = tempdir()?;
        app.config.codewen_home = codewen_home.path().to_path_buf().abs();
        std::fs::write(codewen_home.path().join("config.toml"), "[broken")?;
