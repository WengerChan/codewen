...
pub(crate) const DIRECTIVE_PREFIX: &str = "::codewen-inline-vis{";
...
impl InlineVisualizationContext {
    pub(crate) fn new(codewen_home: &Path, thread_id: ThreadId) -> Option<Self> {
        Self::new_with_writable_roots(codewen_home, thread_id, std::iter::empty())
    }
    ...
    fn new_with_writable_roots<'a>(
        ...
        let visualizations_dir = codewen_home.join("visualizations");
...
fn link_placeholder() -> String {
    ...
    format!("https://codewen.invalid/inline-visualization/{token}")
}
