...
use codewen_config::NetworkUnixSocketPermissionToml;
use codewen_config::RequirementSource;
use codewen_config::ResidencyRequirement;
use codewen_config::SandboxModeRequirement;
use codewen_config::WebSearchModeRequirement;
use codewen_config::format_config_layer_source;
...
mod tests {
    ...
    use codewen_config::ConstraintError;
    use codewen_config::FeatureRequirementsToml;
    use codewen_config::FilesystemConstraints;
    use codewen_config::HookEventsToml;
    use codewen_config::HookHandlerConfig;
    use codewen_config::LoaderOverrides;
    use codewen_config::ManagedHooksRequirementsToml;
    use codewen_config::MatcherGroup;
    ...
    use codewen_config::NetworkUnixSocketPermissionToml;
    use codewen_config::NetworkUnixSocketPermissionsToml;
    use codewen_config::RequirementSource;
    use codewen_config::ResidencyRequirement;
    use codewen_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\codewen\\config.toml")
        } else {
            absolute_path("/etc/codewen/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.codewen")
        } else {
            absolute_path("/repo/.codewen")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_codewen_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            ...
            absolute_path("/etc/codewen/requirements.toml")
        };
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.codewen\\state")
        } else {
            absolute_path("/home/alice/.codewen/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.codewen\\logs")
        } else {
            absolute_path("/home/alice/.codewen/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.codewen\\models.json")
        } else {
            absolute_path("/home/alice/.codewen/models.json")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.codewen\\config.toml")
        } else {
            absolute_path("/home/alice/.codewen/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/codewen.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\codewen")
        } else {
            absolute_path("/var/lib/codewen")
        };
