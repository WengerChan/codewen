...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        codewen_error_info: &AppServerCodexErrorInfo,
    ) -> bool {
        matches!(
            codewen_error_info,
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.codewen_rate_limit_reached_type.map(|kind| {
        ...
        self.codewen_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        codewen_error_info: Option<AppServerCodexErrorInfo>,
    ) {
        if codewen_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if codewen_error_info
            .as_ref()
            ...
            })
        {
            ...
            self.request_redraw();
            self.maybe_send_next_queued_input();
        } else if let Some(info) = codewen_error_info
            .as_ref()
