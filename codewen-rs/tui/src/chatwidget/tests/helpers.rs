...
pub(super) async fn test_config() -> Config {
    ...
    let codewen_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_codewen_home(codewen_home.clone(), Vec::new())
            .await
            .expect("config");
    config.codewen_home = codewen_home.abs();
    config.sqlite_home = codewen_home.clone();
    config.log_dir = codewen_home.join("log");
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::CodewenOpTarget::Direct(op_tx));
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.has_codewen_backend_auth = true;
...
fn token_usage_breakdown(usage: TokenUsage) -> codewen_app_server_protocol::TokenUsageBreakdown {
    codewen_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    codewen_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: codewen_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    codewen_error_info: Option<CodewenErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = codewen_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            codewen_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("codewen-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(codewen_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    codewen_app_server_protocol::PluginHookSummary {
        ...
        hook_run(
            ...
            vec![
                codewen_app_server_protocol::HookOutputEntry {
                    kind: codewen_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: codewen_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        codewen_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        codewen_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        codewen_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        codewen_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        codewen_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        codewen_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        codewen_app_server_protocol::HookEventName::SessionEnd => "SessionEnd",
        codewen_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        codewen_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        codewen_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        codewen_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
