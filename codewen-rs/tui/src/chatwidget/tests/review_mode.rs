...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::CodewenOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::CodewenOp(Op::Review { target }) => {
            assert_eq!(
                target,
                ReviewTarget::Custom {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::CodewenOp should be sent
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        codewen_app_server_protocol::ServerNotification::TurnStarted(
            codewen_app_server_protocol::TurnStartedNotification {
                thread_id: "thread-1".to_string(),
                turn: codewen_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items_view: codewen_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: codewen_app_server_protocol::TurnStatus::InProgress,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
    ...
    chat.handle_server_notification(
        codewen_app_server_protocol::ServerNotification::ThreadGoalUpdated(
            codewen_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: codewen_app_server_protocol::ThreadGoal {
                    ...
                    status: codewen_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        ...
        codewen_app_server_protocol::ServerNotification::TurnCompleted(
            codewen_app_server_protocol::TurnCompletedNotification {
                ...
                turn: codewen_app_server_protocol::Turn {
                    ...
                    items_view: codewen_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: codewen_app_server_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
