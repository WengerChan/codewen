...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<codewen_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<codewen_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(codewen_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`codewen-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `codewen mcp add`.
    ...
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            ...
            crate::terminal_hyperlinks::osc8_hyperlink(
                "https://developers.openai.com/codewen/mcp",
                "MCP docs",
            )
            .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, codewen_protocol::mcp::Tool>,
