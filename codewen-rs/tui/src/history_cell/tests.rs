...
use codewen_otel::RuntimeMetricTotals;
...
async fn test_config() -> Config {
    let codewen_home = std::env::temp_dir();
...
fn composite_cell_preserves_child_web_links() {
    let destination = "https://chatgpt.com/codewen/settings/usage";
...
fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
    ...
    assert_eq!(
        rendered,
        vec![
            "✔ You approved codewen to".to_string(),
...
fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
    ...
    let cmd = "set -o pipefail\ncargo test -p codewen-tui --quiet".to_string();
    let call_id = "c1".to_string();
    let mut cell = ExecCell::new(
        ExecCall {
            call_id: call_id.clone(),
