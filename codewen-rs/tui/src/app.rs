...
use codewen_ansi_escape::ansi_escape_line;
...
use codewen_app_server_protocol::ConfigReadResponse;
...
use codewen_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use codewen_app_server_protocol::SkillErrorInfo;
...
use codewen_app_server_protocol::WriteStatus;
...
use codewen_models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use codewen_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use codewen_protocol::permissions::FileSystemSandboxKind;
...
use codewen_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        codewen_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&codewen_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&codewen_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[codewen_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&codewen_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<codewen_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    codewen_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `codewen unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_codewen_backend_auth: self.chat_widget.has_codewen_backend_auth(),
    ...
    pub async fn run(
        ...
        let has_codewen_backend_auth = matches!(auth_mode, Some(TelemetryAuthMode::Chatgpt));
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            codewen_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("codewen.status_line", /*inc*/ 1, &[]);
        }
...
See the Codex keymap documentation for supported actions and examples."
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.codewen_home.clone();
