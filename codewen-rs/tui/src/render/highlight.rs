...
/// Set the user-configured syntax theme override and codewen home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `codewen_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), codewen_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let codewen_home_set_ok = CODEX_HOME.set(codewen_home.clone()).is_ok();
    ...
    if !override_set_ok || !codewen_home_set_ok {
...
fn custom_theme_path(name: &str, codewen_home: &Path) -> PathBuf {
    codewen_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{codewen_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, codewen_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, codewen_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, codewen_home: Option<&Path>) -> Theme {
...
fn build_default_theme() -> Theme {
    ...
    let codewen_home = CODEX_HOME
        .get()
        .and_then(|codewen_home| codewen_home.as_deref());
    resolve_theme_with_override(name, codewen_home)
}
...
pub(crate) fn resolve_theme_by_name(name: &str, codewen_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// found in `{codewen_home}/themes/`.
pub(crate) fn list_available_themes(codewen_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*codewen_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*codewen_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*codewen_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*codewen_home*/ None).is_none());
    }
