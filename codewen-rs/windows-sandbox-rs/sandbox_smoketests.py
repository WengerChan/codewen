...
# Run a suite of smoke tests against the Windows sandbox via the Codewen CLI
...
def _resolve_codewen_cmd() -> List[str]:
    """Resolve the Codewen CLI to invoke `codewen sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Codewen.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "codewen.exe",
        ws_root / "target" / "release" / "codewen.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "codewen.exe",
            cargo_base / "release" / "codewen.exe",
        ])
    ...
    if shutil.which("codewen"):
        return ["codewen"]

    raise FileNotFoundError(
        "Codewen CLI not found. Build it first, e.g.\n"
        "  cargo build -p codewen-cli --release\n"
        "or for debug:\n"
        "  cargo build -p codewen-cli\n"
    )

CODEWEN_CMD = _resolve_codewen_cmd()
print(CODEWEN_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to codewen CLI overrides.
    ...
    argv = [*CODEWEN_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"CODEWEN_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\codewen_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.codewen artifacts) denied
    codewen_home = Path(os.environ["USERPROFILE"]) / ".codewen"
    cap_sid_target = codewen_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .codewen\\policy.json"], WS_ROOT)
    add("WS: .codewen cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .codewen policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > codewen_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://codewen-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
