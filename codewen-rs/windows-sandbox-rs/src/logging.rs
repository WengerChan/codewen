...
pub fn current_log_file_path_for_codewen_home(codewen_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(codewen_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_codewen_home_uses_sandbox_dir() {
        let codewen_home = Path::new("codewen-home");

        assert_eq!(
            current_log_file_path_for_codewen_home(codewen_home),
            current_log_file_path(&codewen_home.join(".sandbox"))
        );
    }
}
