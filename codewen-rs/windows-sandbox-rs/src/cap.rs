...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.codewen`)
...
pub fn cap_sid_file(codewen_home: &Path) -> PathBuf {
    codewen_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(codewen_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(codewen_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(codewen_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(codewen_home);
    let mut caps = load_or_create_cap_sids(codewen_home)?;
...
pub fn writable_root_cap_sid_for_path(codewen_home: &Path, root: &Path) -> Result<String> {
    let path = cap_sid_file(codewen_home);
    let mut caps = load_or_create_cap_sids(codewen_home)?;
...
pub fn workspace_write_cap_sid_for_root(
    ...
    if canonical_path_key(root) == canonical_path_key(cwd) {
        workspace_cap_sid_for_cwd(codewen_home, cwd)
    } else {
        writable_root_cap_sid_for_path(codewen_home, root)
    }
}
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&codewen_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&codewen_home, alt_spelling.as_path()).expect("second sid");
    ...
    fn write_roots_get_path_scoped_sids() {
        ...
        let extra_sid = workspace_write_cap_sid_for_root(&codewen_home, &workspace, &extra_root)
