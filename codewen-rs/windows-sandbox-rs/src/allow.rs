...
mod tests {
    ...
    #[test]
    fn denies_codewen_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let codewen_dir = command_cwd.join(".codewen");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&codewen_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&codewen_dir).unwrap(),
