...
use anyhow::Context;
use anyhow::Result;
use codewen_utils_pty::JobObject;
use codewen_windows_sandbox::ConsoleMode;
use codewen_windows_sandbox::ErrorPayload;
use codewen_windows_sandbox::ErrorStage;
use codewen_windows_sandbox::ExitPayload;
use codewen_windows_sandbox::FramedMessage;
use codewen_windows_sandbox::IPC_PROTOCOL_VERSION;
use codewen_windows_sandbox::LocalSid;
use codewen_windows_sandbox::Message;
use codewen_windows_sandbox::OutputPayload;
use codewen_windows_sandbox::OutputStream;
use codewen_windows_sandbox::PipeSpawnHandles;
use codewen_windows_sandbox::ResizePayload;
use codewen_windows_sandbox::SpawnReady;
use codewen_windows_sandbox::SpawnRequest;
use codewen_windows_sandbox::StderrMode;
use codewen_windows_sandbox::StdinMode;
use codewen_windows_sandbox::WindowsSandboxTokenMode;
use codewen_windows_sandbox::allow_null_device;
use codewen_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use codewen_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use codewen_windows_sandbox::decode_bytes;
use codewen_windows_sandbox::encode_bytes;
use codewen_windows_sandbox::get_current_token_for_restriction;
use codewen_windows_sandbox::hide_current_user_profile_dir;
use codewen_windows_sandbox::log_note;
use codewen_windows_sandbox::read_frame;
use codewen_windows_sandbox::read_handle_loop;
use codewen_windows_sandbox::spawn_process_with_pipes;
use codewen_windows_sandbox::to_wide;
use codewen_windows_sandbox::token_mode_for_permission_profile;
use codewen_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with codewen_exec_server::CODEX_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--codewen-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\CodewenSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<codewen_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.codewen_home.clone();
    hide_current_user_profile_dir(req.codewen_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = codewen_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
