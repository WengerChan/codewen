...
use codewen_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "CodewenSandboxUsers";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(codewen_home);
...
pub(super) fn prepare_setup_marker(codewen_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(codewen_home).join("setup_marker.json");
