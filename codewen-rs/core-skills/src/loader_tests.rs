...
use codewen_exec_server::CopyOptions;
use codewen_exec_server::CreateDirectoryOptions;
use codewen_exec_server::ExecutorFileSystem;
use codewen_exec_server::ExecutorFileSystemFuture;
use codewen_exec_server::FileMetadata;
use codewen_exec_server::FileSystemReadStream;
use codewen_exec_server::FileSystemSandboxContext;
use codewen_exec_server::LOCAL_FS;
use codewen_exec_server::ReadDirectoryEntry;
use codewen_exec_server::RemoveOptions;
use codewen_exec_server::WalkOptions;
use codewen_exec_server::WalkOutcome;
...
const REPO_ROOT_CONFIG_DIR_NAME: &str = ".codewen";
...
async fn make_config(codewen_home: &TempDir) -> TestConfig {
    make_config_for_cwd(codewen_home, codewen_home.path().to_path_buf()).await
}
            ...
            dot_codex.is_dir().then(|| {
                ConfigLayerEntry::new(
                    ConfigLayerSource::Project {
                        dot_codewen_folder: dot_codex.abs(),
                    },
...
async fn make_config_for_cwd(codewen_home: &TempDir, cwd: PathBuf) -> TestConfig {
    let user_config_path = codewen_home.path().join(CONFIG_TOML_FILE);
    let system_config_path = codewen_home.path().join("etc/codewen/config.toml");
    fs::create_dir_all(
        system_config_path
            .parent()
...
-> anyhow::Result<()> {
    ...
    let system_folder = tmp.path().join("etc/codewen");
    let home_folder = tmp.path().join("home");
    let user_folder = home_folder.join("codewen");
...
fn write_skill(codewen_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(&codewen_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(codewen_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(
        &codewen_home.path().join("skills/.system"),
        dir,
        name,
        description,
    )
...
fn write_plugin_manifest(plugin_root: &Path, contents: &str) {
    let manifest_path = plugin_root.join(".codewen-plugin/plugin.json");
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    ...
    let skill_path = write_skill(&codewen_home, "demo", "dep-skill", "from json");
...
async fn loads_skill_interface_metadata_from_yaml() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&codewen_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&codewen_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&codewen_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&codewen_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(codewen_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &codewen_home.path().join("skills/shared"));
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    let cycle_dir = codewen_home.path().join("skills/cycle");
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let cfg = make_config_for_cwd(&codewen_home, repo_dir.path().to_path_buf()).await;
    let outcome = load_skills_for_test(&cfg).await;
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = codewen_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = codewen_home.path().join("skills");
    let outcome = load_skills_from_roots(
        [SkillRoot {
...
async fn skips_hidden_and_invalid() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = codewen_home.path().join("skills/.hidden");
    ...
    let invalid_dir = codewen_home.path().join("skills/invalid");
...
async fn preserves_overlong_descriptions() {
    ...
    write_skill(&codewen_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&codewen_home).await;
    ...
    write_skill(&codewen_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg).await;
    assert!(
        outcome.errors.is_empty(),
        "unexpected errors: {:?}",
...
#[tokio::test]
async fn loads_skills_from_agents_dir_without_codewen_dir() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
#[tokio::test]
async fn loads_skills_from_all_codewen_dirs_under_project_root() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
    ...
    let cfg = make_config_for_cwd(&codewen_home, nested_dir).await;
...
async fn repo_skill_root_search_limits_concurrent_probes_and_preserves_order() {
    ...
    let expected_probes = directories
        ...
        .collect::<Vec<_>>();
    let cfg = make_config_for_cwd(&codewen_home, cwd).await;
...
#[tokio::test]
async fn loads_skills_from_codewen_dir_when_not_git_repo() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    ...
    let cfg = make_config_for_cwd(&codewen_home, work_dir.path().to_path_buf()).await;
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&codewen_home, "user", "dupe-skill", "from user");
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_codewen_dirs() {
    let codewen_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&codewen_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&codewen_home, file_path).await;
...
async fn loads_skills_from_system_cache_when_present() {
    ...
    let skill_path = write_system_skill(&codewen_home, "system", "system-skill", "from system");
