...
use codewen_core_skills::loader::EnvironmentSkillMetadata;
use codewen_core_skills::loader::load_environment_skills_from_discovery;
use codewen_core_skills::loader::load_environment_skills_from_root;
use codewen_exec_server::CapabilityRootDiscoverRequest;
use codewen_exec_server::CapabilityRootsDiscoverParams;
...
use codewen_exec_server::discover_capability_roots;
...
async fn loads_nearest_plugin_namespaces_without_reading_unused_sibling_manifests() {
    ...
    for (plugin_root, name) in [
        ...
        fs::create_dir_all(plugin_root.join(".codewen-plugin")).expect("manifest dir");
        fs::write(
    ...
    let mut expected_manifest_reads = [&outer_root, &inner_root]
        .into_iter()
        .map(|plugin_root| {
            PathUri::from_host_native_path(plugin_root.join(".codewen-plugin/plugin.json")).unwrap()
        })
        .collect::<Vec<_>>();
...
async fn reuses_walk_inventory_for_missing_skill_metadata() {
    ...
    let root = tempdir().expect("tempdir");
    let manifest_path = root.path().join(".codewen-plugin/plugin.json");
...
async fn host_loading_reuses_walk_inventory_for_symlinked_skill_pack() {
    ...
    use codewen_core_skills::SkillMetadata;
    use codewen_core_skills::SkillPolicy;
    ...
    let manifest_path = shared_plugin_root.path().join(".codewen-plugin/plugin.json");
    fs::create_dir_all(manifest_path.parent().expect("manifest parent")).expect("manifest dir");
...
async fn executor_bundle_parser_matches_the_existing_environment_loader() {
    let root = tempdir().expect("tempdir");
    let plugin_manifest = root.path().join(".codewen-plugin/plugin.json");
...
async fn executor_bundle_preserves_parent_namespace_and_manifest_precedence() {
    ...
    for (relative_path, name) in [
        (".codewen-plugin/plugin.json", "codewen-name"),
    ...
    assert!(
        discovery.namespace_manifests[0]
            .path
            .to_string()
            .ends_with("/.codewen-plugin/plugin.json")
    );
    assert_eq!(bundled.warnings, existing.warnings);
    assert_eq!(
        bundled
    ...
    assert_eq!(bundled.skills[0].metadata.name, "codewen-name:search");
}
