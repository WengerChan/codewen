...
use codewen_extension_api::ConfigContributor;
use codewen_extension_api::ExtensionData;
use codewen_extension_api::ExtensionEventSink;
use codewen_extension_api::ExtensionFuture;
use codewen_extension_api::ExtensionRegistryBuilder;
use codewen_extension_api::ThreadIdleInput;
use codewen_extension_api::ThreadLifecycleContributor;
...
use codewen_extension_api::TokenUsageContributor;
use codewen_extension_api::ToolCallOutcome;
use codewen_extension_api::ToolContributor;
use codewen_extension_api::ToolFinishInput;
use codewen_extension_api::ToolLifecycleContributor;
use codewen_extension_api::ToolLifecycleFuture;
use codewen_extension_api::TurnAbortInput;
use codewen_extension_api::TurnErrorInput;
use codewen_extension_api::TurnLifecycleContributor;
...
pub struct GoalExtension<C> {
    state_dbs: Arc<codewen_state::StateRuntime>,
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                codewen_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                CodewenErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
    ...
    C: Send + Sync + 'static,
{
    ...
    ) -> Vec<Arc<dyn codewen_extension_api::ToolExecutor<codewen_extension_api::ToolCall>>> {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
