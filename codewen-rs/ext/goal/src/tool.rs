...
use codewen_extension_api::JsonToolOutput;
use codewen_extension_api::ToolCall;
use codewen_extension_api::ToolExecutor;
use codewen_extension_api::ToolName;
use codewen_extension_api::ToolOutput;
use codewen_extension_api::ToolSpec;
use codewen_protocol::ThreadId;
use codewen_protocol::protocol::ThreadGoal;
use codewen_protocol::protocol::ThreadGoalStatus;
use codewen_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<codewen_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> codewen_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => codewen_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => codewen_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: codewen_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            codewen_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            codewen_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<codewen_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &codewen_state::StateRuntime,
    thread_id: ThreadId,
    goal: &codewen_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: codewen_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: codewen_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> codewen_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => codewen_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => codewen_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => codewen_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => codewen_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => codewen_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => codewen_state::ThreadGoalStatus::Complete,
    }
}
