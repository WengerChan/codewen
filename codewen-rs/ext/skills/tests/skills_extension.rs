...
use codewen_core_skills::SKILLS_INTRO_WITH_ABSOLUTE_PATHS;
use codewen_core_skills::SkillLoadOutcome;
use codewen_core_skills::injection::InjectedHostSkillPrompts;
use codewen_extension_api::ConversationHistory;
...
use codewen_extension_api::PreviousWorldStateSection;
...
use codewen_extension_api::WorldStateContributionInput;
use codewen_models_manager::model_info::model_info_from_slug;
...
use codewen_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use codewen_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
...
use codewen_skills_extension::SkillProviders;
use codewen_skills_extension::SkillsExtensionConfig;
use codewen_skills_extension::catalog::SkillAuthority;
use codewen_skills_extension::catalog::SkillCatalog;
use codewen_skills_extension::catalog::SkillCatalogEntry;
use codewen_skills_extension::catalog::SkillPackageId;
use codewen_skills_extension::catalog::SkillProviderError;
use codewen_skills_extension::catalog::SkillReadResult;
use codewen_skills_extension::catalog::SkillResourceId;
use codewen_skills_extension::catalog::SkillSearchResult;
use codewen_skills_extension::catalog::SkillSourceKind;
use codewen_skills_extension::install;
use codewen_skills_extension::install_with_providers;
use codewen_skills_extension::provider::SkillListQuery;
use codewen_skills_extension::provider::SkillProvider;
use codewen_skills_extension::provider::SkillProviderFuture;
use codewen_skills_extension::provider::SkillReadRequest;
use codewen_skills_extension::provider::SkillSearchRequest;
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let codewen_home = test_codewen_home();
    let skill_path = codewen_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(codewen_home)?;
    Ok(())
}
...
async fn selected_executor_catalog_follows_step_availability_and_reuses_its_cache() -> TestResult {
    ...
    let available_sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: codewen_protocol::ThreadId::new(),
            turn_id: "turn-1",
...
fn test_codewen_home() -> PathBuf {
    let id = NEXT_CODEX_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "codewen-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
