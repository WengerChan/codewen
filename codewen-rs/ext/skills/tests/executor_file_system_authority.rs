...
use codewen_skills_extension::ExecutorSkillProvider;
...
async fn skill_loading_and_reads_use_the_supplied_executor_file_system() {
    let test_root =
        std::env::temp_dir().join(format!("codewen-executor-skill-fs-{}", std::process::id()));
...
fn create_local_skill_root(label: &str) -> io::Result<std::path::PathBuf> {
    ...
    let test_root = std::env::temp_dir().join(format!(
        "codewen-executor-skill-{label}-{}-{id}",
        std::process::id()
    ));
