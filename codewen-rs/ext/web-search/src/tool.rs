...
use codewen_api::SearchClient;
use codewen_api::SearchCommands;
use codewen_api::SearchQuery;
use codewen_api::SearchRequest;
use codewen_api::SearchSettings;
use codewen_core::X_CODEX_TURN_METADATA_HEADER;
use codewen_core::web_search_action_detail;
...
use codewen_extension_api::parse_tool_input_schema_without_compaction;
use codewen_extension_items::ExtensionItem;
use codewen_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "codewen.web_search.results.payload_bytes";
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        let extra_headers = search_request_headers(
            ...
            call.codewen_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = codewen_otel::global()
