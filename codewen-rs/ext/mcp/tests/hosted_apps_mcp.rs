...
use codewen_extension_api::McpServerContributor;
...
async fn runtime_overlay_preserves_disabled_server() -> TestResult {
    ...
    let config = ConfigBuilder::default()
        ...
        .cli_overrides(vec![
            ("features.apps".to_string(), true.into()),
            (
                "mcp_servers.codewen_apps.url".to_string(),
                "https://example.com/mcp".into(),
            ),
            ("mcp_servers.codewen_apps.enabled".to_string(), false.into()),
        ])
        .build()
        .await?;
    let auth = CodewenAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn later_extension_can_remove_same_name_registration() -> TestResult {
    ...
    let manager = McpManager::new_with_extensions(
        Arc::new(PluginsManager::new(config.codewen_home.to_path_buf())),
...
async fn hosted_apps_mcp_requires_chatgpt_auth() -> TestResult {
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let auth = CodewenAuth::from_api_key("test");
    let manager = installed_manager(&config);
...
impl McpServerContributor<Config> for RemoveCodexApps {
    ...
    ) -> codewen_extension_api::ExtensionFuture<'a, Vec<McpServerContribution>> {
        Box::pin(async move {
