...
use codewen_config::HookStateToml;
...
use codewen_config::version_for_toml;
...
use codewen_protocol::protocol::HookTrustStatus;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_codewen_folder } => layer
            .hooks_config_folder()
            .unwrap_or_else(|| dot_codewen_folder.clone())
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: codewen_protocol::protocol::HookEventName,
            ...
            match handler {
                HookHandlerConfig::Command {
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        codewen_protocol::protocol::HookEventName::PreToolUse
                            | codewen_protocol::protocol::HookEventName::PostToolUse
                            | codewen_protocol::protocol::HookEventName::SessionStart
                            | codewen_protocol::protocol::HookEventName::UserPromptSubmit
                            | codewen_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
fn normalize_command_hook(
    ...
    if event_name != codewen_protocol::protocol::HookEventName::SessionEnd {
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/codewen/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.codewen/config.toml").abs();
        let dot_codewen_folder = test_path_buf("/tmp/worktree/.codewen").abs();
