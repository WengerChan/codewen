...
//!   2. User-defined entries inside `~/.codewen/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use codewen_api::Provider as ApiProvider;
use codewen_api::RetryConfig as ApiRetryConfig;
...
use codewen_protocol::error::EnvVarError;
...
pub const CHATGPT_CODEX_BASE_URL: &str = "https://chatgpt.com/backend-api/codewen";
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "codewen";
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/openai/codewen/discussions/7782";
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> CodewenResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> CodewenResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> CodewenResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    ...
    let default_codewen_oss_base_url = format!(
        "http://localhost:{codewen_oss_port}/v1",
        codewen_oss_port = std::env::var("CODEWEN_OSS_PORT")
            .ok()
    ...
    let codewen_oss_base_url = std::env::var("CODEWEN_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_codewen_oss_base_url);
    create_oss_provider_with_base_url(&codewen_oss_base_url, wire_api)
}
