...
`codewen-otel` is the OpenTelemetry integration crate for Codewen. It provides:

- Provider wiring for log/trace/metric exporters (`codewen_otel::OtelProvider`
  and `codewen_otel::provider`).
- Session-scoped business event emission via `codewen_otel::SessionTelemetry`.
- Low-level metrics APIs via `codewen_otel::metrics`.
- Trace-context helpers via `codewen_otel::trace_context` and crate-root re-exports.
...
```rust
use codewen_otel::config::OtelExporter;
use codewen_otel::config::OtelHttpProtocol;
use codewen_otel::config::OtelSettings;
use codewen_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "codewen-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    codewen_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Codewen
...
Codewen-specific session events. Rich session/business events should go through
...
`codewen-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Codewen-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use codewen_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("codewen.session_started", 1, &[("source", "tui")])?;
metrics.histogram("codewen.request_latency", 83, &[("route", "chat")])?;
```
