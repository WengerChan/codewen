...
use codewen_otel::OtelExporter;
use codewen_otel::OtelHttpProtocol;
use codewen_otel::OtelProvider;
use codewen_otel::OtelSettings;
...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter("codewen.turns", /*inc*/ 1, &[("source", "test")])?;
    ...
    assert!(
        body.contains("codewen.turns"),
        "expected metric name not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("codewen.active"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "codewen.test.log_exported",
    ...
    let body = String::from_utf8_lossy(&request.body);
    assert!(
        body.contains("codewen.test.log_exported"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "codewen.test.trace_event",
    ...
    assert!(
        body.contains("codewen.test.trace_event"),
