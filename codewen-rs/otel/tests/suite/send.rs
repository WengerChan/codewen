...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "codewen-cli"), ("env", "prod")])?;
    ...
    let counter = find_metric(&resource_metrics, "codewen.turns").expect("counter metric missing");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "codewen-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "codewen.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "codewen.tool_latency")
            ...
            .expect("codewen.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "codewen.active").expect("gauge metric missing");
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "codewen.alpha").expect("codewen.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "codewen.beta").expect("codewen.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("codewen.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("codewen.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
