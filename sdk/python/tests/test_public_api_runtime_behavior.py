...
def test_codewen_init_failure_closes_client(monkeypatch: pytest.MonkeyPatch) -> None:
    closed: list[bool] = []
    ...
    monkeypatch.setattr(public_api_module, "CodewenClient", FakeClient)
...
def test_async_codewen_init_failure_closes_client() -> None:
    async def scenario() -> None:
        ...
        codewen._client.start = fake_start  # type: ignore[method-assign]
        codewen._client.initialize = fake_initialize  # type: ignore[method-assign]
        codewen._client.close = fake_close  # type: ignore[method-assign]

        with pytest.raises(RuntimeError, match="missing required metadata"):
            await codewen.models()

        assert close_calls == 1
        assert codewen._initialized is False
        assert codewen._init is None

    asyncio.run(scenario())


def test_async_codewen_initializes_only_once_under_concurrency() -> None:
    async def scenario() -> None:
        codewen = AsyncCodex()
        start_calls = 0
        initialize_calls = 0
        ...
        async def fake_initialize() -> InitializeResponse:
            ...
            return InitializeResponse.model_validate(
                {
                    "userAgent": "codewen-cli/1.2.3",
                    "serverInfo": {"name": "codewen-cli", "version": "1.2.3"},
                }
            )
        ...
        codewen._client.start = fake_start  # type: ignore[method-assign]
        codewen._client.initialize = fake_initialize  # type: ignore[method-assign]
        codewen._client.model_list = fake_model_list  # type: ignore[method-assign]

        await asyncio.gather(codewen.models(), codewen.models())
