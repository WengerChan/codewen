...
from openai_codex import AsyncCodex, Codewen
...
def test_sync_and_async_initialization_round_trip_metadata(tmp_path) -> None:
    ...
    async def async_scenario(harness: AppServerHarness) -> dict[str, object]:
        async with AsyncCodex(config=harness.app_server_config()) as codewen:
            ...
            server = codewen.metadata.serverInfo
            return {
                "thread_id": thread.id,
                "user_agent": codewen.metadata.userAgent,
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            active_thread = codewen.thread_start()
            archived_thread = codewen.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            codewen.thread_archive(archived_thread.id)
            active_list = codewen.thread_list(archived=False)
            archived_list = codewen.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncCodex(config=harness.app_server_config()) as codewen:
                ...
                resumed = await codewen.thread_resume(thread.id)
                forked = await codewen.thread_fork(thread.id)
                archive_response = await codewen.thread_archive(thread.id)
                unarchived = await codewen.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            ...
            forked = codewen.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            ...
            archived = codewen.thread_archive(thread.id)
            unarchived = codewen.thread_unarchive(thread.id)
...
def test_models_rpc(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        with Codewen(config=harness.app_server_config()) as codewen:
            models = codewen.models(include_hidden=True)
