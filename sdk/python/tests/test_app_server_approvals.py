...
from openai_codex import ApprovalMode, AsyncCodex, Codewen
...
def test_thread_resume_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            source = codewen.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            resumed = codewen.thread_resume(source.id)
            resumed_state = codewen._client.thread_resume(  # noqa: SLF001
                resumed.id,
...
def test_thread_fork_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            source = codewen.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            forked = codewen.thread_fork(source.id)
            forked_state = codewen._client.thread_resume(  # noqa: SLF001
                forked.id,
...
def test_turn_approval_mode_persists_until_next_turn(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            ...
            after_turn_override = codewen._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            second_result = thread.run("inherit previous approval mode")
            after_omitted_turn = codewen._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
...
def test_thread_run_approval_mode_persists_until_explicit_override(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            thread = codewen.thread_start(approval_mode=ApprovalMode.deny_all)

            first_result = thread.run("keep approvals denied")
            after_default_run = codewen._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            ...
            after_override_run = codewen._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
...
def test_async_thread_run_approval_mode_persists_until_explicit_override(
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncCodex(config=harness.app_server_config()) as codewen:
                thread = await codewen.thread_start(approval_mode=ApprovalMode.deny_all)
                first_result = await thread.run("keep async approvals denied")
                after_default_run = await codewen._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
                ...
                after_override_run = await codewen._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
