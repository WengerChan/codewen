...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    """CodewenClient.stream_text should stream through a real app-server turn."""
    with AppServerHarness(tmp_path) as harness:
        harness.responses.enqueue_sse(
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            thread = codewen.thread_start()
            chunks = list(codewen._client.stream_text(thread.id, "low-level sync"))  # noqa: SLF001
...
def test_low_level_async_stream_text_allows_parallel_model_list(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncCodex(config=harness.app_server_config()) as codewen:
                thread = await codewen.thread_start()
                stream = codewen._client.stream_text(  # noqa: SLF001
                    thread.id,
                    "low-level async",
                )
                first = await anext(stream)
                models_task = asyncio.create_task(codewen.models())
...
def test_interleaved_sync_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Codewen(config=harness.app_server_config()) as codewen:
            first_thread = codewen.thread_start()
            second_thread = codewen.thread_start()
...
def test_interleaved_async_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncCodex(config=harness.app_server_config()) as codewen:
                first_thread = await codewen.thread_start()
                second_thread = await codewen.thread_start()
