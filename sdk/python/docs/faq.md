...
`openai-codewen` publishes stable releases. Install the latest one with
`pip install openai-codewen`.

## Why does the SDK install a runtime package?

The SDK version tracks the corresponding Codewen CLI release. Each SDK release
...
- `Codewen` is the sync public API.
...
If your app is not already async, stay with `Codewen`.
...
When `sandbox=` is omitted, Codewen uses its configured default. A turn
...
`Codewen()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `openai-codewen-cli-bin` dependency is missing
- local `codewen_bin` override points to a missing file
- a custom local Codewen executable does not support the SDK operation being used
