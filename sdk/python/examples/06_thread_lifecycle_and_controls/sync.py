...
with Codewen(config=runtime_config()) as codewen:
    ...
    reopened = codewen.thread_resume(thread.id)
    listing_active = codewen.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = codewen.thread_archive(reopened.id)
    listing_archived = codewen.thread_list(limit=20, archived=True)
    unarchived = codewen.thread_unarchive(reopened.id)

    resumed = codewen.thread_resume(
        unarchived.id,
        model="gpt-5.4",
