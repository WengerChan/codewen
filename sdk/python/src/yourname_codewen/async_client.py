...
from .client import CodewenClient, CodewenConfig
...
class AsyncCodexClient:
    """Async wrapper around CodewenClient using thread offloading."""

    def __init__(self, config: CodewenConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = CodewenClient(config=config)

    async def __aenter__(self) -> "AsyncCodexClient":
        """Start the Codewen process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the Codewen process when leaving an async context."""
        await self.close()
    ...
    async def initialize(self) -> InitializeResponse:
        """Initialize the Codewen session."""
        ...
        try:
            ...
            def cleanup_cancelled_start(
                ...
                threading.Thread(
                    ...
                    name="codewen-goal-start-cleanup",
                    daemon=True,
                ).start()
