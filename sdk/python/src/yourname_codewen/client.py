...
from .errors import CodewenError, InvalidRequestError, TransportClosedError
...
RUNTIME_PKG_NAME = "openai-codewen-cli-bin"
...
def _installed_codewen_path() -> Path:
    try:
        from codewen_cli_bin import bundled_codewen_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Codewen runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set CodewenConfig.codewen_bin "
            "explicitly."
        ) from exc

    return bundled_codewen_path()


def _installed_codewen_path_dirs() -> tuple[Path, ...]:
    try:
        from codewen_cli_bin import bundled_path_dir
...
class CodewenBinResolverOps:
    installed_codewen_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_codewen_bin_resolver_ops() -> CodewenBinResolverOps:
    return CodewenBinResolverOps(
        installed_codewen_path=_installed_codewen_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_codewen_bin(config: "CodewenConfig", ops: CodewenBinResolverOps) -> Path:
    if config.codewen_bin is not None:
        codewen_bin = Path(config.codewen_bin)
        if not ops.path_exists(codewen_bin):
            raise FileNotFoundError(
                f"Codewen binary not found at {codewen_bin}. Set CodewenConfig.codewen_bin "
                "to a valid binary path."
            )
        return codewen_bin

    return ops.installed_codewen_path()


def _resolve_codewen_bin(config: "CodewenConfig") -> Path:
    return resolve_codewen_bin(config, _default_codewen_bin_resolver_ops())


@dataclass(slots=True)
class CodewenConfig:
    """Configuration for launching and identifying the local Codewen runtime.

    Most callers can use ``Codewen()`` without configuration. Set ``codewen_bin``
    only when intentionally using a specific local Codewen executable.
    """

    codewen_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "codewen_python_sdk"
    client_title: str = "Codewen Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class CodewenClient:
    """Synchronous typed JSON-RPC client for `codewen app-server` over stdio."""

    def __init__(
        self,
        config: CodewenConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or CodewenConfig()
    ...
    def __enter__(self) -> "CodewenClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            codewen_bin = _resolve_codewen_bin(self.config)
            if self.config.codewen_bin is None:
                path_dirs = _installed_codewen_path_dirs()
            args = [str(codewen_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise CodewenError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("Codewen process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"Codewen process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise CodewenError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise CodewenError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_codewen_home() -> str:
    return str(Path.home() / ".codewen")
