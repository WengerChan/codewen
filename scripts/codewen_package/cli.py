"""Command-line interface for building Codewen package directories."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a canonical Codewen package directory and optional archive.",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt codewen-code-mode-host executable. If omitted, "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "scripts/codewen_package/codewen-zsh."
        ),
    )
    parser.add_argument(
        "--codewen-command-runner-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows codewen-command-runner.exe executable. "
            "If omitted for Windows targets, codewen-command-runner is built "
            "with Cargo."
        ),
    )
    parser.add_argument(
        "--codewen-windows-sandbox-setup-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows codewen-windows-sandbox-setup.exe "
            "executable. If omitted for Windows targets, "
            "codewen-windows-sandbox-setup is built with Cargo."
        ),
    )
    parser.add_argument(
        "--rg-bin",
...
def main() -> int:
    ...
    package_dir = (
        ...
        else Path(tempfile.mkdtemp(prefix="codewen-package-")).resolve()
    )
    ...
    source_outputs = build_source_binaries(
        ...
        codewen_command_runner_bin=resolve_optional_input_path(
            args.codewen_command_runner_bin,
            "prebuilt Windows codewen-command-runner.exe executable",
            "--codewen-command-runner-bin",
        ),
        codewen_windows_sandbox_setup_bin=resolve_optional_input_path(
            args.codewen_windows_sandbox_setup_bin,
            "prebuilt Windows codewen-windows-sandbox-setup.exe executable",
            "--codewen-windows-sandbox-setup-bin",
        ),
    )
    ...
    inputs = PackageInputs(
        ...
        codewen_command_runner_bin=source_outputs.codewen_command_runner_bin,
        codewen_windows_sandbox_setup_bin=source_outputs.codewen_windows_sandbox_setup_bin,
    )
    ...
    for archive_output in args.archive_output:
        ...
        print(f"Built Codewen package archive at {archive_path}")

    print(f"Built Codewen package directory at {package_dir}")
    return 0
