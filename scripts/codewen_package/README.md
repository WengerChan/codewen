# Codewen package builder

This package contains the implementation behind `scripts/build_codewen_package.py`.
...
The builder creates a canonical Codewen package directory:
...
├── bin
│   ├── <entrypoint>[.exe]
│   └── codewen-code-mode-host[.exe]
├── codewen-resources
│   ├── bwrap                             # Linux only
│   ├── zsh/bin/zsh                       # supported Unix targets only
│   ├── codewen-command-runner.exe          # Windows only
│   └── codewen-windows-sandbox-setup.exe   # Windows only
└── codewen-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Codewen release
...
`codewen` and `codewen-app-server`. The `version` field in `codewen-package.json` is
read from `[workspace.package].version` in `codewen-rs/Cargo.toml`.
...
- all targets: `codewen-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `codewen-command-runner` and `codewen-windows-sandbox-setup`,
...
`--codewen-command-runner-bin` plus `--codewen-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Codewen-built V8 release pair before invoking
...
DotSlash manifest at `scripts/codewen_package/rg`. Downloaded archives are cached
under `$TMPDIR/codewen-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/codewen_package/codewen-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/codewen-package/<target>-zsh` and installed at
`codewen-resources/zsh/bin/zsh`. Pass `--zsh-manifest` to use a different
