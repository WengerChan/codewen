"""Canonical Codewen package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "codewen-resources"
    path_dir = package_dir / "codewen-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"codewen-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.codewen_command_runner_bin is not None:
        copy_executable(
            inputs.codewen_command_runner_bin,
            resources_dir / "codewen-command-runner.exe",
            is_windows=True,
        )

    if inputs.codewen_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.codewen_windows_sandbox_setup_bin,
            resources_dir / "codewen-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    metadata = {
        ...
        "resourcesDir": "codewen-resources",
        "pathDir": "codewen-path",
    }
    write_json(package_dir / "codewen-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "codewen-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: codewen-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"codewen-code-mode-host{spec.exe_suffix}",
        Path("codewen-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("codewen-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("codewen-resources") / "bwrap")
        executable_files.append(Path("codewen-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("codewen-resources") / "codewen-command-runner.exe",
                Path("codewen-resources") / "codewen-windows-sandbox-setup.exe",
            ]
        )
