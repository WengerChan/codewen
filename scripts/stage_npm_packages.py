#!/usr/bin/env python3
"""Stage one or more Codewen npm packages for release."""
...
BUILD_SCRIPT = REPO_ROOT / "codewen-cli" / "scripts" / "build_npm_package.py"
WORKFLOW_NAME = ".github/workflows/rust-release.yml"
GITHUB_REPO = "openai/codewen"
BINARY_TARGETS = (
...
_SPEC = importlib.util.spec_from_file_location("codewen_build_npm_package", BUILD_SCRIPT)
...
CODEWEN_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "CODEWEN_PLATFORM_PACKAGES", {})
CODEWEN_PACKAGE_COMPONENT = getattr(
    _BUILD_MODULE, "CODEWEN_PACKAGE_COMPONENT", "codewen-package"
)
...
BINARY_COMPONENTS = {
    "codewen-responses-api-proxy": BinaryComponent(
        artifact_prefix="codewen-responses-api-proxy",
        dest_dir="codewen-responses-api-proxy",
        binary_basename="codewen-responses-api-proxy",
    ),
}
...
def install_from_workflow_artifacts(
    ...
    if CODEWEN_PACKAGE_COMPONENT in components:
        install_codewen_package_archives(artifacts_dir, vendor_dir, BINARY_TARGETS)
...
def select_target_artifacts(
    ...
    needs_target_artifacts = CODEWEN_PACKAGE_COMPONENT in components or any(
...
def install_codewen_package_archives(
    artifacts_dir: Path,
    vendor_dir: Path,
    ...
    print(
        "Installing Codewen package archives for targets: " + ", ".join(targets),
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                install_single_codewen_package_archive,
                artifacts_dir,
                vendor_dir,
                target,
            ): target
...
def install_single_codewen_package_archive(
    artifacts_dir: Path,
    vendor_dir: Path,
    target: str,
) -> Path:
    artifact_subdir = artifact_dir_for_target(artifacts_dir, target)
    archive_path = artifact_subdir / f"codewen-package-{target}.tar.gz"
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in CODEWEN_PLATFORM_PACKAGES:
        platform = package.removeprefix("codewen-")
        return f"codewen-npm-{platform}-{version}.tgz"
