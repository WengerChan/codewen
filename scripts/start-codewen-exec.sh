...
remote_path='~/code/codewen-sync'
local_exec_server_port="${CODEWEN_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${CODEWEN_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/codewen-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/codewen-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/codewen-sync"
remote_codewen_rs="$remote_repo_root/codewen-rs"

cd "${remote_codewen_rs}"
cargo build -p codewen-cli --bin codewen

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/codewen exec-server --listen ws://127.0.0.1:0 \
...
echo "Start codewen via: "
printf '  CODEWEN_EXEC_SERVER_URL=ws://127.0.0.1:%s codewen -C %q\n' \
